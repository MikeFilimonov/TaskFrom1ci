...
Procedure AtFillingToDoList(ToDoList) Export
	...
	If Not Users.InfobaseUserWithFullAccess(, True)
		Or ModuleToDoListService.WorkDisabled("AccessGroupsProfiles") Then
		Return;
	EndIf;
	...
	Sections = ModuleToDoListServer.SectionsForObject(Metadata.Catalogs.AccessGroupsProfiles.FullName());
	...
	For Each Section In Sections Do
		...
		Work.Presentation = NStr("en = 'Not compatible with the current version'");
	...
	For Each Role In ProfileRolesDescription Do
		...
		If RoleMetadata = Undefined Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Role ""%2"" is not found in metadata
				     |while checking supplied profile.'"),
...
Procedure FillInDataSuppliedIdentifiers() Export
	...
	While Selection.Next() Do
		...
		UpdateResults.WriteData(ProfileObject);
	EndDo;
	...
	For Each ProfileDescription In ProfileDescriptions Do
		...
		If ValueIsFilled(ProfileDescription.Name) Then
			If ProfilesProperties.Get(ProfileDescription.Name) <> Undefined Then
				Raise ErrorTitle + StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'Profile with name ""%1"" already exists.'"),
					ProfileDescription.Name);
			EndIf;
		...
		For Each ItemOfList In ProfileDescription.AccessKinds Do
			...
			If AccessKindRefiner <> ""
			   ...
			   AND AccessKindRefiner <> "InitiallyAllAllowed" Then
				...
				Raise ErrorTitle + StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'In the profile description ""%1""
					     |for the access kind ""%2"" unknown refiner ""%3"" is specified.
					     |
					     |Only the following refiners are valid:
					     |- ""AllProhibitedInBeginning"" or """",
		...
		For Each ItemOfList In ProfileDescription.AccessValues Do
			...
			If AccessTypeProperties = Undefined Then
				Raise ErrorTitle + StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'In the profile description ""%1"" invalid access kind
					     |""%2"" is specified for access value ""%3"".'"),
					?(ValueIsFilled(ProfileDescription.Name),
					  ProfileDescription.Name,
					  ProfileDescription.ID),
					AccessKind,
			...
			If MetadataObject = Undefined Then
				Raise ErrorTitle + StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'In the profile description ""%1"" for the
					     |access kind ""%2"" specified access value type ""%3"" does not exist.'"),
					?(ValueIsFilled(ProfileDescription.Name),
					  ProfileDescription.Name,
					  ProfileDescription.ID),
					AccessKind,
			...
			If AccessValueEmptyRef = Undefined Then
				Raise ErrorTitle + StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'In the profile description ""%1"" for the
					     |access kind ""%2"" not a reference type of access value ""%3"" is specified.'"),
					?(ValueIsFilled(ProfileDescription.Name),
					  ProfileDescription.Name,
					  ProfileDescription.ID),
					AccessKind,
			...
			If AccessKindPropertiesByType = Undefined
			 OR AccessKindPropertiesByType.Name <> AccessKind Then
				...
				Raise ErrorTitle + StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'In the profile description ""%1"" access value of the ""%3"" type
					     |is specified that is not specified in the access kind ""%2"" properties.'"),
					?(ValueIsFilled(ProfileDescription.Name),
					  ProfileDescription.Name,
					  ProfileDescription.ID),
					AccessKind,
			...
			If AccessValuesTable.FindRows(Filter).Count() > 0 Then
				Raise ErrorTitle + StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'In the profile description ""%1"" for access
					     |kind ""%2"" access value ""%3"" is specified again.'"),
					?(ValueIsFilled(ProfileDescription.Name),
					  ProfileDescription.Name,
					  ProfileDescription.ID),
					AccessKind,
...
//                    in the structure of the AccessGroupProfileNewDescription return function of the AccessManagement
//                    general module.
// 
// Returns:
