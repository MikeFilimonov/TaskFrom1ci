...
#Region ItemEventHandlersAccessKindPresentationFormTableAccessKinds
...
Procedure RestoreByInitialFilling(Command)
	...
	ShowQueryBox(
		...
		NStr("en = 'Restore the profile to its initial state?'"),
...
Procedure RestoreByInitialFillingEnd(Response, NotSpecified) Export
	...
	If UpdateAccessGroups Then
		Text = NStr("en = 'Profile ""%1"" is restored to its initial state, access groups of the profile are updated.'");
	Else
		Text = NStr("en = 'Profile ""%1"" is restored to its initial state, access groups of the profile are not updated.'");
	EndIf;
