...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	If Not AdditionalReportsAndDataProcessors.CheckGlobalProcessing(Type) Then
		If Not UseForObjectForm AND Not UseForListForm 
			...
			CommonUseClientServer.MessageToUser(
				NStr("en = 'Disable publishing or select at least one of the forms to use'")
				,
				,
				,
	...
	If Publication = Enums.AdditionalReportsAndDataProcessorsPublicationOptions.Used Then
		...
		AdditionalReportTypes.Add(Enums.AdditionalReportsAndDataProcessorsTypes.AdditionalReport);
		AdditionalReportTypes.Add(Enums.AdditionalReportsAndDataProcessorsTypes.Report);
		...
		If Conflicting.Count() > 0 Then
			...
			Else
				ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'Name ""%1"" used by the report (data processor) ""%2"" is already in use by another published additional report (data processor).'"),
					ObjectName,
					CommonUse.ObjectAttributeValue(Ref, "Description")
				);
			EndIf;
...
Procedure OnWrite(Cancel)
	...
	If AdditionalProperties.Property("QuickAccess", QuickAccess) Then
		InformationRegisters.UserAccessToDataProcessors.RefreshDataOnAdditionalWriteObject(Ref, QuickAccess);
	EndIf;
	...
	If Type = Enums.AdditionalReportsAndDataProcessorsTypes.AdditionalReport Then
