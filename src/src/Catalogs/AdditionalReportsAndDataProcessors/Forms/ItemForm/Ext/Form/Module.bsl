...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not AddRight Then
		If IsNew Then
			Raise NStr("en = 'Insufficient access rights to add additional reports and data processors.'");
		Else
	...
	IsAdditionalInformationProcessor	= Object.Type = Enums.AdditionalReportsAndDataProcessorsTypes.AdditionalInformationProcessor;					
...
Procedure OnReadAtServer(CurrentObject)
	...
	|	RegistersData.User
	|FROM
	|	InformationRegister.UserAccessToDataProcessors AS RegistersData
	|WHERE
...
Procedure BeforeWriteAtServer(Cancel, CurrentObject, WriteParameters)
	...
	If IsAdditionalInformationProcessor OR Object.Type = TypeAdditionalReport Then
...
Procedure AfterWrite(WriteParameters)
	If Object.Type = PredefinedValue("Enum.AdditionalReportsAndDataProcessorsTypes.BankExchangeProcessor") Then
		Notify("Write_BankStatementProcessingSetting", Object.Ref);
	EndIf;
EndProcedure
...
Procedure AfterWriteAtServer(CurrentObject, WriteParameters)
	...
	If Object.Type = Enums.AdditionalReportsAndDataProcessorsTypes.BankExchangeProcessor Then
		UpdateBankStatementProcessingSetting(CurrentObject);	
	EndIf;
...
Procedure AdditionalReportVariantsBeforeDelete(Item, Cancel)
	...
	If Not Variant.User Then
		ShowMessageBox(, NStr("en = 'Cannot mark predefined report option for deletion.'"));
		Return;
	EndIf;
...
Procedure PlaceInSections(Command)
	...
	// Opens the dialog of placement several report options in command interface sections.
...
Procedure CommandPrescriptionFillingFormsClick(Item, StandardProcessing)
	...
	If Object.Type = TypeAdditionalReport OR IsAdditionalInformationProcessor Then
		// Sections selection
...
Procedure ContinueWriteOnClient(Result, CloseAfterWriting)  Export
	...
	If WriteParameters.DataProcessorRegistration Then
		...
		WarningText = NStr("en = 'For the changes to take effect, reopen the windows referring to this external file.'");
...
Procedure UpdateFromFileStart()
	...
	If Object.Ref.IsEmpty() Then
		...
		DialogueParameters.Title = NStr("en = 'Select a file of external report or data processor'");
	ElsIf Object.Type = TypeAdditionalReport Or Object.Type = ReportKind Then
		DialogueParameters.FilterIndex = 1;
		DialogueParameters.Title = NStr("en = 'Select an external report file'");
	Else
		DialogueParameters.FilterIndex = 2;
		DialogueParameters.Title = NStr("en = 'Select an external data processor file'");
	EndIf;
...
Procedure UpdateFromFileAfterFileSelection(PlacedFiles, RegistrationParameters) Export
	...
	Else
		...
		StandardSubsystemsClient.ReturnResultAfterShowWarning(
			NStr("en = 'File extension does not match those of the external report (ERF) or data processor (EPF).'"),
...
Procedure UpdateFromFileShowConflicts(RegistrationParameters)
	If RegistrationParameters.ConflictsCount > 1 Then
		If RegistrationParameters.IsReport Then
			QuestionTitle = NStr("en = 'Conflicts during external report import'");
		...
		EndIf;
		DisableButtonPresentation = NStr("en = 'Disable conflicting'");
		OpenButtonPresentation = NStr("en = 'Cancel and show list'");
	Else
		If RegistrationParameters.IsReport Then
			QuestionTitle = NStr("en = 'Conflict during external report import'");
		...
		Else
			QuestionTitle = NStr("en = 'Conflict occurred during import of external data processor'");
		...
		EndIf;
		OpenButtonPresentation = NStr("en = 'Cancel and show'");
	EndIf;
	ContinueButtonPresentation = NStr("en = 'In debug mode'");
...
Procedure UpdateFromFileConflictsResolution(Response, RegistrationParameters) Export
	...
	ElsIf Response = "CancelAndOpen" Then
		...
		If ShowList Then // List form with the conflicting selection.
			FormNameString = "Catalog.AdditionalReportsAndDataProcessors.ListForm";
		...
		Else // Item form
			FormNameString = "Catalog.AdditionalReportsAndDataProcessors.ObjectForm";
			ParametersForm = New Structure;
			ParametersForm.Insert("Key", RegistrationParameters.Conflicting[0].Value);
		EndIf;
		ExecuteNotifyProcessing(RegistrationParameters.ResultHandler, RegistrationParameters);
		OpenForm(FormNameString, ParametersForm, Undefined, True);
	Else // Cancel.
		ExecuteNotifyProcessing(RegistrationParameters.ResultHandler, RegistrationParameters);
	EndIf;
EndProcedure
...
Procedure OpenOption()
	...
	If Not ValueIsFilled(Variant.Ref) Then
		ErrorText = NStr("en = 'Report option ""%1"" is not registered.'");
...
Function DisabledSchedulePresentation()
	Return NStr("en = 'Schedule is not set'");
EndFunction
...
Function UsersQuickAccessPresentation(UserCount)
	...
	If LastDigit = 1 Then
		QuickAccessView = NStr("en = 'User %1'");
	ElsIf LastDigit = 2 Or LastDigit = 3 Or LastDigit = 4 Then
		QuickAccessView = NStr("en = 'User %1'");
	Else
		QuickAccessView = NStr("en = '%1 users'");
	EndIf;
...
Function PrepareFormParametersMetadataObjectSelect()
	...
	If Object.Type = Enums.AdditionalReportsAndDataProcessorsTypes.ObjectFill Then
		CommandParameterType = Metadata.CommonCommands.ObjectFill.CommandParameterType;
	ElsIf Object.Type = Enums.AdditionalReportsAndDataProcessorsTypes.Report Then
		CommandParameterType = Metadata.CommonCommands.ObjectReports.CommandParameterType;
	ElsIf Object.Type = Enums.AdditionalReportsAndDataProcessorsTypes.PrintForm Then
		CommandParameterType = Metadata.DefinedTypes.AdditionalPrintForms.Type;
	ElsIf Object.Type = Enums.AdditionalReportsAndDataProcessorsTypes.CreatingLinkedObjects Then
...
Procedure SetVisibleEnabled(Registration = False)
	...
	If Object.Type = TypeAdditionalReport AND Object.UsesVariantsStorage Then
		...
		If Registration OR VariantCount = 0 Then
			...
			Items.PageVariants.Title = NStr("en = 'Report options'");
		Else
	...
	Else
		...
		ElsIf TypeOf(SafeMode) = Type("String") Then
			...
			Items.DecorationPersonalSecurityProfileLabel.Title = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Additional report or data processor will connect to application using ""personal"" security profile %1, which allows only following operations:'"), SafeMode);
		Else
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = '%1 not a correct connection mode for additional reports and data processors that require permissions for security profile usage.'"),
				SafeMode);
		EndIf;
	...
	If IsNew Then
		Title = ?(IsReport, NStr("en = 'Additional report (creation)'"), NStr("en = 'Additional data processor (creation)'"));
	Else
...
Procedure FillCommands(SavedCommands = Undefined)
	...
	For Each ItemCommand In Object.Commands Do
		...
		If IsAdditionalInformationProcessor 
		...
		Else
			ItemCommand.ScheduledJobPresentation = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Not applied for commands with launch option ""%1""'"),
...
Procedure AdditionalReportVariantsFill()
	...
	Except
		ErrorText = NStr("en = 'Cannot receive a list of report options due to the error that occurred when connecting this report:'");
...
Procedure FileNameForDebuggingStartChoice(Item, ChoiceData, StandardProcessing)
	...
	OpeningFileDialogue.Filter		= "EPF (*.epf)|*.epf|ERF (*.erf)|*.erf";
	OpeningFileDialogue.Multiselect	= False;
	OpeningFileDialogue.Title		= NStr("en = 'Select file for debugging'");
	...
	If OpeningFileDialogue.Choose() Then
	    ...
	    EndDo;
	Else
	    ShowMessageBox(,  NStr("en = 'File is not selected!'"));
	EndIf;
	
EndProcedure

&AtServer
Procedure UpdateBankStatementProcessingSetting(CurrentObject)
	
	Query = New Query;
	Query.Text = 
	"SELECT
	|	BankStatementProcessingSettings.Ref AS Ref
	|FROM
	|	Catalog.BankStatementProcessingSettings AS BankStatementProcessingSettings
	|WHERE
	|	BankStatementProcessingSettings.DataProcessor = &DataProcessor";
	
	Query.SetParameter("DataProcessor", CurrentObject.Ref);
	...
	While Selection.Next() Do
		...
		Catalogs.BankStatementProcessingSettings.UpdateSettingsFromDataProcessor(SettingObject, CurrentObject.Ref);
		SettingObject.Write();
	EndDo;
