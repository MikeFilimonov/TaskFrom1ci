...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	For Each EnumValue In Enums.AdditionalReportsAndDataProcessorsTypes Do
		ChoiceList.Add(EnumValue, String(EnumValue));
	EndDo;
...
Function PublicationChange(PublicationVariant)
	...
	If SelectedRows.Count() = 0 Then
		MessageText = NStr("en = 'No additional report (data processor) is selected'");
	...
	If SelectedRows.Count() = 1 Then
		...
		MessageText = NStr("en = 'Publication of additional report (data processor) ""%1"" is changed'");
		MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessageText, ObjectName);
	Else
		MessageText = NStr("en = 'Publication of %1 additional reports (data processors) is changed'");
