...
Procedure ListBeforeRowChange(Item, Cancel)
	...
	If Items.List.CurrentRow <> Undefined Then
		...
		OpenForm("Catalog.AdditionalValues.ObjectForm", FormParameters, Items.List);
	EndIf;
	...
	If IsBlankString(TitleString) Then
		...
		If ValueIsFilled(Property) Then
			If Not Parameters.ChoiceMode Then
				TitleString = NStr("en = 'Attribute values for %1'");
			ElsIf GroupChoice Then
				TitleString = NStr("en = 'Select a group of values for %1'");
			Else
				TitleString = NStr("en = 'Select attribute value for %1'");
			EndIf;
		...
		ElsIf Parameters.ChoiceMode Then
			...
			If GroupChoice Then
				TitleString = NStr("en = 'Select a group of values'");
			Else
				TitleString = NStr("en = 'Select attribute value'");
			EndIf;
		EndIf;
	EndIf;
...
Procedure OnChangeProperties()
	...
	If ValueIsFilled(Property) Then
		...
		If TypeOf(ValueType) = Type("TypeDescription")
		   AND ValueType.ContainsType(Type("CatalogRef.AdditionalValues")) Then
