...
Function ResultFillManufacturingCalendarDefault(BusinessCalendarCode, YearNumber, Type = "MondayFriday") Export
	...
	BusinessCalendarData.Columns.Add("DayKind", New TypeDescription("EnumRef.BusinessCalendarDayType"));
	...
	If Type = "MondayFriday" Then
		...
		While DateOfDay <= Date(YearNumber, 12, 31) Do
			...
			If WeekDayNumber <= 5 Then
				DayKinds.Insert(DateOfDay, Enums.BusinessCalendarDayType.Working);
			Else 
				DayKinds.Insert(DateOfDay, Enums.BusinessCalendarDayType.Weekend);
			EndIf;
	...
	ElsIf Type = "MondaySaturday" Then
	...
	// If a weekend day and a public holiday match, the weekend day is transferred to the next working day after the
	// holiday except of the weekends that match with public holidays during the New Year holidays and Christmas.
	...
	For Each TableRow In HolidayDays Do
		...
		If Year(DateIncludedDays) <> YearNumber Then
			...
			Continue;
		EndIf;
		If DayKinds[DateIncludedDays] = Enums.BusinessCalendarDayType.Working 
			AND HolidayDays.Find(DateIncludedDays, "Date") = Undefined Then
			DayKinds.Insert(DateIncludedDays, Enums.BusinessCalendarDayType.Preholiday);
		EndIf;
		If DayKinds[HoliDay] <> Enums.BusinessCalendarDayType.Working 
			...
			While True Do
				DateOfDay = DateOfDay + LengthOfDay;
				If DayKinds[DateOfDay] = Enums.BusinessCalendarDayType.Working 
			...
			EndDo;
		EndIf;
		DayKinds.Insert(HoliDay, Enums.BusinessCalendarDayType.Holiday);
	EndDo;
...
Function BusinessCalendarsDataFromXML(Val XML) Export
	...
	DataTable.Columns.Add("DayKind", New TypeDescription("EnumRef.BusinessCalendarDayType"));
	...
	For Each ClassifierRow In ClassifierTable Do
		...
		NewRow.DayKind	= Enums.BusinessCalendarDayType[ClassifierRow.DayType];
...
Function DecorColorsOfBusinessCalendarDayTypes() Export
	...
	AppearanceColors.Insert(Enums.BusinessCalendarDayType.Working,			StyleColors.BusinessCalendarDayTypeWorkColor);
	AppearanceColors.Insert(Enums.BusinessCalendarDayType.Weekend,		StyleColors.BusinessCalendarDayTypeSundayColor);
	AppearanceColors.Insert(Enums.BusinessCalendarDayType.Preholiday,	StyleColors.BusinessCalendarDayTypePreHolidayColor);
	AppearanceColors.Insert(Enums.BusinessCalendarDayType.Holiday,			StyleColors.BusinessCalendarDayTypeHolidayColor);
	
	Return AppearanceColors;
	
EndFunction

// Function makes a list of all possible day kinds of the business calendar by the BusinessCalendarDayType enumeration metadata.
//
// Return
...
Function DayKindList() Export
	...
	For Each DayKindMetadata In Metadata.Enums.BusinessCalendarDayType.EnumValues Do
		DayKindList.Add(Enums.BusinessCalendarDayType[DayKindMetadata.Name], DayKindMetadata.Synonym);
	EndDo;
...
Function PrintingFormBusinessCalendar(ParametersForPrintingForms) Export
	...
	TypesOfNonWorkingDays.Add(Enums.BusinessCalendarDayType.Weekend);
	TypesOfNonWorkingDays.Add(Enums.BusinessCalendarDayType.Holiday);
	...
	While SelectionByYear.Next() Do
		...
		While SelectionByQuarter.Next() Do
			...
			NumberOfQuarter.Parameters.NumberOfQuarter = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'quarter %1'"), SelectionByQuarter.QuarterWeek);
			...
			While SelectionByMonth.Next() Do
				...
				While SelectionByTypeOfDay.Next() Do
					If SelectionByTypeOfDay.DayKind = Enums.BusinessCalendarDayType.Weekend 
						Or SelectionByTypeOfDay.DayKind = Enums.BusinessCalendarDayType.Holiday Then
						 WeekEnd = WeekEnd + SelectionByTypeOfDay.CalendarDays
					 ElsIf SelectionByTypeOfDay.DayKind = Enums.BusinessCalendarDayType.Working Then 
					 ...
					 ElsIf SelectionByTypeOfDay.DayKind = Enums.BusinessCalendarDayType.Preholiday Then
