...
Procedure MoveDay(Command)
	...
	DateChoiceParameters.Insert("Title",					NStr("en = 'Select transfer data'"));
	DateChoiceParameters.Insert("ExplanationText",				StringFunctionsClientServer.SubstituteParametersInString(
																NStr("en = 'Select a date day %1 will be transferred to (%2)'"), 
																Format(DestinationDate, "DF=d MMMM'"), 
																DayKind));
	
	OpenForm("CommonForm.SelectDate", DateChoiceParameters, ThisObject);
...
&AtClient
Procedure FillMondaySaturday(Command)
	
	FillOfDefaultData("MondaySaturday");
...
Procedure FillPresentationTransfers(Form)
	...
	For Each KeyAndValue In Form.TransfersOfDays Do
		// From the practical point of view, it is always the holiday that is shifted to the working day, therefore from two
		// dates we select the one to which the holiday corresponded (now it is the working day that corresponds to it).
		DateSource = KeyAndValue.Key;
		DateTransmitters = KeyAndValue.Value;
		DayKind = Form.DayKinds.Get(DateSource);
		If DayKind = PredefinedValue("Enum.BusinessCalendarDayType.Weekend")Then
		...
		Form.ListTransfers.Add(DateSource, StringFunctionsClientServer.SubstituteParametersInString(
															NStr("en = '%1 (%3) is transferred to %2 (%4)'"),
...
Function KindTransferDayPresentation(DayKind, Date)
	...
	If DayKind = PredefinedValue("Enum.BusinessCalendarDayType.Working") 
		Or DayKind = PredefinedValue("Enum.BusinessCalendarDayType.Holiday") Then
