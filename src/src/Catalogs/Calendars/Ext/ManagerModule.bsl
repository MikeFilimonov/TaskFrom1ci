...
Procedure UpdateWorkSchedulesByFactoryCalendarData(UpdateConditions) Export
	...
	|				OR Calendars.EndDate = DATETIME(1, 1, 1))
	|		LEFT JOIN InformationRegister.WorkScheduleAdjustment AS ManualChangesForAllYears
	...
	WorkScheduleVT = New ValueTable;
	WorkScheduleVT.Columns.Add("DayNumber", 		New TypeDescription("Number", New NumberQualifiers(7)));
	WorkScheduleVT.Columns.Add("BeginTime", 	New TypeDescription("Date", , , New DateQualifiers(DateFractions.Time)));
	WorkScheduleVT.Columns.Add("EndTime",	New TypeDescription("Date", , , New DateQualifiers(DateFractions.Time)));
...
Function DaysIncludedInScheduleForWeek(Year, BusinessCalendar, FillTemplate, ConsiderHolidays, Val StartDate = Undefined, Val EndDate = Undefined)
	...
	While DateOfDay <= EndDate Do
		...
		Else
			...
			HoliDay = ConsiderHolidays AND DataDay.DayKind = Enums.BusinessCalendarDayType.Holiday;
		EndIf;
...
Function DaysAreIncludedInScheduleOfArbitraryLength(Year, BusinessCalendar, FillTemplate, ConsiderHolidays, BeginnigDate, Val StartDate = Undefined, Val EndDate = Undefined)
	...
	DataOnPublicHolidays = BusinessCalendarData.FindRows(New Structure("DayKind", Enums.BusinessCalendarDayType.Holiday));
