...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	...
	If Object.FillTemplate.FindRows(New Structure("DayIncludedInSchedule", True)).Count() = 0 Then
		CommonUseClientServer.MessageToUser(
			NStr("en = 'Select working days for the pattern'"), , "Object.FillTemplate", , Cancel);
	EndIf;
...
Procedure CurrentYearNumberOnChange(Item)
	...
	If ModifiedResults Then
		MessageText = StringFunctionsClientServer.SubstituteParametersInString(
							NStr("en = 'Write the changed data for the year of %1?'"), 
...
Procedure SetConditionalAppearance()
	...
	Item.Appearance.SetParameterValue("Text", NStr("en = 'Set working hours'"));
...
Function ScheduleForDayPresentation(Form, DayNumber)
	...
	If Seconds = 0 Then
		Return NStr("en = 'Set working hours'");
	EndIf;
	
	Hours = Round(Seconds / 3600, 1);
	
	Return StringFunctionsClientServer.SubstituteParametersInString(NStr("en = '%1 h. (%2)'"), Hours, IntervalsPresentation);
...
Procedure ReadSignManualEditing(CurrentObject, YearNumber)
	...
	|FROM
	|	InformationRegister.WorkScheduleAdjustment AS ManualChanges
	|WHERE
...
Procedure WriteSignManualEditing(CurrentObject, YearNumber)
	...
	RecordSet = InformationRegisters.WorkScheduleAdjustment.CreateRecordSet();
...
Procedure FillTextFillResultInformation(Form)
	...
	If Form.ManualEdit Then
		InformationalText = NStr("en = 'The work schedule has user-defined exceptions relatively to the work days calendar. You can restore default values by pressing the ""Restore default"" button.'");
		AvailableFillingPattern = True;
	Else
		If Form.ResultFillByPattern Then
			If ValueIsFilled(Form.Object.BusinessCalendar) Then
				InformationalText = NStr("en = 'The work schedule is updated accordingly to the work days calendar.'");
			EndIf;
		Else
			InformationalText = NStr("en = 'Press ""Restore default"" to see how the work days calendar will affect your work schedule'");
			AvailableFillingPattern = True;
		EndIf;
	EndIf;
	...
	FillInformationTextEdit(Form);
	
EndProcedure

&AtClientAtServerNoContext
Procedure FillInformationTextEdit(Form)
	
	InformationalText = "";
	If Form.ManualEdit Then
		InformationalText = NStr("en = 'The work schedule has user-defined exceptions relatively to the work days calendar. Check the result to see these exceptions (highlighted in grey).'");
	EndIf;
