...
Procedure FillInButtonsTableFromLayout()
	...
	Template.LanguageCode = CommonUseClientServer.MainLanguageCode();
...
Procedure FillByProductsAtServer(ProductsGroup)
	...
	CommonUseClientServer.MessageToUser(NStr("en = 'Filling in is completed.'"));
...
Procedure CopySettingsToSourceFromReceiver(Settings, TheReceiver = False)
	...
	If SourceSettings.Ref = ReceiverSettings.Ref Then
		Message = New UserMessage;
		Message.Text = NStr("en = 'Current settings are selected. Copying is not completed.'");
		Message.Message();
		Return;
	EndIf;
	...
	Message = New UserMessage;
	Message.Text = NStr("en = 'Filling in is completed.'");
	Message.Message();
