...
Procedure HierarchySetDeletionMark(Command)
	...
	NotificationText = StrTemplate(NStr("en = 'Deletion mark %1'"),
		?(DeletionMark, NStr("en = 'is set'"), NStr("en = 'is removed'")));
...
Procedure ReadHierarchy()
	...
	TreeRow.GroupPresentation	= NStr("en = '<All groups>'");
	
	TreeRow = CollectionItems.Add();
	TreeRow.IconIndex			= -1;
	TreeRow.CounterpartyGroup	= "WithoutGroup";
	TreeRow.GroupPresentation	= NStr("en = '<No group>'");
...
Procedure SetFormTitle(Form)
	...
	If Form.FilterCustomer Then
		RelationshipKinds.Add(NStr("en = 'Customers'"));
	EndIf;
	
	If Form.FilterSupplier Then
		RelationshipKinds.Add(NStr("en = 'Suppliers'"));
	EndIf;
...
Function SegmentCounterparties(Segment)
	...
	SegmentContent = Catalogs.Segments.GetSegmentContent(Segment);
...
Procedure RefreshContactInformationPanelServer()
	...
	ContactInformationPanelDrive.RefreshPanelData(ThisObject, CurrentCounterparty);
