...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	CanAddToCatalog = Catalogs.Countries.HasRightToAdd();
...
Procedure OnOpen(Cancel)
	...
	If OpenClassifierForm Then
		...
		OpenForm("Catalog.Countries.Form.Classifier", OpenParameters, FormOwner);
		Cancel = True;
	EndIf;
...
Procedure NotificationProcessing(EventName, Parameter, Source)
	...
	If EventName = "Catalog.Countries.Update" Then
...
Procedure ListBeforeAddRow(Item, Cancel, Clone, Parent, Group)
	...
	If Not Clone Then
		Cancel = True;
		QuestionText = NStr("en = 'Do you want to select a country from classifier?'");
...
Procedure OpenClassifier(Command)
	...
	OpenParameters = New Structure;
	OpenParameters.Insert("CurrentRow", Items.List.CurrentRow);
	OpenForm("Catalog.Countries.Form.Classifier", OpenParameters, Items.List);
...
Procedure SetCatalogAndClassifierJunctionFilter()
	...
	Query = New Query(
		...
		|		INNER JOIN Classifier AS Classifier
		|		ON Countries.Code = Classifier.Code
		|			AND Countries.Description = Classifier.Description");
	
	Query.SetParameter("Classifier", Catalogs.Countries.ClassifierTable());
...
Procedure NotifyClassifierSelectionQuestion(QuestionResult, AdditionalParameters) Export
	...
	ElsIf QuestionResult=DialogReturnCode.No Then
		// Adding an item
		OpenForm("Catalog.Countries.ObjectForm");
