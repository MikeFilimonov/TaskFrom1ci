...
Procedure NextAfterCertificateSearch(Result, NotSpecified) Export
	...
	If Result.Property("CertificateNotFound") Then
		Context.Insert("ErrorDescription", NStr("en = 'ElectronicSignature is not found on the computer (it might have been deleted).'"));
	Else
...
Procedure Add(Command)
	...
	If Not ValueIsFilled(ElectronicSignature) Then
	...
	Notify("Record_DigitalSignaturesAndEncryptionKeyCertificates",
		AdditionalParameters, ElectronicSignature);
...
Procedure ShowPageCertificatePropertiesAdjustment(Form, CryptoCertificate, CertificateData)
	...
	Form.ElectronicSignature             = SavedProperties.Ref;
	...
	String = ?(ValueIsFilled(Form.ElectronicSignature), NStr("en = 'Update calendar'"), NStr("en = 'Add'"));
