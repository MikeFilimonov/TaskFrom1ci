...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	DigitalSignatureOverridable.OnCreatingFormCertificateCheck(Parameters.ElectronicSignature,
	...
	CertificateProperties = CommonUse.ObjectAttributesValues(ElectronicSignature,
...
Procedure CheckOnClientSideAfterConnectingCryptographyExtension(Attached, Context) Export
	...
	// ElectronicSignature data validation.
...
Procedure CheckOnClientSideAfterCertificateInitialization(CryptoCertificate, Context) Export
	...
	// ElectronicSignature availability in personal list.
...
// Continue the procedure CheckOnClientSide.
&AtClient
Procedure CheckOnClientSideAfterSignatureCheck(ElectronicSignature, Context) Export
	
	If ElectronicSignature <> Null Then
		If TypeOf(ElectronicSignature) = Type("String") Then
			ErrorDescription = ElectronicSignature;
		Else
			ErrorDescription = "";
		EndIf;
...
Procedure CheckOnClientSideCycleBeginning(Context)
	...
	ExecuteParameters.Insert("ElectronicSignature",           ElectronicSignature);
...
Procedure CheckOnServerSide(Val PasswordValue)
	...
	For Each ItemOfList In AdditionalChecks Do
		...
		IsWarning = False;
		Try
			DigitalSignatureOverridable.OnAdditionalCertificateVerification(ElectronicSignature,
...
Procedure SetItem(Form, StartElement, AtServer, ErrorDescription = Undefined, IsError = False)
	...
	If ErrorDescription = Undefined Then
		...
		ItemPicture.ToolTip   = NStr("en = 'No check was performed.'");
	...
	Else
		...
		ItemPicture.ToolTip   = NStr("en = 'Check succeeded.'");;
