...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Parameters.InsertIntoList Then
		...
		Items.ShowAll.ToolTip =
			NStr("en = 'Show all certificates without filter (for example, including added and expired)'");
	EndIf;
	...
	If ForEncryptionAndDecryption = True Then
		If Parameters.InsertIntoList Then
			Title = NStr("en = 'Add a certificate for data encryption and decryption'");
		Else
			Title = NStr("en = 'Select a certificate to encrypt and decrypt data'");
		EndIf;
	ElsIf ForEncryptionAndDecryption = False Then
		If Parameters.InsertIntoList Then
			Title = NStr("en = 'Add a certificate for signing data'");
		EndIf;
	ElsIf DigitalSignature.UseEncryption() Then
		Title = NStr("en = 'Add a certificate for signing and encrypting data'");
	Else
		Title = NStr("en = 'Add a certificate for signing data'");
	EndIf;
...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	...
	DigitalSignatureService.CheckPresentationUniqueness(
		CertificateName, ElectronicSignature, "CertificateName", Cancel);
...
Procedure SelectAfterCertificateCheck(Result, Context) Export
	...
	If Result.Property("ErrorDescription") Then
		...
		Else
			FormTitle = NStr("en = 'Digital signature verification'");
		EndIf;
		DigitalSignatureServiceClient.ShowRequestToApplicationError(
	...
	NotifyChanged(ElectronicSignature);
	
	If ReturnPassword Then
		InternalData.Insert("SelectedCertificate", ElectronicSignature);
		If Not RememberPassword Then
			InternalData.Insert("SelectedCertificatePassword", PasswordProperties.Value);
		EndIf;
		NotifyChoice(True);
	Else
		NotifyChoice(ElectronicSignature);
	EndIf;
...
Procedure GoToCurrentCertificateChoice(Notification)
	...
	If CertificateAtServer Then
		...
		Else
			Result.ErrorDescription = NStr("en = 'ElectronicSignature is not found on the server (it might have been deleted).'");
			Result.UpdateCertificatesList = True;
			ExecuteNotifyProcessing(Notification, Result);
		EndIf;
		Return;
...
Procedure GoToCurrentCertificateChoiceAfterCertificateSearch(SearchResult, Context) Export
	...
	If TypeOf(SearchResult) <> Type("CryptoCertificate") Then
		If SearchResult.Property("CertificateNotFound") Then
			Context.Result.ErrorDescription = NStr("en = 'ElectronicSignature is not found on the computer (it might have been deleted).'");
		Else
...
Procedure GoToCurrentCertificateChoiceAfterFillingCertificateProperties(Context)
	...
	ElectronicSignature             = Context.SavedProperties.Ref;
	...
	If InsertIntoList Then
		String = ?(ValueIsFilled(ElectronicSignature), NStr("en = 'Update calendar'"), NStr("en = 'Add'"));
...
Procedure CheckCertificateAfterCreatingCryptographyManager(Result, Context) Export
	...
	If TypeOf(Result) <> Type("CryptoManager")
	   AND Result.Common Then
		...
		If ForEncryptionAndDecryption = True Then
			Result.Insert("ErrorTitle", NStr("en = 'Cannot pass the encryption check due to:'"));
		Else
			Result.Insert("ErrorTitle", NStr("en = 'Cannot pass the signing check due to:'"));
		EndIf;
...
Function CheckCertificateAndWriteInCatalog(Val PasswordValue, ErrorOnServer)
	...
	If DigitalSignatureService.CryptoManager("", False, ErrorOnServer) = Undefined
	   AND ErrorOnServer.Common Then
		...
		If ForEncryptionAndDecryption = True Then
			ErrorOnServer.Insert("ErrorTitle", NStr("en = 'Cannot pass the encryption check due to:'"));
		Else
			ErrorOnServer.Insert("ErrorTitle", NStr("en = 'Cannot pass the signing check due to:'"));
		EndIf;
		Return False;
	EndIf;
