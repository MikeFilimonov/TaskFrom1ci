...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If SpecifiedCertificatesSet AND Not Parameters.ChangeSet Then
		Items.ElectronicSignature.Visible = False;
...
Procedure CertificateStartChoice(Item, ChoiceData, StandardProcessing)
	...
	DigitalSignatureClient.CertificateStartChoiceWithConfirmation(Item,
		ElectronicSignature, StandardProcessing, True);
...
Procedure EncryptionCertificatesChoiceProcessing(Item, ValueSelected, StandardProcessing)
	...
	For Each Value In ValueSelected Do
		Filter = New Structure("ElectronicSignature", Value);
		Rows = EncryptionCertificates.FindRows(Filter);
		If Rows.Count() > 0 Then
			Continue;
		EndIf;
		EncryptionCertificates.Add().ElectronicSignature = Value;
	EndDo;
...
Procedure OpenCertificate(Command)
	...
	If Items.EncryptionVariants.CurrentPage = Items.PickFromCatalog Then
		DigitalSignatureClient.OpenCertificate(CurrentData.ElectronicSignature);
	Else
...
Procedure FillEncryptionCertificatesFromSet(CertificatesSetDescription)
	...
	If CommonUse.IsReference(TypeOf(CertificatesSetDescription)) Then
		...
		Query.Text =
		"SELECT
		|	SpeadsheetPartEncryptionCertificates.ElectronicSignature AS ElectronicSignature
		|FROM
		...
		While Selection.Next() Do
			CertificatesArray.Add(Selection.ElectronicSignature.Get());
		EndDo;
	Else
		...
		For Each ElectronicSignature In CertificatesArray Do
			If TypeOf(ElectronicSignature) = TypeOf("CatalogRef.DigitalSignaturesAndEncryptionKeyCertificates") Then
				If AddedCertificates.Get(ElectronicSignature) = Undefined Then
					AddedCertificates.Insert(ElectronicSignature, True);
					EncryptionCertificates.Add().ElectronicSignature = ElectronicSignature;
				EndIf;
			Else
		...
		For Each String In Rows Do
			...
			If TypeOf(CertificateData) <> Type("BinaryData") Then
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'The ""%1"" certificate data is not found in catalog'"), Selection.Presentation);
			EndIf;
			Try
	...
	If AllCertificatesInCatalog Then
		For Each String In CertificatesTable Do
			EncryptionCertificates.Add().ElectronicSignature = String.Ref;
		EndDo;
	Else
		...
		For Each String In CertificatesTable Do
			...
			Properties.Insert("ElectronicSignature",    String.Data);
...
Procedure EncryptData(Notification)
	...
	If CertificatesSet.Count() = 0 Then
		...
		If Items.ElectronicSignature.Visible AND ValueIsFilled(ElectronicSignature) Then
			Refs.Add(ElectronicSignature);
			ExcludePersonalCertificate = True;
		EndIf;
		For Each String In EncryptionCertificates Do
			If Not ExcludePersonalCertificate Or String.ElectronicSignature <> ElectronicSignature Then
				Refs.Add(String.ElectronicSignature);
			EndIf;
		EndDo;
...
Procedure EncryptDataAfterCompletionOnClientSide(Result, Context) Export
	...
	If NotifyAboutExpiration Then
		FormParameters = New Structure("ElectronicSignature", ElectronicSignature);
...
Procedure EncryptDataAfterCompletion(Result)
	...
	If Result.Property("ThereAreProcessedDataItems") Then
		...
		Items.ElectronicSignature.ReadOnly = True;
		Items.EncryptionCertificates.ReadOnly = True;
	EndIf;
...
Function CertificatesProperties(Val Refs, Val FormID)
	...
	While Selection.Next() Do
		...
		If TypeOf(CertificateData) <> Type("BinaryData") Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'The ""%1"" certificate data is not found in catalog'"), Selection.Description);
		EndIf;
		...
		Properties.Insert("ElectronicSignature",    CertificateData);
...
Procedure WriteEncryptionCertificatesAtServer(ObjectsDescription, CertificatesAddress, FormID, Error)
	...
	Except
		ErrorInfo = ErrorInfo();
		RollbackTransaction();
		Error.Insert("ErrorDescription", NStr("en = 'An error occurred when writing encryption certificates:'")
