...
Procedure UsersGroupOnChangingOnServer()
	...
	If UsersGroupUse Then
		...
		|WHERE
		|	Certificates.RequestStatus IN (VALUE(Enum.CertificateIssueRequestStatus.EmptyRef), VALUE(Enum.CertificateIssueRequestStatus.Executed))
		|	AND TRUE In
		|			(SELECT TOP 1
		|				TRUE
		|			IN
		...
		|			WHERE
		|				UserGroupMembers.User = Certificates.User
		|				AND UserGroupMembers.UsersGroup IN (&UsersGroup))";
