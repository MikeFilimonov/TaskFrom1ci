...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Else
		SetAgreement();
		Object.RequestStatus = Enums.CertificateIssueRequestStatus.NotPrepared;
...
Procedure NotificationProcessing(EventName, Parameter, Source)
	...
	If Source = ChiefExecutiveOfficer
...
Procedure OnClose(Exit)
	...
	If Object.RequestStatus = PredefinedValue("Enum.CertificateIssueRequestStatus.Prepared")
		OR Object.RequestStatus = PredefinedValue("Enum.CertificateIssueRequestStatus.NotPrepared")
...
Procedure ActualAddressStartChoice(Item, ChoiceData, StandardProcessing)
	...
	AddressPresentationSelectionStart(ThisObject, Item, ChoiceData, StandardProcessing,
		"ActualAddress", NStr("en = 'Physical address of the counterparty'"));
...
Procedure DecorationKeyGenerationNavigationRefProcessing(Item, URL, StandardProcessing)
	...
	ElsIf URL = "CertificateQuery" Then
		ShowMessageBox(,
			NStr("en = 'ElectronicSignature request is not the secret information. It is created based on the digital signature key, sent together with the certificate issue application and required to issue certificate.'"));
	EndIf;
	...
	If ThisIsIndividualEntrepreneur Then
		WarningText =
			NStr("en = 'Set
			     ...
			     |2. Copy of the Tax Authority Registration ElectronicSignature (TIN).
			     |3. Copy of the identification document of certificate owner.'");
	Else
		WarningText =
			NStr("en = 'Set of documents:
			     |1. Application for certificate issue (prepared in the previous step).
			     |2. Copy* of the Tax Authority Registration ElectronicSignature (TIN).
			     ...
			     | the CEO who signed the statement (minutes of founders meeting, decision of the owner, company charter) or relevant extract from the Unified State Register of Legal Entities certified by tax authority.
			     |
			     |* Copies of documents are certified with CEO''''s signature and company seal.'");
	EndIf;
...
Procedure ExportRootCertificate(Command)
	...
	DigitalSignatureServiceClient.SaveCertificate(, RootCertificateAddress,
		NStr("en = 'Root certificate of 1C RDC LLC'"));
...
Procedure PrintDocuments(Command)
	...
	PrintedFormName = NStr("en = 'Application for certificate issue'");
	...
	PrintManagementModuleClient.PrintDocuments(PrintFormsCollection, ObjectAreas);
...
Procedure GoToPageStatementSending()
	...
	If Not ValueIsFilled(Object.Application) Then
		CommonUseClientServer.MessageToUser(
			NStr("en = 'The ""Digital signature application"" field is not populated'"), , "Application");
		Return;
	EndIf;
...
Procedure GoToPageStatementSendingAfterCreatingCryptographyManager(Result, Context) Export
	...
	If TypeOf(Result) <> Type("CryptoManager") Then
		...
		DigitalSignatureServiceClient.ShowRequestToApplicationError(
			NStr("en = 'Create DS key'"),, Result, New Structure);
		Return;
	EndIf;
...
Procedure GoToPageStatementSendingOnServer()
	...
	Object.RequestStatus = Enums.CertificateIssueRequestStatus.Prepared;
...
Function GoToPageStatementProcessingPendingOnServer(ErrorDescription)
	...
	Object.RequestStatus = Enums.CertificateIssueRequestStatus.Sent;
	
	UpdateDateConditions = CurrentSessionDate();
	RequestProcessingState = NStr("en = 'Application is accepted for processing.'");
...
Procedure UpdateUpdateDateStatesInTitle(Form)
	...
	Form.Items.RequestProcessingState.Title = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = 'Request processing state (on %1)'"),
...
Procedure GoToPageCertificateSetup()
	...
	If Result <> True Then
		AfterWrite(NStr("en = 'Application is rejected.'"));
		Close();
		Return;
	EndIf;
...
Procedure GoToPageCertificateSetupAfterInstallation(Installed, NoSpecified) Export
	...
	If Installed Then
		...
		AfterWrite(NStr("en = 'Application is executed.'"));
		Close();
	Else
		AfterWrite();
		WriteStatement();
...
Function GoToPageCertificateSetupOnServer()
	...
	If Result = True Then
		Object.RequestStatus = Enums.CertificateIssueRequestStatus.ExecutedCertificateNotInstalled;
	Else
		Object.RequestStatus = Enums.CertificateIssueRequestStatus.Rejected;
	EndIf;
...
Procedure UpdateCertificateInstallationDateInHeader(Form)
	...
	Form.Items.ErrorCertificateSetup.Title = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = 'An error occurred when installing certificate (to %1)'"),
...
Procedure CloseAssistantAfterCertificateInstallationEnd(Installed, NoSpecified) Export
	...
	AfterWrite(NStr("en = 'ElectronicSignature is installed.'"));
	Close();
...
Procedure CloseAssistantAfterCertificateInstallationOnServer()
	...
	Object.RequestStatus = Enums.CertificateIssueRequestStatus.Executed;
	...
	Try
		...
		If DetailedResult.Result <> "Correct" Then
			...
			If Not ValueIsFilled(Message) Then
				Message = NStr("en = 'Address is not filled in'");
			EndIf;
		EndIf;
	Except
		ErrorInfo = ErrorInfo();
...
Function BankAccountMeetsRequirements(BankAccount, MessageText)
	...
	If Not StringFunctionsClientServer.OnlyNumbersInString(Value) Then
		MessageText = NStr("en = 'Account number can contain only digits.'");
		Return False;
	EndIf;
	
	If StrLen(Value) <> 20 Then
		MessageText = NStr("en = 'Account number consists of 20 digits.'");
		Return False;
	EndIf;
...
Function BICMeetsRequirements(BIN, MessageText)
	...
	If Not StringFunctionsClientServer.OnlyNumbersInString(Value) Then
		MessageText = NStr("en = 'Branch ID must contain only digits.'");
		Return False;
	EndIf;
	
	If StrLen(Value) <> 9 Then
		MessageText = NStr("en = 'Branch ID must consist of 9 digits.'");
		Return False;
	EndIf;
...
Function CorrespondentBankAccountMeetsRequirements(CorrespondentAccount, MessageText)
	...
	If Not StringFunctionsClientServer.OnlyNumbersInString(Value) Then
		MessageText = NStr("en = 'Correspondent account should contain only digits.'");
		Return False;
	EndIf;

	If StrLen(Value) <> 20 Then
		MessageText = NStr("en = 'Correspondent account should contain 20 digits.'");
		Return False;
	EndIf;
...
Procedure OnChangingCertificateOwnerOnServer(Import = False)
	...
	If Not ValueIsFilled(OwnerKind) Then
		OwnerKind = "ChiefExecutiveOfficer";
	EndIf;
	
	Attributes = New Structure;
	Attributes.Insert("OwnerKind", OwnerKind);
	Attributes.Insert("ChiefExecutiveOfficer");
	...
	Else
		...
		List.Clear();
		Items.ChiefExecutiveOfficer.Visible = False;
		Items.ChiefAccountant.Visible = False;
		
		If ThisIsIndividualEntrepreneur Then
			OwnerKind = "Employee";
		Else
			If Attributes.ChiefExecutiveOfficer <> Undefined Then
				List.Add("ChiefExecutiveOfficer", NStr("en = 'ChiefExecutiveOfficer'"));
				Items.ChiefExecutiveOfficer.Visible = True;
				
			ElsIf OwnerKind = "ChiefExecutiveOfficer" Then
		...
		ChiefExecutiveOfficer         = Attributes.ChiefExecutiveOfficer;
...
Function RFPassportNumberMeetsRequirements(Val RFPassportNumber, MessageText)
	...
	If IsBlankString(RowOfDigits) Then
		MessageText = MessageText + NStr("en = 'Passport number is not specified'");
		Return False;
	EndIf;
	
	If StrLen(RowOfDigits) < 10 Then
		MessageText  =  MessageText + NStr("en = 'Incorrect passport number'");
		Return False;
	EndIf;
	
	If Not StringFunctionsClientServer.OnlyNumbersInString(RowOfDigits) Then
		Result = False;
		MessageText = MessageText + NStr("en = 'Passport number must contain only digits.'");
	EndIf;
...
Function ValidateAttribute(Cancel, AttributeName)
	...
	If Not ValueIsFilled(ThisObject[AttributeName]) Then
		CommonUseClientServer.MessageToUser(StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'The ""%1"" field is not filled in'"), TitleFields), , AttributeName, , Cancel);
...
Function PreparedDocument()
	...
	Template.Parameters.IdentityCard = DocumentViewPresentation + " " + DocumentNumberPresentation + " "
		...
		+ NStr("en = 'issued'") + " " + DocumentWhoIssued;
...
Function SendStatement(ErrorDescription)
	...
	If ResultCode = "202" Then
		ErrorDescription = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Cannot check the contract for ITS (its.1c.ru) due to: The ticket received for the %1 user is incorrect or obsolete. Try to send the statement again.'"),
			AuthenticationParametersOnSite.Login);
		
	ElsIf ResultCode = "206" Then
		ErrorDescription = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Statement is not accepted due to: User %1 does not have a valid contract for ITS (its.1c.en).
...
Function TicketToSupportSite(ErrorDescription)
	...
	Except
		...
		If Find(BriefErrorDescription, "IncorrectLoginOrPasswordExceptionApi") > 0 Then
			BriefErrorDescription = NStr("en = 'Incorrect user name or password.'");
			AuthenticationParametersOnSite = Undefined;
		EndIf;
		ErrorDescription = StringFunctionsClientServer.SubstituteParametersInString(
...
Function GetCertificate()
	...
	ErrorDescriptionTemplate =
		NStr("en = 'Cannot update the state due to: %1'");
	...
	ElsIf ResultCode = "1" Then
		ErrorDescription = NStr("en = 'The application has not been processed yet, try again later.'");
	Else
		DOMNode = DOMBuilder.GetElementByTagName("errorMessage");
		If DOMNode.Count() > 0 Then
			ErrorDescription = DOMNode[0].TextContent;
		Else
			ErrorDescription = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Server returned error code: %1'"), String(ResultCode));
		EndIf;
	EndIf;
	...
	Try
		...
		If Not FileFound Then
			Raise NStr("en = 'Invalid data format.'");
		EndIf;
	Except
		ErrorInfo = ErrorInfo();
		ErrorDescription = StringFunctionsClientServer.SubstituteParametersInString(
	...
	Try
		...
		For Counter = 0 To archive.Items.Count() - 1 Do
			...
			If archive.Items[Counter].Extension = "xml" Then
				...
				DOMNode = DOMDocument.GetElementByTagName("ElectronicSignature");
	...
	SubjectProperties     = DigitalSignatureClientServer.CertificateSubjectProperties(ElectronicSignature);
	...
	Object.Description = DigitalSignatureClientServer.CertificatePresentation(ElectronicSignature);
	
	Object.Signing = ElectronicSignature.UseToSign;
	Object.Encryption = ElectronicSignature.UseForEncryption;
...
Procedure SetCertificateAfterCreatingCryptographyManager(Manager, Context) Export
	...
	If TypeOf(Manager) <> Type("CryptoManager") Then
		DigitalSignatureServiceClient.ShowRequestToApplicationError(
			NStr("en = 'Install the certificate on this computer'"),, Manager, New Structure);
...
Procedure SetCertificateWhenCryptographyObjectCreationErrorAppearsAfterConnectingFilesExtension(Attached, Context) Export
	...
	If Not Attached Then
		ErrorCertificateSetup = NStr("en = 'File operation extension is not installed.'");
	Else
...
Procedure SetCertificateWhenCryptographyObjectCreationErrorAppearsAfterConnectingCryptographyExtension(Attached, Context) Export
	...
	If Not Attached Then
		ErrorCertificateSetup = NStr("en = 'Cryptography operation extension is not installed.'");
	Else
...
Procedure SetCertificateAfterGettingPermission(PermissionsReceived, Context) Export
	...
	If Not PermissionsReceived Then
		ErrorCertificateSetup =
			NStr("en = 'Saving certificates to the temporary folder is canceled by user.'");
		ExecuteNotifyProcessing(Context.Notification, False);
		Return;
	EndIf;
...
Procedure SetCertificateAfterReceivingFiles(ReceivedFiles, Context) Export
	...
	If ReceivedFiles = Undefined
	 Or ReceivedFiles.Count() <> 2 Then
		...
		ErrorCertificateSetup = NStr("en = 'Certificates were not saved into a temporary folder.'");
...
Procedure SetCertificateAfterCallSetCertificateToContainerAndStorage(Result, CallParameters, Context) Export
	...
	If Result <> True AND Not ValueIsFilled(ErrorCertificateSetup) Then
		ErrorCertificateSetup = NStr("en = 'Cannot install a personal certificate.'");
	EndIf;
...
Procedure SetCertificateAfterCertificateSearch(Result, Context) Export
	...
	If TypeOf(Result) <> Type("CryptoCertificate") Then
		If Result.Property("CertificateNotFound") Then
			SearchError =
				NStr("en = 'Cannot find a personal certificate installed on the computer.'");
		Else
...
Procedure SetCertificateAfterCallingImportCertificate(Result, CallParameters, Context) Export
	...
	If Result <> True AND Not ValueIsFilled(ErrorCertificateSetup) Then
		ErrorCertificateSetup = NStr("en = 'Cannot install the root certificate.'");
	EndIf;
...
Procedure SetCertificateAfterRootCertificateSearch(Result, Context) Export
	...
	If TypeOf(Result) <> Type("CryptoCertificate") Then
		If Result.Property("CertificateNotFound") Then
			SearchError =
				NStr("en = 'Cannot find a root certificate installed on the computer.'");
		Else
			SearchError = StringFunctionsClientServer.SubstituteParametersInString(
...
Procedure ImportStatement()
	...
	If Content.Property("ElectronicSignature")
	   AND TypeOf(Content.ElectronicSignature) = Type("BinaryData") Then
		
		CertificateAddress = PutToTempStorage(Content.ElectronicSignature, UUID);
	EndIf;
	...
	If CurrentObject.RequestStatus = Enums.CertificateIssueRequestStatus.NotPrepared Then
		GoToPageCertificateQueryPreparationOnServerOnImport();
		
	ElsIf CurrentObject.RequestStatus = Enums.CertificateIssueRequestStatus.Prepared Then
		GoToPageStatementSendingOnServerOnImporting();
		
	ElsIf CurrentObject.RequestStatus = Enums.CertificateIssueRequestStatus.Sent Then
		GoToPagePendingStatementProcessingOnServerOnImport();
		
	ElsIf CurrentObject.RequestStatus = Enums.CertificateIssueRequestStatus.ExecutedCertificateNotInstalled Then
		GoToPageCertificateSetupOnServerOnImporting();
		
	ElsIf CurrentObject.RequestStatus = Enums.CertificateIssueRequestStatus.Rejected Then
		AutoTitle = False;
		Title = NStr("en = 'The application using which the certificate was not received'");
		Items.FormNext.Visible = False;
		Items.FormClose.Title = NStr("en = 'Close'");
	...
	ElsIf CurrentObject.RequestStatus = Enums.CertificateIssueRequestStatus.Executed Then
		AutoTitle = False;
		Title = NStr("en = 'Application which was used to get the certificate'");
...
Procedure FillAttributesStatements(All = False)
	...
	FillAttribute(NStr("en = 'Company information'"),, True);
	...
	FillAttribute(NStr("en = 'ElectronicSignature owner information'"),, True);
	...
	String = RequestAttributes.Add();
	String.Attribute = NStr("en = 'Identity document'");
	...
	If Not ThisIsIndividualEntrepreneur Then
		...
		String = RequestAttributes.Add();
		String.Attribute = NStr("en = 'Full name of the manager'");
		String.Value = DocumentsHead;
		
		String = RequestAttributes.Add();
		String.Attribute = NStr("en = 'Manager position'");
		String.Value = DocumentsHeadPosition;
		
		String = RequestAttributes.Add();
		String.Attribute = NStr("en = 'Manager action basis'");
		String.Value = DocumentsHeadBasis;
	EndIf;
	
	// Completion of information about printed documents.
	FillAttribute(NStr("en = 'Information on service company'"),, True);
...
Function AttributePresentationDocumentIssueDate(DocumentIssueDate)
	...
	Return Format(DocumentIssueDate, "DLF=D");
...
Procedure SetAgreement()
	...
	HTMLText = StringFunctionsClientServer.SubstituteParametersInString(HTMLText,
		...
		NStr("en = 'authorized federal authority on the use of digital signature'"));
...
Procedure AfterSelectingApplicationAfterCreatingCryptographyManager(Manager, Context) Export
	...
	If TypeOf(Manager) <> Type("CryptoManager") Then
		...
		If Context.ShowError Then
			DigitalSignatureServiceClient.ShowRequestToApplicationError(
				NStr("en = 'Select a digital signature application'"),, Manager, New Structure);
		EndIf;
...
Procedure WriteStatement()
	...
	If CurrentObject.RequestStatus = Enums.CertificateIssueRequestStatus.NotPrepared Then
		CurrentObject.Description = Surname + " " + Name + " " + Patronymic + " (" + NStr("en = 'Application for new certificate'") + ")";
		// Company.
		...
		UpdateValue(Content, "ChiefExecutiveOfficer");
	...
	ElsIf CurrentObject.RequestStatus = Enums.CertificateIssueRequestStatus.ExecutedCertificateNotInstalled Then
		If ThisIsNewState Then
			...
			UpdateValue(Content, "ElectronicSignature",         GetFromTempStorage(CertificateAddress));
			UpdateValue(Content, "RootCertificate", GetFromTempStorage(RootCertificateAddress));
			UpdateValue(Content, "SubscriberIdentifier");
			UpdateValue(Content, "UpdateDateConditions");
			UpdateValue(Content, "RequestProcessingState");
			If CurrentObject.CertificateData.Get() <> Content.ElectronicSignature Then
				CurrentObject.CertificateData = New ValueStorage(Content.ElectronicSignature);
...
Procedure AfterWrite(NotificationText = "")
	...
	If Not ValueIsFilled(NotificationText) Then
		NotificationText = NStr("en = 'Application is saved.'");
	EndIf;
...
Procedure CreateKeyAndCertificateQuery(Notification)
	...
	Else
		CreateCryptographyObject(New NotifyDescription(
			...
			NStr("en = 'To create a digital signature key and a certificate request, install an additional external component.'"));
	EndIf;
...
Procedure CreateKeyAndCertificateQueryAfterKeyContainerExistenceCheck(Exists, Context) Export
	...
	If Exists Then
		ShowUserNotification(NStr("en = 'Previously created key container is used:'"),,
			KeyContainerPath);
...
Procedure CreateKeyAndCertificateQueryAfterCallErrorCreateContainer(ErrorInfo, StandardProcessing, Context) Export
	...
	DigitalSignatureServiceClient.ShowRequestToApplicationError(
		NStr("en = 'Create DS key'"),, Error, New Structure);
...
Procedure CreateKeyAndCertificateQueryAfterCallErrorCreateCertificateQuery(ErrorInfo, StandardProcessing, Context) Export
	...
	CreateKeyAndCertificateQueryEnd(Context, ErrorInfo,
		NStr("en = 'Cannot execute the operation due to:'"));
...
Procedure CreateKeyAndCertificateQueryAfterCallCreateCertificateQuery(Result, CallParameters, Context) Export
	...
	File = New File(Context.QueryFileName);
	CreateKeyAndCertificateQueryAfterFileInitialization(File, Context);
	
EndProcedure

// Continue the procedure CreateKeyAndCertificateQuery.
&AtClient
Procedure CreateKeyAndCertificateQueryAfterFileInitialization(File, Context)
...
Procedure CreateKeyAndCertificateQueryAfterCallErrorGetOpenKey(ErrorInfo, StandardProcessing, Context) Export
	...
	CreateKeyAndCertificateQueryEnd(Context, ErrorInfo,
		NStr("en = 'Cannot receive a public key part due to:'"));
...
Procedure CreateKeyAndCertificateQueryAfterCallErrorCalculateEntityKeyIdentifier(ErrorInfo, StandardProcessing, Context) Export
	...
	CreateKeyAndCertificateQueryEnd(Context, ErrorInfo,
		NStr("en = 'Cannot calculate the subject key ID due to:'"));
...
Procedure CreateKeyAndCertificateQueryAfterFilesPlacement(PlacedFiles, Context) Export
	...
	RequestDate = Format(CommonUseClient.SessionDate(), "DLF=D");
...
Procedure CreateKeyAndCertificateQueryEnd(Context, ErrorInfo = Undefined, ErrorTitle = "")
	...
	If ErrorInfo <> Undefined Then
		...
		DigitalSignatureServiceClient.ShowRequestToApplicationError(
			NStr("en = 'Create certificate request'"),, Error, New Structure);
		
	ElsIf Not Context.QueryCreated Then
		ShowMessageBox(, NStr("en = 'ElectronicSignature request is not created.'"));
	EndIf;
...
Procedure CreateCryptographyObjectAfterConnectingExternalComponent(Connected, Context) Export
	...
	Else
		...
		Buttons.Add("DoNotInstall", NStr("en = 'Do not set'"));
	...
	If Not Connected Then
		ShowMessageBox(
			New NotifyDescription("CreateCryptographyObjectAfterErrorWarning",
				ThisObject, Context),
			NStr("en = 'Cannot connect an additional external component.'"));
	Else
		CreateCryptographyObjectContinuation(Context);
	EndIf;
...
Procedure CreateCryptographyObjectAfterInstallationDoNotShowErrorMessages(Context) Export
	...
	PathToApplication = String(DigitalSignatureClientServer.PersonalSettingsTip(
