...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	InformationSimpCurrency = Constants.PresentationCurrency.Get();
...
Procedure FillInInformationOnDiscountCard()
	...
	DiscountPercentByDiscountCard = DriveServer.CalculateDiscountPercentByDiscountCard(CurrentDate(), Object, AdditionalParameters);
	...
	If ValueIsFilled(AdditionalParameters.PeriodPresentation) Then
		Items.SalesAmountOnDiscountCard.Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Sales amount (%1)'"), AdditionalParameters.PeriodPresentation);
	Else
		Items.SalesAmountOnDiscountCard.Title = NStr("en = 'Sales amount'");
	EndIf;
	...
	If ReceivedCodeType = PredefinedValue("Enum.CardCodesTypes.MagneticCode") Then
		...
		Else
			...
			ElsIf Result.Count() = 0 Then
				...
				EndIf;
			Else
				ValueSelected = Result.ChooseItem(NStr("en = 'Select magnetic stripe card code'"));
