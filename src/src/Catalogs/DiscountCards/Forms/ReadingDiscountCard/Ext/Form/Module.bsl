...
Function WriteDiscountCard()
	...
	If CheckFilling() Then
		Try
			...
			Return True;
		Except
			CommonUseClientServer.MessageToUser(ErrorDescription(), ThisObject); 
			Return False;
		EndTry;			
	Else
		Return False;
...
Function HandleReceivedCodeOnServer(Data, CardCodeType, Preprocessing, ThereAreFoundCards = False)
	...
	For Each TSRow In DiscountCards.RegisteredDiscountCards Do
		...
		NewRow.Description = String(TSRow.Ref) + ?(ValueIsFilled(TSRow.Counterparty) AND ValueIsFilled(TSRow.Ref), StringFunctionsClientServer.SubstituteParametersInString(" " + NStr("en = 'Client: %1'"), String(TSRow.Counterparty)), "");
	...
	If DiscountCards.RegisteredDiscountCards.Count() = 0 Then
		For Each TSRow In DiscountCards.NotRegisteredDiscountCards Do
			...
			NewRow.Description = StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'Register a new card: %1'"), String(TSRow.CardKind))+?(TSRow.ThisIsMembershipCard, " (Named, ", " (")+TSRow.CardType+")";
...
Procedure HandleReceivedCodeOnClient(Data, ReceivedCodeType, Preprocessing)
	...
	If Not Result Then
		...
		If CodeType = PredefinedValue("Enum.CardCodesTypes.Barcode") Then
			MessageText = NStr("en = 'Card with the ""%1"" barcode is not registered and there is no suitable discount card kind.'");
		Else
			MessageText = NStr("en = 'Card with the ""%1"" magnetic code is not registered and there is no suitable discount card kind.'");
		EndIf;
