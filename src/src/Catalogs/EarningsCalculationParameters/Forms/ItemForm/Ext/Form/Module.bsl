...
Procedure GenerateQueryFillParameters()
	...
	For Each FilterItem In DataFilter.Settings.Filter.Items Do
		If Object.QueryParameters.FindRows(New Structure("Name", String(FilterItem.LeftValue))).Count() = 0 Then
			...
			NewRow.ComparisonType = CommonUse.ValueToXMLString(FilterItem.ComparisonType);
...
Function QueryCorrect()
	...
	Try
        ...
        If QueryBuilder.Dimensions.Count() > 0 Then
			...
			Message.Text = NStr("en = 'Query should not contain totals.'");
			Message.Message();			
			Return False;
		EndIf; 
		If QueryBuilder.SelectedFields.Count() > 1 Then
		    Message = New UserMessage();
			Message.Text = NStr("en = 'The request cannot contain more than one indicator.'");
			Message.Message();			
			Return False;
		EndIf;
		Return True;
...
Function CheckForIDDuplication(Cancel)
	...
	If Cancel Then
	  	...
	  	Message = New UserMessage();
		Message.Text = NStr("en = 'The calculation parameter with the same ID already exists.'");
		Message.Message();
	...
	If Not Object.CustomQuery Then
		...
		For Each ParameterString In Object.QueryParameters Do
			...
			If ParameterString.Name <> "PointInTime"
				...
				Else
				    FilterItem.ComparisonType = CommonUse.ValueFromXMLString(ParameterString.ComparisonType);
				EndIf; 
...
Procedure SpecifyValueAtPayrollCalculationOnChange(Item)
	...
	If Object.SpecifyValueAtPayrollCalculation Then
		...
		MessageText = NStr("en = 'After the check box is selected, all attributes will be cleared. Continue?'");
...
Procedure CreateFixedAmount(Command)
	...
	ShowQueryBox(Notification,NStr("en = 'Calculation parameter will be completely repopulated. Continue?'"), QuestionDialogMode.YesNo, 0);
