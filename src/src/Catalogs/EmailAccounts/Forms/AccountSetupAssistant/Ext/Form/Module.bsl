...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Items.AccountSetupTitle.Title = ?(ContextMode,
		NStr("en = 'To send emails, configure the email account'"),
...
Procedure ShowQuestionBoxBeforeClosingForm()
	QuestionText = NStr("en = 'Entered data is not written. Close the form?'");
	NotifyDescription = New NotifyDescription("FormClosingConfirmed", ThisObject);
	Buttons = New ValueList;
	Buttons.Add("Close", NStr("en = 'Close'"));
	Buttons.Add(DialogReturnCode.Cancel, NStr("en = 'Keep open'"));
	ShowQueryBox(NotifyDescription, NStr("en = 'Entered data is not written. Close the form?'"), Buttons,,
		DialogReturnCode.Cancel, NStr("en = 'Account setting'"));
EndProcedure
...
Procedure CheckFillingOnPageAccountSetting(Cancel)
	...
	ElsIf Not CommonUseClientServer.EmailAddressMeetsRequirements(EmailAddress, True) Then
		CommonUseClientServer.MessageToUser(NStr("en = 'Email address is invalid'"), , "EmailAddress", , Cancel);
	EndIf;
	
	If IsBlankString(PasswordForReceivingEmails) Then
		CommonUseClientServer.MessageToUser(NStr("en = 'Enter a password'"), , "PasswordForReceivingEmails", , Cancel);
	EndIf;
...
Procedure SetCurrentPageItems()
	...
	Else
		If CurrentPage = Items.AccountSetup
			AND CheckingCompletedWithErrors Then
				NextButtonTitle = NStr("en = 'Repeat'");
		ElsIf CurrentPage = Items.AccountSetup
			AND SettingsMethod = "automatically" Then
			If ContextMode Then
				NextButtonTitle = NStr("en = 'Customize'");
			Else
				NextButtonTitle = NStr("en = 'Generate'");
			EndIf;
		Else
	...
	If CurrentPage = Items.AccountConfiguredSuccessfully Then
		Items.LabelAccountConfiguredSuccessfully.Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Account %1 parameter setting is complete.'"), EmailAddress);
	EndIf;
...
Function CheckConnectionToOutgoingMailServer()
	...
	Body = NStr("en = 'This message is sent by subsystem of 1C:Enterprise email'");
...
Procedure CheckAccountSettings()
	...
	If Not IsBlankString(OutgoingMailServerMessage) Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Cannot connect to the outgoing email server: 
			     |%1'"),
			OutgoingMailServerMessage);
	EndIf;
	...
	If Not IsBlankString(IncomingMailServerMessage) Then
		ErrorText = ErrorText +
			StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Cannot connect to the incoming email server:
				     |%1'"),
				IncomingMailServerMessage);
	EndIf;
