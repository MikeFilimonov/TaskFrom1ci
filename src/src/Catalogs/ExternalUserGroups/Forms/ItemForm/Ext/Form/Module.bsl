		...
		If String.Check AND Left(String.Synonym, 1) = "?" Then
			CommonUseClientServer.AddUserError(Errors,
				...
				StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'Role ""%1"" was not found in metadata.'"),
					String.Synonym),
				"Roles",
				TreeItems.IndexOf(String),
				StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'The ""%2"" role in line %1 is not found in the metadata.'"),
					"%1", String.Synonym));
		EndIf;
	EndDo;
...
Procedure ContentParticipantsOnChange(Item)
	...
	Object.AllAuthorizationObjects = (UserGroupsMembers = "AllOneType");
	If Object.AllAuthorizationObjects Then
		Object.Content.Clear();
	EndIf;
	
	If UserGroupsMembers = "AllOneType" Or UserGroupsMembers = "OneType" Then
...
Function UserTransferToGroup(UserArray, NewGroupOwner)
	...
	Return UsersService.GeneratingMessageToUser(DisplacedUsersArray, NewGroupOwner,
													False, ArrayIsNotDisplacedUsers);
...
Procedure FilterAvailableForSelectTypesOfGroups()
	...
	If ValueIsFilled(Object.Parent)
		...
		If Object.Parent.TypeOfAuthorizationObjects <> Undefined Then
			...
			UserGroupsMembers = Items.UserGroupsMembers.ChoiceList.FindByValue("OneType").Value;
			Items.UsersType.Enabled = False;
		Else
			Items.UsersType.Enabled = True;
			FoundValue = Items.UserGroupsMembers.ChoiceList.FindByValue("AllOneType");
			If FoundValue <> Undefined Then
				Items.UserGroupsMembers.ChoiceList.Delete(FoundValue);
			EndIf;
	...
	Else
		...
		FoundValue = Items.UserGroupsMembers.ChoiceList.FindByValue("AllOneType");
		If FoundValue = Undefined Then
			Items.UserGroupsMembers.ChoiceList.Insert(0, "AllOneType", NStr("en = 'All users of the specified kind'"));
		EndIf;
		Items.UsersType.Enabled = True;
...
Procedure SetEnabledOfProperties(Form)
	...
	Items.AuthorizationObjectsTypePresentation.Visible = 
		...
		AND ((Form.UserGroupsMembers = "OneType" Or Form.UserGroupsMembers = "AllOneType"));
...
Function AnExtendedFormOfSelectionOptions()
	...
	ExternalUsersUserGroupsMembers = Object.Content.Unload(, "ExternalUser");
	
	For Each Item In ExternalUsersUserGroupsMembers Do
	...
	FormHeaderSelection = NStr("en = 'Select participants of external user group'");
