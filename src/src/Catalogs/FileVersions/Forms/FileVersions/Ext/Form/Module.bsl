...
Procedure ActivateExecute()
	...
	Else
		ShowMessageBox(, NStr("en = 'It is allowed to change the active version only for the files that are not locked.'"));
	EndIf;
...
Procedure Compare(Command)
	...
	#If WebClient Then
		ShowMessageBox(, NStr("en = 'Version comparison is not supported in web client.'"));
		Return;
	#EndIf
	...
	If SelectedRowCount = 2 OR SelectedRowCount = 1 Then
		...
		If Not ExtensionIsSupported Then
			WarningText =
				NStr("en = 'Versions matching is supported only for files of the following types:
				     |Text document (.txt)
				     |RTF document (.rtf)
		...
		If StandardSubsystemsClientReUse.ClientWorkParameters().ThisIsBasicConfigurationVersion Then
			CommonUseClientServer.MessageToUser(
				NStr("en = 'This operation is not supported in base version.'"));
			Return;
		EndIf;
...
Procedure MarkToDeleteUnmark()
	...
	If CurrentData.DeletionMark Then 
		QuestionText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Clear mark for deletion for ""%1""?'"),
...
Procedure VersionsComparisonAutomatic(Result, ExecuteParameters) Export
	...
	If Result <> -1 Then
		If ExecuteParameters.CurrentStep = 1 Then
			...
			ExecuteParameters.FileVersionComparisonMethod = PersonalSettingsTip.FileVersionComparisonMethod;
	...
	If ExecuteParameters.CurrentStep = 2 Then
		...
		StatusText = NStr("en = 'Comparing versions of file ""%1""...'");
