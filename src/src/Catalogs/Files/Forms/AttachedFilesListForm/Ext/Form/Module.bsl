...
Procedure ListSelection(Item, SelectedRow, Field, StandardProcessing)
	...
	HowToOpen = PersonalSettingsTip.DoubleClickAction;
	...
	If FileData.IsEditing.IsEmpty() Then
		PromptForEditModeOnOpenFile = PersonalSettingsTip.PromptForEditModeOnOpenFile;
...
//   DragParameters - Drag-and-drop parameters.
	...
	ElsIf TypeOf(DragParameters.Value) = Type("File") AND DragParameters.Value.IsFile() = False Then
		...
		ShowMessageBox(, NStr("en = 'Select files only (without directory).'"));
		Return;
