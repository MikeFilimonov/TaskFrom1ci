...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not EditMode Then
		TitleString = StringFunctionsClientServer.SubstituteParametersInString(NStr("en = '%1 (read only)'"), TitleString);
	EndIf;
	Title = TitleString;
...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	If Modified Then
		...
		If Exit Then
			WarningText = NStr("en = 'Data changed. All changes will be lost.'");
			Return;
		EndIf;
		...
		NameAndExtension = CommonUseClientServer.GetNameWithExtention(
			...
			FileData.Extension);
		QuestionText = StrReplace(NStr("en = 'File ""%1"" was changed.'"), "%1", NameAndExtension);
...
Procedure SaveAs(Command)
	...
	If FileChoice.Choose() Then
		...
		Status(NStr("en = 'File successfully saved'"), , SelectedFullFileName);
