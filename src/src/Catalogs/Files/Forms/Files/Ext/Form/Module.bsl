...
Procedure FindExecute()
	...
	If SearchString = "" Then
		CommonUseClientServer.MessageToUser(
			NStr("en = 'Search text is not entered.'"), , "SearchString");
		Return;
	EndIf;
...
Procedure MoveToFolder(Command)
	...
	FormParameters = New Structure;
	FormParameters.Insert("Title",    NStr("en = 'Install directory'"));
	...
	ElsIf TypeOf(DraggingValue) = Type("Array") Then
		...
		ElsIf ValueType = Type("CatalogRef.Files") Then
			...
			If Action = DragAction.Copy Then
				...
				If DraggingValue.Count() = 1 Then
					NotificationTitle = NStr("en = 'File is copied.'");
				...
				Else
					...
					NotificationText = StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en = 'Files (%1 pcs) were copied into folder ""%2""'"),
						DraggingValue.Count(),
						String(FolderForAdding));
				EndIf;
			...
			Else
				...
				If DraggingValue.Count() = 1 Then
					NotificationTitle = NStr("en = 'File is moved.'");
					NotificationText = StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en = 'The
						...
						String(FolderForAdding));
				Else
					NotificationTitle = NStr("en = 'Files are moved.'");
					NotificationText = StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en = 'Files (%1 pcs) were moved into folder ""%2""'"),
		...
		ElsIf ValueType = Type("CatalogRef.FileFolders") Then
			...
			If DraggingValue.Count() = 1 Then
				...
				NotificationTitle = NStr("en = 'Folder is moved.'");
				NotificationText = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'Folder
					     |""%1"" moved to ""%2""'"),
					String(DraggingValue[0]),
					String(FolderForAdding));
			Else
				NotificationTitle = NStr("en = 'Folders are moved.'");
				NotificationText = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'Folders (%1 pcs.) are moved to the ""%2"" folder'"),
					String(DraggingValue.Count()),
					String(FolderForAdding));
			EndIf;
			ShowUserNotification(NotificationTitle, , NotificationText, PictureLib.Information32);
...
Procedure ChooseModeAndEditFile(ResultHandler, FileData, EditCommandEnabled) Export
	...
	OpeningMethod = PersonalSettingsTip.TextFilesOpeningMethod;
	If OpeningMethod = PredefinedValue("Enum.FilesAssociation.InEmbeddedEditor") Then
		
		ExtensionInList = FileFunctionsServiceClientServer.FileExtensionInList(
			PersonalSettingsTip.TextFilesExtension,
			FileData.Extension);
	...
	OpeningMethod = PersonalSettingsTip.GraphicSchemesOpeningMethod;
	If OpeningMethod = PredefinedValue("Enum.FilesAssociation.InEmbeddedEditor") Then
		
		ExtensionInList = FileFunctionsServiceClientServer.FileExtensionInList(
			PersonalSettingsTip.GraphicalSchemaExtension,
			FileData.Extension);
	...
	If FileData.IsEditing.IsEmpty()
		AND PersonalSettingsTip.PromptForEditModeOnOpenFile = True
