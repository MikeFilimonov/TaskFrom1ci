...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If CommonUse.SubsystemExists("StandardSubsystems.DigitalSignature") Then
		...
		If CommonSettings.UseDigitalSignature Then
	...
	If FileEdited AND Not AnswerQuestionOnReceivedFileIsBusy Then
		...
		If ClientWorkParameters.AuthorizedUser = Object.IsEditing Then
			QuestionText = NStr("en = 'File is not available for editing.
			                    |Do you want to close the card?'");
			CommonUseClient.ShowArbitraryFormClosingConfirmation(ThisObject, Cancel, Exit, QuestionText, "AnswerQuestionOnReceivedFileIsBusy");
			Return;
		EndIf;
	EndIf;
	...
	If CreationMode = "FromTemplate" AND Not Object.DigitallySigned Then
		If NewFile AND NewFileWritten AND (NOT FileEdited) Then
			If Not AnswerToQuestionReceivedOpenForEditing Then
				Cancel = True;
				QuestionText = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'Do you want to open ""%1"" for editing?'"),
...
Procedure Delete(Command)
	...
	ShowQueryBox(Handler, NStr("en = 'Delete the selected signatures?'"), QuestionDialogMode.YesNo);
...
Procedure FillEncryptionList()
	...
	If Object.Encrypted Then
		...
		For Each CertificateStructure In ArrayOfEncryptionCertificates Do
			...
			If CertificateStructure.ElectronicSignature <> Undefined Then
				NewRow.CertificateAddress = PutToTempStorage(CertificateStructure.ElectronicSignature, UUID);
			EndIf;
		EndDo;
	EndIf;
...
Procedure FillListOfSignatures()
	...
	If Object.DigitallySigned Then
		...
		Query.Text = "SELECT ALLOWED
		               ...
		               |	DigitalSignatures.ElectronicSignature AS ElectronicSignature
		               |FROM
		...
		While QuerySelection.Next() Do
			...
			CertificateBinaryData = QuerySelection.ElectronicSignature.Get();
