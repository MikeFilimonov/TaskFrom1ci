...
// Button "Rescan" replaces the selected image (or the only one image) (or adds new images to the end if no images are
// selected) with a new image(s).
&AtClient
...
Procedure AcceptAllAsSeparateFiles(Command)
	...
	If ErrorsCount > 0 Then
		...
		Else
			ShortAllTextErrors = StrReplace(NStr("en = 'Performing failed (%1).'"), "%1", String(ErrorsCount));
...
Procedure ConvertEnumsToParametersAndGetPresentation()
	...
	Chromaticity = -1;
	If ChromaticityEnum = Enums.ImageColorMode.Monochrome Then
		Chromaticity = 0;
	ElsIf ChromaticityEnum = Enums.ImageColorMode.GrayGradations Then
		Chromaticity = 1;
	ElsIf ChromaticityEnum = Enums.ImageColorMode.Color Then
		Chromaticity = 2;
	EndIf;
	...
	Presentation = Presentation + NStr("en = 'Multi-page storage format:'") + " ";
