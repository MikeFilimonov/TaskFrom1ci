...
Procedure SchedulePresentationStartChoice(Item, ChoiceData, StandardProcessing)
	...
	If ValueIsFilled(Object.Pattern) Then
		ShowMessageBox(, NStr("en = 'Schedule is set in the template for jobs created from templates.'"));
		Return;
	EndIf;
...
Procedure SetPresentationSchedule(Val Form)
	...
	ElsIf ValueIsFilled(Form.Object.Pattern) Then
		Form.SchedulePresentation = NStr("en = '<Specified in the template>'");
	Else
		Form.SchedulePresentation = NStr("en = '<Not specified>'");
	EndIf;
