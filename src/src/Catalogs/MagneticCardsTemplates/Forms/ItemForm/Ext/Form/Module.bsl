...
Procedure BeforeWrite(Cancel, WriteParameters)
	...
	If FieldCounter = 0 Then
		CommonUseClientServer.MessageToUser(NStr("en = 'Not a single field was not added to the any of available tracks.'"), , , , Cancel);
	EndIf;
...
Procedure DetermineFieldCoordinates(Notification, TrackData, Prefix, Suffix, Delimiter, NCol, CCol)
	...
	If Not IsBlankString(Prefix)
		...
		If NCol < 1 Then
			...
			CommonUseClientServer.MessageToUser(NStr("en = 'Selected part of the code should not overlap the suffix, prefix or block separator.'"));
			Return;
		EndIf;
	EndIf;
	...
	If CCol = StrLen(DataRow)+1
		...
		ButtonList.Add(DialogReturnCode.Yes, NStr("en = 'Field length fixed'"));
		ButtonList.Add(DialogReturnCode.No, NStr("en = 'Field length is limited by the separator or by the row end'"));
		...
		// |""No""  - length fields limited by delimiter or string end'"), QuestionDialogMode.YesNo, , DialogReturnCode.No,
		//            NStr("en = 'Field length in characters'; ru = 'Длина поля в символах'"));
		//	
...
Procedure ControlOfFieldUniqueness(DoublesList, Field, CurrentLineNumber, TableName, Cancel)
				...
				For Each curField In Object["TrackFields"+String(G)] Do
					If curField.Field = Field Then
						...
						If TwinCounter > 1 Then
							StrMessage = NStr("en = 'Track %1, line %2: Field should be unique.'");
	...
	Else
		StrMessage = NStr("en = 'Track %1, line %2: Field cannot be empty.'");
...
Procedure CheckTemplateEnd(Result, Parameters) Export
	...
	If Result = DialogReturnCode.Yes Then
		...
		If Not Write() Then
			CommonUseClientServer.MessageToUser(NStr("en = 'Cannot write the template'"));
			Return;
		EndIf;
...
Procedure CheckTemplateCommand(Command)
	...
	If Modified Then
		QuestionText = NStr("en = 'Template was changed. Save changes?'");
