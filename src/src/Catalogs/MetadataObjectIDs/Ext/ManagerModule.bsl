...
//                  are added or required to be added, as well as full names of metadata objects, full names of which
//                  are changed or required to be changed.
//
Procedure ExecuteDataRefreshing(HasChanges, HasDeleted, CheckOnly = False,
	...
	Try
		...
		If ValueIsFilled(MetadataObjectsRenamingsList) Then
			CriticalChangesList = NStr("en = 'Rename IDs of metadata objects OldFullName -> NewFullName:'")
		...
		If ValueIsFilled(ListOfNewMetadataObjects) Then
			CriticalChangesList = CriticalChangesList
				+ NStr("en = 'Add new metadata object identifiers:'")
			...
			If TypeOf(MetadataObjectKey) = Type("Type")
			   AND MetadataObjectKey <> Type("Undefined") Then
				...
				If ImportedProperty.MetadataObjectByKey <> ImportedProperty.MetadataObjectByDescriptionFull Then
					Raise StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en = 'An error occurred while importing metadata objects IDs.
						     |Unable to import item metadata object of which ""%1"" does not correspond to the full name ""%2"".'"),
...
Function MetadataObjectCollectionProperties() Export
	...
	String.Synonym         = NStr("en = 'Document logs'");
	String.SingularName     = "DocumentJournal";
	String.SynonymInSingularNumber = NStr("en = 'Document journal'");
	String.WithoutData       = True;
	...
	String.SynonymInSingularNumber = NStr("en = 'Processing'");
	String.WithoutData       = True;
	...
	String.SynonymInSingularNumber = NStr("en = 'Primary chart of accounts'");
...
Procedure CallExceptionByError(ErrorText) Export
	...
	Raise
		NStr("en = 'An error occurred while working with the ""Metadata object IDs"" catalog.'") + "
		           |
		           |" + ErrorText;
...
Function MetadataFindByFullName(FullName)
	...
	If Upper(MetadataObject.FullName()) <> Upper(FullName) Then
		...
		Else
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'An error occurred when searching for metadata object by full name (searched for ""%1"", ""%2"" was found).'"),
				FullName,
				MetadataObject.FullName());
		EndIf;
	EndIf;
...
#Region IDReplacementInTheDataBase
	...
	Try
		For Each TableRow In RefsTable Do
			...
			ElsIf Metadata.InformationRegisters.Contains(TableRow.Metadata) Then
				...
				EndTry;
			Else
				ShowMessageAboutError(NStr("en = 'Values are not changed in the type data'") + ": " + TableRow.Metadata);
			EndIf;
		EndDo;
...
Procedure ShowMessageAboutError(Val Definition)
	...
	WriteLogEvent(
		NStr("en = 'Metadata objects IDs. Change identifier'",
		     CommonUseClientServer.MainLanguageCode()),
		EventLogLevel.Error,
		,
		,
