...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	UserLanguageCode = DriveReUse.GetCurrentUserLanguageCode();
	If UserLanguageCode = Undefined Then
		UserLanguageCode = Metadata.DefaultLanguage.LanguageCode;
	EndIf;
	List.Parameters.SetParameterValue("LangKey", UserLanguageCode);
...
Procedure SubsystemsTreeDrag(Item, DragParameters, StandardProcessing, String, Field)
	...
	If ArrangementParameters.Source.Ref = ArrangementParameters.Receiver.Ref Then
		ShowMessageBox(, NStr("en = 'The selected report options are already in this section.'"));
		Return;
	EndIf;
	
	If ArrangementParameters.Variants.Total = 1 Then
		If ArrangementParameters.Action = "Copy" Then
			QuestionTemplate = NStr("en = 'Place ""%1"" to ""%4""?'");
		Else
	...
	Else
		...
		EndDo;
		If ArrangementParameters.Action = "Copy" Then
			QuestionTemplate = NStr("en = 'Place report options ""%1"" (%2 pcs.) in ""%4""?'");
		Else
			QuestionTemplate = NStr("en = 'Move report options ""%1"" (%2 pcs.) from ""%3"" to ""%4""?'");
		EndIf;
	EndIf;
...
Function PlaceOptionsInSubsystem(ArrangementParameters)
	...
	If ArrangementParameters.Variants.Total = Placed Then
		...
		If ArrangementParameters.Variants.Total = 1 Then
			If ArrangementParameters.Action = "Move" Then
				Pattern = NStr("en = 'Successfully transferred to %1"".'");
			Else
				Pattern = NStr("en = 'Successfully placed in %1"".'");
			EndIf;
	...
	Else
		...
		If Not IsBlankString(CannotBeArranged) Then
			ErrorsText = ?(ErrorsText = "", "", ErrorsText + Chars.LF + Chars.LF)
				+ NStr("en = 'Cannot be placed in command interface:'")
				+ Chars.LF
				+ CannotBeArranged;
		EndIf;
		If Not IsBlankString(ArrangedAlready) Then
			ErrorsText = ?(ErrorsText = "", "", ErrorsText + Chars.LF + Chars.LF)
				+ NStr("en = 'Already located in this section:'")
				+ Chars.LF
				+ ArrangedAlready;
		EndIf;
		
		If ArrangementParameters.Action = "Move" Then
			Pattern = NStr("en = 'Transferred report options: %1 out of %2.'");
		Else
			Pattern = NStr("en = 'Placed report options: %1 of %2.'");
		EndIf;
