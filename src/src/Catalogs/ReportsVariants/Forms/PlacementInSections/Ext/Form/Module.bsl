...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	MixedImportance = NStr("en = 'Different'");
...
Procedure Place(Command)
	If CheckCountOfVariants() Then
		WriteAtServer();
		NotificationText = NStr("en = 'Report option settings were changed (%1 items).'");
...
Function CheckCountOfVariants()
	...
	If VariantCount = 0 Then
		CommonUseClientServer.MessageToUser(
			NStr("en = 'Fill in the list ""Report options""'"),
			,
			"CustomizableOptions");
		Return False;
	EndIf;
...
Procedure RefillTree(JustUncheckCheckBoxes)
	...
	If VariantCount = 0 Then
		...
		MessageText = NStr("en = 'Select report options'");
	...
	If ErrorsCount > 0 Then
		...
		For Each TableRow In FilteredVariants Do
			If CurrentCause <> TableRow.Cause Then
				...
				ElsIf CurrentCause = 3 Then
					ErrorsText = ErrorsText + NStr("en = 'The report is disabled or unavailable for the rights:'");
				ElsIf CurrentCause = 4 Then
					ErrorsText = ErrorsText + NStr("en = 'Report option is disabled using functional option:'");
				EndIf;
			EndIf;
		...
		If VariantCount = 0 Then
			MessageText = NStr("en = 'Insufficient rights to place the selected report options in sections.'");
			Items.SubsystemsTree.Enabled = False;
			ExecutionResult.Insert("Cancel", True);
		Else
			MessageText = NStr("en = 'Insufficient rights to place some report options (%1) in sections.'");
