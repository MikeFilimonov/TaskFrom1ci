...
Procedure SendAndReceiveMessages(Command)
	...
	If ThisIsSessionWithEstablishedValuesOfDelimiters Then
		MessageText = NStr("en = 'Messages can be processed only in an undivided session of service administrator.'");
	...
	If Items.List.CurrentData <> Undefined Then
		...
		Else
			...
			If Items.List.SelectedRows.Count() > 1 Then
				...
				QuestionString = NStr("en = 'Delete the selected messages?'");
