	...
	If CommonUseReUse.DataSeparationEnabled() Then
		...
		If Not CanChangeUsers Then
			If Object.Ref.IsEmpty() Then
				Raise
					NStr("en = 'New users cannot be created in the demo mode.'");
			EndIf;
			ReadOnly = True;
		EndIf;
...
Procedure BeforeWrite(Cancel, WriteParameters)
	...
	If CanLogOnToApplication Then
		QuestionsTitle = NStr("en = 'Infobase user record'");
		   ...
		   AND IBUserRoles.Count() = 0 Then
			...
			If Not WriteParameters.Property("WithEmptyListOfRoles") Then
				...
				ShowQueryBox(
					...
					NStr("en = 'Infobase user was not assign any role. Continue?'"),
...
#Region SupportContactInformation
...
#Region SupportAdditionalAttributes
...
#Region ProcessingAnInfobaseUser
	...
	ElsIf AccessLevel.ListManagement Then
		...
		If PropertiesAdjustment.Count() > 0 Then
			...
			Items.PropertiesMismatchNote.Title =
				...
				+ Chars.LF
				+ ?(ShowCommandsDifferences,
					NStr("en = 'Click ""Write"" to resolve the differences and not to show this message again.'"),
					NStr("en = 'Contact your administrator to resolve the differences.'"));
		Else
			ShowMismatch = False;
		EndIf;
	Else
