...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	CustomizeOrderFoldersAllUsers(UserGroups);
	...
	If Not Users.InfobaseUserWithFullAccess(, CommonUseReUse.ApplicationRunningMode().Local) Then
		...
		EndIf;
		Items.UserDetails.Visible = False;
	EndIf;
	...
	If Parameters.ChoiceMode Then
		...
		Items.UserGroups.ChoiceMode = SettingsStored.UserGroupChoice;
	...
	SettingsStored.Insert("GroupAllUsers", Catalogs.UserGroups.AllUsers);
...
#Region UserGroupsFormTableItemsEventsHandlers
...
Procedure UserGroupsDrag(Item, DragParameters, StandardProcessing, String, Field)
	...
	FolderIsMarkedForDelete = Items.UserGroups.RowData(String).DeletionMark;
	UserCount = DragParameters.Value.Count();
	
	ActionToDeleteUser = (SettingsStored.GroupAllUsers = String);
	
	ActionWithUser = ?((SettingsStored.GroupAllUsers = Items.UserGroups.CurrentRow),
		NStr("en = 'Enable'"),
	...
	If UserCount = 1 Then
		...
		QuestionText = StringFunctionsClientServer.SubstituteParametersInString(
			...
			String(String), String(Items.UserGroups.CurrentRow));
	...
	Notification = New NotifyDescription("UserGroupsDragEnd", ThisObject, AdditionalParameters);
	ShowQueryBox(Notification, QuestionText, QuestionDialogMode.YesNo, 60, DialogReturnCode.Yes);
	
EndProcedure

&AtClient
Procedure UserGroupsCheckingDrag(Item, DragParameters, StandardProcessing, String, Field)
...
&AtClient
Procedure UserDetails(Command)
	
	OpenForm(
		"Report.UserDetails.ObjectForm",
		New Structure("VariantKey", "UserDetails"),
		ThisObject,
		"UserDetails");
	
EndProcedure

#Region SupportBulkObjectEditing
...
Procedure SetupFormByUsingUserGroups()
	...
	If SettingsStored.Property("CurrentRow") Then
		...
		If TypeOf(SettingsStored.CurrentRow) = Type("CatalogRef.UserGroups") Then
			
			If SettingsStored.UseGroups Then
				Items.UserGroups.CurrentRow = SettingsStored.CurrentRow;
			Else
	...
	If Parameters.ChoiceMode Then
		...
		If Parameters.CloseOnChoice = False Then
			...
			If UserGroupChoice Then
				...
				Else
					Title = NStr("en = 'Select users and groups'");
				EndIf;
...
&AtClient
Procedure UserGroupsDragEnd(Response, AdditionalParameters) Export
