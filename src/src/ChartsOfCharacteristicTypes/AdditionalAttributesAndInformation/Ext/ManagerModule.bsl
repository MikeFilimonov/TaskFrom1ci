...
Procedure ChangePropertiesConfiguration(Parameters, StorageAddress) Export
	...
	Try
		...
		If HasAdditionalValues Then
			...
			If PropertyObject.ValueType.ContainsType(Type("CatalogRef.AdditionalValues")) Then
				CatalogName = "AdditionalValues";
				IsFolder      = "Values.IsFolder";
			Else
				CatalogName = "AdditionalValuesHierarchy";
			...
			|	Values.Weight
			|FROM
			|	Catalog.AdditionalValues AS Values
			|WHERE
			|	Values.Owner = &Owner
			|TOTALS BY
			|	Ref HIERARCHY";
			Query.Text = StrReplace(Query.Text, "AdditionalValues", CatalogName);
		...
		If Property <> PropertyObject.Ref
		 OR ReferenceMap.Count() > 0 Then
			...
			// ref), and if not only the target dataset has the replaced common  property then it is necessary to add new
			// property and value.
			//
			...
			If Property = PropertyObject.Ref Then
				...
				|		ON (VALUETYPE(AdditionalInformation.Object) = Type(Catalog.AdditionalValues))
			...
			Else
				...
				|	InformationRegister.AdditionalInformation AS AdditionalInformation
				|WHERE
				|	VALUETYPE(AdditionalInformation.Object) = Type(Catalog.AdditionalValues)
				|	AND NOT AdditionalInformation.Object IN (&ProcessedObjects)
				|	AND AdditionalInformation.Property = &Property";
			EndIf;
			
			Query.Text = StrReplace(Query.Text, "Catalog.AdditionalValues", OwnerFullName);
