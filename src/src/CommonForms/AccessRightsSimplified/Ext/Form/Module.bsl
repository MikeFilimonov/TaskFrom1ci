...
#Region FormTableItemsEventsHandlersAllAllowedPresentation
...
Procedure ImportData(FilterProfilesOnlyCurrentUser)
	...
	|	AND Not Profiles.IDSuppliedData = &ProfileIDProductsEdit
	...
	|			THEN 98
	|		WHEN Profiles.Ref.Description = ""Cash management""
	|			THEN 97
	...
	AllowEditProducts   = AllowEditProducts(Parameters.User);
		...
		While Selection.Next() Do
			...
			If Selection.Profile = Catalogs.AccessGroupsProfiles.Administrator Then
				...
				Else
					...
					EndIf;
				EndIf;
			ElsIf Selection.Profile.IDSuppliedData = New UUID("76337581-bff4-11df-9174-e0cb4ed5f4c3") Then // ReturnsFromCustomers
...
Procedure OnChangeCurrentProfile(Val Form)
	...
	If CurrentData <> Undefined Then
		...
		If CurrentProfileName = "Sales" Then
			...
			Items.AllowEditProducts.Visible 	 = True;
		Else
			Items.AllowEditPricesInDocuments.Visible = False;
			Items.AllowEditProducts.Visible 	 = False;
		EndIf;
		Items.AllowEditPricesInDocuments.Enabled = CurrentData.Check;
		Items.AllowEditProducts.Enabled   = CurrentData.Check;
...
&AtServerNoContext
Function AllowEditProducts(User)
	...
	|	Catalog.AccessGroups.Users AS AccessGroupsUsers
	|WHERE
	|	AccessGroupsUsers.Ref.Profile.IDSuppliedData = &ProfileIDProductsEdit
	|	AND AccessGroupsUsers.User = &User";
...
Function AccessKindsSettingIsAvailableForProfile(AccessKinds, Profile)
	...
	If Not GetFunctionalOption("UseRowLevelSecurity") Then
		Return False;
	EndIf;
...
Function AddAdditionalProfiles(SelectedProfiles)
	...
	ProfileProductsEdit = Catalogs.AccessGroupsProfiles.FindByAttribute("IDSuppliedData", IDSuppliedData);
	If ValueIsFilled(ProfileProductsEdit) Then
		NewRow = SelectedProfiles.Add();
		NewRow.Profile = ProfileProductsEdit;
		NewRow.Check = HasSaleProfile AND AllowEditProducts;
	EndIf;
