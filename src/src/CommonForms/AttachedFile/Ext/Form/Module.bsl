...
Procedure StandardReread(Command)
	...
	QuestionText = NStr("en = 'Data is changed. Reread?'");
...
#Region HandlersOfEPCommandsAndEncryption
...
Procedure DeleteDS(Command)
	...
	ShowQueryBox(NotifyDescription, NStr("en = 'Delete the selected signatures?'"), QuestionDialogMode.YesNo);
...
#Region CommandHandlersForFileCollaborationSupporting
...
Procedure UpdateTitle()
	...
	Else
		Title = NStr("en = 'Attached file (Creation)'")
	EndIf;
...
Procedure FillEncryptionList()
	...
	If ThisObject.Object.Encrypted Then
		For Each CertificateStructure In FormAttributeToValue("Object").EncryptionCertificates Do
			...
			CertificateBinaryData = CertificateStructure.ElectronicSignature.Get();
	...
	HeaderText = NStr("en = 'Decryption allowed'");
...
Procedure FillListOfSignatures()
	...
	If ThisObject.Object.DigitallySigned Then
		...
		For Each ItemEP In FormAttributeToValue("Object").DigitalSignatures Do
			...
			CertificateBinaryData = ItemEP.ElectronicSignature.Get();
			If CertificateBinaryData <> Undefined Then 
...
Function WriteFile(Val ObjectParameter = Undefined)
	...
	Except
		If TransactionActive Then
			RollbackTransaction();
			WriteLogEvent(NStr("en = 'Files.An error occurred when writing the attached file'", CommonUseClientServer.MainLanguageCode()),
...
Procedure ReadSignaturesCertificatesCycleAfterCertificateExport(CertificateData, Context) Export
	...
	TableRow.Imprint = Base64String(Context.ElectronicSignature.Imprint);
	TableRow.CertificateAddress = PutToTempStorage(CertificateData, UUID);
	TableRow.CertificateIsIssuedTo =
		Context.ModuleDigitalSignatureClientServer.SubjectPresentation(Context.ElectronicSignature);
