...
Function ValidateActionAllowed(Val CurrentAction = "")
	...
	If TypeOf(Items.List.CurrentRow) = CatalogWithFilesType Then
		Return True;
	Else
		ShowMessageBox(, NStr("en = 'The action is not available for the list grouping row.'"));
		Return False;
	EndIf;
...
Procedure ConfigureDynamicList(CatalogNameFilesStorage)
	...
	ErrorTitle = NStr("en = 'An error occurred when configuring the dynamic list of attached files.'");
	EndErrors = NStr("en = 'In this case, dynamic list configuration is not supported.'");
