...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If ExchangePlanList.Count() = 0 Then
		MessageText = NStr("en = 'Configuring data synchronization is not applicable.'");
	...
	Else
		...
		IBPrefix = DataExchangeServer.GlobalNumerationPrefix();
...
Procedure ListOfNodesStatusSelection(Item, SelectedRow, Field, StandardProcessing)
	...
	ShowQueryBox(NotifyDescription, NStr("en = 'Synchronize data?'"), QuestionDialogMode.YesNo,, DialogReturnCode.Yes);
...
Procedure SetOneSynchronizationItems()
	...
	If ConfiguredSynchronization.InfobaseNode = Undefined Then
		...
		Raise NStr("en = 'Data synchronization monitor is not supported for undivided session'");
...
Procedure CheckConversionRulesCompatibility(Val ExchangePlanName, ContinuationProcessor)
	...
	Else
		...
		If ErrorDescription.ErrorKind = "IncorrectConfiguration" Then
			FormParameters.Title = NStr("en = 'Data synchronization cannot be executed'");
		Else
			FormParameters.Title = NStr("en = 'Data synchronization may be executed incorrectly'");
		EndIf;
...
Function ConversionRulesAreCompatibleWithCurrentVersion(ExchangePlanName, ErrorDescription)
	...
	If ConfigurationNameFromRules <> InfobaseConfigurationName Then
			...
			ErrorDescription.Insert("ErrorText", NStr("en = 'Cannot sync data because you use rules designed for application ""%1"". Use rules from configuration or import the correct set of rules from the file.'"));
		...
		If ComparisonResult <> 0 Then
			...
			Else
				...
				ErrorText = NStr("en = 'Data may be synchronized incorrectly due to using rules for a newer version of the """"%1"""" application (%2). It is recommended to update application version or use rules set designed for the current application version (%3).'");
