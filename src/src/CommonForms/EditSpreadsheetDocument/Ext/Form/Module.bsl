...
Procedure OnOpen(Cancel)
	...
	If Not IsBlankString(Parameters.PathToFile) Then
		...
		WhenInitializationFileIsComplete(File,);
		Return;
	EndIf;
...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	If Exit Then
		...
		WarningText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Data will be lost (%1)'"), DocumentName);
	...
	QuestionText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Do you want to save changes in %1?'"), DocumentName);
	NotifyDescription = New NotifyDescription("ConfirmAndClose", ThisObject);
	CommonUseClient.ShowFormClosingConfirmation(NotifyDescription, Cancel, Exit, QuestionText);
...
&AtClient
Procedure WhenInitializationFileIsComplete(File, AdditionalParameters)
	
	If IsBlankString(DocumentName) Then
		DocumentName = File.BaseName;
	EndIf;
	
	NotifyDescription = New NotifyDescription("BeginGettingReadOnlyEnd", ThisObject);
	File.BeginGettingReadOnly(NotifyDescription);
	
EndProcedure

&AtClient
Procedure BeginGettingReadOnlyEnd(ReadOnly, AdditionalParameters) Export
