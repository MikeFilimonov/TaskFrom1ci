...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Else
		...
		Items.ApplicationsExplanation.Title =
			NStr("en = 'Applications specified by administrator that can be used on computer.'");
	EndIf;
...
Procedure RefreshCurrentItemsVisible()
	...
	If Constants.UseEncryption.Get() Then
		Items.AddFromInstalledOnComputer.Title =
			NStr("en = 'From the ones installed on computer...'");
	Else
		Items.AddFromInstalledOnComputer.Title =
			NStr("en = 'From the ones installed on computer'");
	EndIf;
	...
	If Constants.UseDigitalSignature.Get() Then
		CheckBoxTitle = NStr("en = 'Check signatures and certificates on the server'");
		CheckBoxToolTip =
	...
	If Not Constants.UseDigitalSignature.Get() Then
		CheckBoxTitle = NStr("en = 'Encrypt and decrypt on server'");
		CheckBoxToolTip =
	...
	ElsIf Not Constants.UseEncryption.Get() Then
		...
		CheckBoxToolTip =
			NStr("en = 'It is not necessary to
			     ...
			     |on each computer with working 1C:Enterprise server or Web server which uses a file infobase.'");
	Else
		CheckBoxTitle = NStr("en = 'Sign and decrypt on server'");
		CheckBoxToolTip =
...
Procedure ExpectationHandlerDefineInstalledApplicationsContinueCycle(Manager, Context) Export
	...
	If Error.PathNotSpecified Then
		...
		UpdateValue(ApplicationDescription.Use, "");
	Else
		ErrorText = NStr("en = 'It is not installed on the computer.'") + " " + Error.Description;
		If Error.ToAdmin AND Not InfobaseUserWithFullAccess Then
			ErrorText = ErrorText + " " + NStr("en = 'Contact administrator.'");
		EndIf;
...
Procedure FillApplicationsAndSettings(RefreshReUse = False)
	...
	PersonalSettingsTip = DigitalSignature.PersonalSettingsTip();
	
	ActionsOnSavingDS         = PersonalSettingsTip.ActionsOnSavingDS;
	ExtensionForEncryptedFiles = PersonalSettingsTip.ExtensionForEncryptedFiles;
	ExtensionForSignatureFiles       = PersonalSettingsTip.ExtensionForSignatureFiles;
	PathToApplications                  = PersonalSettingsTip.PathToDigitalSignatureAndEncryptionApplications;
...
Procedure SaveSettingsAtServer(SavedSettings)
	...
	PersonalSettingsTip = DigitalSignature.PersonalSettingsTip();
	FillPropertyValues(PersonalSettingsTip, SavedSettings);
	DigitalSignatureService.SavePersonalSettings(PersonalSettingsTip);
...
Procedure SaveLinuxPathAtServer(Application, PathLinux)
	...
	PersonalSettingsTip = DigitalSignature.PersonalSettingsTip();
	PersonalSettingsTip.PathToDigitalSignatureAndEncryptionApplications.Insert(Application, PathLinux);
	DigitalSignatureService.SavePersonalSettings(PersonalSettingsTip);
