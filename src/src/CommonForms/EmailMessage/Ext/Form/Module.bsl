...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not ValueIsFilled(Parameters.UserAccount) Then
		...
		If AvailableAccounts.Count() = 0 Then
			MessageText = NStr("en = 'Available email accounts are not found, contact the system administrator.'");
	...
	ElsIf TypeOf(Parameters.UserAccount) = Type("ValueList") Then
		...
		If AccountsSet.Count() = 0 Then
			MessageText = NStr("en = 'Email accounts are not specified, contact the system administrator.'");
			CommonUseClientServer.MessageToUser(MessageText,,,, Cancel);
			Return;
		EndIf;
...
Procedure ContinueSendingEmailsWithPassword()
	...
	If EmailParameters = Undefined Then
		CommonUseClientServer.MessageToUser(NStr("en = 'An error occurred while generating email message parameters'"));
		Return;
	EndIf;
