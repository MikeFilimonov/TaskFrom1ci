...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not ValueIsFilled(Parameters.DataSource) Then 
		CommonUseClientServer.Validate(TypeOf(Parameters.CommandParameter) = Type("Array") Or CommonUse.ReferenceTypeValue(Parameters.CommandParameter),
			...
			NStr("en = 'Invalid value of CommandParameter parameter when calliing PrintManagementClient.ExecutePrintCommand method.
			     |Expected: Array, AnyRef. Actual: %1'"), 
	...
	Items.ButtonGoToTemplatesManagement.Visible = 
		AccessRight("Edit", Metadata.InformationRegisters.UserPrintTemplates);
	...
	If Upper(ChoiceSource.FormName) = Upper("CommonForm.SavePrintForm") Then
		...
		If ValueSelected <> Undefined AND ValueSelected <> DialogReturnCode.Cancel Then
			...
			Else
				...
				EndDo;
				Status(NStr("en = 'Saving completed successfully.'"));
			EndIf;
		EndIf;
		
	ElsIf Upper(ChoiceSource.FormName) = Upper("CommonForm.SelectAttachmentFormat")
		Or Upper(ChoiceSource.FormName) = Upper("CommonForm.SendPrintFormByMail") Then
...
Procedure AddNumberOfInstancesInPrintFormsPresentations()
	For Each PrintFormSetting In PrintFormsSettings Do
		If PrintFormSetting.Quantity <> 1 Then
			PrintFormSetting.Presentation = PrintFormSetting.Presentation + StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = '%1 copies'"),
			PrintFormSetting.Quantity); 
		EndIf;
	EndDo;
EndProcedure
	...
	If IsAllowedOutput Then
		For Each PrintFormSetting In PrintFormsSettings Do
			...
			If SpreadsheetDocumentField.Output = UseOutput.Disable Then
				PrintFormSetting.Presentation = PrintFormSetting.Presentation + NStr("en = '(cannot print)'");
...
Procedure SetPrinterNameInPrintButtonToolTip()
	If PrintFormsSettings.Count() > 0 Then
		...
		If Not IsBlankString(PrinterName) Then
			ThisObject.Commands["Print"].ToolTip = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Print on %1'"),
				PrinterName);
		EndIf;
	EndIf;
...
Function DefaultPrintedFormFileName(PrintObject, PrintedFormName)
	...
	If CommonUse.ThisIsDocument(Metadata.FindByType(TypeOf(PrintObject))) Then
		...
		Pattern = NStr("en = '[PrintedFormName] [Number] dated [Date]'");
	Else
...
Procedure RegeneratePrintForm(TemplateName, AttributeName)
	...
	If Cancel Then
		Raise NStr("en = 'Print form was not regenerated.'");
	EndIf;
...
Procedure SendPrintFormsByMailAccountSetupOffered(UserAccountIsConfigured, AdditionalParameters) Export
	...
	FormParameters = New Structure;
	OpenableFormName = "CommonForm.SelectAttachmentFormat";
	If MoreThenOneRecipient(OutputParameters.SendingParameters.Recipient) Then
		...
		OpenableFormName = "CommonForm.SendPrintFormByMail";
	EndIf;
