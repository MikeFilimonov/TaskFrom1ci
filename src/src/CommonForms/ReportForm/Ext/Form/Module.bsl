...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	ParametersForm.Insert("Filter", New Map);
	...
	If Parameters.Property("DecryptionFilter") AND TypeOf(Parameters.DecryptionFilter) = Type("Map") Then
		For Each KeyValue In Parameters.DecryptionFilter Do
			ParametersForm.Filter.Insert(KeyValue.Key, KeyValue.Value);
		EndDo; 
	EndIf;
	...
	// StandardSubsystems.PerformanceMeasurement
	If PerformanceEstimationServerCallReUse.MeasurePerformance() Then
		Try
		...
		If ReportVariantsCommandsVisible // Commands are not enabled by the form opening parameters.
			...
			If ValueIsFilled(PredefinedVariant) Then
				...
				If Disabled.Find(PredefinedVariant) <> Undefined Then // Option is disabled.
					...
					Text = NStr("en = 'Context variant ""%1"" of the ""%2"" report is opened without context.'");
...
Procedure ChoiceProcessing(Result, SubordinateForm)
	...
	If TypeOf(SubordinateForm) = Type("ManagedForm") Then
		...
		ElsIf TypeOf(Result) = Type("Structure") Then
			...
			If SourseFormSuffix = Upper("ReportPredefinedFilterSettingsForm")
				Or SourseFormSuffix = Upper("SettingsForm")
				Or SourseFormSuffix = Upper("ReportDataCompositionForm")
...
Procedure OnLoadVariantAtServer(NewSettingsDC)
	...
	If TypeOf(ParametersForm.Filter) = Type("Map") Then
		...
		Period = DCParameters.Items.Find("ItmPeriod");
		If Period = Undefined Then
			Period = DCParameters.Items.Find("Period");
		EndIf;
		...
		For Each KeyAndValue In ParametersForm.Filter Do
			...
			If (Name = "BeginOfPeriod" OR Name = "EndOfPeriod") AND NOT Period = Undefined Then
				
				If Name = "BeginOfPeriod" Then
					Period.Value.StartDate = Value;
				ElsIf Name = "EndOfPeriod" Then
					Period.Value.EndDate = Value;
				EndIf;
				
				Period.UserSettingID = "";
			...
			If TypeOf(AvailableDCField) = Type("DataCompositionFilterAvailableField") Then
				...
				If TypeOf(Value) = Type("Structure") Then
					ComparisonTypeCD	= Value.ComparisonType;
					Value				= Value.Value
			...
			ErrorText = NStr("en = 'Cannot set fixed filter ""%1"".'");
	...
	For Each DCUsersSetting In DCUserSettings.Items Do
		...
		If Type = "SettingsParameterValue"
			...
			ElsIf Value.StartDate > Value.EndDate Then
				ErrorText = NStr("en = 'Period end should be later than its start'");
...
Procedure SendByEMail(Command)
	...
	If StatePresentation.Visible = True
		...
		QuestionText = NStr("en = 'Report not generated. Generate?'");
...
Procedure Attachable_LoadReportVariant(Command)
	...
	If Found.Count() = 0 Then
		ShowMessageBox(, NStr("en = 'Report option was not found.'"));
		Return;
	EndIf;
	FormVariant = Found[0];
...
Procedure BackgroundJobCheckAtClient()
	If BackGroundJobFinished() Then
		ShowUserNotification(NStr("en = 'Report is generated'"), , Title);
...
Procedure ShowSendingByEMailDialog()
	...
	OpenForm("CommonForm.SendSpreadsheetByMail", FormParameters, , );
EndProcedure
...
Procedure VisibleEnabledCorrectness(Changes = "")
	...
	EndIf;
	If EncryptingMode Then
		Title = Title + " (" + NStr("en = 'Explanation'") + ")";
	EndIf;
...
Function BackGroundJobStart()
	...
	Try
		BackgroundJobResult = LongActions.ExecuteInBackground(
			...
			NStr("en = 'Report options: Report generation'"));
	Except
		ErrorInfo = ErrorInfo();
	EndTry;
...
Procedure LoadVariant(VariantKey)
	...
	StatePresentation.Text     = NStr("en = 'Another report option is selected. Click ""Generate"" to receive the report.'");
EndProcedure
...
Procedure FormationErrorsShow(ErrorInfo)
	If TypeOf(ErrorInfo) = Type("ErrorInfo") Then
		...
		DetailErrorDescription = NStr("en = 'Generation error:'") + Chars.LF + DetailErrorDescription(ErrorInfo);
...
Procedure AfterChangingKeyStates(FillingParameters)
	If FillingParameters.Event.Name <> "AfterGenerating" Then
		...
		ElsIf FillingParameters.VariantModified
			...
			StatePresentation = Items.ReportSpreadsheetDocument.StatePresentation;
			StatePresentation.Visible = True;
			StatePresentation.Text     = NStr("en = 'Settings were changed. Click ""Generate"" to get the report.'");
