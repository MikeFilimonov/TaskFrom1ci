...
Procedure VariantPress(Item)
	...
	ElsIf Not ValueIsFilled(Variant.ReportName) Then
		...
		WarningText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Report name for the ""%1"" option is not filled in.'"),
			Variant.Description);
...
Function EnteredSearchStringIsTooShort(Text)
	...
	If Not ThereIsNormalWord Then
		ShowMessageBox(, NStr("en = 'Words for search are too short.'"));
		Return True;
	EndIf;
...
Procedure ResetSettings(Command)
	QuestionText = NStr("en = 'Reset report placement settings?'");
...
Procedure AddTakeAwayOptionFromQuickAccess(Variant, Item, QuickAccess)
	...
	// Related activity: if option added to the quick access is not visible - then show it.
...
Procedure ResetSettingsAndRefreshReportsPanel()
	SettingMode = False;
	InformationRegisters.ReportOptionSettings.ResetUserSettingsSection(CurrentSectionRef);
	RefreshReportsPanel();
EndProcedure
...
Procedure SaveCustomSettings()
	...
	InformationRegisters.ReportOptionSettings.WriteSettingsPackage(SettingsPackage, Dimensions, Resources, False);
EndProcedure
...
Procedure FillInformationAboutSubsystemsAndSetTitle()
	...
	If CurrentSectionRef = Catalogs.MetadataObjectIDs.EmptyRef() Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Section ""%1"" is not connected to the ""%2"" subsystem. See module ReportsVariantsPredefined, procedure DetermineSectionsWithReportsVariants.'"),
...
#Region ServerFillingTheReportsPanel
...
Procedure FillReportsPanel()
	...
	Else
		...
		If FillingParameters.OtherSections.Count() = 0 Then
			...
			Label.Title = "    " + NStr("en = 'Reports are not found in other sections.'") + Chars.LF;
			Label.Height = 2;
		EndIf;
		...
		If FillingParameters.DontOutput > 0 Then // Output of information text.
			LabelTitle = NStr("en = 'The first %1 reports from other sections are displayed, specify a search query.'");
...
Procedure FindReportsVariantsForWithdrawal(FillingParameters)
	...
	|	CASE
	|		WHEN ReportsVariantsMultilingualValuesReports.Definition IS NULL
	|			THEN CASE
	...
	|				END
	|		ELSE CAST(ReportsVariantsMultilingualValuesReports.Definition AS STRING(1000))
	|	END AS Definition,
	|	ISNULL(ReportsVariantsMultilingualValuesReports.Description, ReportsVariants.Description) AS Description,
	...
	|		LEFT JOIN Catalog.ReportsVariants.MultilingualValuesReports AS ReportsVariantsMultilingualValuesReports
	|		ON (ReportsVariantsMultilingualValuesReports.Ref = ReportsVariants.Ref)
	|			AND (&UserLanguageCode = ReportsVariantsMultilingualValuesReports.LanguageKey)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|SELECT ALLOWED
	|	LocationVariants.Ref AS Ref,
	|	LocationVariants.Subsystem AS Subsystem,
	|	LocationVariants.Use AS Use,
	|	LocationVariants.Important AS Important,
	|	LocationVariants.SeeAlso AS SeeAlso,
	|	CASE
	...
	|	END AS Definition,
	|	LocationVariants.Ref.ForAuthorOnly AS ForAuthorOnly,
	|	LocationVariants.Ref.Description AS Description,
	|	LocationVariants.Ref.Report AS Report,
	|	LocationVariants.Ref.ReportType AS ReportType,
	|	LocationVariants.Ref.VariantKey AS VariantKey,
	|	LocationVariants.Ref.Author AS Author,
	|	LocationVariants.Ref.Parent AS Parent,
	|	LocationVariants.Ref.VisibleByDefault AS VisibleByDefault
	|INTO ttOptions
	|FROM
	...
	|		ELSE FALSE
	|	END AS Additional,
	|	ISNULL(PersonalSettingsTip.Visible, ttAllVariants.VisibleByDefault) AS Visible,
	|	ISNULL(PersonalSettingsTip.QuickAccess, FALSE) AS QuickAccess,
	|	CASE
	...
	|	ttAllVariants.Definition AS Definition,
	...
	|		ON ttAllVariants.Subsystem = PersonalSettingsTip.Subsystem
	|			AND ttAllVariants.Ref = PersonalSettingsTip.Variant
	|			AND (PersonalSettingsTip.User = &CurrentUser)
	|WHERE
	|	ISNULL(PersonalSettingsTip.Visible, ttAllVariants.VisibleByDefault)
	|
	|ORDER BY
	|	SubsystemOfPriority,
	|	Description";
	...
	Query.SetParameter("UserLanguageCode",		DriveReUse.GetCurrentUserLanguageCode());
	...
	If SettingMode Or UseBacklight Then
		RemovedFragment = 
			"WHERE ISNULL(PersonalSettingsTip.Visible, AllVariants.VisibleByDefault)";
	...
	If FillingParameters.VariantsQuantity = 0 Then
		...
		If FillingParameters.CurrentSectionVariantsDisplayed Then
			...
			If ValueIsFilled(SearchString) Then
				...
				Else
					Label.Title = NStr("en = 'Reports are not found in the current section.'");
					Label.Height = 2;
				EndIf;
			Else
				Label.Title = NStr("en = 'Report panel of this section does not contain any reports.'");
			EndIf;
		...
		If HighlightParameters <> Undefined Then
			...
			If HighlightParameters.ParametersAndFiltersNames.FoundWordsCount > 0 Then
				If ToolTipContent.Count() > 0 Then
					ToolTipContent.Add(Chars.LF);
				EndIf;
				ToolTipContent.Add(NStr("en = 'Setting:'") + " ");
