...
Procedure DeviceTestEnd(ExecutionResult, Parameters) Export
	...
	MessageText = ?(ExecutionResult.Result,  NStr("en = 'Test succeeded. %AdditionalDetails%'"),
	                               NStr("en = 'Test failed. %AdditionalDetails%'"));
...
Procedure AdditionalAction(Command)
	...
	MessageText = ?(Result,  NStr("en = 'Operation is performed successfully.'"),
...
Procedure StartAdditionalCommandExecutionEnd(ExecutionResult, Parameters) Export
	...
	Else
		...
		DriverVersion  = NStr("en = 'Not defined'");
		If Not IsBlankString(DriverMessage) AND DriverIsSet = NStr("en = 'Set'") Then
	...
	Items.SetupDriver.Enabled = Not (DriverIsSet = NStr("en = 'Set'"));
