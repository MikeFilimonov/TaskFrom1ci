...
#Region TabularDocumentOutputProcedures
...
Procedure DisplayTableDocument()
	...
	Template = GetCommonTemplate("SubordinateDocumentStructure");
...
Function GetDocumentPresentationToPrint(Selection)
	...
	If (Selection.DocumentAmount <> 0) AND (Selection.DocumentAmount <> NULL) Then
		DocumentPresentation = DocumentPresentation + " " + NStr("en = 'to the amount of'") + " " + Selection.DocumentAmount + " " + Selection.Currency + ".";
	EndIf;
...
#Region ProcedureOfTheDocumentSubordinationTreeBuilding
...
Procedure RefreshTreeStructureOfSubjection()
	...
	Else
		CommonUseClientServer.MessageToUser(
			NStr("en = 'The document for which the dependence structure report is generated is no longer available.'"));
	EndIf;
...
Function GetSelectionByDocumentAttributes(DocumentRef)
	...
	ReplaceQueryTextCurrency(QueryText, DocumentMetadata, "#Currency", "Currency");
...
Procedure OutputParentDocuments(CurrentDocument,ParentalTree)
	...
	For Each Attribute In DocumentMetadata.Attributes Do
		...
		If Metadata.FilterCriteria.LinkedDocuments.Content.Contains(Attribute) Then
...
Function GetDocumentsListOnFilterCriterion(ValueOfFilterCriterion)
	...
	If Metadata.FilterCriteria.LinkedDocuments.Type.ContainsType(TypeOf(ValueOfFilterCriterion))  Then
		
		Query = New Query;
		Query.Text = "SELECT ALLOWED
		|	LinkedDocuments.Ref
		|FROM
		|	FilterCriterion.LinkedDocuments(&ValueOfFilterCriterion) AS LinkedDocuments";
...
Function PostDocumentAtServer(DocumentRef)
	...
	If DocumentRef.DeletionMark Then
		...
		Return NStr("en = 'Document marked for deletion cannot be posted.'");
	...
	Except
		...
		ErrorText = NStr("en = 'Action (posting) cannot be performed as the %1 document is locked by the user.'");
...
Function UndoPostingDocumentAtServer(DocumentRef)
	...
	Except
		...
		ErrorText = NStr("en = 'Action (posting cancellation) cannot be performed as the %1 document is locked by the user.'");
		Return StringFunctionsClientServer.SubstituteParametersInString(ErrorText, DocumentRef);
...
Function SetDeletionMarkOfDocumentAtServer(DocumentRef)
	...
	Except
		...
		ErrorText = NStr("en = 'Action (set / clear deletion mark) cannot be performed as the %1 document is locked by the user.'");
		Return StringFunctionsClientServer.SubstituteParametersInString(ErrorText, DocumentRef);
...
Procedure PostDocument(Command)
	...
	If ValueIsFilled(CurrentDocument) Then
		...
		Except
			...
			ErrorText = NStr("en = 'Cannot post the %1 document.'");
...
Procedure UndoPostingDocument(Command)
	...
	If ValueIsFilled(CurrentDocument) Then
		...
		Except
			...
			ErrorText = NStr("en = 'Cannot cancel the %1 document posting.'");;
			ErrorText = StringFunctionsClientServer.SubstituteParametersInString(ErrorText, CurrentDocument);
...
Procedure SetDeletionMarkDocument(Command)
	...
	If ValueIsFilled(CurrentDocument) Then
		...
		Except
			...
			ErrorText = NStr("en = 'Cannot set the deletion mark for document %1.'");
			ErrorText = StringFunctionsClientServer.SubstituteParametersInString(ErrorText, CurrentDocument);
...
Procedure OpenDocument(Command)
	...
	If ValueIsFilled(CurrentDocument) Then
		...
		Except
			...
			ErrorText = NStr("en = 'Failed to open the %1 document.'");
			CommonUseClientServer.MessageToUser(
				StringFunctionsClientServer.SubstituteParametersInString(ErrorText, CurrentDocument)
				);
...
Procedure FindInList(Command)
	...
	Except
		...
		ErrorText = NStr("en = 'Cannot open the %1 document in the list.'");
		CommonUseClientServer.MessageToUser(
			StringFunctionsClientServer.SubstituteParametersInString(ErrorText, CurrentDocument)
			);
