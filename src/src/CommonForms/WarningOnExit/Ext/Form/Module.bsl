...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Hash.Append(KeyVar);
	WindowOptionsKey = "WarningOnExit" + StrReplace(Hash.HashSum, " ", "");
...
Function GenerateFormItemGroup()
	...
	NewGroup = Items.Add(GroupName, Type("FormGroup"), Items.MainGroup);
	NewGroup.Type = FormGroupType.UsualGroup;
	
	NewGroup.HorizontalStretch	= True;
...
Procedure CreateCheckBoxOnForm(QuestionStructure)
	...
	DefaultValue = True;
	NewGroup  = GenerateFormItemGroup();
	
	If Not IsBlankString(QuestionStructure.ExplanationText) Then
		LabelName = FindLabelNameOnForm("QuestionLabel");
		LabelType = Type("FormDecoration");
		
		LabelParent = NewGroup;
