...
#Region ProceduresAndFunctionsUsedToCheckRights
...
// it is additionally checked whether access group provides the right Reading of the specified object (or access value
// set is allowed).
//
// Parameters:
...
Function IsRole(Val Role, Val ObjectReference = Undefined, Val User = Undefined) Export
	...
	If ObjectReference = Undefined OR Not UseRowLevelSecurity() Then
		...
		|		ON (UserGroupMembers.User = &AuthorizedUser)
	...
	|																	INNER JOIN InformationRegister.UserGroupMembers AS UserGroupMembers
	|																	ON
	|																		UserGroupMembers.User = &AuthorizedUser
	|																			AND UserGroupMembers.UsersGroup = RightSettings.User)
	|														AND Not FALSE In
	|																(SELECT TOP 1
	|																	FALSE
	|																FROM
...
//                   the OnFillingPossibleRightsForObjectRightsSetting procedure of the AccessManagementOverridable
//                   general module.
//
//  ObjectReference - CatalogRef, ChartOfCharacteristicTypesRef - ref to one of
//                   the right owners specified
//                   in the OnFillingPossibleRightsForObjectRightsSetting procedure of the AccessManagementOverridable
//                   general module.
//
//  User   - CatalogRef.Users, CatalogRef.ExternalUsers, Undefined - if
//                   parameter is not specified, right is checked for the current user.
...
Function IsRight(Right, ObjectReference, User = Undefined) Export
	...
	If Not UseRowLevelSecurity() Then
		Return True;
	EndIf;
	...
	If RightsDescriptionFull = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Description of possible rights for the ""%1"" table is not found'"),
	...
	If RightDetails = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Right description ""%1"" for table ""%2"" is not found'"),
			Right,
			ObjectReference.Metadata().FullName());
	EndIf;
...
#Region ProceduresAndFunctionsForGettingSubsystemGeneralSettings
...
//
Function UseRowLevelSecurity() Export
	
	SetPrivilegedMode(True);
	
	Return GetFunctionalOption("UseRowLevelSecurity");
	
EndFunction

#EndRegion

#Region ProceduresAndFunctionsUsedToSetManagedFormsInterface
...
// AccessGroup attribute sets a list of access value groups to the selection parameter that provide access for changing
// access value.
//
...
Procedure OnFormCreationAccessValues(Form,
	...
	If ValueGroupsForChanges.Count() = 0
	   AND NewCreation Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Permitted ""%1"" are required for adding.'"),
...
//    set for one string of set, * Change       - Boolean - (optional if the set is for all rights) set for one string
//    of set.
//
...
Procedure FillAccessValueSets(Val Object, Table, Val RefOnSubordinatedObject = Undefined) Export
	...
	// (even including dependent sets) as in the standard templates rights check is built into the logic of the "Object"
	// access kind work.
...
#Region ProceduresAndFunctionsUsedInTheOverridableModule
...
// of the predefined item, for example, "Catalog.UserGroups.AllUsers".
// 
...
//                           several values groups in the access values additional type (there is the AccessGroups
//                           tabular section).
// 
...
#Region ProceduresAndFunctionsUsedDuringTheInfobaseUpdate

// Returns reference to the supplied profile by ID.
//
// Parameters:
//  ID - String - name or unique ID of the
//                  supplied profile as it is specified in the OnFillingAccessKinds procedure of the
//                  AccessManagementOverridable general module.
//
// Returns:
...
#Region ProceduresAndFunctionsUsedToUpdateServiceData
