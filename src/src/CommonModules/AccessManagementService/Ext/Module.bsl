...
#Region DeclaringServiceEventsToWhichHandlersCanBeAttached
...
Procedure SessionParametersSetting(ParameterName, SpecifiedParameters) Export
	...
	If Not Constants.UseRowLevelSecurity.Get() Then
	...
	|							AND AccessGroupsUsers.User = UserGroupMembers.UsersGroup
	|							AND UserGroupMembers.User = &CurrentUser)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|SELECT DISTINCT
...
Procedure UpdatePerformersGroupsUsers(PerformersGroups = Undefined) Export
	...
	InformationRegisters.AccessValuesGroups.UpdateUserGroups(Parameters);
				...
				If OldRoles[String.Role] = Undefined Then
					...
					Else
						...
						For Each Profile In Profiles Do
							WriteLogEvent(
								NStr("en = 'Access management.Role is not found in metadata'",
...
Procedure OnReceiveDataFromSubordinate(DataItem, ItemReceive, SendBack, Sender) Export
	...
	ElsIf TypeOf(DataItem) = Type("ConstantValueManager.UseRowLevelSecurity")
...
//           It should explain the report option
//           content for the user and should not duplicate the report option name.
...
//   
// IMPORTANT:
//   Report serves as options container.
//     By modifying the report settings you can change the settings of all its options at the same time.
...
//
//  It is possible to use
// the "AccessManagement" subsystem when the specified subscription does not exist if the dependent access value sets
// are not applied.
//
...
#Region HandlersOfScheduledJobs
...
Procedure DataFillingForAccessLimit(DataQuantity = 0, OnlyCacheAttributes = False, HasChanges = Undefined) Export
	...
	If AccessManagement.UseRowLevelSecurity() AND Not OnlyCacheAttributes Then
	...
	If DataQuantity < 10000 Then
		WriteLogEvent(
			NStr("en = 'Access management.Data population for access restriction'",
				 CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Information,
			,
			,
			NStr("en = 'Data population for access restriction is completed.'"),
			EventLogEntryTransactionMode.Transactional);
	...
	Else
		WriteLogEvent(
			...
			,
			,
			NStr("en = 'Part of data for access restriction is written.'"),
			EventLogEntryTransactionMode.Transactional);
	EndIf;
...
#Region ProceduresAndFunctionsForWorkWithUsersAndUserGroups
...
Procedure AfterUserGroupStavesUpdating(ParticipantsOfChange, ChangedGroups) Export
	...
	Parameters.Insert("UserGroups", ChangedGroups);
...
#Region ProceduresAndFunctionsForWorksWithAccessKinds
...
#Region ProceduresAndFunctionsForWorkWithAccessValueSets
...
Procedure RecordSetsOfAccessValues(Val Object, HasChanges = Undefined) Export
	...
	If AccessManagement.UseRowLevelSecurity() Then
...
// some of the files from the "Files" catalog can be part of some business processes "Job" created on the basis of files
// and sets of files are not required to be written to the register.
//
// Calls procedure of
// the AccessManagementOverridable applied handler.OnChangeAccessValueSets()
// that is used to
...
#Region ProceduresAndFunctionsForActionsDuringSubsystemSettingsChange

// If it is necessary, it enables data filling
// for the access restrictions and updates some data at once.
//
// Called from the OnWrite handler of the UseRowLevelSecurity constant.
//
Procedure OnChangeLimitAccessOnRecordsLevel(RecordLevelSecurityEnabled) Export
	...
	If RecordLevelSecurityEnabled Then
		...
		WriteLogEvent(
			...
			,
			,
			NStr("en = 'Data population for access restriction is started.'"),
			EventLogEntryTransactionMode.Transactional);
...
#Region ProceduresAndFunctionsForUserInterfacesWork
...
#Region ServicingTablesAccessKindsAndAccessValuesInEditForms
		...
		If IndexOfSeparator = 0 Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'An error occurred while executing the AccessManagement procedure.SetFilterCriteriaInQuery().
				     |
				     |In the ParameterNameFilterConditionsFieldName parameter separator (colon) is not found in the next string of ""<Condition parameter name>:<Field name>"" ""%1"" format.'"),
				CurrentRow);
		EndIf;
...
//                                  calculate the difference between set records, for example, "Dimension1, Dimension2,
//                                  Resource1", the ChangeDate attribute is not included in the list.
//
...
// call the UpdateExecutiveGroupsUsers procedure in the AccessManagement module to apply the changes to the subsystem
// internal data.
//
// Parameters:
...
//    of UpdateResults common module.
...
Procedure FillParameters(InputParameters, Val AllParameters, Val MandatoryParameters = "")
	...
	For Each KeyAndValue In Parameters Do
		If Not AllParameters.Property(KeyAndValue.Key) Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Non-existing parameter %1 is specified'"), KeyAndValue.Key);
		EndIf;
	...
	If ValueIsFilled(MandatoryParameters) Then
		...
		For Each KeyAndValue In MandatoryParameters Do
			If Not Parameters.Property(KeyAndValue.Key) Then
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'Required parameter %1 is not specified'"), KeyAndValue.Key);
			EndIf;
		EndDo;
	EndIf;
...
Function ProcessUserLinkWithAccessGroup(User, ProvidedProfile, Enable = Undefined)
	...
	|	TRUE AS TrueValue
	|FROM
	|	Catalog.AccessGroups.Users AS UserGroupsMembers
	|WHERE
	|	UserGroupsMembers.Ref = &Ref
	|	AND UserGroupsMembers.User = &User";
...
Function CurrentUserProperties(UserArray)
	...
	|	CheckedUsers AS CheckedUsers
	|		INNER JOIN InformationRegister.UserGroupMembers AS UserGroupMembers
	|		ON CheckedUsers.User = UserGroupMembers.User
	|			AND (UserGroupMembers.Used)
	|			AND (&ExcludeExternalUsers)
	|		INNER JOIN Catalog.AccessGroups.Users AS AccessGroupsUsers
	|		ON (UserGroupMembers.UsersGroup = AccessGroupsUsers.User)
...
Function UserProfilesWithRole(CurrentUser, Role)
	...
	|		INNER JOIN Catalog.AccessGroups.Users AS AccessGroupsUsers
	|		ON (UserGroupMembers.User = &CurrentUser)
	|			AND UserGroupMembers.UsersGroup = AccessGroupsUsers.User
	|			AND (UserGroupMembers.Used)
	|			AND (NOT AccessGroupsUsers.Ref.DeletionMark)
	|		INNER JOIN Catalog.AccessGroupsProfiles.Roles AS Roles
...
Procedure FillPresentationTableAllAllowedInForm(Form, IsProfile)
	...
	If IsProfile Then
		...
		String.Presentation = NStr("en = 'All prohibited, exceptions are assigned in access groups'");
		
		String = Form.PresentationsAllAllowed.Add();
		String.Name = "InitiallyAllAllowed";
		String.Presentation = NStr("en = 'All permitted, exceptions are assigned in access groups'");
		
		String = Form.PresentationsAllAllowed.Add();
		String.Name = "AllProhibited";
		String.Presentation = NStr("en = 'All prohibited, exceptions are assigned in a profile'");
		
		String = Form.PresentationsAllAllowed.Add();
		String.Name = "AllAllowed";
		String.Presentation = NStr("en = 'All permitted, exceptions are assigned in a profile'");
	Else
		String = Form.PresentationsAllAllowed.Add();
		String.Name = "AllProhibited";
