...
// CommandParameter) Return AdditionalReportsAndDataProcessors.RunCommandFromExternalObjectForm(CommandID,
// CommandParameter, ThisObject); EndFunction
//
...
// AdditionalReportsAndDataProcessorsClient.FormNameLongOperation() Then
// AdditionalReportsAndDataProcessorsClient.ShowCommandResult (Thisobject, ValueSelected); EndIf;
// EndProcedure
//
...
Procedure RunCommandInBackground(CommandID, CommandParameters, Form) Export
	...
	If WrongType OR AdditionalInformationProcessorRef = PredefinedValue("Catalog.AdditionalReportsAndDataProcessors.EmptyRef") Then
		...
		ErrorText = NStr("en = 'Incorrect value of parameter AdditionalDataProcessorRef:'") + Chars.LF;
		If WrongType Then
			ErrorText = ErrorText + StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Type ""%1"" is transferred, ""%2"" was expected.'"),
				String(TypeOf(AdditionalInformationProcessorRef)),
				String(Type("CatalogRef.AdditionalReportsAndDataProcessors")));
		Else
			ErrorText = ErrorText + NStr("en = 'Null reference passed. Data processor might have been opened directly.'");
		EndIf;
...
Function ExecuteAllocatedCommandAtClient(Form, ItemName) Export
	...
	If Object.Ref.IsEmpty() OR Form.Modified Then
		QuestionText = StrReplace(
			NStr("en = 'To run the ""%1"" command, write the data.'"),
			"%1",
...
Procedure ReportMailingPickupAddReport(FormItem) Export
	...
	AdditionalReport = PredefinedValue("Enum.AdditionalReportsAndDataProcessorsTypes.AdditionalReport");
	Report               = PredefinedValue("Enum.AdditionalReportsAndDataProcessorsTypes.Report");
...
Function ExecuteAssignedPrintCommand(ExecuteCommand, Form) Export
	...
	ExecuteCommand.Insert("Kind", PredefinedValue("Enum.AdditionalReportsAndDataProcessorsTypes.PrintForm"));
...
Procedure ShowNotificationOnCommandExecution(ExecuteCommand) Export
	If ExecuteCommand.ShowAlert Then
		ShowUserNotification(NStr("en = 'Executing the command...'"), , ExecuteCommand.Presentation);
	EndIf;
EndProcedure
...
Procedure RunClientMethodOfDataProcessor(ExecuteCommand, Form, DestinationObjects) Export
	...
	If ExecuteCommand.Type = PredefinedValue("Enum.AdditionalReportsAndDataProcessorsTypes.AdditionalInformationProcessor")
		Or ExecuteCommand.Type = PredefinedValue("Enum.AdditionalReportsAndDataProcessorsTypes.AdditionalReport") Then
		
		ProcessingForm.RunCommand(ExecuteCommand.ID);
		
	ElsIf ExecuteCommand.Type = PredefinedValue("Enum.AdditionalReportsAndDataProcessorsTypes.CreatingLinkedObjects") Then
	...
	ElsIf ExecuteCommand.Type = PredefinedValue("Enum.AdditionalReportsAndDataProcessorsTypes.PrintForm") Then
		
		ProcessingForm.Print(ExecuteCommand.ID, DestinationObjects);
		
	ElsIf ExecuteCommand.Type = PredefinedValue("Enum.AdditionalReportsAndDataProcessorsTypes.ObjectFill") Then
	...
	ElsIf ExecuteCommand.Type = PredefinedValue("Enum.AdditionalReportsAndDataProcessorsTypes.Report") Then
...
Procedure ExportToFile(ExportParameters) Export
	MessageText = NStr("en = 'To export an external data processor (repot) to the file, install extension for 1C:Enterprise web client.'");
...
#Region ServiceHandlersForAsynchronousDialogs
