...
Function GetFileFromInternet(Val SessionKey, Val URL, Val Port = 0, Val UserName = "", Val Password = "", Val Timeout = 20,Val SecureConnection = Undefined, Val PassiveConnection = Undefined) Export
	...
	If Not CommonUse.SubsystemExists("StandardSubsystems.GetFilesFromInternet") Then
		Raise NStr("en = 'Call of function AdditionalReportsAndDataProcessorsInSafeMode.GetFileFromInternet is not supported in this configuration.'");
	EndIf;
	...
	If Upper(Protocol) = "FTP" Then
		...
		Except
			ErrorInfo = ErrorInfo();
			ErrorInfo = NStr("en = 'An error occurred when establishing FTP connection with server %1:'") + Chars.LF + "%2";
	...
	Else
		...
		Except
			ErrorInfo = ErrorInfo();
			ErrorInfo = NStr("en = 'An error occurred when setting up HTTP connection with server %1:'") + Chars.LF + "%2";
	...
	Except
		ErrorInfo = ErrorInfo();
		ErrorInfo = NStr("en = 'An error occurred when receiving the file from server %1:'") + Chars.LF + "%2";
...
Function ImportFileInInternet(Val SessionKey, Val BinaryDataAddress, Val URL, Val Port = 0, Val UserName = "", Val Password = "", Val Timeout = 20,Val SecureConnection = Undefined, Val PassiveConnection = Undefined) Export
	...
	If Not CommonUse.SubsystemExists("StandardSubsystems.GetFilesFromInternet") Then
		Raise NStr("en = 'Call of function AdditionalReportsAndDataProcessorsInSafeMode.ImportFileInInternet is not supported in this configuration.'");
	EndIf;
	...
	Except
		ErrorInfo = ErrorInfo();
		ErrorInfo = NStr("en = 'An error occurred when transferring file to the server %1:'") + Chars.LF + "%2";
...
Procedure CheckCorrectnessOfMethodNameOfExternalObject(Val MethodName)
	...
	For Each ForbiddenSymbol In ForbiddenSymbols Do
		...
		If Find(MethodName, ForbiddenSymbol) > 0 Then
			...
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = '%1 not a correct method name for COM object or external component object.'"),
				MethodName);
