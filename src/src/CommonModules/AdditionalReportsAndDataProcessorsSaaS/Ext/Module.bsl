...
Procedure SetIncludedProcessingInDataArea(Val InstallationDetails, Val QuickAccess, Val Tasks, Val Sections, Val CatalogsAndDocuments, Val SettingsLocationOfCommands, Val AdditionalReportVariants, Val Responsible) Export
	...
	WriteLogEvent(
		NStr("en = 'Supplied additional reports and data processors.Installation of supplied data processor in data area initiated'",
	...
	Try
		...
		If CommonUse.RefExists(SuppliedDataProcessor) Then
			...
			WriteLogEvent(NStr("en = 'Additional supplied reports and data processors.Installation in the data area'",
				CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Information,
				,
		...
		Else
			...
			WriteLogEvent(NStr("en = 'Additional supplied reports and data processors. Installation to the data area is deferred'",
				CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Information,
				,
				String(InstallationDetails.ID),
...
Procedure DeleteComesFromDataProcessingAreas(Val SuppliedDataProcessor, Val IDOfUsedDataProcessor) Export
	...
	Try
		...
		Try
			...
			WriteLogEvent(NStr("en = 'Supplied additional reports and data processors.Delete from the data area'",
				CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Information,
				,
				SuppliedDataProcessor,
		...
		Except
			...
			WriteLogEvent(NStr("en = 'Supplied additional reports and data processors.Error deleting from the data area'",
				CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Error,
				,
				SuppliedDataProcessor,
...
Procedure ProcessAdditionalInformationProcessorSettingToDataAreaError(Val SuppliedDataProcessor, Val InstallationID, Val ErrorMessage) Export
	...
	WriteLogEvent(NStr("en = 'Additional supplied reports and data processors. An error occurred during installation to the data area'",
		CommonUseClientServer.MainLanguageCode()),
		EventLogLevel.Error,
		,
		SuppliedDataProcessor,
...
//      StandardProcessing - Boolean, in this procedure this parameter is used to set the flag showing that the right is
//                           checked by standard processor
//
...
//      additional report or data processor from file, StandardProcessing - Boolean, in this procedure this parameter is
//      used to set the flag showing the execution of the standard processing checking the option to import an
...
//    sets the flag of checking standard processing completion possibility of additional report or data processor import
//    in file.
//
...
// Parameters:
//  AdditionalInformationProcessor - CatalogRef.AdditionalReportsAndDataProcessors,
//  Result - Boolean, in this procedure this parameter is used to set the flag showing the necessity to display detailed
//           information on additional reports and data processors import in user interface.
		...
		For Each ControlledAttribute In ControlledAttributes Do
			...
			If SourceProp <> ResultAttribute Then
				...
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'Invalid attempt to change attribute value %1 for additional data processor %2 received from the additional data processor directory of the service manager.'"), 
...
Procedure OnEnableExternalProcessor(Val Ref, StandardProcessing, Result) Export
	...
	If CommonUseReUse.DataSeparationEnabled() Then
		...
		If TypeOf(Ref) <> Type("CatalogRef.AdditionalReportsAndDataProcessors") Or Ref = Catalogs.AdditionalReportsAndDataProcessors.EmptyRef() Then
			Raise NStr("en = 'A non-existent additional data processor connection requested.'");
		EndIf;
...
Procedure OnCreateExternalDataProcessor(Val Ref, StandardProcessing, Result) Export
	...
	If Not StandardProcessing Then
		...
		If DataProcessorName = Undefined Then
			Raise NStr("en = 'The creation is requested for object of a non-existent additional data processor.'");
		EndIf;
...
Procedure OnGetSafeModeSessionsPermissions(Val SessionKey, PermissionDescriptions, StandardProcessing) Export
	...
	If CommonUseReUse.DataSeparationEnabled() Then
		...
		Else
			...
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Supplied data processor for startup key %1 is not found.'"),
				String(SessionKey));
...
// 
// If NewDataAvailable sets the Import argument to the True value, the data is imported, the handle and the file path
// with data are passed to the ProcessNewData procedure. File will be automatically deleted after procedure completed.
...
Procedure UpdateSettingsUsedByDataProcessors(Val Ref) Export
	...
	Else
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Additional data processor with %1 ID is not supplied.'"),
			String(Ref.UUID()));
...
// IN the body you should check whether this data is necessary for the application, and if so, - select the Import check
// box.
// 
// Parameters:
...
Procedure AvailableNewData(Val Handle, Import) Export
	...
	If Handle.DataType = ProvidedDataKindId() Then
		...
		Else
			...
			WriteLogEvent(
				NStr("en = 'Supplied additional reports and data processors.Supplied data processor import canceled'", 
				CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Information,
				,
				,
				NStr("en = 'The supplied processor is not compatible with this configuration'") + Chars.LF + Chars.CR + ProvidedDataProcessorDescription.Compatibility);
...
Procedure ProcessSuppliedAdditionalReportsAndDataProcessors(Handle, PathToFile)
	...
	WriteLogEvent(NStr("en = 'Supplied additional reports and data processors.Import supplied data processor'", 
		CommonUseClientServer.MainLanguageCode()),
		EventLogLevel.Information,
		,
		,
		NStr("en = 'Supplied data processor import initiated'") + Chars.LF + Chars.CR + ProvidedDataProcessorDescription.Manifest);
	...
	If SuppliedDataProcessor.GUIDVersion <> ProvidedDataProcessorDescription.Version Then
		...
		WriteLogEvent(NStr("en = 'Supplied additional reports and data processors.Supplied data processor is imported'",
			CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Information,
			,
			SuppliedDataProcessor.Ref,
			NStr("en = 'Supplied data processor import is completed.'") + Chars.LF + Chars.CR + ProvidedDataProcessorDescription.Manifest);
		...
		For Each InstallationOfDataProcessors In UsedDataProcessors Do
			...
			WriteLogEvent(NStr("en = 'Supplied additional reports and data processors.Update of the supplied data processor settings is scheduled'",
				CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Information,
				,
				SuppliedDataProcessor.Ref,
		...
		For Each ItemInQueue In InstallationsQueue Do
			...
			WriteLogEvent(
				NStr("en = 'Additional supplied reports and data processors. Deferred installation of the supplied data processor in the data area is scheduled'",
				CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Information,
				,
				SuppliedDataProcessor.Ref,
...
Function IsReport(Val Ref)
	...
	Return (Kind = Enums.AdditionalReportsAndDataProcessorsTypes.Report) Or (Kind = Enums.AdditionalReportsAndDataProcessorsTypes.AdditionalReport);
	...
	If CommonUseReUse.DataSeparationEnabled() Then
		...
		Else
			...
			If Not Constants.AdditionalReportsAndDataProcessorsIndependentUseSaaS.Get() Then
				...
				Raise NStr("en = 'This additional report cannot be used in the service.'");
...
Procedure CheckSuppliedDataProcessorExecutionPossibility(Val UsingDataProcessor)
	...
	If UsedDataProcessorPublicationParameters.Publication = Enums.AdditionalReportsAndDataProcessorsPublicationOptions.Disabled Then
		Raise NStr("en = 'Use of additional data processor is prohibited in your application. Contact the user with administrative rights in this application.'");
	EndIf;
	...
	If ValueIsFilled(SuppliedDataProcessor) Then
		...
		If UsedDataProcessorPublicationParameters.Version <> ProvidedDataProcessorPublicationParameters.Version Then
			Raise NStr("en = 'Additional data processor is temporarily unavailable as it is being updated. It may take several minutes. We apologize for the inconvenience.'");
		EndIf;
