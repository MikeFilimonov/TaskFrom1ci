...
Procedure SetJobParameters(Task, Use, Parameters, Schedule) Export
	...
	If Not Constants.AllowAdditionalReportsAndDataProcessorsPerformByProceduralTasksSaaS.Get() Then
		Raise NStr("en = 'It is prohibited by the service administration to periodically perform additional data processor commands as jobs.'");
	EndIf;
	...
	If Schedule.ExecutionRequired(ChackedDate, OriginalDate) Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Schedule set for execution of commands of additional report or data processor as jobs should be maximum once in %1 seconds.'"), MinInterval);
