...
//          Return value of
//        the function ClusterAdministrationClientServer.SessionProperties(), ComparisonType - value of the systematic
//        enum ComparisonType, kind of comparing sessions values with the specified in the filter condition. Possible values:
...
//    that will be used for the comparison to be done, 2. Structure (simplified version), key - Name of the session's
...
//  as the security profile of the safe mode for the infobase. If the security profile is not assigned for the infobase
...
Function SecurityProfile(Val ClusterAdministrationParameters, Val ProfileName) Export
	...
	If SecurityProfiles.Count() <> 1 Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'Security profile %2 is not registered in server cluster %1.'"), ClusterIdentifier, ProfileName);
	EndIf;
...
Procedure CreateSecurityProfile(Val ClusterAdministrationParameters, Val SecurityProfileProperties) Export
	...
	If SecurityProfiles.Count() = 1 Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'Security profile %2 is already registered in server cluster %1.'"), ClusterIdentifier, ProfileName);
	EndIf;
...
Function GetCluster(Val ClusterAdministrationParameters)
	...
	Else
		Raise StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'Server cluster with port %1 is not found'"), ClusterAdministrationParameters.ClusterPort);
	EndIf;
...
Function GetInfobase(Val ClusterIdentifier, Val ClusterAdministrationParameters, Val InformationBaseAdministrationParameters)
	...
	Else
		Raise StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'Infobase %2 is not registered in the %1 server cluster.'"), ClusterIdentifier, InformationBaseAdministrationParameters.NameInCluster);
	EndIf;
...
//          values of which form the period
//    that will be used for the comparison to be done, 2. Structure (simplified version), key - Name of the session's
//    property (see above), value - the value with which the comparison is executed. When this variant of the filter
//    description is used, the
//  comparison is always executed on the equality, UseDictionary - Boolean if True - return result will be filled in
//  using the dictionary, otherwise, - without usage.
//
...
//  utility and in the required description, ItemProperties - Structure, properties values of the item of the access
//  control list.
//
...
Function RunCommand(Val Pattern, Val ClusterAdministrationParameters, Val ParameterValues = Undefined)
	...
	#If Server Then
		...
		If SafeMode() Then
			Raise NStr("en = 'Cluster administration is unavailable in the safe mode.'");
		EndIf;
		
		If CommonUseReUse.DataSeparationEnabled() Then
			Raise NStr("en = 'Applied infobase cannot administer cluster in SaaS.'");
		EndIf;
...
//  flow of the administer server console client to matching. Match keys - names of the properties, map values -
//  properties values.
//
// Parameters:
...
//  API notation, Dictionary - Structure, description of the properties names match in the API notation and the rac
//  utility notation.
//
...
//         in
//  API (see ClusterAdministrationClientServer.SessionsAndScheduledJobsLockingProperties()), Value - String, the name of
//  the object property.
//
