...
Function GetFileBinaryData(Val AttachedFile) Export
	...
	CommonUseClientServer.Validate(FileObject <> Undefined, 
		StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'Attached file ""%1"" is not found (%2)'"),
	...
	If FileObject.FileStorageType = Enums.FileStorageTypes.InInfobase Then
		...
		Else
			...
			ErrorInfo = StringFunctionsClientServer.SubstituteParametersInString(
				...
				GetURL(AttachedFile));
			WriteLogEvent(NStr("en = 'Files.Open file'", CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Error,
	...
	Else
		...
		Except
			...
			WriteLogEvent(NStr("en = 'Files.Receive a file from the volume'", CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Error,
				Metadata.Catalogs[AttachedFile.Metadata().Name],
				AttachedFile,
				ErrorInfo);
...
//                                            the file change (UTC + 0:00) if it is not specified, then
//                                            CurrentUniversalDate() is used.
...
Function AddFile(Val FilesOwner,
	...
	ErrorTitle = NStr("en = 'An error occurred when adding the attached file.'");
	
	If NewRefToFile = Undefined Then
		CatalogName = AttachedFilesService.CatalogNameStorageFiles(FilesOwner, "", ErrorTitle,
			NStr("en = 'In this case, parameter NewRefToFile must be specified.'"));
	...
	Except
		...
		WriteLogEvent(
			NStr("en = 'File.Adding an attached file'",
...
Function NewRefToFile(FilesOwner, CatalogName = Undefined) Export
	...
	ErrorTitle = NStr("en = 'An error occurred when receiving a new reference to the attached file.'");
...
Function GetAttachedFilesObjectFormNameByOwner(Val FilesOwner) Export
	...
	ErrorTitle = NStr("en = 'An error occurred when receiving a form name of the attached file.'");
	EndErrors = NStr("en = 'In this case, the form cannot be received.'");
...
#Region ProceduresConnectedToTheManagedFormsEvents
...
#Region ProceduresCalledFromTheModuleOfCatalogsManagerWithTheAttachedFiles
...
#Region OperationsWithTheDigitalSignature
...
Procedure AddInformationOfOneSignature(Val AttachedFile, Val SignatureData) Export
	...
	SignatureProperties.Insert("ElectronicSignature",          SignatureData.CertificateBinaryData);
...
Procedure PerformActionsBeforeWriteAttachedFile(Source, Cancel) Export
	...
	If Not ValueIsFilled(Source.FileOwner) Then
		...
		If UpdateResults.InfobaseUpdateInProgress() Then
			...
			WriteLogEvent(
				NStr("en = 'Files. An error occurred when writing the file while updating IB'",
				     CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Error,
				,
				Source.Ref,
...
Procedure OnCreateAtServerAttachedFile(Val Form) Export
	...
	Decoration.Title = NStr("en = 'In order to proceed to the file card, click the hyperlink'");
...
Procedure MarkToDeleteAttachedFiles(Val Source, CatalogName = Undefined)
	...
	Except
		...
		Raise NStr("en = 'An error occurred when marking the attached files for deletion.'")
			+ Chars.LF
			+ ErrorPresentation;
	EndTry;
