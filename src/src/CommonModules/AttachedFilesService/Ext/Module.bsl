...
#Region ProceduresUsedWhileExchangingDataAndDownoadingImportingData
		...
		Else
			...
			Else // Enums.FileStorageTypes.InInfobase
				...
				Except
					...
					WriteLogEvent(
						NStr("en = 'Files.Cannot send the file during data exchange'",
...
Procedure OnReceiveFile(DataItem, ItemReceive) Export
	...
	ElsIf IsItemAttachedFiles(DataItem) Then
		...
		// Delete from volumes for existing files placed in volumes as while receiving a new file it will be placed to volume
		// or infobase again.
...
Procedure UpdateBinaryDataOfFileAtServer(Val AttachedFile,
	...
	Try
		...
		Else
			...
			Except
				ErrorInfo = ErrorInfo();
				WriteLogEvent(
					NStr("en = 'Files.Writing file on disk'", CommonUseClientServer.MainLanguageCode()),
					EventLogLevel.Error,
	...
	Except
		...
		EndIf;
		WriteLogEvent(
			NStr("en = 'Files.Updating the attached file data in the file storage'",
			     CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Error,
			,
			,
...
Procedure CopyAttachedFiles(Val Source, Val Recipient) Export
	...
	For Each CopiedFile In CopiedFiles Do
		...
		For Each CopyTableRow In CopiedFile.DigitalSignatures Do
			...
			TableRowCopy.ElectronicSignature = CopyTableRow.ElectronicSignature;
		EndDo;
...
Procedure WhenConfidentialityOfFilesInVolumeWhenPlacing(FilesPathCompliance, StoreFilesInDirectoriesOnHardDisk, AttachedFiles) Export
	...
	AddFilesToVolumesOnPlacing(FilesPathCompliance, StoreFilesInDirectoriesOnHardDisk, AttachedFiles);
