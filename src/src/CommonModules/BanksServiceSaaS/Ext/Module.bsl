...
// Imports the file of the bank classifier from the data supplied by a service manager.
//
Procedure ImportBankClassifier() Export
	
	Descriptors = SuppliedData.ProvidedDataFromManagerDescriptors("Banks");
	
	If Descriptors.Descriptor.Count() < 1 Then
		Raise(NStr("en = 'There is no data of the Banks kind in the service manager'"));
	EndIf;
...
Procedure RegisterProvidedDataHandlers(Val Handlers) Export
	...
	Handler.DataKind = "Banks";
	Handler.ProcessorCode = "ImportingBankClassifier";
...
// Parameters:
//   Handle       - XDTOObject Descriptor.
//   PathToFile   - String or Undefined. The full name of the extracted file. File will be automatically deleted after
//                  procedure completed. If the file was not specified
//                  in the service manager - the argument value is Undefined.
//
Procedure ProcessNewData(Val Handle, Val PathToFile) Export
	
	If Handle.DataType = "Banks" AND ValueIsFilled(PathToFile) Then
		WorkWithBanks.ImportDataFromFile(PathToFile);
	EndIf;
