...
Function GetDatesArrayByCalendar(Val WorkSchedule, Val DateFrom, Val DaysArray, Val CalculateNextDateFromPrevious = False, CallingException = True) Export
	...
	|	AND CalendarSchedules.DayKind IN (VALUE(Enum.BusinessCalendarDayType.Working), VALUE(Enum.BusinessCalendarDayType.Preholiday))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|SELECT
	...
	While Selection.Next() Do
		If Selection.DateInCalendar = Undefined Then
			ErrorInfo = NStr("en = 'The ""%1"" business calendar is not filled in from date %2 for the specified number of working days.'");
...
Function GetDatesDiffByCalendar(Val WorkSchedule, Val StartDate, Val EndDate, CallingException = True) Export
	...
	If TypeOf(WorkSchedule) = Type("CatalogRef.BusinessCalendars") Then
		...
		|	CalendarSchedules.Date AS ScheduleDate,
		|	CASE
		|		WHEN CalendarSchedules.DayKind IN (VALUE(Enum.BusinessCalendarDayType.Working), VALUE(Enum.BusinessCalendarDayType.Preholiday))
		|			THEN TRUE
		|		ELSE FALSE
	...
	If Result.IsEmpty() Then
		If CallingException Then
			ErrorInfo = NStr("en = 'The ""%1"" work schedule is not filled in for period %2.'");
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				ErrorInfo,
	...
	// Subtract all the following dates from the value set for the first selection date. As a result, get a number of days
	// included into schedule for the whole period with the minus sign. If the first selection day is a business day, and
	// the following one - weekend, then a number of days included in both dates will be the same.
...
Function GetWorkingDaysDates(Schedule, BeginningDates, GetPreceding = False, CallingException = True, IgnoreUnfilledGraphics = False) Export
	...
	If TypeOf(Schedule) = Type("CatalogRef.BusinessCalendars") Then
		...
		|			VALUE(Enum.BusinessCalendarDayType.Working), 
		|			VALUE(Enum.BusinessCalendarDayType.Preholiday)
		|			))
		|
		|GROUP BY
		|	BeginningDates.Date";
		...
		Else 
			...
			If CallingException Then
				ErrorMessage = StringFunctionsClientServer.SubstituteParametersInString(
									NStr("en = 'Cannot determine the nearest workday for date %1, work schedule might not be populated.'"), 
...
Function SchedulesOfWorkOnPeriod(Graphics, StartDate, EndDate) Export
	...
	Raise NStr("en = 'Subsystem ""Work schedules"" not found.'");
...
// Creates temporary table VTDaysIncrement in which a string with an item index and value is created for each item from
// DaysArray. - by number of days.
// 
// Parameters:
