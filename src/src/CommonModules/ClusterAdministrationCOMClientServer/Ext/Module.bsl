...
Function GetCluster(ConnectionToServerAgent, Val ClusterPort, Val ClusterAdministratorName, Val ClusterAdministratorPassword)
	...
	Raise StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = 'Cluster %2 is not found on server %1'"),
...
Function WorkingProcessConnection(COMConnector, ConnectionToServerAgent, Cluster)
	...
	Raise StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = 'Active processes are not found in server cluster %1:%2.'"),
		Cluster.HostName,
...
Function GetIBDescription(ConnectionToServerAgent, Cluster, Val NameInCluster)
	...
	Raise StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = 'Infobase ""%3"" was not found in server cluster %1:%2.'"),
		Cluster.HostName,
		Format(Cluster.MainPort, "NG=0"),
		NameInCluster
	);
	...
	For Each Infobase In WorkingProcessConnection.GetInfobases() Do
		...
		If Infobase.Name = NameInCluster Then
			...
			If Not ValueIsFilled(Infobase.DBMS) Then
				...
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'The administrator user name or password is incorrect in the cluster of servers %2:%3 in infobase %1 (name: ""%4"").'"),
					NameInCluster,
...
//    that will be used for the comparison to be done, 2. Structure (simplified version), key - Name of the session's
//    property (see above), value - the value with which the comparison is executed. When using this option of the
//    filter description, the comparison
//  is always done by equality, Descriptions - Boolean if value is False, the function returns the array
//    of COMObjects that implement the ISessionInfo interface, else - array of structures that describe the properties
//    of the sessions (fields of structures - see return value of the function. ClusterAdministrationClientServer.SessionProperties()).
//
...
//  the infobase administrator, Filter - Description of the connection filtering conditions, descriptions of which must
//  be received. Variants:
//      1. Array of structures that describe conditions of the connections filtering. Fields of structures included in
//      the array:
//        Property - String, the name of the property used for filtering. Possible values - see
//          Return value of
...
//             array of COMObjects that implement the IConnectionShort interface, else - array of structures that
//             describe the connections properties (fields of structures - see return value of the function.
...
Function GetSecurityProfile(ConnectionToServerAgent, Cluster, ProfileName)
	...
	Raise StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = 'Security profile ""%3"" is not found in server cluster %1:%2.'"),
		Cluster.HostName,
		Format(Cluster.MainPort, "NG=0"),
		ProfileName
	);
...
//  the interface ISecurityProfile, SecurityProfileProperties - Structure that describes the security profile. Content -
...
//  filling the properties of the object, Dictionary - Maping contains correspondence of the object properties and in
//  the description:
//    Key - Property name in
//    the description, Value - The name of the object property.
//
