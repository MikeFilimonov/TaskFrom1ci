...
//    administrator's account (if the list of administrators is not specified for the cluster or if the password is not
//    set for the account) - an empty row is used),
//
...
//    if the session executes an installing of managed transactional blocks and waits for removal of blocks established
//    by another session (otherwise - value is 0),
//  BlockDBMS - Number, the number of session that
//    is the reason for the waiting of a transactional block, in that case,
//    if a session executes a request to DBMS and waits a transactional block established by another session (otherwise
//    - value is 0,
...
//    since the beginning of the call, in case the call of the 1C:Enterprise server is executed in the session (else -
//    value is 0),
...
//    a transaction is opened or temporary tables are defined (i.e. the connection with DBMS is captured). If the
//    connection with the DBMS is not captured - value is an empty row,
...
//          ComparisonType.PeriodIncludingBorders, ComparisonType.IntervalIncludingLowerBound and
//          ComparisonType.IntervalIncludingUpperBound shall be passed as the structures with the From and To fields the
//          values of which form the period that will be used for the comparison to be done,
...
//    at the time of getting the connections list this connection executed request to the server DBMS, otherwise - value
//    is an empty row). Identifier returns in terms of DBMS server.
//  TimeDBMS - Number, time in seconds, during which the request to DBMS server is executed
//    (if at the time of getting the connections list this connection executed the request to DBMS server, otherwise -
//    value is 0),
//    
...
//    Variants:
//      1. Array of structures that describe conditions of the connections filtering. Fields of structures included in
//      the array:
//        Property - String, the name of the property used for filtering. Possible values - see
//          Return value of
//        the function ClusterAdministrationClientServer.ConnectionProperties(), ComparisonType - value of the
//        systematic enum ComparisonType, kind of comparing connections values with the specified in the filter
//        condition. Possible values: ComparisonType.Equal,
//            ComparisonType.NotEqual,
...
//          ComparisonType.PeriodIncludingBorders, ComparisonType.IntervalIncludingLowerBound and
//          ComparisonType.IntervalIncludingUpperBound shall be passed as the structures with the From and To fields the
//          values of which form the period that
//    will be used for the comparison to be done, 2. Structure (simplified version), key - Name of the connection
//    property (see above), value - the value with which the comparison is executed. When using this option of the
//    filter description, the comparison is always executed on equality.
//
// Return value: Array (Structure) array of structures that describe the connections properties. Description of
// structures - see ClusterAdministrationClientServer.ConnectionProperties().
//
...
//    Variants:
//      1. Array of structures that describe the filtering conditions of the terminated connections. Fields of
//      structures included in the array:
//        Property - String, the name of the property used for filtering. Possible values - see
//          Return value of
//        the function ClusterAdministrationClientServer.ConnectionProperties(), ComparisonType - value of the
//        systematic enum ComparisonType, kind of comparing connections values with the specified in the filter
...
//  assigned as the security profile of the safe mode for the infobase. If the security profile is not assigned for the
//  infobase - an empty string is returned.
//
...
//  of the file system, listed in the property of VirtualDirectories, COMObjectsFullAccess - Boolean determines the
//  existence of restrictions on access to use. COMObjects. When you set the value as False the access will
//    be given only to
...
//  external components, listed in the property of ExternalComponents, FullAccessToExternalModules - Boolean, defines
//  the access estrictions to use the external modules (external reports and processors, Execute() and Calculate()
//  calls) in unsafe mode. When you set the value as False, the option to
//    use only external modules in an unsafe mode
//  will be given that are listed in the property of ExternalModules, FullAccessToOperatingSystemApplications - Boolean,
//  defines the access restrictions to the use the applications of operating system. When you set the value as
//    False, there will be the option to use only the
//  operating system applications that are listed in the property of OSApplications, FullAccessToInternetResources -
//  Boolean determines the existence of restrictions on access to use. Internet resources. When you set the value as
//  False, the option to use only internet resources
//  listed in a property will be given, VirtualDirectories - Array(Structure), array of structures that
//    describes the virtual directories that give access when installing FullAccessToFileSystem = False. Descriptions of
//    the structures fields -
//   see ClusterAdministrationClientServer.VirtualDirectoryProperties(),
//  COMClasses - Array(Structure), array of structures that
//    describe COM classes that give access when installing COMObjectsFullAccess = False. Descriptions of the structures
//    fields - see ClusterAdministrationClientServer.COMClassProperties(),
//  ExternalComponents - Array(Structure), array of structures that describe
//    external components that give access when installing FullAccessToExternalComponents = False. Descriptions of the
//    structures fields - see ClusterAdministrationClientServer.ExternalComponentProperties(),
//  ExternalModules - Array(Structure), array of structures that describe
//    the external modules permitted to access in the unsafe mode when setting the FullAccessToExternalModules = False.
//    Descriptions of the structures fields - see ClusterAdministrationClientServer.ExternalModuleProperties(),
//  OSApplications - Array(Structure), array of structures that describe
//    the applications of the operating system that give access when you installing
//    FullAccessToOperatingSystemApplications = False. Descriptions of the structures fields - see ClusterAdministrationClientServer.OSApplicationsProperties(),
//  InternetResources - Array (Structure), array of structures that
//    describe internet resources that give access when installing FullAccessToInternetResources = false. Descriptions
//    of the structures fields - see ClusterAdministrationClientServer.InternetResourceProperties().
//
...
//  which can be created in the operating system, Computer - String, name of the computer on which the COM object can be
//  created.
//
...
//          ComparisonType.PeriodIncludingBorders, ComparisonType.IntervalIncludingLowerBound and
//          ComparisonType.IntervalIncludingUpperBound shall be passed as the structures with the From and To fields the
//          values of which form the period
//    that will be used for the comparison to be done, 2. Structure (simplified version), key - Property name (see
//    above), value - the value with which the comparison is executed. When using this option of the filter description,
//    the comparison is always executed on equality.
//
...
// application interface for the administration of the servers cluster that corresponds to the type of connection to
// administered servers cluster.
//
// Parameters:
//  ClusterAdministrationParameters - Structure that describes the connection parameters
//    to the server cluster, Description - see ClusterAdministrationClientServer.ClusterAdministrationParameters().
