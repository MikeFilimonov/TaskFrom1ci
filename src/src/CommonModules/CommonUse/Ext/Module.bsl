...
#Region CommonProceduresAndFunctionsForWorkWithDataInBase
...
Function PostDocuments(Documents) Export
	...
	For Each DocumentRef In documents Do
		...
		If DocumentObject.CheckFilling() Then
			...
			EndTry;
		Else
			ErrorPresentation = NStr("en = 'Document fields are not populated.'");
		EndIf;
...
//
//                                * RemovalMethod - String - the deletion method, one of the following:
//                                      "Directly"      - if a reference is not used anywhere after the replacement, delete it.
//                                      "Check"        - if a reference is not used anywhere after the replacement, mark it for deletion.
//                                      Any other value - do not delete the reference.
//                                      
...
//                                                                     the write is executed in the exclusive mode, else
//                                                                     - with the current rights.
		...
		If Errors.Count() > 0 Then
			...
			For Each KeyValue In SubstitutionsPairs Do
				If Errors[KeyValue.Key] = Undefined Then
					...
					ErrorText = NStr("en = 'Not replaced due to the previous issues.'");
					Cause = ReplacementErrorDescription("RecordingError", ForWhat, SubjectString(ForWhat), ErrorText);
					AddReplacementResult(ReplacementResult, What, Cause);
				EndIf;
			EndDo;
...
#Region CommonProceduresAndFunctionsForWorkWithAppliedTypesAndValuesCollections
...
// Procedure deletes from the AttributesArray items corresponding to the names of the object attributes from the
// UncheckedAttributesArray array. To use in the handler of the FillCheckProcessing event.
//
// Parameters:
...
// Function creates a structure with properties like
// values table columns of the passed rows and
...
Function ReadXMLToTable(Val XML) Export
	...
	ElsIf Read.Name <> "Items" Then
		Raise NStr("en = 'An error occurred in XML structure'");
	EndIf;
...
// Function compares two strings collections for which the bypass using the For each ... operator is available. From ...
// Cycle. Compared collections should meet the following requirements:
...
// IncludingRowOrder - Boolean - if True, then the collections are considered to be identical only if the same rows are
//                               located on the same places in collections.
//
// Returns:
//  Boolean.
//
Function IdenticalCollections(RowsCollection1, RowsCollection2, ColumnNames = "", ExcludingColumns = "", IncludingRowOrder = False) Export
	...
	If IsBlankString(ColumnNames) Then
		If SpecialCollectionTypes.Find(TypeOf(RowsCollection1)) <> Undefined 
			...
			EndIf;
		Else
			ErrorMessage = NStr("en = 'Specify names of fields for comparison for the collection of type %1'");
		...
		For Each FilterRow In RowsCollection1 Do
			...
			For Each CollectionRow2 In RowsCollection2 Do
				...
				If RowFits Then
					...
					// If there are more such rows in the second collection than in the first one, then it concludes that the
					// collections are not identical.
...
#Region MathematicalProceduresAndFunctions
...
#Region ProceduresAndFunctionsForWorkWithExternalConnection
...
Function COMConnectorIdentifier(Val COMConnectorName) Export
	...
	ErrorMessage = NStr("en = 'CLSID is not specified for class %1'");
...
// Only those objects can be converted to an XML string (serialized) for those it is stated in the description that they
...
// Sets or clears the deletion mark for all objects that refer to the specified owner object.
//
// Parameters:
...
//
//  DeletionMark - Boolean - if True, set or clear the deletion mark for all subordinate objects.
//
...
Function ExecuteQueryBeyondTransaction(Val Query) Export
	...
	If TransactionActive() Then
		Raise(NStr("en = 'Transaction is active. Cannot execute a query outside the transaction.'"));
	EndIf;
...
Function GeneralBasicFunctionalityParameters() Export
	...
	CommonParameters.Insert("MinimallyRequiredPlatformVersion",	"8.3.12.1412");
	CommonParameters.Insert("MustExit",							True); // Lock the start if the version is less than min.
...
//  where
//  DataAreaBasicData separator value is not set (exclude possibility of knowingly erroneous call to the separated data
//  from the undivided session);
//
...
Function TimeIntervalAsString(BeginTime, EndTime = Undefined) Export
	...
	ElsIf BeginTime > EndTime Then
		Raise NStr("en = 'The interval end date cannot be earlier than the start date.'");
	EndIf;
	...
	Else
		DetailsOfInterval = NStr("en = 'less than a day'");
	EndIf;
...
#Region ProceduresAndFunctionsForWorkWithForms
...
#Region ProceduresAndFunctionsForWorkWithTypesMetadataObjectsAndTheirRowPresentations
...
// 
// Array = New Array;
// Array.Add(Constant.UseDataSync);
...
Function GetConfigurationMetadataTree(Filter = Undefined) Export
	...
	NewMetadataObjectCollectionRow("ChartsOfCharacteristicTypes", NStr("en = 'Charts of characteristic types'"), PictureLib.ChartOfCharacteristicTypes, PictureLib.ChartOfCharacteristicTypesObject, CollectionsOfMetadataObjects);
	...
	NewMetadataObjectCollectionRow("ChartsOfCalculationTypes",       NStr("en = 'Charts of characteristic types'"), PictureLib.ChartOfCharacteristicTypes, PictureLib.ChartOfCharacteristicTypesObject, CollectionsOfMetadataObjects);
...
Function ObjectByDescriptionFull(FullName) Export
	...
	If RowArray.Count() >= 2 Then
		...
		Name = RowArray[1];
	Else
		Raise StrReplace(NStr("en = 'Incorrect full name of the report or data processor %1.'"), "%1", FullName);
	EndIf;
	...
	Else
		Raise StrReplace(NStr("en = '%1 is not a report or a data processor.'"), "%1", FullName);
	EndIf;
EndFunction
...
Function ObjectKindByKind(Type) Export
	...
	Else
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Incorrect type of parameter value (%1)'"), String(Type));
...
Function MetadataObjectID(MetadataObjectDesc) Export
	...
	Else
		...
		Except
			Message("MetadataObjectDescriptionType = " + MetadataObjectDescriptionType);
...
Function MetadataObjectByID(ID) Export
	...
	If Exporting[0].DeletionMark Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Error in function CommonUse.MetadataObjectByID().
			     |
			     |The %1 ID is found in the ""Metadata object IDs"" catalog, but its ""Deletion mark"" attribute is set to True.'")
			+ StandardSubsystemsServer.SpecificationOfErrorParametersWorkApplicationForDeveloper(),
			String(ID));
	EndIf;
...
// To use in the
// OnAddMetadataObjectsRenaming procedure of the CommonUseOverridable common module for the description of objects
// metadata renaming.
// 
// Parameters:
...
//                      For
//  example: Metadata.Document.GoodsAndServicesImplementation Properties - String - attributes properties separated by
//  commas, and it is required to receive their values. For example: Name, Type, Synonym, ToolTip.
//
// Returns:
...
//  Delimiter      - String - name of the common attribute-separator and the metadata object is checked whether it was
//                            separated by it.
//
// Returns:
...
#Region FunctionsOfMetadataObjectsTypesDefinition
...
// Defines if metadata object belongs to the Primary chart of accounts common type.
// 
// Parameters:
//  MetadataObject - MetadataObject - object for which it is required to define whether it belongs to the specified type.
// 
...
#Region SavingReadingAndDeletingSettingsFromStorage
...
#Region FunctionsForWorkWithTheWorkingDateSetting
...
#Region FunctionsForWorkWithTabularDocuments
...
#Region CommonProceduresAndFunctionsForWorkInTheDataSeparationMode
...
Procedure LockInfobase(Val CheckNoOtherSessions = True) Export
	...
	Else
		...
		Else
			Raise(NStr("en = 'Subsystem ""SaaS operations"" is not available'"));
		EndIf;
	EndIf;
...
#Region ApplicationInterfacesVersioning
...
Function GetInterfaceVersions(Val Address, Val User, Val Password = Undefined, Val Interface = Undefined) Export
	...
	If Not ConnectionParameters.Property("URL") 
		...
		Raise(NStr("en = 'URL of the service is not specified.'"));
	EndIf;
...
Function GetInterfaceVersionsViaExternalConnection(ExternalConnection, Val InterfaceName) Export
	...
	Except
		...
		WriteLogEvent(NStr("en = 'Receive interface versions'", CommonUseClientServer.MainLanguageCode()),
...
#Region ReceiveHandlersOfServerEvents
...
Function WSDefinitions(Val WSDLAddress, Val UserName, Val Password, Val Timeout = 10) Export
	...
	Except
		WriteLogEvent(NStr("en = 'Receive WSDL'", CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Error, , , DetailErrorDescription(ErrorInfo()));
	EndTry;
...
#Region WorkWithOperationWarnings

// Return unsafe operation protection description without warnings.
//
// Returned value:
//	UnsafeOperationProtectionDescription - with properties UnsafeOperationWarnings = False.
Function ProtectDescriptionWithoutWarnings() Export
	
	TypeName = "UnsafeOperationProtectionDescription";
	ProtectDescriptionWithoutWarnings = New(TypeName);
	ProtectDescriptionWithoutWarnings.UnsafeOperationWarnings = False;
	
	Return ProtectDescriptionWithoutWarnings;
		...
		If Not CommonUseReUse.SessionWithoutSeparator() Then
			...
			Else 
				If DataArea <> SessionSeparatorValue() Then
					Raise(NStr("en = 'Cannot access data from another data area in this session.'"));
				EndIf;
			EndIf;
		EndIf;
...
Function GetWSDL(Val Address, Val UserName, Val Password, Val Timeout)
	...
	If Not FileDescription.Status Then
		Raise(NStr("en = 'An error occurred when receiving a description file of the web service:'") + Chars.LF + FileDescription.ErrorInfo)
	EndIf;
...
Function ManagerServerModule(Name)
	...
	If Not ObjectFound Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = '%1 metadata object is not found or manager module receipt is not supported for it.'"), Name);
	EndIf;
...
#Region HelperProceduresAndFunctionsReferencesReplacement
...
Procedure ReplaceInConstant(Results, Val UsagePlace, Val WriteParameters, Val InternalTransaction = True)
	...
	If InternalTransaction Then
		...
		Except
			...
			Error = NStr("en = 'Cannot lock constant %1'");
	...
	If ActionState = "" Then
		...
		If ReplacementMade Then
			...
			Except
				...
				Error = NStr("en = 'Cannot write %1 due to: %2'");
				Error = StrReplace(Error, "%1", DataPresentation);
...
Procedure ReplaceInObject(Results, Val UsagePlace, Val WriteParameters, Val InternalTransaction = True)
	...
	If InternalTransaction Then
		...
		Except
			// Add record about an unsuccessful attempt to lock the result.
			Error = NStr("en = 'Unable to lock one or several objects from the list %1'");
...
Procedure ReplaceInSet(Results, Val UsagePlace, Val WriteParameters, Val InternalTransaction = True)
	...
	If InternalTransaction Then
		...
		Except
			// Add record about an unsuccessful attempt to lock the result.
			Error = NStr("en = 'Cannot lock set %1'");
			Error = StrReplace(Error, "%1", DataPresentation);
	...
	For Each Ref In RefsList Do
		...
		Except
			AddReplacementResult(DeletionResult, Ref, 
				ReplacementErrorDescription("LockError", Ref, REFPRESENTATION, NStr("en = 'An error occurred when locking reference for deletion.'")));
		EndTry
	EndDo;
	...
	For Each Ref In ToDelete Do
		...
		Except
			...
			AddReplacementResult(DeletionResult, Ref, 
				...
				NStr("en = 'Removal failed'") + Chars.LF + TrimAll( BriefErrorDescription(ErrorInfo))));
		EndTry;
	...
	For Each String In TableSearchAgain Do
		...
		If FinalTable.FindRows(Filter).Count() = 0 Then
			AddReplacementResult(FinalTable, Ref, 
				...
				NStr("en = 'Data was added or changed by another user'")));
		EndIf;
	EndDo;
...
Function SetDimensionsDescription(Val Meta, Cache)
	...
	If Metadata.InformationRegisters.Contains(Meta) Then
		...
		ElsIf MetaPeriod = Periodicity.Year Then
			...
			DimensionData.Presentation = NStr("en = 'Accounting period'");
...
Procedure WriteObject(Val Object, Val WriteParameters)
	...
	If AttributesTest.Hierarchical Or AttributesTest.ExtDimensionTypes <> Undefined Then 
		...
		If Object.Parent = Object.Ref Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Circular reference occurs in hierarchy when writing %1.'"),
				String(Object));
			EndIf;
	...
	For Each MetaOwner In AttributesTest.Owners Do
		...
		If Object.Owner = Object.Ref Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Circular reference occurs in subordination when writing %1.'"),
				String(Object));
		EndIf;
		Break;
...
Procedure ReplaceRefWithLongTransaction(ReplacementResult, Val Ref, Val ReplacementParameters, Val SearchTable)
	...
	Except
		...
		Error = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Cannot lock all usage locations %1'"),
			Ref
		);
