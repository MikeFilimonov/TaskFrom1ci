...
Function TextFillingErrors(FieldKind = "Field", MessageKind = "Filling",
	...
	If Upper(FieldKind) = "FIELD" Then
		If Upper(MessageKind) = "FILLING" Then
			Pattern = NStr("en = 'The ""%1"" field is not filled in'");
	...
	ElsIf Upper(FieldKind) = "COLUMN" Then
		If Upper(MessageKind) = "FILLING" Then
			Pattern = NStr("en = 'Column ""%1"" is not filled out in line %2 of list ""%3""'");
		ElsIf Upper(MessageKind) = "CORRECTNESS" Then
	...
	ElsIf Upper(FieldKind) = "LIST" Then
		If Upper(MessageKind) = "FILLING" Then
			Pattern = NStr("en = 'No lines entered to list ""%3""'");
		ElsIf Upper(MessageKind) = "CORRECTNESS" Then
...
// Generates a path to the specified LineNumber row in the AttributeName column of the TabularSectionName tabular
// section for showing messages in the form. For the shared use with
...
// for filling, UniqueValuesOnly - Boolean, optional if True, then only those values will be included to the array that
		...
		If SearchKey AND StructureReceiver.Property(KeyAndValue.Key) Then
			...
			Else
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'Structure intersection of source and receiver by key ""%1"".'"),
					KeyAndValue.Key);
			EndIf
		EndIf;
...
Procedure SetSpreadsheetDocumentFieldState(SpreadsheetDocumentField, Status = "DontUse") Export
	...
	If TypeOf(SpreadsheetDocumentField) = Type("FormField") 
		...
		ElsIf Upper(Status) = "IRRELEVANCE" Then
			...
			StatePresentation.Picture                       = New Picture;
			StatePresentation.Text                          = NStr("en = 'The report is not generated. Click ""Run report"" to generate the report.'");;
...
Function CompareVersions(Val VersionString1, Val VersionString2) Export
	...
	If Version1.Count() <> 4 Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Invalid format for parameter VersionRow1: %1'"), VersionString1);
	EndIf;
	Version2 = StringFunctionsClientServer.DecomposeStringIntoSubstringsArray(Row2, ".");
	If Version2.Count() <> 4 Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
	    	NStr("en = 'Invalid format for parameter VersionRow2: %1'"), VersionString2);
	EndIf;
...
Function InstallOuterDatabaseJoin(Parameters) Export
	...
	#If Server Or ThickClientOrdinaryApplication Or ExternalConnection Then
		...
		ErrorShortInfo = NStr("en = 'Direct connection to the infobase on a server under OS Linux is not available.'");
	#Else
		IsLinux = IsLinuxClient();
		ErrorShortInfo = NStr("en = 'Direct connection to the infobase on a client under OS Linux is not available.'");
	#EndIf
	...
	Except
		...
		ErrorMessageString = NStr("en = 'Cannot connect to another application: %1'");
	...
	If FileModeWork Then
		...
		If IsBlankString(Parameters.InfobaseDirectory) Then
			ErrorMessageString = NStr("en = 'Location of the infobase directory is not specified.'");
	...
	Else
		...
		If IsBlankString(Parameters.PlatformServerName) Or IsBlankString(Parameters.InfobaseNameAtPlatformServer) Then
			ErrorMessageString = NStr("en = 'Required connection parameters are not specified: ""Server name""; ""Name of infobase on the server"".'");
			FillCheckingError = True;
		EndIf;
...
#Region WorkWithMailingAddresses
...
//                                       does not correspond to standards requirements, then such text is written to the
//                                       Alias field.
	...
	For Each AddressString In ArrayOFAddresses Do
		...
		If AddressDefined Then 
			If HasProhibitedCharsInAlias Then
				ErrorDescription = NStr("en = 'The presentation contains invalid characters'");
			EndIf;
		Else
			If StringContainsEmail Then 
				ErrorDescription = NStr("en = 'Email address contains errors'");
			Else
				ErrorDescription = NStr("en = 'The string does not contain an email address'");
			EndIf;
		EndIf;	
...
Function ParseStringWithPostalAddresses(Val EmailAddressString, CallingException = True) Export
	...
	ProhibitedCharsMessage = NStr("en = 'Invalid character ""%1"" in email address ""%2""'");
	MessageInvalidEmailFormat = NStr("en = 'Incorrect email address ""%1""'");
...
#Region WorkWithFiltersAndParametersOfDynamicLists
...
#Region WorkingWithManagedForms

// Receives form attribute value.
// Parameters:
// 	Form
//		AttributePath - String, path to data, for example: Object.MonthEarnings.
...
// Parameters:
// 	Form
// 	AttributePath - String, path to data, for example, Object.Earnings
//		Value
...
//       ChartOfCalculationTypes.Earnings.PaymentOnSalary
//
// Returns: 
...
Function PredefinedItem(FullPredefinedName) Export
	If Upper(Right(FullPredefinedName, 9)) = ".EMPTYREF" Then
	...
	If Condition <> True Then
		...
		If Not IsBlankString(CheckContext) Then
			ErrorMessage = ErrorMessage + " " +
				StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'in %1'"), CheckContext);
		EndIf;
		Raise ErrorMessage;
	EndIf;
...
Procedure CheckParameter(Val ProcedureOrFunctionName, Val ParameterName, Val ParameterValue, 
	...
	Validate(TypeOf(ProcedureOrFunctionName) = Type("String"), 
		NStr("en = 'Invalid value of the ProcedureOrFunctionName parameter'"), Context);
	Validate(TypeOf(ParameterName) = Type("String"), 
		NStr("en = 'Invalid value of the ParameterName parameter'"), Context);
	...
	Validate(ThisTypeDescription Or TypeOf(ExpectedTypes) = Type("Type"), 
		NStr("en = 'Invalid value of parameter ExpectedTypes'"), Context);
...
//
Function DistributeAmountProportionallyToFactors(Val AmountToBeDistributed, Ratios, Val Precision = 2) Export
	
	If Ratios.Count() = 0 Or Not ValueIsFilled(AmountToBeDistributed) Then
		Return Undefined;
	EndIf;
	...
	For IndexOf = 0 To Ratios.Count() - 1 Do
		Result[IndexOf] = Round(AmountToBeDistributed * Ratios[IndexOf] / RatiosAmount, Precision, 1);
		DistributedAmount = DistributedAmount + Result[IndexOf];
	EndDo;
	
	// Assign rounding errors to ratio with the maximum weight.
	If Not DistributedAmount = AmountToBeDistributed Then
		Result[MaxRatioIndex] = Result[MaxRatioIndex] + AmountToBeDistributed - DistributedAmount;
	EndIf;
...
Function CheckAndPrepareMailAddress(Val FullNameAddressee, Val MailAddress)
	...
	ProhibitedCharInRecipientName = NStr("en = 'Recipient name contains invalid characters.'");
	EmailContainsProhibitedChar = NStr("en = 'Invalid character in mailing address.'");
	BorderChars = "<>[]";
