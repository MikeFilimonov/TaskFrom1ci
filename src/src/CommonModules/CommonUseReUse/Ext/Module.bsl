...
Function GetXSLTransformFromCommonTemplate(Val CommonTemplateName) Export
	...
	Except
		WriteLogEvent(NStr("en = 'Receive XSL'", CommonUseClientServer.MainLanguageCode()),
...
//  Separator parameter value. It also returns metadata object properties values that can be required for its subsequent
//  processor in the universal algorithm.
...
Function SeparatedMetadataObjects(Val Delimiter) Export
	...
	If CommonAttributeMetadata = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Common attribute %1 was not found in configuration.'"), Delimiter);
	EndIf;
	...
	Else
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Data separation is not used for common attribute %1.'"), Delimiter);
		
	EndIf;
	
	// II. Define separation by the incoming documents for sequences and logs.
	
	// 1) Sequences. Enumeration with the checking of the first incoming document. If there are no documents, consider it
	// to be separated.
	For Each SequenceMetadata In Metadata.Sequences Do
		...
		If SequenceMetadata.Documents.Count() = 0 Then
			...
			MessagePattern = NStr("en = 'Sequence %1 does not include any documents.'");
			MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, SequenceMetadata.Name);
			WriteLogEvent(NStr("en = 'Receive separate metadata objects'", 
	...
	For Each DocumentJournalMetadata In Metadata.DocumentJournals Do
		...
		If DocumentJournalMetadata.RegisteredDocuments.Count() = 0 Then
			...
			MessagePattern = NStr("en = 'Journal %1 does not contain any documents.'");
	...
	If WantedUpdate Then
		...
		Else
			...
			DescriptionSchTask = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Updating version cache. Identifier of record %1. Data type %2.'"),
				ID,
				DataType);
				...
				Except
					For Each OriginalTask In Tasks Do
						...
						If Job.ErrorInfo <> Undefined Then
							WriteLogEvent(NStr("en = 'Updating version cache'", CommonUseClientServer.MainLanguageCode()),
								EventLogLevel.Error,
								,
								,
						...
						Else
							...
							Raise(NStr("en = 'An unknown error occurred while updating the version cache'"));
						EndIf;
					EndDo;
...
Function InterfaceOptions() Export
	...
	Except
		ErrorInfo = ErrorInfo();
		EventName = NStr("en = 'Interface configuration'", CommonUseClientServer.MainLanguageCode());
