...
Procedure ActionCIClick(Form, Item, DataCI = Undefined) Export
	
	If DataCI = Undefined Then
	...
	If DataCI.Type = PredefinedValue("Enum.ContactInformationTypes.Skype") Then
		...
		List = New ValueList;
		List.Add("Call", NStr("en = 'Call'"));
		List.Add("StartChat", NStr("en = 'Start chat'"));
...
Procedure PresentationCIOnChange(Form, Item) Export
	...
	If IsBlankString(DataCI.Presentation) Then
		DataCI.FieldValues = "";
	Else
		DataCI.FieldValues = ContactInformationDriveServerCall.ContactInformationXMLByPresentation(DataCI.Presentation, DataCI.Kind);
	EndIf;
...
Procedure CommentCIOnChange(Form, Item) Export
	...
	ContactInformationDriveServerCall.SetContactInformationComment(DataCI.FieldValues, DataCI.Comment, ExpectedKind);
	
EndProcedure

Procedure ExecuteCommand(Form, Command) Export
	
	If Command.Name = "AddFieldContactInformation"
		OR StrStartsWith(Command.Name, "AddFieldContactInformation") AND Form.ContactLineIdentifierAttributeExists Then
		
		AdditionalParameters = New Structure;
		AdditionalParameters.Insert("Form", Form);
		If Form.ContactLineIdentifierAttributeExists Then
			AdditionalParameters.Insert("ContactLineIdentifier", Number(StrReplace(Command.Name, "AddFieldContactInformation_", "")));
		EndIf;
		
		NotifyDescription = New NotifyDescription("AddContactInformationTypeselected", ThisObject, AdditionalParameters);
		
		ListAvailableKinds = ContactInformationDriveClientServer.KindsListForAddingContactInformation(Form);
		
		Form.ShowChooseFromList(NotifyDescription, ListAvailableKinds, Form.Items[Command.Name]);
		
	ElsIf StrStartsWith(Command.Name, "ContextMenuMapGoogle_") Then
...
Procedure AddContactInformationTypeselected(SelectedItem, AdditionalParameters) Export
	...
	If KindProperties.ShowInFormAlways = False Then
		...
		QuestionText = StrTemplate(NStr("en = 'Add the ability to input the type of contact information ""%1""?'"), SelectedItem.Value);
		QuestionTitle = NStr("en = 'Confirm adding'");
		
		ShowQueryBox(NotifyDescription, QuestionText, QuestionDialogMode.YesNo, , DialogReturnCode.Yes, QuestionTitle);
		
	Else
		
		If AdditionalParameters.Property("ContactLineIdentifier") Then
			Form.AddContactInformationServer(SelectedItem.Value, False, AdditionalParameters.ContactLineIdentifier);
		Else
