...
Function BackupInUse() Export
	...
	Return Constants.BackupUsage.Get();
...
Procedure OnAddParametersJobsClientLogicStandardSubsystems(Parameters) Export
	...
	Parameters.Insert("DataAreasBackup", GetFunctionalOption("BackupUsage"));
...
Procedure WhenCompletingTablesOfParametersOfIB(ParameterTable) Export
	...
	If CommonUse.SubsystemExists("StandardSubsystems.SaaS") Then
		...
		CurParameterString = ModuleSaaSOperations.AddConstantToInfobaseParameterTable(ParameterTable, "BackupUsage");
	EndIf;
...
Procedure SetUserActivityFlagInZone() Export
	...
	If Not CommonUseReUse.IsSeparatedConfiguration()
		...
		OR Not GetFunctionalOption("BackupUsage")
...
Procedure SetActivityFlagInZone(Val DataArea = Undefined, Val Status = True)
	...
	If DataArea = Undefined Then
		...
		Else
			Raise NStr("en = 'Parameter DataArea is required while calling procedure SetActivityFlagInZone from undivided session.'");
		EndIf;
	Else
		If Not CommonUseReUse.SessionWithoutSeparator()
				AND DataArea <> CommonUse.SessionSeparatorValue() Then
			
			Raise(NStr("en = 'Working with data of the area apart from the current one is prohibited'"));
...
Procedure CopiesCreation(Val CreationParameters, Val StateBuilding) Export
	...
	If Not RequiredCreating Then
		WriteLogEvent(
			EventLogMonitorEvent() + "." 
				+ NStr("en = 'Skip creation'", CommonUseClientServer.MainLanguageCode()),
	...
	WriteEventToLog(
		NStr("en = 'Create'", CommonUseClientServer.MainLanguageCode()),
		ParametersEvents);
...
Procedure ErrorCreatingCopies(Val JobParameters, Val ErrorInfo) Export
	...
	Else
		...
		Event = NStr("en = 'Creation error'", CommonUseClientServer.MainLanguageCode());
	EndIf;
...
Procedure ScheduleArchivingQueue(Val ExportParameters) Export
	...
	Else
		If ActiveTasks1[0].JobState <> Enums.JobStatuses.Planned Then
...
Procedure ExportAreaToMSStorage(Val ExportParameters, StorageAddress = Undefined) Export
	...
	Except
		...
		WriteLogEvent(NStr("en = 'Data area backup'", CommonUseClientServer.MainLanguageCode()), 
...
Function EventLogMonitorEvent()
	...
	Return NStr("en = 'Application backup'", CommonUseClientServer.MainLanguageCode());
