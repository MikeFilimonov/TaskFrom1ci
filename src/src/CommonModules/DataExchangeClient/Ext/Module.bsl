...
Procedure SettingFormBeforeClose(Cancel, Form) Export
	...
	If Form.Modified Then
		...
		QuestionText = NStr("en = 'Data was changed. Close the form without saving the changes?'");
...
Procedure OpenDataExchangeSettingAssistant(Val ExchangePlanName, ExchangeWithServiceSetup = False) Export
	...
	OpenForm("DataProcessor.DataSyncWizard.Form.Form", FormParameters, , ExchangePlanName + ExchangeWithServiceSetup, , , , FormWindowOpeningMode.LockOwnerWindow);
...
Function HyperlinkHeadersOfDataImport() Export
	...
	Structure = New Structure;
	Structure.Insert("Undefined",               NStr("en = 'Data receive was not performed'"));
	Structure.Insert("Error",                     NStr("en = 'Cannot receive data'"));
	Structure.Insert("CompletedWithWarnings", NStr("en = 'Data is received with warnings'"));
	Structure.Insert("Success",                      NStr("en = 'Data is successfully received'"));
	Structure.Insert("Execution",                 NStr("en = 'Receiving data ...'"));
	
	Structure.Insert("Warning_ExchangeMessageHasBeenPreviouslyReceived", NStr("en = 'No new data to receive'"));
	Structure.Insert("Error_MessageTransport",                      NStr("en = 'Cannot receive data'"));
...
Function HyperlinkHeadersOfDataDump() Export
	...
	Structure.Insert("Error",       NStr("en = 'Cannot send data'"));
	Structure.Insert("Success",        NStr("en = 'Data is successfully sent'"));
	Structure.Insert("Execution",   NStr("en = 'Sending data...'"));
	
	Structure.Insert("Warning_ExchangeMessageHasBeenPreviouslyReceived", NStr("en = 'Data is sent with warnings'"));
	Structure.Insert("CompletedWithWarnings",                     NStr("en = 'Data is sent with warnings'"));
	Structure.Insert("Error_MessageTransport",                      NStr("en = 'Cannot send data'"));
...
Procedure OnStart(Parameters) Export
	...
	If ClientWorkParameters.Property("OpenCommunicationAssistantToConfigureSlaveNode") Then
		...
		OpenForm("DataProcessor.DataSyncWizard.Form.Form", FormParameters, , , , , , FormWindowOpeningMode.LockWholeInterface);
	EndIf;
...
//
// Parameters:
//     Object               - Arbitrary - Object from which by attachment file name for opening will be received the
//                                        property name.
...
Procedure FileDirectoryChoiceHandler(Object, Val PropertyName, StandardProcessing = False, Val DialogueParameters = Undefined, CompletionAlert = Undefined) Export
	...
	DialogDefaults.Insert("Title", NStr("en = 'Specify directory'") );
	
	SetStructureDefaultValues(DialogueParameters, DialogDefaults);
	
	WarningText = NStr("en = 'Install extension for 1C:Enterprise web client for this operation.'");
...
Procedure FileChoiceHandler(Object, Val PropertyName, StandardProcessing = False, Val DialogueParameters = Undefined, CompletionAlert = Undefined) Export
	...
	DialogDefaults.Insert("Title",                   NStr("en = 'Select file'"));
...
Procedure PassFilesToServer(CompletionAlert, Val FileNames, Val FormID = Undefined, Val WarningText = Undefined) Export
	...
	For Each FileName In FileNames Do
		...
		If IsBlankString(FileName) Then
			HasEmpty = True;
			FileDescription.ErrorDescription = NStr("en = 'File is not selected.'");
		EndIf;
	...
	If WarningText = Undefined Then
		If FileNames.Count() = 1 Then
			WarningText = NStr("en = 'To transfer the file to the server, install the 1C:Enterprise web client.'");
		Else
			WarningText = NStr("en = 'To transfer files to the server, install extension for web client 1C:Enterprise.'");
		EndIf;
	EndIf;
...
Procedure PassFileToServerEndExtensionConnectionFillListForPass(AdditionalParameters)
	...
	CheckedFile = New File(Path);
	PassFileToServerEndExtensionConnectionAfterFileInitialization(CheckedFile, AdditionalParameters)
	
EndProcedure

// Procedure continued (see above).
// 
Procedure PassFileToServerEndExtensionConnectionAfterFileInitialization(File, AdditionalParameters)
...
Procedure PassFileToServerEndExtensionConnectionAfterFileExistenceCheck(Exists, AdditionalParameters) Export
	...
	If Not Exists Then
		...
		Item.ErrorDescription = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'File ""%1"" does not exist or cannot be accessed'"), Item.Name);
...
Procedure SelectAndSaveFileOnClientAfterWorksWithFilesExtensionConnection(Attached, AdditionalParameters) Export
	...
	DialogDefaults = New Structure;
	DialogDefaults.Insert("Title",               NStr("en = 'Select a file to save to'"));
	DialogDefaults.Insert("Multiselect",      False);
	DialogDefaults.Insert("Preview", False);
...
Procedure OpenSentDataContent(Val InfobaseNode) Export
	...
	FormParameters.NamesOfHiddenMetadata.Add("InformationRegister.MappingOfInfobaseObjects");
	...
	OpenForm("DataProcessor.MarkDataForExchange.Form", FormParameters,, InfobaseNode);
EndProcedure
...
Procedure DeleteSynchronizationSetting(Val InfobaseNode) Export
	...
	QuestionText = NStr("en = 'Delete data synchronization setting?'");
...
Procedure OnInstructionOpenHowToChangeDataSynchronizationPassword(Val AddressForAccountPasswordRecovery) Export
	...
	If IsBlankString(AddressForAccountPasswordRecovery) Then
		...
		ShowMessageBox(, NStr("en = 'Address for account password recovery is not set.'"));
...
#Region ServiceProceduresAndFuctionsOfInteractiveExportAddition
//
