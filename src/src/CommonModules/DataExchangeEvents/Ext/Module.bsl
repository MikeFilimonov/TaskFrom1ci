...
#Region ProceduresAndFunctionsForUseInRegistrationRuleEventHandlers
...
//                  constant record manager, register records set etc CheckExportPermission - Boolean   - Optional check
//                  box. If you set to False, then additional check on the match to node general settings is
...
Procedure RecordChangesData(Val Recipient, Val Data, Val CheckPermissionOfUplouding=True) Export
	...
	Else
		...
		If ObjectExportMode = Enums.ExchangeObjectsExportModes.ExportIfNecessary Then
			...
			If IsNewObject Then
				Raise NStr("en = 'Registration of unwritten objects that can be exported by reference is not supported.'");
			EndIf;
			...
			Try
				...
				Else
					InformationRegisters.MappingOfInfobaseObjects.AddObjectToFilterOfPermittedObjects(Data.Ref, Recipient);
				EndIf;
...
// Determines nodes list of the ExchangePlanName exchange plan receivers for which it is required to register the Object
// object for the subsequent export.
//
...
// Then, using objects registration mechanism (ORM, registration rules) it is determined on which nodes of each exchange
// plan object should be registered.
//
// Parameters:
//  ExchangePlanName - String - exchange plan name for which registration mechanism is executed.
		...
		If CommonUseReUse.DataSeparationEnabled() Then
			...
			Else
				...
				If SeparatedData(MetadataObject) Then
					Raise NStr("en = 'Change registration of separated data in the split mode.'");
				EndIf;
...
#Region SelectiveObjectsRegistration
...
Function ObjectModifiedForExchangePlan(Source, MetadataObject, ExchangePlanName, WriteMode, RecordObjectChangeToExport)
	...
	Except
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'An error occurred when determining modification of object: %1'"),
...
#Region ObjectRegistrationRules
...
Function GetArrayOfNodesForRegistrationExportIfNeeded(Ref, Val ExchangePlanName, Val FlagAttributeName) Export
	...
	Else
		...
		|ON
		|	MappingOfInfobaseObjects.UniqueSourceHandle = &Object
		|WHERE
		|	     ExchangePlanHeader.Ref <> &ThisNode
		|	AND    ExchangePlanHeader.[FlagAttributeName] = VALUE(Enum.ExchangeObjectsExportModes.ExportIfNecessary)
		|	AND Not ExchangePlanHeader.DeletionMark
		|	AND    MappingOfInfobaseObjects.UniqueSourceHandle = &Object
		|";
...
// Determines nodes list of the ExchangePlanName exchange plan receivers for which it is required to register the Object
// object according to ORR (universal part) for the subsequent export.
//
// Parameters:
//  ArrayOfNodesResult - Array - array of the ExchangePlan exchange
//   plan receivers node for which it is required to execute registration.
...
// Determines nodes list of the ExchangePlanName exchange plan receivers for which it is required to register the Object
// object according to ORR for the subsequent export.
//
// Parameters:
//  ArrayOfNodesResult - Array - array of the ExchangePlan exchange
//   plan receivers node for which it is required to execute registration.
...
Function RecordSetByType(MetadataObject)
	...
	Else
		...
		MessageString = NStr("en = 'The record set is not provided for metadata object %1.'");
...
#Region RegistrationRulesByObjectProperties
		...
		Else
			...
			If TreeRow.FilterItemKind = DataExchangeServer.FilterItemPropertyValueAlgorithm() Then
				...
				Except
					...
					MessageString = NStr("en = 'An error occurred while calculating constant value:
...
#Region EventsOfObjectsRegistrationRules
...
Procedure ExecuteORRHandlerOnProcessing(Cancel, ORR, Object, AdditionalParameters)
	...
	If ORR.HasOnProcessHandler Then
		...
		Except
			...
			MessageString = StrReplace(MessageString, "[HandlerName]",      NStr("en = 'When processing'"));
			MessageString = StrReplace(MessageString, "[ExchangePlanName]",      ORR.ExchangePlanName);
			MessageString = StrReplace(MessageString, "[MetadataObjectName]", ORR.MetadataObjectName);
			MessageString = StrReplace(MessageString, "[Definition]",            DetailErrorDescription(ErrorInfo()));
			Raise MessageString;
...
Procedure ExecuteORRHandlerOnProcessingAdditional(Cancel, ORR, Object, QueryText, QueryParameters, UseCache, AdditionalProperties = Undefined)
	...
	If ORR.HasOnProcessHandlerAdditional Then
		...
		Except
			MessageString = NStr("en = 'An error occurred while executing handler: ""[HandlerName]""; Exchange plan: [ExchangePlanName]; Metadata object: [MetadataObjectName] Error description: [Description]'");
			MessageString = StrReplace(MessageString, "[HandlerName]",      NStr("en = 'When processing (optional)'"));
			MessageString = StrReplace(MessageString, "[ExchangePlanName]",      ORR.ExchangePlanName);
			MessageString = StrReplace(MessageString, "[MetadataObjectName]", ORR.MetadataObjectName);
			MessageString = StrReplace(MessageString, "[Definition]",            DetailErrorDescription(ErrorInfo()));
			Raise MessageString;
...
Procedure ProcessRegistrationRulesError(ExchangePlanName)
	...
	If UpdateResults.InfobaseUpdateInProgress()
...
Function DataDifferent(Data1, Data2, PropertyList = Undefined, ExcludingProperties = Undefined) Export
	...
	Else
		...
		Raise NStr("en = 'Invalid value of parameter [1] of method CommonUse.DataDifferent is set.'");
...
#Region WorkWithCollisionsOfDataChangesDuringExchange
...
Procedure ValidateImportProhibitionExistanceByDate(DataItem, ItemReceive, Val Sender)
	...
	If CommonUse.SubsystemExists("StandardSubsystems.ClosingDates") Then
		ProhibitionDateChangesModule = CommonUse.CommonModule("ClosingDates");
...
// by date, see procedure of the general module ClosingDates.ImportProhibited.
//
Procedure RegisterDataProhibitionImportByDate(DataItem, Sender, ErrorInfo)
	...
	If DataExchangeReUse.UseVersioning(Sender) AND CommonUse.ThisIsObjectOfReferentialType(DataItem.Metadata()) Then
		...
		If CommonUse.RefExists(ObjectReference) Then
			...
			Comment = NStr("en = 'Version was created during data synchronization.'");
			InfoAboutObjectVersion = New Structure("Comment", Comment);
		...
		Else
			...
			ErrorMessageString = NStr("en = '%1 cannot be imported to the prohibited period.%2%2%3'");
...
Function GetDataObjectAsStringBeforeChange(Object, ObjectMetadata, IsReferenceType, RefExists, ObjectInDataBase)
	...
	If IsReferenceType Then
		...
		Else
			...
			ObjectString = NStr("en = 'Object is removed'");
