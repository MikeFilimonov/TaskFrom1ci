...
#Region ObjectRecordingMechanismAtTheExchangePlanNodes
...
Function ThisInfobaseName() Export
	...
	Result = Constants.ApplicationTitle.Get();
...
Function ErrorMessages() Export
	...
	ErrorMessages.Insert(8,  NStr("en = 'Incorrect file name for data export'"));
	...
	ErrorMessages.Insert(177, NStr("en = 'Exchange plan name from the exchange message is not as expected.'"));
	ErrorMessages.Insert(178, NStr("en = 'Recipient from the exchange message is not as expected.'"));
...
Function OfflineWorkExchangePlan() Export
	...
	If Result.Count() = 0 Then
		...
		Raise NStr("en = 'Offline work in the application is not supported.'");
		
	ElsIf Result.Count() > 1 Then
		
		Raise NStr("en = 'More than one exchange plan for offline work was created.'");
...
//
// Parameters:
//  ObjectName       - String - Metadata object full name, for example Catalog.Products.
//  ExchangePlanName - String - exchange plan name as specified in the designer.
//
// Returns:
...
// 
//  Returns:
//   Array - Used exchange message transport for the node.
//
