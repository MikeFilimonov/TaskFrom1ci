...
// It specifies the
// constant-flag of the data change and sends the message about the change with the number of the current area to the
// service manager.
//
...
//    of the UpdateResults common module.
...
#Region HandlersOfConditionalCallsFromSSL
...
Procedure OnDataSynchronizationDisabling(Cancel) Export
	...
	Constants.UseDataSyncSaaSWithLocalApplication.Set(False);
	Constants.UseDataSyncSaaSWithApplicationInInternet.Set(False);
...
// during the implementation the body of the procedure shall be changed so it will return relevant version sets (see
// example below).
//
// Parameters:
...
Procedure OnAddParametersJobsClientLogicStandardSubsystemsRunning(Parameters) Export
	...
	Parameters.Insert("OfflineWorkParameters", 
		AutonomousWorkParametersOnExit());	
...
Procedure WhenFillingTypesExcludedFromExportImport(Types) Export
	...
	Types.Add(Metadata.Constants.GlobalNumerationPrefix);
	...
	Types.Add(Metadata.InformationRegisters.MappingOfInfobaseObjects);
...
Procedure OnDeleteOutdatedMessagesExchange() Export
	...
	While Selection.Next() Do
		...
		If MessageFile.Exist() Then
			...
			Except
				WriteLogEvent(NStr("en = 'Data exchange description'", CommonUseClientServer.MainLanguageCode()),
...
Procedure OnGettingObjectExceptionsOfExchangePlan(Objects, Val DistributedInfobase) Export
	...
	If DistributedInfobase Then
		...
		Objects.Add(Metadata.Constants.UseDataSyncSaaSWithLocalApplication);
		Objects.Add(Metadata.Constants.UseDataSyncSaaSWithApplicationInInternet);
...
// with data are passed to the ProcessNewData procedure. File will be deleted automatically after completion of the
// procedure. If the file was not specified in the service manager - The argument value is Undefined.
//
// Parameters: 
...
#Region ExportInternalProceduresAndFunctions
	...
	If ScenarioString.InfobaseNumber = 1 Then
		...
		Else
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'The ""%1"" unknown action on data exchange between data areas.'"),
...
Function GeneratePredefinedNodeDescription() Export
	...
	Return ?(IsBlankString(ApplicationName), NStr("en = 'online application'"), ApplicationName);
EndFunction
...
Function CorrespondentEndPoint(Val Correspondent) Export
	...
	If QueryResult.IsEmpty() Then
		MessageString = NStr("en = 'The correspondent''s endpoint is not set for ""%1"".'");
...
Procedure AvailableNewData(Val Handle, Import) Export
	...
	If Handle.DataType = ProvidedDataKindId() Then
		...
		Else
			...
			MessageText = NStr("en = 'Supplied exchange rules are not suitable for the current configuration and are intended for exchange plan %1 of configuration %2 of version %3'");
			...
			WriteLogEvent(NStr("en = 'Supplied data exchange rules. Import of the supplied rules is canceled'",
...
Procedure CreateExchangeSetting(
	...
	Try
		...
		If IsBlankString(ThisNodeCode) Then
			...
			MessageString = NStr("en = 'Code is not specified for the ""%1"" predefined exchange node.'");
		...
		If IsBlankString(GetFunctionalOption("GlobalNumerationPrefix")) Then
			
			If IsBlankString(Prefix) Then
				Raise NStr("en = 'Prefix for this application is not set up in the service manager.'");
			EndIf;
		...
		If TransportSettings.ExchangeMessageTransportKindByDefault = Enums.ExchangeMessagesTransportKinds.FILE Then
			...
			If IsBlankString(FILEInformationExchangeCommonDirectory) Then
				...
				MessageString = NStr("en = 'Information exchange directory is not specified for endpoint ""%1"".'");
			...
			If Not CommonDirectory.Exist() Then
				...
				MessageString = NStr("en = 'The ""%1"" data exchange directory does not exist.'");
		...
		Else
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Kind of exchange message transport ""%1"" for endpoint %2 is not supported.'"),
...
Procedure SaveSessionData(Val Message, Val Presentation = "") Export
	...
	MessageString = NStr("en = 'Message exchange session of the ""%1"" application successfully completed.%2'",
...
Procedure FixUnsuccessfullSessionCompletion(Val Message, Val Presentation = "") Export
	...
	MessageString = NStr("en = 'Error of the %1 system message exchange session performance. %2 Error description from the correspondent: %3.'", CommonUseClientServer.MainLanguageCode());
	MessageString = StringFunctionsClientServer.SubstituteParametersInString(MessageString,
...
Function EventLogMonitorEventDataSyncronizationSetting() Export
	...
	Return NStr("en = 'Data exchange in SaaS.Data synchronization settings'",
		CommonUseClientServer.MainLanguageCode());
...
Function SystemMessagesExchangeSessionEventLogMonitorEvent()
	...
	Return NStr("en = 'Data exchange in SaaS.Sessions of application message exchange'",
		CommonUseClientServer.MainLanguageCode());
...
#Region ProceduresAndFunctionsOfTheExchangeDataMonitor
...
#Region LocalInternalProceduresAndFunctions
...
Function FindInfobaseNode(Val ExchangePlanName, Val NodeCode)
	...
	If Result = Undefined Then
		Message = NStr("en = 'Exchange plan node is not found. Exchange plan name %1; node code %2 or %3'");
...
#Region EventSubscriptionsForTheIndependentWorkingPlace
...
#Region ReceiveReferencesToTheGeneralInfoPagesOfTheService
