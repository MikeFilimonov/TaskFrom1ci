...
//  Cancel - Boolean           - shows that form creation has been denied. If it is set to True, then the form will not
//                               be created.
// 
...
//   String
//
Function GlobalNumerationPrefix() Export
	
	Return GetFunctionalOption("GlobalNumerationPrefix");
...
Procedure SetIBPrefix(Val Prefix) Export
	...
	Constants.GlobalNumerationPrefix.Set(TrimAll(Prefix));
...
Function BackupCopiesParameters(Val Sender, Val ReceivedNo) Export
	...
	// message in the correspondent. That means that this base will receive the number of the received message that was not
	// sent - message from the future.
...
//   ParameterNameWithNodeName - Name of the parameter in the default settings from which the node name is required to
//                               be received.
//
// Returns:
...
#Region WorkWithTheMonitorOfDataExchangeIssues
...
Procedure OnAddOfficeEvent(ClientEvents, ServerEvents) Export
	...
	//     False value for this parameter in the body of the procedure-processor, there will be no standard processing of
	//     the event. Denial from the standard processor does not stop action.
	//     Default value: True.
	...
	//     parameter in the body of the procedure-processor, there will be no standard processing of the event. Denial from
	//     the standard processor does not stop action. Default value: True.
...
Procedure BeforeCheckingIdentifiersOfMetadataObjectsInSubordinateNodeDIB(Cancel = False) Export
	...
	Except
		...
		WriteLogEvent(
			NStr("en = 'Data exchange. Import the ""Metadata object IDs"" catalog'", CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Error,,,
			DetailErrorDescription(ErrorInfo()));
		
		Raise
			NStr("en = 'Changes of the ""Metadata object IDs"" catalog are not imported from the main node: data import error. For more information, see event log.'");
	EndTry;
	SetPrivilegedMode(True);
	SetDataExchangeMessageImportModeBeforeStart("ImportAllowed", False);
	SetPrivilegedMode(False);
...
Procedure WhenErrorChecksIdsMetadataObjectsInSubordinateSiteDIB(OnImport, CallingException = False) Export
	...
	ElsIf CallingException Then // Setting in the IB subordinate node during the first start.
			ErrorText =
				NStr("en = 'The Metadata objects identifiers catalog is not updated in the main node: the check showed that you need to execute the critical changes (for details, see the events log monitor in the Metadata objects identifiers.It is required to execute critical changes event).
				     |
				     |Update the infobase on the main node and try to create a subordinate node again.
				     |
				     |To update the infobase in the main node, start the application once with the StartInfobaseUpdate start parameters.'");
...
Procedure ExecuteUpdateOfDataExchangeRules() Export
	...
	If Cancel Then
		Raise NStr("en = 'Errors occurred during update of data exchange rules (see the event log).'");
	EndIf;
...
Function SelectChanges(Val Node, Val MessageNo, Val FilterSample = Undefined) Export
	...
	If TransactionActive() Then
		Raise NStr("en = 'Data change selection is prohibited in an active transaction.'");
	EndIf;
...
//                                                                         is required to check versions variance in the
//                                                                         configuration rules. Check is executed during
...
//                                                                         the data exchange creation assistant in the
//                                                                         user interface.
...
//                                                                         the node of base-correspondent in the
//                                                                         exchange creation assistant.
...
Function DefaultExchangePlanSettings(ExchangePlanName) Export
	...
	AssistantFormTitle = NStr("en = 'Data synchronization with %Application%...'");
...
// 
// Returns:
// - Arbitrary - Type of the return value depends on the type of the received setting value.
//
...
#Region WorkWithObjectFTPConnection
...
Function FTPServerNameAndPath(Val ConnectionString) Export
	...
	If (Upper(Left(ConnectionString, 6)) <> "FTP://"
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'FTP connection string does not match the format: ""%1""'"), ConnectionString
		);
	EndIf;
	...
	If ConnectionParameters.Count() < 3 Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Server name is not specified in FTP resource connection string: ""%1""'"), ConnectionString
		);
	EndIf;
...
Procedure OnGettingObjectExceptionsOfExchangePlan(Objects, Val DistributedInfobase) Export
	...
	If DistributedInfobase Then
		...
		Objects.Add(Metadata.Constants.UseDataSync);
		Objects.Add(Metadata.Constants.UseDataSyncInLocalMode);
		Objects.Add(Metadata.Constants.UseDataSyncSaaS);
		...
		Objects.Add(Metadata.Constants.GlobalNumerationPrefix);
		...
		Objects.Add(Metadata.InformationRegisters.MappingOfInfobaseObjects);
...
Procedure OnEnableSeparationByDataAreas() Export
	...
	If GetFunctionalOption("UseDataSync") = False Then
		Constants.UseDataSync.Set(True);
	EndIf;
...
Procedure AtFillingToDoList(ToDoList) Export
	...
	OnFillingToDoListForSynchronizationWarning(ToDoList);
	OnFillingToDoListUpdateRequired(ToDoList);
	OnFillCurrensTodosListCheckCompatibilityWithCurrentVersion(ToDoList);
...
Procedure OnDefenitionOfUsageOfProhibitionDatesChange(Used) Export
	...
	If CommonUse.SubsystemExists("StandardSubsystems.ClosingDates") Then
		Used = True;
	EndIf;
...
// Handler of the UseDataSync constant setting.
//
//  Parameters:
...
// Handler to remove the UseDataSync constant.
//
//  Parameters:
...
// Event occurs in the data record transaction in IB when the recipients of data changes are defined according to the
// rules of objects registration.
//
// Parameters:
...
Procedure SkipChangeProhibitionCheck(Skip = True) Export
	...
	If CommonUse.SubsystemExists("StandardSubsystems.ClosingDates") Then
		ChangeProhibitionDateModuleService = CommonUse.CommonModule("ClosingDatesService");
...
//                           by default - True.
//  ExchangeMessageTransportKind (optional) - EnumRef.ExchangeMessagesTransportKinds - kind of transport that will be
...
// Initialization stage is executed once per session and saved in the session cache on server before the restart of
// session or the reset of the reused values of the data exchange subsystem. The reset of the reused values is executed
// while changing the data influencing the process of the data exchange (transport setings, exchange setting, filters on
// the exchange plan nodes setting).
//
...
Procedure ExecuteDataExchangeByScenarioOfExchangeData(Cancel, ExchangeExecutionSettings, LineNumber = Undefined) Export
	...
	While Selection.Next() Do
		...
		Else
			...
			MessageString = NStr("en = 'Data exchange process start for setting %1'", CommonUseClientServer.MainLanguageCode());
...
Procedure ExecuteDataExchangeByScheduledJob(ExchangeScenarioCode) Export
	...
	Else
		MessageString = NStr("en = 'Data exchange script with code %1 is not found.'");
...
Function GetExchangeMessageToTemporaryDirectory(Cancel, InfobaseNode, ExchangeMessageTransportKind, OutputMessages = True) Export
	...
	If ExchangeSettingsStructure.Cancel Then
		...
		If OutputMessages Then
			NString = NStr("en = 'Errors occurred while initializing processing of exchange message transport.'");
	...
	If ExchangeSettingsStructure.ExchangeProcessResult <> Undefined Then
		...
		If OutputMessages Then
			NString = NStr("en = 'Errors occurred while receiving exchange messages.'");
			CommonUseClientServer.MessageToUser(NString,,,, Cancel);
		EndIf;
...
Function GetExchangeMessageFromCorrespondentInfobaseToTempDirectory(Cancel, InfobaseNode, OutputMessages = True) Export
	...
	Except
		If OutputMessages Then
			Message = NStr("en = 'Cannot exchange: %1'");
	...
	If Cancel Then
		...
		If OutputMessages Then
			...
			Message = NStr("en = 'Cannot export data: %1'");
...
//  LongAction      - Boolean - Shows that the long operation was used.
...
Function GetExchangeMessageToTempDirectoryFromCorrespondentInfobaseOverWebService(
	...
	If Proxy = Undefined Then
		...
		Cancel = True;
		Message = NStr("en = 'An error occurred when establishing connection to the second infobase: %1'");
	...
	Except
		...
		Cancel = True;
		Message = NStr("en = 'When exporting data, errors occurred in the second infobase: %1'", CommonUseClientServer.MainLanguageCode());
		Message = StringFunctionsClientServer.SubstituteParametersInString(Message, DetailErrorDescription(ErrorInfo()));
		ExchangeSettingsStructure = New Structure("EventLogMonitorMessageKey");
		ExchangeSettingsStructure.EventLogMonitorMessageKey = GetEventLogMonitorMessageKey(InfobaseNode, Enums.ActionsAtExchange.DataImport);
		WriteLogEventOfDataExchange(Message, ExchangeSettingsStructure, True);
	...
	If LongAction Then
		ExchangeSettingsStructure = New Structure("EventLogMonitorMessageKey");
		ExchangeSettingsStructure.EventLogMonitorMessageKey = GetEventLogMonitorMessageKey(InfobaseNode, Enums.ActionsAtExchange.DataImport);
		WriteLogEventOfDataExchange(NStr("en = 'Pending data from the correspondent base...'",
	...
	Except
		...
		Cancel = True;
		Message = NStr("en = 'Errors occurred when receiving an exchange message from file transfer service: %1'", CommonUseClientServer.MainLanguageCode());
		Message = StringFunctionsClientServer.SubstituteParametersInString(Message, DetailErrorDescription(ErrorInfo()));
		ExchangeSettingsStructure = New Structure("EventLogMonitorMessageKey");
		ExchangeSettingsStructure.EventLogMonitorMessageKey = GetEventLogMonitorMessageKey(InfobaseNode, Enums.ActionsAtExchange.DataImport);
		WriteLogEventOfDataExchange(Message, ExchangeSettingsStructure, True);
	...
	Except
		Cancel = True;
		Message = NStr("en = 'Errors occurred while receiving exchange messages: %1'", CommonUseClientServer.MainLanguageCode());
		Message = StringFunctionsClientServer.SubstituteParametersInString(Message, DetailErrorDescription(ErrorInfo()));
		ExchangeSettingsStructure = New Structure("EventLogMonitorMessageKey");
		ExchangeSettingsStructure.EventLogMonitorMessageKey = GetEventLogMonitorMessageKey(InfobaseNode, Enums.ActionsAtExchange.DataImport);
		WriteLogEventOfDataExchange(Message, ExchangeSettingsStructure, True);
...
#Region ForWorkViaTheExternalConnection
...
Procedure ExecuteDataExchangeForInfobaseNodeOverFileOrString(InfobaseNode = Undefined,
	...
	If InfobaseNode = Undefined Then
		...
		If InfobaseNode.IsEmpty() Then
			ErrorMessageString = NStr("en = 'Exchange plan node %1 with code %2 is not found.'");
	...
	If ExchangeSettingsStructure.Cancel Then
		ErrorMessageString = NStr("en = 'An error occurred when initializing the data exchange process.'");
		FixEndExchange(ExchangeSettingsStructure);
		Raise ErrorMessageString;
	EndIf;
	...
	MessageString = NStr("en = 'Data exchange process start for node %1'", CommonUseClientServer.MainLanguageCode());
		...
		If ExchangeSettingsStructure.UseLargeDataTransfer Then
			...
			If ExchangeProcessResultCompleted(ExchangeSettingsStructure.ExchangeProcessResult) Then
				...
				Try
					...
					If LongAction Then
						WriteLogEventOfDataExchange(NStr("en = 'Pending data import in the correspondent base...'",
							CommonUseClientServer.MainLanguageCode()), ExchangeSettingsStructure);
						Return;
					EndIf;
...
Procedure ExecuteExchangeActionForInfobaseNodeByExternalConnection(Cancel, InfobaseNode,
	...
	If ExchangeSettingsStructure.DoDataImport Then
		...
		If ObjectConversionRules = Undefined Then
			...
			NString = NStr("en = 'Conversion rules for exchange plan %1 are not specified in the second infobase. Exchange is canceled.'",
		...
		Except
			WriteLogEventOfDataExchange(
				StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'An error occurred in the second infobase: %1'"),
...
Procedure BeforeReadingMessageExchange(Val Recipient, ExchangeMessage, StandardProcessing)
	...
	If IsSubordinateDIBNode()
		...
		If TypeOf(SavedExchangeMessage) = Type("BinaryData") Then
			...
			WriteEventGetData(Recipient, NStr("en = 'The exchange message was received from cache.'"));
...
#Region ExchangeSerializationMethods
...
Procedure ExecuteStandardNodeChangeImport(
	...
	If MessageReader.Sender <> InfobaseNode Then // Message is intended not for this node.
		...
		WriteLogEvent(EventLogMonitorMessageKey, EventLogLevel.Error,
			InfobaseNode.Metadata(), InfobaseNode, NStr("en = 'The exchange message contains data for another infobase node.'",
			CommonUseClientServer.MainLanguageCode()));
		//
		Return;
	EndIf;
...
#Region ExportServiceFunctionsProperties
...
#Region TheExchangeMessagesTransport
...
Function GetFileFromStorageInService(Val FileID, Val InfobaseNode, Val PartSize = 1024, Val AuthenticationParameters = Undefined) Export
	...
	Else
		...
		Comment = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Start of the Internet exchange message receiving (number of file parts is %1).'"),
		...
		Comment = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'End of exchange message receiving from the Internet (size of a compressed exchange message is %1 MB).'"),
...
Procedure OnReceiveFileFromStore(Val FileID, FileName)
	...
	If QueryResult.IsEmpty() Then
		Definition = NStr("en = 'A file with ID %1 not found.'");
...
#Region RegistrationOfChangesForInitialDataExport
...
// is not set and the registration rules are not specified, then the changes registration will be unconditionally
// executed for all data of this type. If the registration rules are set for the metadata object, then the registration
// of changes will be executed taking into account export start date and list of companies. The registration of changes
// by the date of export start and list of companies is supported for documents. For business processes and jobs the
// registration of changes is supported by export start date. Registration of changes by the date of export start and
// list of companies is supported for sets of the register records. This procedure can be a prototype for developing
// their own procedures of changes registration for the initial data export.
//
// Parameters:
//
...
#Region HelperServiceProceduresAndFunctions
...
Procedure CheckIfExchangesPossible() Export
	...
	If Not Users.RolesAvailable("DataSynchronization, DataSynchronizationSetting") Then
		...
		Raise NStr("en = 'Insufficient rights to data synchronization.'");
		
	ElsIf UpdateResults.InfobaseUpdateRequired()
	        AND Not DataExchangeServerCall.DataExchangeMessageImportModeBeforeStart("ImportAllowed") Then
		
		Raise NStr("en = 'Infobase is being updated.'");
...
Procedure FixEndExchange(ExchangeSettingsStructure) Export
	...
	Else
		MessageString = NStr("en = '%1, %2; Processed objects: %3'", CommonUseClientServer.MainLanguageCode());
		MessageString = StringFunctionsClientServer.SubstituteParametersInString(MessageString,
							ExchangeSettingsStructure.ExchangeProcessResult,
							ExchangeSettingsStructure.ActionOnExchange,
...
Function UnpackZIPFile(Val ArchiveFileFullName, Val FileUnpackPath, Val ArchivePassword = "") Export
	...
	Except
		...
		MessageString = NStr("en = 'An error occurred when unpacking archive files %1 to directory: %2'");
...
Function PackIntoZipFile(Val ArchiveFileFullName, Val FilePackingMask, Val ArchivePassword = "") Export
	...
	Except
		...
		MessageString = NStr("en = 'An error occurred when packing archive files: %1 from directory: %2'");
...
Function GetEventLogMonitorMessageKey(InfobaseNode, ActionOnExchange) Export
	...
	MessageKey = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = 'Data exchange.%1.Node %2.%3'",
		CommonUseClientServer.MainLanguageCode()), ExchangePlanName, ExchangePlanNodeCode, ActionOnExchange);
...
Function DataAreaNumberByExchangePlanNodeCode(Val NodeCode) Export
	...
	If TypeOf(NodeCode) <> Type("String") Then
		Raise NStr("en = 'Invalid type of parameter number [1].'");
	EndIf;
...
Procedure ValidateRequiredFormAttributes(Form, Val Attributes)
	...
	If MissingAttributes.Count() > 0 Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'No required attributes of node configuration form: %1'"),
...
Procedure ExternalConnectionRefreshExchangeSettingsData(Val ExchangePlanName, Val NodeCode, Val DefaultValuesAtNode) Export
	...
	DataSyncWizard.InfobaseNode = InfobaseNode;
	DataSyncWizard.ExternalConnectionRefreshExchangeSettingsData(GetFilterSettingsValues(DefaultValuesAtNode));
...
Function InfobaseParameters(Val ExchangePlanName, Val NodeCode, ErrorInfo) Export
	...
	Result.Insert("GlobalNumerationPrefix");
	...
	If Result.ExchangePlanExists Then
		...
		GlobalNumerationPrefix = Undefined;
		GlobalNumerationPrefix = DataExchangeOverridable.InfobasePrefixByDefault();
		DataExchangeOverridable.OnDefineDefaultInfobasePrefix(GlobalNumerationPrefix);
		
		Result.GlobalNumerationPrefix                 = GetFunctionalOption("GlobalNumerationPrefix");
		Result.InfobasePrefixByDefault      = GlobalNumerationPrefix;
...
Procedure ExecuteCheckupOfExchangeRulesImportedFromFilePresence(ImportedFromFileExchangeRules, ImportedFromFileRecordRules)
	...
	If Not Result.IsEmpty() Then
		...
		WriteLogEvent(UpdateResults.EventLogMonitorEvent(), EventLogLevel.Error,,, MessageString);
...
Procedure CheckConnectionOfExchangeMessagesTransportDataProcessor(Cancel, SettingsStructure, TransportKind, ErrorInfo = "") Export
	...
	If Not DataProcessorObject.ConnectionIsDone() Then
		...
		AdditionalMessage = NStr("en = 'See technical information on the error in the event log.'");
		...
		WriteLogEvent(NStr("en = 'Exchange message transport'", CommonUseClientServer.MainLanguageCode()),
...
Function InstallOuterDatabaseJoin(SettingsStructure) Export
	...
	If NoFullRights Then
		Result.DetailedErrorDescription = NStr("en = 'User specified for connection to another application should have been assigned roles ""System administrator"" and ""Full rights""'");
		Result.ErrorShortInfo   = Result.DetailedErrorDescription;
		Result.Join = Undefined;
	Else
		Try 
			StateUnacceptable = ExternalConnection.UpdateResults.InfobaseUpdateRequired();
		Except
			StateUnacceptable = False
		EndTry;
		
		If StateUnacceptable Then
			Result.DetailedErrorDescription = NStr("en = 'Another application is being updated.'");
			Result.ErrorShortInfo   = Result.DetailedErrorDescription;
			Result.Join = Undefined;
		EndIf;
...
Function IsConnectionToCorrespondent(Val Correspondent, Val SettingsStructure, UserMessage = "") Export
	...
	EventLogMonitorEvent = NStr("en = 'Data exchange.Connection check'", CommonUseClientServer.MainLanguageCode());
	...
	For Each ExchangePlanName In DataExchangeReUse.SSLExchangePlans() Do
		...
		If ImportedFromFileExchangeRules.Find(ExchangePlanName) = Undefined
			...
			MessageText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Updating data conversion rules for exchange plan %1'"), ExchangePlanName);
			WriteLogEvent(EventLogMonitorMessageTextDataExchange(),
		...
		If ImportedFromFileRecordRules.Find(ExchangePlanName) = Undefined
			...
			MessageText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Updating data registration rules for exchange plan %1'"), ExchangePlanName);
			WriteLogEvent(EventLogMonitorMessageTextDataExchange(),
				EventLogLevel.Information,,, MessageText);
	...
	If File.Exist() AND File.IsFile() Then
		...
		If MaximumValidMessageSize <> 0 Then
			...
			If PackageSize > MaximumValidMessageSize Then
				...
				MessageString = NStr("en = 'Outgoing package size is %1 KB and exceeds allowable limit of %2 KB.'");
...
Procedure RegisterDataForInitialExport(InfobaseNode, Data = Undefined) Export
	...
	If DataExchangeReUse.ExchangePlanContainsObject(DataExchangeReUse.GetExchangePlanName(InfobaseNode),
		Metadata.InformationRegisters.MappingOfInfobaseObjects.FullName()) Then
		
		ExchangePlans.DeleteChangeRecords(InfobaseNode, Metadata.InformationRegisters.MappingOfInfobaseObjects);
	...
	If GetFunctionalOption("UseDataSync") = True Then
		...
		If InfobaseNode <> Undefined Then
			...
			Try
				...
				If Cancel OR UpdateResults.InfobaseUpdateRequired() Then
					EnableExchangeMessageDataExportRepetitionBeforeRunning();
				EndIf;
				
				If Cancel Then
					Raise NStr("en = 'Receiving data from the main node is completed with errors.'");
				EndIf;
			Except
				SetPrivilegedMode(True);
				SetDataExchangeMessageImportModeBeforeStart("ImportAllowed", False);
...
// node for the current infobase if the distributed infobase is created according to the exchange plan served by the
// subsystem of SSL exchange data.
//
// Returns:
...
#Region WrapperForWorkWithExternalInterfaceOfTheExchangePlanManager
...
#Region WorkWithPasswordsOfTheDataSynchronization
...
Procedure RunControlRecordsUndividedData(Val Data) Export
	...
	If CommonUseReUse.DataSeparationEnabled()
		...
		ExceptionsRepresentation = NStr("en = 'Access right violation'");
		ErrorMessage = NStr("en = 'Access right violation'", CommonUseClientServer.MainLanguageCode());
...
#Region WorkWithTheDataExchangeMonitor
...
// To define whether there is exchange with an exchange plan, you should check whether this plan has nodes exchange
// except of the redefined one.
// 
// Parameters:
//  No.
// 
...
#Region InitializationOfTheDataExchangeSettingsStructure
...
// It receives the transport setting structure for the data exchange.
//
...
Procedure RunInitOfExchangeSettingsStructure(ExchangeSettingsStructure, ExchangeExecutionSettings, LineNumber)
	...
	ExchangeSettingsStructure.EventLogMonitorMessageKey = NStr("en = 'Data exchange description'");
...
Procedure RunCheckOfMainFieldsOfExchangeSettingsStructure(ExchangeSettingsStructure)
	...
	If Not ValueIsFilled(ExchangeSettingsStructure.InfobaseNode) Then
		...
		ErrorMessageString = NStr("en = 'Infobase node with which information shall be exchanged is not specified. Exchange is canceled.'",
			CommonUseClientServer.MainLanguageCode());
		WriteLogEventOfDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
		SetExchangeInitEnd(ExchangeSettingsStructure);
	...
	ElsIf Not ValueIsFilled(ExchangeSettingsStructure.ExchangeTransportKind) Then
		...
		ErrorMessageString = NStr("en = 'Exchange transport kind is not specified. Exchange is canceled.'",
			CommonUseClientServer.MainLanguageCode());
		WriteLogEventOfDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
	...
	ElsIf Not ValueIsFilled(ExchangeSettingsStructure.ActionOnExchange) Then
		...
		ErrorMessageString = NStr("en = 'Executed action (export/ import) is not specified. Exchange is canceled.'",
			CommonUseClientServer.MainLanguageCode());
		WriteLogEventOfDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
...
Procedure RunExchangeStructureValidityCheck(ExchangeSettingsStructure, UseTransportSettings = True)
	...
	ElsIf ExchangeSettingsStructure.InfobaseNode.DeletionMark Then
		...
		ErrorMessageString = NStr("en = 'Infobase node is marked for deletion. Exchange is canceled.'",
			CommonUseClientServer.MainLanguageCode());
		WriteLogEventOfDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
	...
	ElsIf ExchangeSettingsStructure.InfobaseNode = ExchangeSettingsStructure.CurrentExchangePlanNode Then
		...
		ErrorMessageString = NStr("en = 'Unable to properly communicate with the current infobase node. The exchange has been canceled.'",
			CommonUseClientServer.MainLanguageCode());
		WriteLogEventOfDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
	...
	ElsIf IsBlankString(ExchangeSettingsStructure.InfobaseNodeCode)
		...
		ErrorMessageString = NStr("en = 'One of exchange nodes has an empty code. Exchange is canceled.'",
			CommonUseClientServer.MainLanguageCode());
		WriteLogEventOfDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
	...
	ElsIf ExchangeSettingsStructure.DebuggingExportHandlers Then
		...
		If Not ExportDataProcessorFile.Exist() Then
			...
			ErrorMessageString = NStr("en = 'External data processor file for export debugging does not exist. Exchange is canceled.'",
				CommonUseClientServer.MainLanguageCode());
			WriteLogEventOfDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
	...
	ElsIf ExchangeSettingsStructure.DebuggingImportHandlers Then
		...
		If Not ImportDataProcessorFile.Exist() Then
			...
			ErrorMessageString = NStr("en = 'External data processor file for import debugging does not exist. Exchange is canceled.'",
				CommonUseClientServer.MainLanguageCode());
			WriteLogEventOfDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
...
Procedure SetExchangeRulesOfDataDump(DataExchangeXMLDataProcessor, ExchangeSettingsStructure)
	...
	If ObjectConversionRules = Undefined Then
		...
		// Exchange rules should be specified.
		NString = NStr("en = 'Conversion rules are not specified for exchange plan %1. Data export is canceled.'",
			CommonUseClientServer.MainLanguageCode());
...
Procedure SetDataImportExchangeRules(DataExchangeXMLDataProcessor, ExchangeSettingsStructure)
	...
	If ObjectConversionRules = Undefined Then
		...
		// Exchange rules should be specified.
		NString = NStr("en = 'Conversion rules are not specified for exchange plan %1. Data import is canceled.'",
			CommonUseClientServer.MainLanguageCode());
		ErrorMessageString = StringFunctionsClientServer.SubstituteParametersInString(NString, ExchangeSettingsStructure.ExchangePlanName);
		WriteLogEventOfDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
		SetExchangeInitEnd(ExchangeSettingsStructure);
...
#Region QuantityOfItemsInTransaction
...
Procedure RegisterErrorDocument(Object, ExchangeNode, ErrorMessage) Export
	...
	MessageString = NStr("en = 'Cannot post document %1 received from another infobase.%2 Maybe not all required attributes are filled in.'",
		CommonUseClientServer.MainLanguageCode());
...
Procedure RegisterErrorRecordsObject(Object, ExchangeNode, ErrorMessage) Export
	...
	MessageString = NStr("en = 'Cannot write object %1 received from another infobase. %2 Maybe not all required attributes are filled in.'",
		CommonUseClientServer.MainLanguageCode());
	MessageString = StringFunctionsClientServer.SubstituteParametersInString(MessageString, String(Object), CauseErrors);
...
Function SynchronizationDatePresentation(Val SynchronizationDate) Export
	...
	If Not ValueIsFilled(SynchronizationDate) Then
		Return NStr("en = 'Synchronization has never been performed.'");
	EndIf;
	
	Return StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'Last synchronization: %1'"),
...
Function CorrespondentVersionsDiffer(ExchangePlanName, EventLogMonitorMessageKey, VersionInThisApplication,
	...
	If ValueIsFilled(VersionInThisApplication) AND ValueIsFilled(VersionInAnotherApplication)
		...
		If VersionInThisApplicationWithoutBuildNumber <> VersionInAnotherApplicationWithoutBuildNumber Then
			...
			MessagePattern = NStr("en = 'Data may be synced incorrectly because version of application ""%1"" (%2) is different from the version %3 specified in conversion rules of another application. Make sure that you imported the rules relevant for both applications.'");
...
Function EventLogMonitorMessageTextEstablishingConnectionToWebService() Export
	...
	Return NStr("en = 'Data exchange.Connecting to web service'", CommonUseClientServer.MainLanguageCode());
...
Function DataExchangeRuleImportingEventLogMonitorMessageText() Export
	...
	Return NStr("en = 'Data exchange.Rule import'", CommonUseClientServer.MainLanguageCode());
...
Function DataExchangeCreationEventLogMonitorMessageText() Export
	...
	Return NStr("en = 'Data exchang.Creating data exchange'", CommonUseClientServer.MainLanguageCode());
...
Function EventLogMonitorMessageTextRemovingTemporaryFile() Export
	...
	Return NStr("en = 'Data exchange.Removing temporary file'", CommonUseClientServer.MainLanguageCode());
...
Function EventLogMonitorMessageTextDataExchange() Export
	...
	Return NStr("en = 'Data exchange description'", CommonUseClientServer.MainLanguageCode());
...
#Region ProceduresAndFunctionsForWorksWithTheDataExchangeMessageFromMainNodeConstant
//
...
Procedure SetDataExchangeMessageFromMainNode(ExchangeMessage, MasterNode) Export
	...
	WriteEventGetData(MasterNode, NStr("en = 'The exchange message was written to cache.'"));
...
Procedure ClearDataExchangeMessageFromMainNode() Export
	...
	WriteEventGetData(MasterNode(), NStr("en = 'The exchange message was deleted from cache.'"));
...
Function ExternalModuleContainerDictionary() Export
	...
	Result.Insert("Nominative", NStr("en = 'Configure data synchronization'"));
...
#Region ServiceProgramInterfaceOfInteractiveExportAddition
//
...
Function InteractiveExportChange(Val InfobaseNode, Val AddressOfFormStore, Val IsNodeScript=Undefined) Export
	...
	ScriptParametersAdditions.VariantNoneAdds.Insert("Explanation", 
		NStr("en = 'Only data according to general settings will be sent.'")
	); 
	...
	ScriptParametersAdditions.VariantAllDocuments.Insert("Explanation",
		NStr("en = 'All the period documents which satisfy the filter conditions will be sent additionally.'")
	); 
	...
	ScriptParametersAdditions.VariantArbitraryFilter.Insert("Explanation",
		NStr("en = 'Data will be sent additionally according to the filter.'")
	); 
	...
	ScriptParametersAdditions.VariantAdditionally.Insert("Explanation",
		NStr("en = 'Additional data on settings will be sent.'")
	); 
...
Function ExportAdditionNameAutoSaveSettings() Export
	Return NStr("en = 'Last sent (saved automatically)'");
EndFunction
...
//    * ToolTip     - String - The tooltip wording.
//
Procedure OnFillingToDoListForSynchronizationWarning(ToDoList)
	...
	If Not AccessRight("view", Metadata.InformationRegisters.DataExchangeResults)
		Or ModuleToDoListService.WorkDisabled("AlertOnSynchronization") Then
		Return;
	EndIf;
	...
	Sections = ModuleToDoListServer.SectionsForObject(Metadata.CommonForms.DataSync.FullName());
	...
	For Each Section In Sections Do
		...
		Work.Presentation  = NStr("en = 'Synchronization warnings'");
...
//    * ToolTip     - String - The tooltip wording.
//
Procedure OnFillingToDoListUpdateRequired(ToDoList)
	
	ModuleToDoListService = CommonUse.CommonModule("ToDoListService");
	If Not AccessRight("Administration", Metadata)
		Or ModuleToDoListService.WorkDisabled("UpdateRequiredDataExchange") Then
		Return;
	EndIf;
	ModuleToDoListServer = CommonUse.CommonModule("ToDoListServer");
	...
	For Each Section In Sections Do
		...
		Work.Important         = True;
		Work.Presentation  = NStr("en = 'Update application version'");
		...
		Else
			...
			Work.FormParameters = New Structure("Title,TemplateName",
				NStr("en = 'Install update'"), "ManualUpdateTip");
		EndIf;
		Work.Owner       = Section;
...
//    * ToolTip     - String - The tooltip wording.
//
Procedure OnFillCurrensTodosListCheckCompatibilityWithCurrentVersion(ToDoList)
	
	ModuleToDoListService = CommonUse.CommonModule("ToDoListService");
	If Not AccessRight("Edit", Metadata.InformationRegisters.DataExchangeRules)
		Or ModuleToDoListService.WorkDisabled("ExchangeRules") Then
		Return;
	EndIf;
	...
	OutputToDo = True;
	CheckedForVersion = CommonSettingsStorage.Load("ToDoList", "ExchangePlans");
	...
	Else
		...
		If Work.ThereIsWork Then
			ToDosGroup.Count = ToDosGroup.Quantity + Work.Quantity;
		EndIf;
	EndIf;
