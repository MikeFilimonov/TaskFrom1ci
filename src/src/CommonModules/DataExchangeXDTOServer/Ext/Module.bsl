...
Function OCRByName(ExchangeComponents, Name) Export
	...
	If ConversionRule = Undefined Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Credit slip with name %1 is not found'"), Name);
...
Function XDTOObjectFromXDTOData(
	...
	For Each Property In XDTOType.Properties Do
		...
		Else
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Unknown property type <%1>. Object type: %2'"),
				Property.Name,
				String(XDTOType)
			);
		EndIf;
...
//                                               data received as a result of the XDTO ConvertAndWrite data conversion.
//                                               - get object, ready.
//
// Returns:
...
Procedure OpenExportFile(ExchangeComponents, ExchangeFileName = "") Export
	...
	If ExchangeComponents.IsExchangeThroughExchangePlan Then
		...
		If TransactionActive() Then
			Raise NStr("en = 'Exchange data lock cannot be set in an active transaction.'");
		EndIf;
...
Procedure OpenImportFile(ExchangeComponents, ExchangeFileName) Export
	...
	While IterationNumber = 0 Do
		...
		Except
			...
			ErrorMessageString = NStr("en = 'An error occurred when importing data: %1'");
		...
		If IsExchangeThroughExchangePlan Then
			...
			If ExchangePlanManager.ExchangeFormat() <> ExchangeFormat.BasicFormat Then
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'Exchange message format <%1> does not correspond to exchange plan format <%2>.'"),
			...
			If ExchangeFormatVersionsArray(ExchangeComponents.CorrespondentNode).Find(ExchangeFormat.Version) = Undefined Then
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'Version of exchange message format <%1> is not supported.'"),
		...
		If MaxCommonVersion <> CorrespondentVersionNumber Then
			...
			WriteInExecutionProtocol(ExchangeComponents, 
										NStr("en = 'Exchange format version number is changed.'"),,False,,, True);
		EndIf;
...
// If the search is successful, the reference returns from the register. If the search is unsuccessful, the initial
// reference returns.
// 
// Parameters:
...
Procedure WriteObjectToIB(ExchangeComponents, Object, Type, WriteObject = False, Val SendBack = False, UUIDString = "") Export
	...
	If CommonUseReUse.DataSeparationEnabled()
		...
		ErrorMessageString = NStr("en = 'Attempting to change shared data (%1) in split mode.'");
...
Procedure ExecuteDocumentRealizationOnImport(
	...
	// Set a sender node to prevent the object from being registered on the node for which importing is run, posting should
	// not be in the import mode.
...
Function FormatVersionExchangeManager(Val InfobaseNode, Val FormatVersion) Export
	...
	If Result = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Conversion Manager for exchange format version <%1> is not defined.'"),
			FormatVersion
		);
	EndIf;
...
Procedure XDTOObjectStructurePropertyConversion(
	...
	ElsIf StageNumber = 2 Then
		...
		// properties of the obtained data object and represent a structure that contains either instruction for converting or
		// the XDTO value. If the value receiver is a row of tabular section,
...
// The procedure fills in the tabular section of an object taking into account the previous version of tabular section
// (before data import).
//
// Parameters:
...
//  ColumnsForInclusion                 - Other columns (except for the key ones) with changing values (row separated by
//                                        comma).
...
Procedure PerformPostponedObjectsRecording(ExchangeComponents)
	...
	For Each MappingObject In ExchangeComponents.ObjectsForPostponedRecording Do
		...
		// Set a sender node to prevent the object from being registered on the node for which importing is run, posting
		// should not be in the import mode.
...
Function ArrangeExchangeFormat(Val ExchangeFormat)
	...
	If FormatItems.Count() = 0 Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Non-canonical name of the exchange format <%1>'"),
			ExchangeFormat
		);
	EndIf;
...
Function FindRefbyPublicIdidentifier(XDTOObjectUDID, ExchangeComponents, IBObjectValueType)
	...
	If Selection.Count() > 0 Then
		...
		If  SuitableTypeRefsQuantity > 1 Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Several links are assigned for unique identifier <%1> and node <%2>.'"),
...
Function DERByName(ExchangeComponents, Name)
	...
	If DataProcessorRule = Undefined Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'DER with name %1 is not found'"), Name);
...
//  corresponds to the processed POD ProcessingObject  - Reference to an object that needs to be processed, a structure
//  that corresponds to the XDTO object (on import), or a reference to the infobase object (on export).
...
Procedure CheckVersion(Val Version)
	...
	If Versions.Count() = 0 Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Non-canonical presentation of the exchange format version: <%1>.'"),
			Version
		);
	EndIf;
...
Procedure DeleteDocumentRegisterRecords(DocumentObject, Cancel)
	...
	For Each RegisterRecordRow In RegisterRecordTable Do
		...
		If Not AccessRight("Update", Set.Metadata()) Then
			...
			Raise NStr("en = 'Access right violation:'") + " " + RegisterRecordRow.Name;
			Return;
		EndIf;
	...
	For Each RegisterRecordRow In RecordTableRowToProcessArray Do		
		...
		Except
			...
			Raise NStr("en = 'Operation failed:'") + " " + RegisterRecordRow.Name
