...
//    Keys:
//      ImportedTypes - Array(MetadataObject) - the array of metadata objects, the data of which is required to be
...
Procedure ImportDataFromArchive(Val ArchiveName, Val ExportParameters) Export
	...
	If Not ValidateRights() Then
		Raise NStr("en = 'Insufficient access rights to import data.'");
	EndIf;
...
Function ExportArchiveIsCompatibleWithCurrentConfiguration(Val ArchiveName) Export
	...
	Try
		...
		If ImportDescriptionItem = Undefined Then
			Raise NStr("en = 'File DumpInfo.xml is missing in the export file.'");
		EndIf;
