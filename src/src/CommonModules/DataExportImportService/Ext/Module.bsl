...
// ExportParameters - Structure - Export parameters, see parameter "ExportParameters" of the procedure
//                                "ExportCurrentDataAreaFromArchive" of the common module "DataAreaExportImport"
//
Procedure ImportDataFromDirectory(Val ImportingDirectory, Val ExportParameters) Export
	...
	If CommonUseClientServer.CompareVersions(PlatformVersion, "8.2.19.0") < 0
		...
		Raise
			NStr("en = 'To import data, update 1C:Enterprise technological platform. For version 8.2 it is required to use release 8.2.19 (or newer). For version 8.3 it is required to use release 8.3.4 (or newer).'");
	...
	If Not ExportArchiveIsCompatibleWithCurrentConfiguration(ExportInfo) Then
		...
		Raise ServiceTechnologyIntegrationWithSSL.PlaceParametersIntoString(
			NStr("en = 'Cannot import data from file as the file was exported from another configuration (the file is exported from configuration %1 and cannot be imported to configuration %2)'"),
	...
	If Not ExportInArchiveIsCompatibleWithCurrentConfigurationVersion(ExportInfo) Then
		...
		Raise ServiceTechnologyIntegrationWithSSL.PlaceParametersIntoString(
			NStr("en = 'Cannot import data from file as the file was exported from another configuration version (the file is exported from configuration version %1 and cannot be imported to configuration version %2)'"),
...
#Region FileTypesAndStructureOfTheImportExportDirectories
...
// file for the specified metadata object Used for searching and replacing refs when exporting, modifying schemas
// currentconfig and recording
// 
// Parameters:
...
Function XMLReferenceType(Val Value) Export
	...
	Else
		...
		Raise ServiceTechnologyIntegrationWithSSL.PlaceParametersIntoString(
			NStr("en = 'An error occurred while determining XMLType reference for object %1: the object is not a reference type.'"),
...
Function GetNamespacesForWritePackage(Val NamespaceURI)
	...
	SubordinateDocumentStructure = XDTOFactory.packages.Get(NamespaceURI).SubordinateDocumentStructure;
	For Each Dependence In SubordinateDocumentStructure Do
