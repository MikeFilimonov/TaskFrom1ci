...
Function VerifySignature(CryptoManager, SourceData, Signature, ErrorDescription = Null) Export
	...
	Try
		CryptographyManagerForCheck.VerifySignature(SourceDataForChecking, SignatureForCheck, ElectronicSignature);
	Except
	...
	If Not CheckCertificate(CryptographyManagerForCheck, ElectronicSignature, ErrorDescription) Then
		Return False;
	EndIf;
...
//
Function CheckCertificate(CryptoManager, ElectronicSignature, ErrorDescription = Null) Export
	...
	CertificateForCheck = ElectronicSignature;
	
	If TypeOf(ElectronicSignature) = Type("String") Then
		CertificateForCheck = GetFromTempStorage(ElectronicSignature);
	EndIf;
...
//
//  ObjectVersion      - String - version of object data if the reference to object was passed. Used
...
//     * UseDigitalSignature       - Boolean - if True, then digital signatures are used.
...
//
// See also:
//   DigitalSignatureClientServer.PersonalSettingsTip() - applicationmatic interface for getting.
...
Function PersonalSettingsTip() Export
	...
	// Initials values.
	PersonalSettingsTip.Insert("ActionsOnSavingDS", "Ask");
	PersonalSettingsTip.Insert("PathToDigitalSignatureAndEncryptionApplications", New Map);
	PersonalSettingsTip.Insert("ExtensionForSignatureFiles", "p7s");
	PersonalSettingsTip.Insert("ExtensionForEncryptedFiles", "p7m");
	...
	For Each KeyAndValue In PersonalSettingsTip Do
		...
		If ValueIsFilled(SavedValue)
		   AND TypeOf(KeyAndValue.Value) = TypeOf(SavedValue) Then
			...
			PersonalSettingsTip.Insert(KeyAndValue.Key, SavedValue);
		EndIf;
	EndDo;
	
	Return PersonalSettingsTip;
...
//       * ElectronicSignature                - ValueStorage.
//
Function ReadSignatureData(Signature) Export
	...
	If Certificates.Count() > 0 Then
		ElectronicSignature = Certificates[0];
		
		Result = New Structure;
		Result.Insert("Imprint", Base64String(ElectronicSignature.Imprint));
		Result.Insert("CertificateIsIssuedTo", DigitalSignatureClientServer.SubjectPresentation(ElectronicSignature));
		Result.Insert("CertificateBinaryData", ElectronicSignature.Unload());
		Result.Insert("Signature", New ValueStorage(Signature));
		Result.Insert("ElectronicSignature", New ValueStorage(ElectronicSignature.Unload()));
	EndIf;
...
Procedure AddSignatureRow(ObjectData, SignatureProperties)
	...
	FillPropertyValues(NewRecord, SignatureProperties,, "Signature, ElectronicSignature");
	
	NewRecord.Signature    = New ValueStorage(SignatureProperties.Signature);
	NewRecord.ElectronicSignature = New ValueStorage(SignatureProperties.ElectronicSignature);
...
Procedure DeleteSignatureRow(DigitallySignedObject, RowIndex)
	...
	If DigitallySignedObject.DigitalSignatures.Count() < RowIndex + 1 Then
		Raise NStr("en = 'String with signature was not found.'");
	EndIf;
