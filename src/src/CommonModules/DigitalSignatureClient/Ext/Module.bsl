...
Function UseDigitalSignature() Export
	...
	Return DigitalSignatureClientServer.CommonSettings().UseDigitalSignature;
...
//
//  ShowError - Boolean - if True, then the ApplicationAccessError form will be opened, form which you can go to the
//      list of installed applications to the form of personal settings in the Installed applications page. There you
//      can see why you did not manage to use the application and also open the installation guide.
//
...
//                              password to the certificate at the time of a session, c) or a password was set earlier
//                              using the SetCertificatePassword method. If an error occurred during signing, then the form
...
//                              an additional data preparation handler after selecting the certificate using which the
//                              data will be signed. IN this handler, you can fill the Data parameter
//                              if it depends on the certificate that at the time
//                              of the call is already inserted into DataDescription, as SelectedCertificate (see
//                              below). A common approach should be taken into account (see above).
//    * ExecuteAtServer - Undefined, Boolean - (optional) - when it is
//                              not specified or Undefined, then the execution will be defined
//                              automatically: if there is a server, then first on server and later on client was
//                              unsuccessful and then a message about two errors.
//                             When True: if the execution is allowed on server, then the execution is only on server if
//                             it fails, one error message on server. When False: execution is only on the client as if
//                             there is no server.
...
//                              perform an action with that same certificate once again (user is asked neither for a
//                              password, nor for action confirmation).
...
//                              CertificatesSelection and ExecuteOnServer parameters are ignored, their values remain as
//                              during first call. The AdditionalActionsParameters parameter is ignored.
...
//                              certificate can be used as if the password was saved at the time of the session.
//                              Otherwise the context is ignored.
//    Variant 1:
...
//                                 a signature to the tabular section DigitalSignatures. A common approach should be
//                                 taken into account (see above). At the time of call, the SignatureProperties
//                                 parameter is already inserted into DataDescription. IN the case of the DataSet
//                                 parameter, the DataSetCurrentItem is inserted into DataDescription containing the
//                                 SignatureProperties parameter.
...
//                                 Object property. Structure contains properties Value and Presentation.
//    Variant 2:
...
//                                 DataSet is used if not specified, then the presentation is calculated according to
//                                 the Item object of data set property value.
...
//               partial completion is determined by the availability of the SignatureProperties property. If there is,
...
//         Derived properties:
//         * ElectronicSignature          - BinaryData - contains
...
//                              DigitalSignatures, in which a signature needs to be added.
//    //* --  --           - NotifyDescription - (optional) - handler of adding
//                              a signature to the tabular section DigitalSignatures. A common approach should be taken
//                              into account (see above). At the time of the call, the Signatures parameter has already
//                              been inserted into DataDescription.
//    * ObjectVersioning      - String - (optional) - version of object data
//                              to check and lock object before adding a signature.
...
Procedure AddSignatureFromFile(DataDescription, Form = Undefined, ResultProcessing = Undefined) Export
	...
	AddingForm = OpenForm("CommonForm.AddElectronicSignatureFromFile", ServerParameters,,,,, 
...
//                         - Array - (optional)
//                              contains values of
//                              the CatalogRef.DigitalSignaturesAndEncryptionKeyCertificates type or the BinaryData type
//                              (export certificate).
...
//                              not specified or Undefined, then the execution will be defined
//                              automatically: if there is a server, then first on server and later on client was
//                              unsuccessful and then a message about two errors. When True: if the execution is allowed
//                              on server, then the execution is only on server if it fails, one error message on server.

//                              When False: execution is only on the client as if there is no server.
//    * OperationContext   - Undefined - (optional) - if it is specified, then
//                              a certain value of a custom type
//                              will be set into the property that allows you to
//                              perform an action with that same encryption certificates once again (user is asked to
//                              confirm an action).
//    * ------  ------  // - Arbitrary - (optional) - if it is defined, then
//                              the action will be complete with the same encryption certificates.
//                              The WithoutConfirmation parameter is assumed to be True.
//                              Operation, DataTitle, CertificatesSet,
//                              ChangeSet and ExecuteOnServer parameters are ignored, their values remain as during
//                              first call.
//
//    Variant 1:
...
//     The DataDescription input parameter is passed to the result, to which properties are added:
//     * Success - Boolean - True if all succeeded. If Success = False, then
//               a partial completion is defined by the presence of the EncryptedData property. If there is, the step is
//               executed.
...
//                              protection of a private key, b)or a user remembered a
//                              password to the certificate at the time of a session, c) or a password was set earlier
//                              using the SetCertificatePassword method. If an error occurred during decryption, then
//                              the form with the ability to specify a password will be opened.
...
//    * BeforeExecution   - NotifyDescription - (optional) - description of
//                              an additional data preparation handler after selecting the certificate using which the
//                              data will be decrypted. IN this handler you can fill the Data parameter if it is required.
//                              During the call in the DataDescription the selected
//                              certificate is already  as SelectedCertificate (see below). A common approach should be
...
//                              IsAuthentification and ExecuteOnServer parameters are ignored, their values remain as
//                              during first call. The AdditionalActionsParameters parameter is ignored.
//                              The BeforeOperationBegin procedure is not called.
//                              If you pass the context returned
...
//                                 IN the case of the DataSet
//                                 parameter, the DataSetCurrentItem is inserted into DataDescription containing the
//                                 DecryptedData parameter.
//    (prohibition is set for the object                - Refs - (optional) - ref to object with the tabular section.
//                                 EncryptionCertificates, from
//                                 which you need to obtain certificates and also clear after a successful decryption completion.
//                                 If it is not specified, then the certificates are not required to be received //from
//                                 an object and cleared.
...
//     The DataDescription input parameter is passed to the result, to which properties are added:
//     * Success - Boolean - True if all succeeded. If Success = False, then
//               a partial completion is defined by the presence of the DecryptedData property. If there is, the step is
//               executed.
//     * SelectedCertificate - Structure - contains certificate properties:
//         * Ref    - CatalogRef.DigitalSignaturesAndEncryptionKeyCertificates - reference to certificate.
//         * Thumbprint - String - certificate thumbprint in the Base64 row format.
...
Procedure Decrypt(DataDescription, Form = Undefined, ResultProcessing = Undefined) Export
	...
	ServerParameters = New Structure;
	ServerParameters.Insert("Operation",            NStr("en = 'Data decryption'"));
	ServerParameters.Insert("DataTitle",     NStr("en = 'Data'"));
	ServerParameters.Insert("FilterCertificates");
...
Procedure CheckCertificate(Notification, ElectronicSignature, CryptoManager = Undefined) Export
	...
	DigitalSignatureServiceClient.CheckCertificate(Notification, ElectronicSignature, CryptoManager);
...
//
// Parameters:
//  ElectronicSignature - CatalogRef.DigitalSignaturesAndEncryptionKeyCertificates - certificate being checked.
//
...
//                                 Result.ChecksComplete is transferred to the procedure(see below) with the False
//                                 initial value. If you do not set True
//                                 in the CheckOnSelect mode, then the form will not be closed
//                                 after returning from the alert procedure and a warning that it can not continue will
//                                 be shown.
...
//                                  the ResultProcessing parameter is set, then a form will not be opened if there is 
//                                  the ChecksComplete parameter and True is set.
...
//
Procedure CheckCatalogCertificate(ElectronicSignature, AdditionalParameters = Undefined) Export
	
	DigitalSignatureServiceClient.CheckCatalogCertificate(ElectronicSignature, AdditionalParameters);
...
Procedure OpenDigitalSignaturesAndEncryptionSettings(Page = "Certificates") Export
	...
	Form = OpenForm("CommonForm.ElectronicSignatureAndEncriptionSettings", FormParameters);
...
//
//  PasswordExplanation   - Structure - with the properties describing an explanation that will be written under the
...
//          * ElectronicSignature - CatalogRef.DigitalSignaturesAndEncryptionKeyCertificates - reference
...
//  ElectronicSignature - CatalogRef.DigitalSignaturesAndEncryptionKeyCertificates - the
...
//
Procedure CertificateStartChoiceWithConfirmation(Item, ElectronicSignature, StandardProcessing, ForEncryptionAndDecryption = False) Export
...
#Region OutdatedProceduresAndFunctions
...
Function GenerateSignatureData(
	...
	NewSignatureBinaryData = CryptoManager.Sign(BinaryData, StructureOfSignatureParameters.ElectronicSignature);
	ErrorDescription = "";
	If DigitalSignatureServiceClientServer.EmptySignatureData(NewSignatureBinaryData, ErrorDescription) Then
		Raise ErrorDescription;
	EndIf;
	
	Imprint = Base64String(StructureOfSignatureParameters.ElectronicSignature.Imprint);
	IssuedToWhom = DigitalSignatureClientServer.SubjectPresentation(StructureOfSignatureParameters.ElectronicSignature);
	CertificateBinaryData = StructureOfSignatureParameters.ElectronicSignature.Unload();
...
Procedure OpenSignature(CurrentData) Export
	...
	OpenForm("CommonForm.ElectronicSignature", FormParameters);
