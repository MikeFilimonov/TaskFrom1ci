...
//
// Parameters:
//   Handlers - ValueTable - see UpdateResults.UpdateHandlersNewTable().
//
...
Procedure OnAddParametersJobsClientLogicStandardSubsystems(Parameters) Export
	...
	If CommonUseReUse.CanUseSeparatedData() Then
		...
		SubsystemSettings.Insert("PersonalSettingsTip", DigitalSignature.PersonalSettingsTip());
...
//                         * FromException    - Boolean - description contains a brief presentation of information about
//                                                        error.
...
Function CryptoManager(Operation, ShowError = True, ErrorDescription = "", Application = Undefined) Export
	...
	ElsIf Operation = "SignatureCheck" Then
		ErrorTitle = NStr("en = 'Cannot check the signature on server %1 due to:'");
	...
	ElsIf Operation = "CertificateCheck" Then
		ErrorTitle = NStr("en = 'Cannot check the certificate on server %1 due to:'");
	...
	Else
		ErrorTitle = NStr("en = 'Cannot execute the operation on server %1 due to:'");
	EndIf;
	...
	ElsIf ValueIsFilled(Application) Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Application %1 is not available or installed.'"), Application);
	Else
		ErrorText = NStr("en = 'None of the applications are available or installed.'");
	EndIf;
...
Function GetCertificateByImprint(Imprint, InPersonalStorageOnly,
	...
	If Not ValueIsFilled(ErrorPresentation) Then
		Try
			ElectronicSignature = StorageOfCertificates.FindByThumbprint(BinaryDataImprint);
		Except
			If ShowError Then
				Raise;
			EndIf;
	...
	If ValueIsFilled(ErrorPresentation) Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
			...
			ErrorPresentation);
	Else
		ErrorText = NStr("en = 'ElectronicSignature is not found on the server.'");
	EndIf;
		
	If Not Users.InfobaseUserWithFullAccess(,, False) Then
		ErrorText = ErrorText + Chars.LF + NStr("en = 'Contact administrator.'")
	EndIf;
...
Procedure SavePersonalSettings(PersonalSettingsTip) Export
	
	SubsystemKey = SettingsStorageKey();
	
	For Each KeyAndValue In PersonalSettingsTip Do
...
Procedure BeforeEditKeyCertificate(Refs, ElectronicSignature, AttributesParameters) Export
	...
	DigitalSignatureOverridable.BeforeEditKeyCertificate(Refs, ElectronicSignature, Table);
...
Procedure CheckPresentationUniqueness(Presentation, CertificatRef, Field, Cancel) Export
	...
	If Not Query.Execute().IsEmpty() Then
		MessageText = NStr("en = 'ElectronicSignature with such presentation already exists'");
		...
		If CryptoManager <> Undefined Then
			...
			If Not Pesonal Then
				...
				For Each ElectronicSignature In Array Do
					CertificatesArray.Add(ElectronicSignature);
				EndDo;
			EndIf;
...
Procedure ConfigureSigningEncryptionDecryptionForm(Form, Encryption = False, Details = False) Export
	...
	If Details Then
		For Each ItemOfList In Form.CertificateChoiceList Do
			If TypeOf(ItemOfList.Value) = Type("String") Then
				Continue;
			EndIf;
			ElectronicSignature = ItemOfList.Value;
			Break;
		EndDo;
		
	ElsIf AccessRight("SaveUserData", Metadata) Then
		If Encryption Then
			ElectronicSignature = CommonSettingsStorage.Load("Cryptography", "CertificateForEncryption");
		Else
			ElectronicSignature = CommonSettingsStorage.Load("Cryptography", "CertificateForSigning");
		EndIf;
	EndIf;
	...
	ElsIf Form.FilterCertificates.Count() = 1 Then
		ElectronicSignature = Form.FilterCertificates[0].Value;
	EndIf;
	
	Form.ElectronicSignature = ElectronicSignature;
	
	If ValueIsFilled(Form.ElectronicSignature)
	   AND CommonUse.ObjectAttributeValue(Form.ElectronicSignature, "Ref") <> Form.ElectronicSignature Then
		
		Form.ElectronicSignature = Undefined;
	EndIf;
	
	If ValueIsFilled(Form.ElectronicSignature) Then
		If Encryption Then
		...
		EndIf;
	Else
		Items.ElectronicSignature.DefaultItem = True;
	EndIf;
...
Procedure CertificateOnChangeAtServer(Form, CertificateThumbprintsAtClient, Encryption = False, Details = False) Export
	...
	If Form.FilterCertificates.Count() = 0
	   AND AccessRight("SaveUserData", Metadata) Then
		...
		If Encryption Then
			CommonSettingsStorage.Save("Cryptography", "CertificateForEncryption", Form.ElectronicSignature);
		ElsIf Not Details Then
			CommonSettingsStorage.Save("Cryptography", "CertificateForSigning", Form.ElectronicSignature);
		EndIf;
	EndIf;
...
Procedure WriteCertificateToCatalog(Form, Application = Undefined, ForEncryption = False) Export
	...
	If ValueIsFilled(Form.ElectronicSignature) Then
		CertificateObject = Form.ElectronicSignature.GetObject();
	Else
	...
	If CertificateObject.Modified() Then
		...
		Form.ElectronicSignature = CertificateObject.Ref;
	EndIf;
...
Procedure SetConditionalCertificatesListAppearance(List, RemoveApplications = False) Export
	...
	If RemoveApplications Then
		...
		ListOfState.Add(Enums.CertificateIssueRequestStatus.EmptyRef());
		ListOfState.Add(Enums.CertificateIssueRequestStatus.Executed);
...
Procedure ProcessAddedCertificates(CertificatesPropertiesTable, ExceptAlreadyAdded)
	...
	|	CASE
	|		WHEN Certificates.RequestStatus = VALUE(Enum.CertificateIssueRequestStatus.EmptyRef)
	|			THEN FALSE
	|		WHEN Certificates.RequestStatus = VALUE(Enum.CertificateIssueRequestStatus.Executed)
	|			THEN FALSE
	|		ELSE TRUE
	|	END AS ThisRequest
	|FROM
...
Function EncryptionCertificatesFromDescription(Description)
	...
	While Selection.Next() Do
		CertificateProperties = New Structure("Thumbprint, Presentation, ElectronicSignature");
		FillPropertyValues(CertificateProperties, Selection);
		CertificateProperties.ElectronicSignature = CertificateProperties.ElectronicSignature.Get();
...
Procedure FillCertificateAdditionalProperties(Form)
	...
	If Not ValueIsFilled(Form.ElectronicSignature) Then
		Return;
	EndIf;
	
	Items = Form.Items;
	
	AttributeValues = CommonUse.ObjectAttributesValues(Form.ElectronicSignature,
