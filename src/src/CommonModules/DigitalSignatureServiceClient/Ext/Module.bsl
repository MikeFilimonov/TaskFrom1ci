...
//                      If a reference to the catalog item is sent, and the catalog item is not found by a thumbprint,
//                      then the certificate data will be opened.
...
Procedure OpenCertificateContinuation(Context)
	...
	ElsIf TypeOf(Context.CertificateData) <> Type("String") Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'An error occurred calling the OpenCertificate procedure of the DigitalSignatureClient general module: Incorrect value of the %1 CertificateData parameter.'"), String(Context.CertificateData));
...
//
//   ElectronicSignature - CryptoCertificate - certificate.
//              - BinaryData - certificate binary data.
//              - String - address of a temporary storage containing certificate binary data.
//
Procedure SaveCertificate(Notification, ElectronicSignature, FileDescriptionWithoutExtension = "") Export
	
	Context =  New Structure;
	Context.Insert("Notification",            Notification);
	Context.Insert("ElectronicSignature",            ElectronicSignature);
...
Procedure SaveCertificateAfterExpansionSetting(ExtensionAttached, Context) Export
	...
	If TypeOf(Context.ElectronicSignature) = Type("CryptoCertificate") Then
...
Procedure SaveCertificateContinuation(Context)
	...
	ElsIf TypeOf(Context.ElectronicSignature) = Type("BinaryData") Then
		Context.CertificateAddress = PutToTempStorage(Context.ElectronicSignature);
		
	ElsIf TypeOf(Context.ElectronicSignature) = Type("String")
	        AND IsTempStorageURL(Context.ElectronicSignature) Then
		
		Context.CertificateAddress = Context.ElectronicSignature;
	Else
	...
	Dialog.Title = NStr("en = 'Select a file to save the certificate to'");
	Dialog.Filter    = NStr("en = 'ElectronicSignature files (*.cer)|*.cer|All files (*.*)|*.*'");
...
Procedure SaveCertificateAfterReceivingFiles(ReceivedFiles, Context) Export
	...
	Else
		...
		ShowUserNotification(NStr("en = 'ElectronicSignature is saved to file:'"),,
...
Procedure SaveCertificateQueryAfterSettingExpansion(ExtensionAttached, Context) Export
	...
	Dialog = New FileDialog(FileDialogMode.Save);
	Dialog.Title = NStr("en = 'Select a file to save the certificate request to'");
	Dialog.Filter    = NStr("en = 'ElectronicSignature files (*.p10)|*.p10|All files (*.*)|*.*'");
	Dialog.Multiselect = False;
...
Procedure SaveCertificateQueryAfterReceivingFiles(ReceivedFiles, Context) Export
	...
	Else
		HasReceivedFiles = True;
		ShowUserNotification(NStr("en = 'ElectronicSignature request is saved to file:'"),,
			ReceivedFiles[0].Name);
	EndIf;
...
Procedure SaveSignatureAfterConnectingFileOperationsExtension(Attached, Context) Export
	...
	ExtensionForSignatureFiles = DigitalSignatureClientServer.PersonalSettingsTip().ExtensionForSignatureFiles;
	...
	FileOpeningDialog.Title = NStr("en = 'Select a file to save the signature to'");
...
// Continue the GetCertificateByThumbprint procedure.
Procedure GetCertificateByThumbprintAfterSearch(ElectronicSignature, Context) Export
	
	GetCertificateByThumbprintEnd(ElectronicSignature, , Context);
	
EndProcedure

// Continue the GetCertificateByThumbprint procedure.
Procedure GetCertificateByThumbprintEnd(ElectronicSignature, ErrorPresentation, Context)
	
	If TypeOf(ElectronicSignature) = Type("CryptoCertificate") Then
		ExecuteNotifyProcessing(Context.Notification, ElectronicSignature);
		Return;
	EndIf;
	
	If ValueIsFilled(ErrorPresentation) Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'ElectronicSignature is not found
			     |on the computer as: %1'"),
			ErrorPresentation);
	Else
		ErrorText = NStr("en = 'ElectronicSignature is not found on the computer.'");
	EndIf;
...
// ElectronicSignature is always checked on
...
//                          check of digital signatures on server if the cryptography manager is specified and you
//                          failed to obtain it without error. It increases productivity and safety during a
...
//                          cryptography manager (manager of a first application on the list like the administrator has
//                          set it).
...
Procedure CheckSignatureAfterCreatingCryptographyManager(Result, Context) Export
	...
	If CryptoManager <> Undefined
	   AND Not (  TypeOf(Context.SourceData) = Type("String")
		...
		// ElectronicSignature is checked on server and on client.
		CheckSignatureOnClient(Context);
		Return;
	EndIf;
...
// Continue the CheckSignature procedure.
Procedure CheckSignatureOnClientAfterSignatureCheck(ElectronicSignature, Context) Export
	
	If ElectronicSignature = Undefined Then
		ExecuteNotifyProcessing(Context.Notification,
			NStr("en = 'ElectronicSignature is not found in signature data.'"));
		Return;
	EndIf;
	...
	CheckCertificate(Context.Notification, ElectronicSignature, CryptoManager);
...
//
Procedure CheckCertificate(Notification, ElectronicSignature, CryptoManager = Undefined) Export
	...
	If Context.CryptoManager = Undefined
	   AND DigitalSignatureClientServer.CommonSettings().VerifyDigitalSignaturesAtServer Then
		...
		If TypeOf(ElectronicSignature) = Type("CryptoCertificate") Then
			...
			ElectronicSignature.BeginUnloading(New NotifyDescription(
				"CheckCertificateAfterExportCertificate", ThisObject, Context));
		Else
			CheckCertificateAfterExportCertificate(ElectronicSignature, Context);
		EndIf;
	Else
...
Procedure CheckCertificateAfterExportCertificate(ElectronicSignature, Context) Export
	
	// ElectronicSignature check on server.
	If TypeOf(ElectronicSignature) = Type("BinaryData") Then
		CertificateAddress = PutToTempStorage(ElectronicSignature);
	Else
		CertificateAddress = ElectronicSignature;
	EndIf;
...
Procedure CheckCertificateAfterCreatingCryptographyManager(Result, Context) Export
	...
	CertificateForCheck = Context.ElectronicSignature;
...
//
//  ShowError - Boolean - if True, then the ApplicationAccessError form will be opened, from which you can go to the
//      list of installed applications to the form of personal settings on the Installed applications page. There you
//      can see why you did not manage to use the application e installation guide.
...
Procedure CreateCryptographyManagerAfterWorkWithCryptographyExpansionConnection(Attached, Context) Export
	...
	FormTitle = NStr("en = 'Digital signature and encryption application is required'");
	...
	ElsIf Operation = "SignatureCheck" Then
		ErrorTitle = NStr("en = 'Cannot check the signature due to:'");
	...
	ElsIf Operation = "CertificateCheck" Then
		ErrorTitle = NStr("en = 'Cannot check the certificate due to:'");
	...
	Else
		ErrorTitle = NStr("en = 'Cannot execute the operation due to:'");
	EndIf;
	...
	If Not Attached Then
		ErrorText =
			NStr("en = 'Install an Internet browser extension to work with digital signature and encryption.'");
...
Procedure CreateCryptographyManagerAfterCycle(Context) Export
	...
	If ErrorProperties.Errors.Count() = 0 Then
		...
		If Not StandardSubsystemsClientReUse.ClientWorkParameters().InfobaseUserWithFullAccess Then
			ErrorText = ErrorText + Chars.LF + Chars.LF + NStr("en = 'Contact administrator.'");
		EndIf;
	...
	Else
		If ValueIsFilled(Context.Application) Then
			ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Application %1 is not available or installed on the computer.'"), Context.Application);
		Else
			ErrorText = NStr("en = 'None of the applications are available or installed on computer.'");
		EndIf;
		ErrorProperties.Insert("Description", ErrorText);
	EndIf;
...
Procedure GetCertificatePropertiesOnClientAfterReceivingAllCustomerCertificates(Array, Context) Export
	...
	For Each ElectronicSignature In Array Do
		Context.CertificatesArray.Add(ElectronicSignature);
	EndDo;
...
Procedure AddCertificateForEncryptionOnlyFromFileAfterConnectingExtension(Attached, Context) Export
	...
	Dialog.Title = NStr("en = 'Select a certificate file (only for encryption)'");
	Dialog.Filter = NStr("en = 'ElectronicSignature X.509 (*.cer;*.crt)|*.cer;*.crt|All files(*.*)|*.*'");
...
Procedure AddCertificateForEncryptionOnlyFromFileAfterPostingFile(Address, Context)
	...
	If Form = Undefined Then
		ShowMessageBox(,
			NStr("en = 'ElectronicSignature file must have DER X.509 format, operation aborted.'"));
		Return;
	EndIf;
	...
	If Not Form.IsOpen() Then
		...
		ShowQueryBox(
			New NotifyDescription("AddCertificateForEncryptionOnlyFromFileAfterWarningAboutExisting",
				ThisObject, Form.ElectronicSignature),
			NStr("en = 'ElectronicSignature is already added.'"), Buttons);
	EndIf;
	
EndProcedure

// Continue the AddCertificateForEncryptionOnlyFromFile procedure.
Procedure AddCertificateForEncryptionOnlyFromFileAfterWarningAboutExisting(Response, ElectronicSignature) Export
	
	If Response <> "Open" Then
		Return;
	EndIf;
	
	OpenCertificate(ElectronicSignature);
...
Procedure StartSelectingCertificateWhenFilterIsSet(Form) Export
	...
	If ValueIsFilled(UnavailableCertificates) Then
		If DigitalSignatureClientServer.CommonSettings().CreateDigitalSignaturesAtServer Then
			If ValueIsFilled(AvailableCertificates) Then
				Title = NStr("en = 'The following trusted certificates were not found either on computer, or on server:'");
			Else
				Title = NStr("en = 'None of the following trusted certificates was found either on the computer, or on the server:'");
			EndIf;
		Else
			If ValueIsFilled(AvailableCertificates) Then
				Title = NStr("en = 'The following trusted certificates were not found on computer:'");
			Else
				Title = NStr("en = 'None of the following trusted certificates was found on the computer:'");
			EndIf;
		EndIf;
...
// Only for internal use.
Procedure CheckCatalogCertificate(ElectronicSignature, AdditionalParameters) Export
	...
	ServerParameters.Insert("ElectronicSignature", ElectronicSignature);
...
//
// A general approach to processing of the properties values with the NotificationDescription type in the
// DataDescription parameter. During the notification processing the parameters structure is being inserted into it
...
//                                it is not specified, then the presentation is calculated according to value of the
//                                Object property.
...
//                              FileDescriptionFull property of the Row for saving digital signatures type (see above a
//                              general approach). If the extension for work with files is not connected,
//                              you need to return the attachment file name without path.
//                              If the property is not inserted or filled - it is
//                              considered to be a denial from continuing, and ResultProcessing will be called with the
//                              False result.
//
...
Procedure SaveDataWithSignature(DataDescription, ResultProcessing = Undefined) Export
	...
	SaveAllSignatures = DigitalSignatureClientServer.PersonalSettingsTip(
	...
	SavingForm = OpenForm("CommonForm.SaveAndSignWithElectronicSignature", ServerParameters,,,,,
...
Procedure SaveDataWithSignatureAfterSavingDataFile(Result, Context) Export
	...
	If Result.Property("ErrorDescription") Then
		Error = New Structure("ErrorDescription",
			NStr("en = 'An error occurred when writing file:'") + Chars.LF + Result.ErrorDescription);
		
		ShowRequestToApplicationError(
			NStr("en = 'Cannot save signatures with the file'"), "", Error, New Structure);
		Return;
...
Procedure SaveDataWithSignatureCycleInternalCycleBegin(Context)
	...
	ElsIf Context.Attached Then
		File = New File(Context.FullNameOfSignatureFile);
		SaveDataWithSignatureCycleInternalCycleAfterFileInitialization(File, Context);
	Else
...
// Continue the DigitalSignatureClient procedure.SaveDataWithSignature.
Procedure SaveDataWithSignatureCycleInternalCycleAfterFileInitialization(File, Context)
...
Procedure SaveDataWithSignatureAfterGettingFiles(ReceivedFiles, Context) Export
	...
	OpenForm("CommonForm.ElectronicSignatureFilesSavingResult", FormParameters);
...
Procedure SetExtensionAfterWorkWithCryptographyExtensionConnection(Attached, Context) Export
	...
	OpenForm("CommonForm.CryptographyBrowserExtensionQuestion",
...
#Region CommonProceduresAndFunctionsOfTheManagedForms
...
Procedure ProcessPasswordInForm(Form, InternalData, PasswordProperties, AdditionalParameters = Undefined, NewPassword = Null) Export
	...
	AdditionalParameters.Insert("ElectronicSignature", Form.ElectronicSignature);
...
Procedure ExplanationSetPasswordClick(Form, Item, PasswordProperties) Export
	...
	If TypeOf(PasswordProperties.PasswordExplanationsProcessing) = Type("NotifyDescription") Then
		Result = New Structure;
		Result.Insert("ElectronicSignature", Form.ElectronicSignature);
...
Procedure ExecuteOnSideCertificatesPreparationCycleBegin(Context)
	...
	CryptoCertificate.BeginInitialization(New NotifyDescription(
		...
		Context.CertificatesProperties[Context.IndexOf].ElectronicSignature);
		...
		If Context.DataItem.Property("Object")
		   AND Not TypeOf(Context.DataItem.Object) = Type("NotifyDescription") Then
			...
			If Context.DataItem.Property("ObjectVersion") Then
				DataForServerItem.Property("ObjectVersion", Context.DataItem.ObjectVersion);
			EndIf;
		EndIf;
		...
		If Context.OnClientSide Then
			...
			Try
				DigitalSignatureServiceServerCall.AddSignature(DataItem.Object,
					SignatureProperties, Context.FormID, ObjectVersion);
			Except
				ErrorInfo = ErrorInfo();
				DataItem.Delete("SignatureProperties");
				ErrorOnClient = New Structure("ErrorDescription",
					NStr("en = 'An error occurred when writing the signature:'")
...
Procedure ExecuteOnSideCycleAfterWriteEncryptedData(Result, Context) Export
	...
	If Result.Property("ErrorDescription") Then
		...
		Error = New Structure("ErrorDescription",
			NStr("en = 'An error occurred when writing encrypted data:'")
			+ Chars.LF + Result.ErrorDescription);
		ExecuteOnSideAfterCycle(Error, Context);
		Return;
	EndIf;
...
Procedure ExecuteOnSideCycleAfterWriteDecryptedData(Result, Context) Export
	...
	If Result.Property("ErrorDescription") Then
		...
		Error = New Structure("ErrorDescription",
			NStr("en = 'An error occurred when writing decrypted data:'")
			+ Chars.LF + Result.ErrorDescription);
		ExecuteOnSideAfterCycle(Error, Context);
		Return;
	EndIf;
...
Procedure ProcessPassword(InternalData, AttributePassword, PasswordProperties,
	...
	ElectronicSignature = AdditionalParameters.ElectronicSignature;
	...
	SetPassword = SetPasswords.Get(ElectronicSignature);
	AdditionalParameters.Insert("PasswordIsSetApplicationmatically", SetPassword <> Undefined);
	If SetPassword <> Undefined Then
		AdditionalParameters.Insert("PasswordExplanation",
			InternalData.Get("SetPasswordsExplanations").Get(ElectronicSignature));
	EndIf;
	...
	If AdditionalParameters.EnhancedProtectionPrivateKey Then
		...
		AttributePassword = "";
		Value = PasswordsStorage.Get(ElectronicSignature);
		If Value <> Undefined Then
			PasswordsStorage.Delete(ElectronicSignature);
			Value = Undefined;
		EndIf;
		AdditionalParameters.Insert("PasswordInMemory", False);
	...
	Password = PasswordsStorage.Get(ElectronicSignature);
	...
	If AdditionalParameters.WhenChangingAttributeRememberPassword Then
		...
		ElsIf PasswordProperties.PasswordChecked Then
			PasswordsStorage.Insert(ElectronicSignature, PasswordProperties.Value);
	...
	If NewPassword <> Null Then
		...
		If NewPassword <> Undefined Then
			...
			PasswordProperties.PasswordChecked = True;
			NewPassword = "";
			If PasswordsStorage.Get(ElectronicSignature) <> Undefined Or AttributeRememberPassword Then
				PasswordsStorage.Insert(ElectronicSignature, PasswordProperties.Value);
				AdditionalParameters.Insert("PasswordInMemory", True);
			EndIf;
		ElsIf PasswordsStorage.Get(ElectronicSignature) <> Undefined Then
