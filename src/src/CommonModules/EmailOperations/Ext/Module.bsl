...
Function SendE_Mail(Val UserAccount,
	...
	If TypeOf(UserAccount) <> Type("CatalogRef.EmailAccounts")
		...
		Raise NStr("en = 'Account is not filled in or filled in incorrectly.'");
	EndIf;
	
	If EmailParameters = Undefined Then
		Raise NStr("en = 'Sending parameters are not specified.'");
	EndIf;
...
Function ImportEMails(Val UserAccount,
	...
	If Not UseForReceiving Then
		Raise NStr("en = 'Account is not intended for receiving messages.'");
	EndIf;
