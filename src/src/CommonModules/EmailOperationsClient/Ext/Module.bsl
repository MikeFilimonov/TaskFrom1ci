...
//  
//  FormClosingAlert - NotifyDescription - procedure to which you need to pass the contol after closing the email
//                                         sending form.
//                           
//
...
// If user does not have configured account for sending emails  that depending on rights there is setup assistent of
// account or message about impossibility of sending.
// 
// It is used for the scripts for which it is required to configure an account before requesting the additional
// parameters of sending.
// 
//
// Parameters:
...
Procedure CreatEmailAccountPerformedVerification(UserAccountIsConfigured, SendingParameters) Export
	...
	ElsIf True Then
		...
		DriveClient.OpenEmailMessageSendForm(Sender, Recipient,
...
Procedure OpenSimpleFormOfEmailSending(Sender,
	...
	OpenForm("CommonForm.EmailMessage", EmailParameters, , , , , OnCloseNotifyDescription);
EndProcedure
...
Procedure CheckAccount(Val UserAccount) Export
	...
	Status(NStr("en = 'Check account'"),,NStr("en = 'Checking the account. Please wait...'"));
	...
	Else
		...
		OpenForm("CommonForm.PasswordConfirmation", FormParameters);
	EndIf;
...
Procedure CheckPossibilityOfSendingAndReceivingOfEmails(ResultHandler, UserAccount, PasswordParameter) Export
	...
	If ValueIsFilled(ErrorInfo) Then
		ShowMessageBox(ResultHandler, StringFunctionsClientServer.SubstituteParametersInString(
			...
			NStr("en = 'Check account'"));
	Else
		ShowMessageBox(ResultHandler, StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Account parameter check is completed successfully. %1'"),
			AdditionalMessage),,
			NStr("en = 'Check account'"));
	EndIf;
