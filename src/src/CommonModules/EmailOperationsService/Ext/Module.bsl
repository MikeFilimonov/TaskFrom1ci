...
#Region InitialFillingAndUpdateOfIB
...
Procedure FillSystemAccount() Export
	...
	UpdateResults.WriteData(UserAccount);
	...
	If AccountSettings.UseForSending Then
		...
		Except
			ErrorInfo = StringFunctionsClientServer.SubstituteParametersInString(
									NStr("en = 'An error occurred while sending a message: %1'"),
									BriefErrorDescription(ErrorInfo()) );
		EndTry;
		If Not AccountSettings.UseForReceiving Then
			AdditionalMessage = Chars.LF + NStr("en = '(Email sending check is completed.)'");
		EndIf;
	EndIf;
		...
		Except
			...
			ErrorInfo = ErrorInfo
								+ StringFunctionsClientServer.SubstituteParametersInString(
										NStr("en = 'An error occurred while accessing the incoming message server: %1'"),
										BriefErrorDescription(ErrorInfo()) );
		EndTry;
		If Not AccountSettings.UseForSending Then
			AdditionalMessage = Chars.LF + NStr("en = '(Email receiving check is completed).'");
		EndIf;
	EndIf;
...
Procedure CheckPossibilityOfSendingTestMessages(Val UserAccount, Val Password = Undefined)
	...
	EmailParameters.Insert("Body", NStr("en = 'This message is sent by subsystem of 1C:Enterprise email'"));
...
//                 type Object is predefined as literal, it is not included into predefined elements.
//                 ChartsOfCharacteristicTypesRef.AccessKinds. This kind of access is used in the templates of access
//                 restrictions, such as "link" to another object by which the table is limited. When access type
//                 "Object" is assigned, you shall also indicate the types of tables used for this type of access.I.e. you
