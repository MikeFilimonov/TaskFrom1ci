...
//     NStr("en = 'Cannot connect to the Internet to check for updates.'; ru = 'Невозможно подключиться к сети Интернет
...
Procedure OpenDataForViewing(CurrentData) Export
	...
	If CurrentData = Undefined Or CurrentData.Data = Undefined Then
		ShowMessageBox(, NStr("en = 'This event log entry is not related to data (see the ""Data"" column)'"));
		Return;
	EndIf;
		...
		If CurrentData.Event = "_$Data$_.Delete" Then 
			...
			WarningText =
					StringFunctionsClientServer.SubstituteParametersInString(
						WarningText,
						NStr("en = 'Data is deleted from infobase'"));
		Else
			WarningText =
				StringFunctionsClientServer.SubstituteParametersInString(
						WarningText,
						NStr("en = 'Maybe the data is deleted from the infobase.'"));
		EndIf;
