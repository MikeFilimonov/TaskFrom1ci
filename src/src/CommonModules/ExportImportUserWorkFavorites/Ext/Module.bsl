...
//          application data processor interface    DataExportImportInfobaseDataExportManager. The parameter is passed
//          only on the call of the handler procedures for which version higher than 1.0.0.1 is specified on registration,
//        Serializer - XDTOSerializer, initiated with support of references abstracts execution. If a random exporting
//          handler requires additional data export - You should use SerializerXDTO that is passed to
...
//      (in original and current IB) that belong to the metadata object. If the True value is set - in a general module
//      of the handler, exported procedure BeforeMatchRefs() must be implemented that supports the following parameters:
...
Function NavigationRefOnDisplayingInArtifact(Val Representation)
	...
	If Representation.MainRef <> Undefined Then
		...
		KeyProperty = Representation.MainRef.Key;
		Refs = Representation.MainRef.Value;
		
		Result = StrReplace(Result, String(KeyProperty) + ".Type", Refs.Metadata().FullName());
		Result = StrReplace(Result, String(KeyProperty) + ".UUID",
	...
	For Each DisplayAdditionalRef In Representation.AdditionalRef Do
		...
		KeyProperty = DisplayAdditionalRef.Key;
		...
		Result = StrReplace(Result, String(KeyProperty) + ".UUID", LookupString);
		...
		If MetadataObject <> Undefined Then
			...
			For Each String In RefsInParameters Do
				...
				Else
					...
					DisplayAdditionalRef.Key = KeyProperty;
