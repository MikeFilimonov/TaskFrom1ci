...
Procedure OnAddHandlersOfServiceEvents(ClientHandlers, ServerHandlers) Export
	...
	ServerHandlers[
		"StandardSubsystems.BasicFunctionality\OnAddWorkParametersClientOnStart"].Add(
			"FileFunctionsService");	
...
#Region StandardApplicationmingInterface
...
Procedure AddClientWorkParameters(Parameters) Export
	...
	Parameters.Insert("PersonalFileOperationsSettings", New FixedStructure(
		FileOperationsSettings.PersonalSettingsTip));
...
Procedure DeleteFilesAtServer(FormerPathOnVolume) Export
	...
	If FileTemporary.Exist() Then
		...
		Except
			WriteLogEvent(
				NStr("en = 'Files.Delete files from the volume on exchange'",
...
#Region OperationsWithFileVolumes
...
Function StoringFilesInVolumesOnDrive() Export
	...
	StoreFilesInDirectoriesOnHardDisk = Constants.StoreFilesInDirectoriesOnHardDisk.Get();
	
	Return StoreFilesInDirectoriesOnHardDisk;
...
Function TypeOfFileStorage() Export
	...
	If StoreFilesInDirectoriesOnHardDisk Then
...
Function AddFileToVolume(BinaryDataOrPath, ModificationTimeUniversal, BaseName, Extension,
	...
	If Selection.Count() = 0 Then
		Raise NStr("en = 'There are no volumes to place files in'");
	EndIf;
			...
			If VolumeRef.MaximumSize <> 0 Then
				...
				If NewSize > VolumeRef.MaximumSize Then
					...
					Raise StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en = 'Maximum volume size exceeded (%1 MB).'"),
	...
	WriteLogEvent(
		NStr("en = 'Files. Adding a file'", CommonUseClientServer.MainLanguageCode()),
		EventLogLevel.Error,,,
...
#Region DigitalSignatureAndEncryptionForFiles
...
Procedure CryptographyOnCreateFormAtServer(Form, ThisIsListForm = True, RowsPictureOnly = False) Export
	...
	If CommonUse.SubsystemExists("StandardSubsystems.DigitalSignature")
	   ...
	   AND Not CommonUseClientServer.ThisIsMacOSWebClient() Then
		...
		ESigning = ModuleDigitalSignature.UseDigitalSignature();
	...
	ElsIf ESigning Then
		...
		ToolTip = NStr("en = 'Digital signature existence'");
		Picture  = PictureLib["DigitallySigned"];
	Else // Encryption
		Title = NStr("en = 'Encryption'");
		ToolTip = NStr("en = 'Encryption existence'");
...
Procedure OnGettingObjectExceptionsOfExchangePlan(Objects, Val DistributedInfobase) Export
	...
	Objects.Add(Metadata.Constants.StoreFilesInDirectoriesOnHardDisk);
...
Procedure WhenFillingOutPermitsForAccessToExternalResources(PermissionsQueries) Export
	...
	If GetFunctionalOption("StoreFilesInDirectoriesOnHardDisk") Then
...
Function CreateFileInitialImageAtServer(Node, FormUUID, Language, WindowsFileBaseFullName, LinuxFileBaseFullName, PathToArchiveWithWindowsVolumesFiles, PathToArchiveWithLinuxVolumeFiles) Export
	...
	If IsBlankString(FileBaseFullName) Then
		...
		CommonUseClientServer.MessageToUser(
			NStr("en = 'Specify a full name of the file base (file 1cv8.1cd)'"),,
			"WindowsFileBaseFullName");
		Return False;
	...
	If AreFilesInVolumes Then
		...
		If IsBlankString(PathToArchiveWithVolumeFiles) Then
			CommonUseClientServer.MessageToUser(
				NStr("en = 'Specify a full name of an archive with volume files (file *.zip)'"),, 
				"PathToArchiveWithWindowsVolumesFiles");
			Return False;
		EndIf;
	...
	Try
		...
		For Each Handler In EventHandlers Do
			Handler.Module.WhenConfidentialityOfFilesInVolumeWhenPlacing(
				FilesPathCompliance, TypeOfFileStorage, AttachedFilesArray);
		EndDo;
			...
			If ExchangePlanObject.Ref <> ThisNode Then
				...
				For Each Handler In EventHandlers Do
					Handler.Module.RegistrationChangesOnDelete(
						ExchangePlanObject.Ref, AttachedFilesArray);
				EndDo;
...
Procedure ExtractTextFromFilesOnServer() Export
	...
	WriteLogEvent(
		...
		,
		,
		NStr("en = 'Scheduled text extraction is started'"));
	...
	WriteLogEvent(
		...
		,
		,
		NStr("en = 'Scheduled text extraction is completed'"));
...
Procedure WhenAddSettingsFileOperations(CommonSettings, PersonalSettingsTip) Export
	...
	If CommonUse.SubsystemExists("StandardSubsystems.FileOperations") Then
		...
		ModuleFileOperationsServiceServerCall.AddFileOperationsSettings(CommonSettings, PersonalSettingsTip);
	EndIf;
