...
Procedure OpenScanSettingsForm() Export
	...
	If CommonUseClientServer.IsLinuxClient() Then
		MessageText = NStr("en = 'Scanning is not supported in client under OS Linux.'");
...
#Region AdditionalProceduresAndFunctionsForWorkWithFiles
...
// EndProcedure
//
Procedure ShowFormClosingConfirmationWithFiles(Form, Cancel, Exit, ObjectRef, AttributeName = "CanCloseFormWithFiles") Export
	...
	QuestionText = NStr("en = 'One or several files are occupied by you for editing.
	                    |
	                    |Continue?'");
	CommonUseClient.ShowArbitraryFormClosingConfirmation(Form, Cancel, Exit, QuestionText, AttributeName);
