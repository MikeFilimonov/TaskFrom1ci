...
Procedure OnAddParametersJobsClientLogicStandardSubsystemsRunning(Parameters) Export
	...
	CountEmployedFiles = 0;	
	If CommonUseReUse.CanUseSeparatedData() Then		
		User = UsersClientServer.AuthorizedUser();
		
		If TypeOf(User) = Type("CatalogRef.Users") Then			
			CountEmployedFiles = FileOperationsServiceServerCall.GetLockedFilesCount(, User);
		EndIf;
	EndIf;
	
	Parameters.Insert("CountEmployedFiles", CountEmployedFiles);
...
Procedure OnTransferringToFolderExecution(TransportParameters, Attachments) Export
	...
	Comment = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = 'Mailing reports %1 from %2'"),
...
//
Procedure WhenConfidentialityOfFilesInVolumeWhenPlacing(FilesPathCompliance, StoreFilesInDirectoriesOnHardDisk, AttachedFiles) Export
	
	FileOperationsServiceServerCall.AddFilesToVolumesOnPlacing(FilesPathCompliance, StoreFilesInDirectoriesOnHardDisk);
...
Procedure OnFillingInPossibleRightsForObjectRightsSettings(PossibleRights) Export
	...
	Right.ToolTip     = NStr("en = 'Reading folders and files'");
	...
	Right.Title     = NStr("en = 'Folder change'");
	Right.ToolTip     = NStr("en = 'Addition, change and deletion mark of files folders'");
	...
	Right.Title     = NStr("en = 'File change'");
	Right.ToolTip     = NStr("en = 'Change files in folder'");
	// Rights required for this right.
	Right.RequiredRights.Add("Read");
	// Rights for standard templates of access restrictions.
	...
	Right.RightsOwner  = "Catalog.FileFolders";
	Right.Name           = "FilesAdd";
	Right.Title     = NStr("en = 'Add files'");
	Right.ToolTip     = NStr("en = 'Add files to folder'");
	// Rights required for this right.
	...
	Right.ToolTip     = NStr("en = 'File deletion mark in the folder'");
	// Rights required for this right.
	Right.RequiredRights.Add("FilesUpdate");
	...
	Right.Title     = NStr("en = 'Right management'");
	Right.ToolTip     = NStr("en = 'Folder right management'");
	// Rights required for this right.
	Right.RequiredRights.Add("Read");
...
Procedure AtFillingToDoList(ToDoList) Export
	...
	If Not AccessRight("Read", Metadata.Catalogs.Files)
		Or ModuleToDoListService.WorkDisabled("EditableFiles") Then
		Return;
	EndIf;
	...
	For Each Section In Sections Do
		...
		Work.Presentation  = NStr("en = 'Files being edited'");
...
Function ScannerParametersInEnumerations(PermissionNumber, ChromaticityNumber, RotationNumber, PaperSizeNumber) Export 
	...
	If ChromaticityNumber = 0 Then
		Chromaticity = Enums.ImageColorMode.Monochrome;
	ElsIf ChromaticityNumber = 1 Then
		Chromaticity = Enums.ImageColorMode.GrayGradations;
	ElsIf ChromaticityNumber = 2 Then
		Chromaticity = Enums.ImageColorMode.Color;
	EndIf;
...
Function EventLogMonitorForExchange() 
	...
	Return NStr("en = 'Files.Cannot send the file during data exchange'", CommonUseClientServer.MainLanguageCode());
