...
Procedure BeforeExit(Cancel, Warnings) Export
	...
	UserWarning.HyperlinkText = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = 'Open list of edited files (%1)'"),
		Response.CountEmployedFiles);
	UserWarning.ExplanationText = NStr("en = 'You have locked files for editing'");
...
Procedure TransferAllFilesInVolumes() Export
	...
	OpenForm("DataProcessor.MoveFilesToVolume.Form");
...
Procedure ReleaseFileWithoutQuestion(FileData, UUID = Undefined) Export
	...
	ShowUserNotification(NStr("en = 'File is released'"),
...
#Region EndOfFileEditingAndPlacingOnServer
...
#Region SelectFileAndCreateNewVersionBasedOnIt
...
Procedure UpdateFromFileOnDrive(ResultHandler, FileData, FormID) Export
		...
		#If Not WebClient Then
			If StandardSubsystemsClientReUse.ClientWorkParameters().ThisIsBasicConfigurationVersion Then
				CommonUseClientServer.MessageToUser(
					NStr("en = 'This command is not supported in base version.'"));
				ReturnResult(ResultHandler, False);
				Return;
			EndIf;
		#EndIf
	...
	If CommonUseClient.SubsystemExists("StandardSubsystems.DigitalSignature") Then
		...
		If ModuleDigitalSignatureClientServer.CommonSettings().UseEncryption Then
			ExtensionForEncryptedFiles =
				ModuleDigitalSignatureClientServer.PersonalSettingsTip(
					).ExtensionForEncryptedFiles;
		EndIf;
	EndIf;
...
#Region MarkingTheFileAsLockedForEditing
...
#Region MarkingTheFilesAsLockedForEditing
...
Procedure LockFilesByReferencesAfterInstallingExtension(ExtensionIsSet, ExecuteParameters) Export
	...
	ShowUserNotification(
		...
		,
		StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Files (%1 of %2) are locked for editing.'"),
			LockedCount,
...
#Region OpeningTheFileForEditingByReference
...
#Region OpeningTheFileForEditing
...
Procedure UnlockFilesByReferencesAfterAnswerToQuestionCancelEditing(Response, ExecuteParameters) Export
	...
	ShowUserNotification(
		...
		StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'File editing is canceled (%1 of %2).'"),
...
#Region UnlockingTheFileWithoutUpdate
...
Procedure SaveFileChangesWithExtensionAfterCheckingEncrypted(NOTSpecified, ExecuteParameters) Export
	...
	If ExecuteParameters.ShowAlert Then
		If VersionUpdated Then
			ShowUserNotification(
				NStr("en = 'New version is saved'"),
				ExecuteParameters.FileData.URL,
				ExecuteParameters.FileData.FullDescrOfVersion,
				PictureLib.Information32);
		Else
			ShowUserNotification(
				NStr("en = 'New version is not saved'"),,
				NStr("en = 'File is not changed'"),
				PictureLib.Information32);
...
Procedure ReleaseFiles(ResultHandler, ObjectsRef) Export
	...
	If ListImpossibleRelease.Count() > 0 Then
		QuestionWithListDialog(
			...
			NStr("en = 'Unlock the files left?'"),
			NStr("en = 'The following errors occurred while trying to unlock the files:'"),
...
#Region OpeningQuestionDialogWithTheListOfFilesOrInformation
...
#Region FileDeletion
// Before deletion "Read only" attribute is removed.
...
Procedure DeleteFile(ResultHandler, FullFileName, AskQuestion = Undefined, QuestionHeader = Undefined) Export
	...
	If AskQuestion = Undefined Then
		PersonalSettingsTip = FileFunctionsServiceClientServer.PersonalFileOperationsSettings();
		AskQuestion = PersonalSettingsTip.ConfirmWhenDeletingFromLocalFilesCache;
	EndIf;
...
#Region ReceivingTheFileFromTheStorageInWorkingDirectory
...
#Region ReceivingFileFromTheApplicationOnDisk
...
#Region OpeningWindowsExplorerWithPositioningOnTheFile
...
#Region DeletingTheFileFromDiskAndInformationRegister
...
#Region ClearingWorkingDirectory
...
Procedure FreePlaceInWorkingDirectory(ResultHandler, VersionAttributes)
	...
	#If WebClient Then
		...
		ReturnResultAfterShowWarning(
			ResultHandler,
			NStr("en = 'Working directory cleanup is not supported in web client.'"),
			Undefined);
		Return;
	#EndIf
...
Procedure ClearWorkingDirectory(ResultHandler, FilesSizeInWorkingDirectory, AddedFileSize, ToClearAll) Export
	...
	#If WebClient Then
		ReturnResultAfterShowWarning(ResultHandler, NStr("en = 'Working directory cleanup is not supported in web client.'"), Undefined);
		Return;
	#EndIf
...
Procedure ClearWorkingDirectoryStart(ExecuteParameters)
	...
	PersonalSettingsTip = FileFunctionsServiceClientServer.PersonalFileOperationsSettings();
	MaxSize = PersonalSettingsTip.LocalFilesCacheMaximumSize;
...
Procedure ClearWorkingDirectoryCycleHandler(ExecuteParameters)
	...
	If ExecuteParameters.StepNumber = 1 Then
		If Not ExecuteParameters.YesForAll
			...
			Buttons.Add("YesForAll",  NStr("en = 'Yes to all'"));
			Buttons.Add("No",        NStr("en = 'No'"));
			Buttons.Add("NoForAll", NStr("en = 'No for all'"));
	...
	If ExecuteParameters.StepNumber = 2 Then
		...
		FileOnDrive.SetReadOnly(False);
		If ExecuteParameters.ToClearAll = False Then
			QuestionHeader = NStr("en = 'Clearing the working directory while adding a file.'");
		Else
			QuestionHeader = NStr("en = 'Clearing the working directory.'");
		EndIf;
...
#Region ReceivingTheFileFromServerAndRegisteringInLocalCache
			...
			If StrLen(ExecuteParameters.FullFileName) > ExecuteParameters.FullPathMaxLength Then
				...
				If StrLen(ExecuteParameters.FileName) > FileNameMaxLength Then
					...
					MessageText = MessageText + Chars.CR + Chars.CR
						+ NStr("en = 'Replace the file name with a shorter one.'");
...
Procedure GetFromServerAndRegisterInLocalFilesCacheOfferToSelectDirectory(Response, ExecuteParameters) Export
	...
	QuestionText = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = 'File path length exceeds %1 characters: %2 Choose another working directory?'"),
		ExecuteParameters.FullPathMaxLength,
		ExecuteParameters.FullFileName);
...
#Region ReceivingTheFileFromServerAndRegisteringInWorkingDirectory
...
#Region CheckMaximumLengthOfWorkingDirectoryWithReplacementAndTransferOfFiles
...
Procedure CheckFullPathMaxLengthInWorkingDirectoryStart(ExecuteParameters)
	...
	If StrLen(ExecuteParameters.FileData.OwnerWorkingDirectory) > ExecuteParameters.FullPathMaxLength - 5 Then
		MessageText = MessageText + Chars.CR + Chars.CR
			+ NStr("en = 'Change folder names or move this folder to another one.'");
		ReturnResultAfterShowWarning(ExecuteParameters.ResultHandler, MessageText, False);
		Return;
	EndIf;
...
#Region CopyingTheContentsFromOneDirectoryToAnother
...
#Region TransferOfWorkingDirectoryContentToANewOne
...
#Region DeletingTheArrayOfPathsOfTransferredFoldersAndFiles
...
Procedure FilesImportAfterSizesCheck(Result, ExecuteParameters) Export
	...
	If ExecuteParameters.QuantitySummary = 0 Then
		If ExecuteParameters.ImportMode Then
			ReturnResult(ExecuteParameters.ResultHandler, Undefined);
		Else
			ReturnResultAfterShowWarning(ExecuteParameters.ResultHandler, NStr("en = 'There are no files to add'"), Undefined);
		EndIf;
		Return;
	EndIf;
...
Procedure FilesImportAfterCycleContinued(ExecuteParameters)
	...
	If ExecuteParameters.AllFilesStructuresArray.Count() > 1 Then
		...
		StatusText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'File are imported. Imported files: %1'"), String(ExecuteParameters.AllFilesStructuresArray.Count()) );
			
		If ExecuteParameters.ImportMode Then
			StatusText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Files have been imported. Files imported: %1'"), String(ExecuteParameters.AllFilesStructuresArray.Count()) );
		EndIf;
	...
	If ExecuteParameters.FilenamesWithErrorsArray.Count() <> 0 Then
		...
		If ExecuteParameters.ImportMode Then
			Parameters.Insert("Title", NStr("en = 'Report on file import'"));
		EndIf;
...
Procedure SaveAsWithExtensionAfterDecryption(DataDescription, ExecuteParameters) Export
	...
	Status(NStr("en = 'File successfully saved'"), , FullFileName);
...
#Region DisplaysTheReminderBeforeFilePlacingIfItIsConfigured
...
Procedure ShowReminderBeforePutFile(ResultHandler)
	...
	PersonalSettingsTip = FileFunctionsServiceClientServer.PersonalFileOperationsSettings();
	If PersonalSettingsTip.ShowFileEditTips = True Then
		If Not FileFunctionsServiceClient.FileOperationsExtensionConnected() Then
...
Procedure CheckFilesLimitSize(ResultHandler, CheckParameters)
	...
	If ArrayTooLargeFiles.Count() <> 0 Then 
		...
		For Each File In ArrayTooLargeFiles Do
			...
			StringText = StringFunctionsClientServer.SubstituteParametersInString(NStr("en = '%1 (%2 MB)'"), String(File), String(FileSizeInMb));
			BigFiles.Add(StringText);
		EndDo;
		
		Parameters.Insert("BigFiles", BigFiles);
		Parameters.Insert("ImportMode", CheckParameters.ImportMode);
		Parameters.Insert("Title", NStr("en = 'File import warning'"));
...
#Region DisplaysTheInformationThatTheFileWasNotChanged
...
Procedure ShowInformationFileWasNotChanged(ResultHandler)
	...
	PersonalSettingsTip = FileFunctionsServiceClientServer.PersonalFileOperationsSettings();
	If PersonalSettingsTip.ShowFileNotChangedMessage = True Then
...
#Region ImportsAnEditedFileIntoTheApplication
...
#Region CapturesAFileOrSeveralFilesAndSendsANotification
...
#Region UnlocksTheFileAndSendsAnAlert
...
#Region OpensTheFileAndSendsNotification
...
#Region ImportsTheFileIntoTheApplicationAndSendsAnAlert
...
Procedure Encrypt(ResultHandler, FileData, UUID) Export
	...
	If Not ExecuteParameters.FileData.IsEditing.IsEmpty() Then
		WarningText = NStr("en = 'Cannot encrypt locked file.'");
		ReturnResultAfterShowWarning(ExecuteParameters.ResultHandler, WarningText, ExecuteParameters);
		Return;
	EndIf;
...
Procedure AddFromFileSystemWithoutExtensionAfterFileImport(Placed, Address, SelectedFileName, ExecuteParameters) Export
	...
	If PathStrings.Count() < 2 Then
		QuestionText = NStr("en = 'Specify file with extension.'");
		Buttons = New ValueList;
...
#Region SendsTheNotificationAboutTheEndOfFileEncryptionOrDecryption
...
Procedure SignFile(FilesArray, FormID, EndProcessor) Export
	...
	DataDescription.Insert("Operation",            NStr("en = 'File signing'"));
	DataDescription.Insert("DataTitle",     NStr("en = 'File'"));
	DataDescription.Insert("DataSet",         DataSet);
	DataDescription.Insert("SetPresentation", NStr("en = 'Files (%1)'"));
...
#Region ImportsTheStructureOfFilesAndDirectoriesFromDiskIntoTheApplication
...
Procedure FilesImportRecursivelyWithoutDialogs(Val Owner, Val SelectedFiles, Val ExecuteParameters, Val AskQuestionFolderAlreadyExists)
	...
	For Each SelectedFile In SelectedFiles Do
		...
		If Not SelectedFile.Exist() Then
			...
			Record.Insert("Error", NStr("en = 'File is not present on disk.'"));
		...
		Try
			...
			If ExecuteParameters.ImportMode Then
				If FileOperationsServiceServerCall.IsFileWithSuchName(BaseName, Owner) Then
					Record = New Structure;
					Record.Insert("FileName", SelectedFile.FullName);
					Record.Insert("Error", NStr("en = 'A file with this name already exists in the file storage.'"));
					ExecuteParameters.FilenamesWithErrorsArray.Add(Record);
					Continue;
				EndIf;
			EndIf;
...
Procedure OpenFileByApplication(FileData, FileNameToOpen, ResultHandler = Undefined) Export
	...
	If ExtensionAttached Then
		...
		If TextFilesOpeningMethod = PredefinedValue("Enum.FilesAssociation.InEmbeddedEditor") Then
	...
	Else
		...
		If CountTotal > 0 Then 
			...
			FullDetails = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Files (%1 pcs) are transferred to %2'"),
				CountTotal,
				FileOwner);
			
			ShowUserNotification(
				NStr("en = 'Files are moved'"),
				,
				FullDetails,
				PictureLib.Information32);
...
Function CheckLockedFilesOnExit()
	...
	ShowLockedFilesOnExit = StandardSubsystemsClient.ClientParameter("PersonalFileOperationsSettings").ShowLockedFilesOnExit;
	...
	CountEmployedFiles = StandardSubsystemsClient.ClientParameter("CountEmployedFiles");
	...
	ApplicationWarningFormParameters.Insert("MessageTitle",		NStr("en = 'You have locked the following files for editing:'"));
...
#If Not WebClient Then
	...
	For Each SelectedFile In FilesArray Do
		...
		// If you don't find on disk - then minimum date will be the oldest - and it will be deleted from working directory
		// during clearing of the oldest files.
...
#Region ProceduresAndFunctionForWorkWithScanner
...
Procedure SetComponent(ResultHandler) Export
	...
	If InitializeComponent() Then
		Status(NStr("en = 'Scan component already installed.'"));
