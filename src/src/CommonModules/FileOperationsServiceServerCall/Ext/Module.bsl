...
Function SaveChangesAndReleaseFile(FileData, FileInformation,
	...
	If Not FileDataCurrent.CurrentUserIsEditing Then
		Raise NStr("en = 'File is not used by the current user'");
	EndIf;
...
//                                                   "A1/Order.doc "; it is specified if
//                                                   DoNotChangeRecordInWorkingDirectory = False.
//   FullPathToFile                    - String    - path on client in the working
//                                                   directory; it is specified if DoNotChangeRecordInWorkingDirectory =
//                                                   False.
...
// It receives file data and makes a loan (checkout) - to save client server calls we moved GetFileData and LockFile to
...
// It receives file data for opening and making loan (checkout) - to save the client server calls we moved
// FileDataForOpening and LockFile to one function. Parameters:
//  FileRef     - CatalogRef.Files - file.
//  FileData    - Structure - structure with file data.
//  ErrorString - String used to return the error reason in case of failure
//                 (for example, the file is busy by the other user).
...
// Returns:
//   Structure - with properties:
//     Data recovery executed successfully     - Boolean    - True if the version creation is completed (and the file is
//     binary changed).
//     * FileData - Structure - structure with file data.
//
...
Function FilesImportGenerateReport(FilenamesWithErrorsArray) Export
	...
	HeaderArea.Parameters.Description = NStr("en = 'Cannot import the following files:'");
...
Function DoubleClickAction()
	...
	If HowToOpen = Undefined
	 OR HowToOpen = Enums.FileActionOnDoubleClick.EmptyRef() Then
		
		HowToOpen = Enums.FileActionOnDoubleClick.ToOpenFile;
		
		CommonUse.CommonSettingsStorageSave(
			"OpenFileSettings", "DoubleClickAction", HowToOpen);
	EndIf;
	
	If HowToOpen = Enums.FileActionOnDoubleClick.ToOpenFile Then
		Return "ToOpenFile";
	Else
		Return "ToOpenCard";
	EndIf;
...
// The function changes the Parent attribute for the Catalog type objects.FileFolders, it returns True in case of
// success, for LoopFound variable it returns True if one of the folders is moved to its child folder.
//
// Parameters:
//  RefsToFilesArray - Array - file array.
//  NewParent  - AnyRef - new owner of the file.
	...
	Try
		...
		If Encrypt Then
			For Each ThumbprintStructure In ThumbprintArray Do
				...
				NewRow.ElectronicSignature = New ValueStorage(ThumbprintStructure.ElectronicSignature);
			EndDo;
		Else
...
Function GetArrayOfEncryptionCertificates(Refs) Export
	...
	|	EncryptionCertificates.ElectronicSignature AS ElectronicSignature
	|FROM
	...
	While QuerySelection.Next() Do
		ThumbprintStructure = New Structure("Thumbprint, Presentation, ElectronicSignature",
			QuerySelection.Imprint, QuerySelection.Presentation, QuerySelection.ElectronicSignature.Get());
...
//  PersonalSettingsTip - Structure - different settings for different users.
//  
Procedure AddFileOperationsSettings(CommonSettings, PersonalSettingsTip) Export
	
	SetPrivilegedMode(True);
	
	PersonalSettingsTip.Insert("DoubleClickAction", DoubleClickAction());
	PersonalSettingsTip.Insert("FileVersionComparisonMethod",  FileVersionComparisonMethod());
	
	PersonalSettingsTip.Insert("PromptForEditModeOnOpenFile",
		PromptForEditModeOnOpenFile());
	
	PersonalSettingsTip.Insert("InfobaseUserWithFullAccess",
	...
	PersonalSettingsTip.Insert("ShowLockedFilesOnExit",
		ShowLockedFilesOnExit);
