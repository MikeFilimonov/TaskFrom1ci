...
Procedure FullTextSearchIndexMerge() Export
	...
	UpdateIndex(NStr("en = 'Merging full-text search index.'"), True);
...
Procedure AtFillingToDoList(ToDoList) Export
	...
	If Not CommonUse.SubsystemExists("StandardSubsystems.ApplicationSettings")
		...
		Or ModuleToDoListService.WorkDisabled("FullTextSearch") Then
		Return;
	EndIf;
	...
	IDFullTextSearch = "FullTextSearch" + StrReplace(Section.FullName(), ".", "");
	Work = ToDoList.Add();
	Work.ID  = IDFullTextSearch;
	Work.ThereIsWork	= (DaysSinceLastUpdate >= 1 AND Not FullTextSearch.IndexTrue());
	Work.Presentation	= NStr("en = 'Full-text search index is out of date'");
...
Procedure UpdateIndex(ProcedureRepresentation, AllowMerge = False, ByPortions = False)
	...
	LogRecord(Undefined, NStr("en = 'Start the ""%1"" procedure.'"), , ProcedureRepresentation);
	
	Try
		FullTextSearch.UpdateIndex(AllowMerge, ByPortions);
		LogRecord(Undefined, NStr("en = 'Procedure ""%1"" is completed successfully.'"), , ProcedureRepresentation);
	Except
		LogRecord(Undefined, NStr("en = 'An error occurred when executing procedure ""%1"":'"), ErrorInfo(), ProcedureRepresentation);
	EndTry;
