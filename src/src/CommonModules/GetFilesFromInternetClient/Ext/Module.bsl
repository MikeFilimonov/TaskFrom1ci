...
//   RecordError   - Boolean - The flag showing that it is necessary to write an error to the event log when receiving a
//                             file.
//
// Returns:
...
Function ExportFileAtClient(Val URL, Val ReceivingParameters = Undefined, Val RecordError = True) Export
	...
	#If WebClient Then
		Raise NStr("en = 'Downloading files to the client is not available on the web client.'");
	#EndIf
