...
Procedure OnStart(Parameters) Export
	...
	If Not WorkParameters.Property("BackupInfobase", FixedParametersOfInfobaseBackup) Then
		Return;
	EndIf;
	...
	If FixedParametersOfInfobaseBackup.RecoverHasBeenPerformed Then
		NotificationText = NStr("en = 'Data is restored successfully.'");
	...
	If CommonUseClient.SubsystemExists("StandardSubsystems.ToDoList") Then
...
// Defines the list of warnings.
//
// Parameters:
//  Warnings - Array - you can add items of the Structure type to the array, 
//                     its properties see  StandardSubsystemsClient.WarningOnWorkEnd.
//
Procedure BeforeExit(Cancel, Warnings) Export
	...
	If NOT CommonUseClientServer.IsWindowsClient() Then
		Return;
	EndIf;
	
	ClientWorkParameters = StandardSubsystemsClient.ClientParameter();
	
	If ClientWorkParameters.DataSeparationEnabled Or Not ClientWorkParameters.FileInfobase Then
		Return;
	EndIf;
	
	If NOT ClientWorkParameters.InfobaseBackupOnComplete.NotificationRolesAvailability
		OR NOT ClientWorkParameters.InfobaseBackupOnComplete.ExecuteOnWorkCompletion Then
			Return;
	EndIf;
		
	WarningParameters = StandardSubsystemsClient.AlertOnEndWork();
	WarningParameters.FlagText = NStr("en = 'Back up'");
	WarningParameters.Priority = 50;
	WarningParameters.ExplanationText = Nstr("en = 'The data was not backed up.'");
	
	ActionIfMarked = WarningParameters.ActionIfMarked;
	ActionIfMarked.Form = "DataProcessor.BackupInfobase.Form.DataBackup";
...
Function BackupSettingsFormName() Export
	...
	If StandardSubsystemsClientReUse.ClientWorkParameters().FileInfobase Then
		...
		Return "DataProcessor.BackupSetup.Form.Form";
		
	Else
		
		Return "DataProcessor.BackupSetup.Form.ReminderClientServer";
...
Function OpenBackupForm(Val Owner = Undefined, Val FormOpenParameters = Undefined) Export
	...
	OpenForm("DataProcessor.BackupInfobase.Form.DataBackup",
...
Procedure DeleteBackupsBySetting() Export
	...
	FixedParametersOfInfobaseBackup = StandardSubsystemsClientReUse.ClientWorkParameters().BackupInfobase;
	...
	If DeletionParameters.RestrictionType <> "StoreAll" AND StoringDirectory <> Undefined Then
		...
		Except
			...
			EventLogMonitorClient.AddMessageForEventLogMonitor(EventLogMonitorEvent(), "Error",
				NStr("en = 'Cannot clear a directory with backups.'") + Chars.LF 
...
Procedure CheckInfobaseBackup(Parameters)
	...
	Else
		...
		ShowUserNotification(NStr("en = 'Backup'"),
			...
			NStr("en = 'Backup was successfully performed'"), PictureLib.Information32);
...
Procedure NotifyUserAboutBackingUp(VariantNotifications) Export
	...
	If VariantNotifications = "Overdue" Then
		...
		ShowUserNotification(NStr("en = 'Backup'"),
			"e1cib/app/DataProcessor.BackupInfobase", ExplanationText, PictureLib.Warning32);
	...
	ElsIf VariantNotifications = "YetNotConfigured" Then
		...
		ExplanationText = NStr("en = 'We recommend that you configure backup for the infobase.'"); 
		ShowUserNotification(NStr("en = 'Backup'"),
...
Function EventLogMonitorEvent() Export
	...
	Return NStr("en = 'Infobase backup'", CommonUseClientServer.MainLanguageCode());
