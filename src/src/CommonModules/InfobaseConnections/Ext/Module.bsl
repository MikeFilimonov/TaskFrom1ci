...
#Region LockAndEndOfConnectionsWithIB
...
Procedure SetDataAreaSessionLock(Parameters, Val LocalTime = True, Val DataArea = -1) Export
	...
	If CommonUseReUse.CanUseSeparatedData() Then
		...
		ElsIf DataArea <> CommonUse.SessionSeparatorValue() Then
			Raise NStr("en = 'Running a session with used separator values, you cannot set a session lock for a data area that is different from the one used in the session.'");
		EndIf;
		
	Else
		
		If DataArea = -1 Then
			Raise NStr("en = 'Cannot set lock for data area sessions as data area was not specified.'");
		EndIf;
...
Procedure OnAddOfficeEvent(ClientEvents, ServerEvents) Export
	...
	//    server agent via COM-connection or administer server with the query of connection parameters to the cluster of
	//    the current user). May be set to the False value inside the event processor, in
	...
	//    (you should pass DialogReturnCode.OK  as a value of the Result parameter if the session is completed
	//    successfully). Parameter can be omitted - in this case do not process the alert.
	//
	// Syntax:
...
Procedure WhenCompletingTablesOfParametersOfIB(Val ParameterTable) Export
	...
	If CommonUse.SubsystemExists("StandardSubsystems.SaaS") Then
		...
		ModuleSaaSOperations.AddConstantToInfobaseParameterTable(ParameterTable, "WarningMessageOnDatabaseUpdate");
	EndIf;
...
Procedure OnAddParametersJobsClientLogicStandardSubsystemsRunning(Parameters) Export
	...
	If ValueIsFilled(CurrentMode.End) Then
		LockPeriod = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'for the period from %1 to %2'"),
	...
	If ValueIsFilled(CurrentMode.Message) Then
		LockReason = NStr("en = 'due to:'") + Chars.LF + CurrentMode.Message;
	Else
		LockReason = NStr("en = 'to conduct scheduled operations'");
	EndIf;
...
Procedure AtFillingToDoList(ToDoList) Export
	...
	If Not AccessRight("DataAdministration", Metadata)
		Or ModuleToDoListService.WorkDisabled("SessionsLock") Then
		Return;
	EndIf;
	...
	Sections = ModuleToDoListServer.SectionsForObject(Metadata.DataProcessors.UserWorkBlocking.FullName());
	...
	If LockParameters.Use Then
		If CurrentSessionDate < LockParameters.Begin Then
			If LockParameters.End <> Date(1, 1, 1) Then
				Message = StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'Scheduled from %1 to %2'"), 
					Format(LockParameters.Begin, "DLF=DT"), Format(LockParameters.End, "DLF=DT"));
			Else
				Message = StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'Scheduled from %1'"), 
		...
		ElsIf LockParameters.End <> Date(1, 1, 1) AND CurrentSessionDate > LockParameters.End AND LockParameters.Begin <> Date(1, 1, 1) Then
			Importance = False;
			Message = StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'Not valid (expired %1)'"), 
				Format(LockParameters.End, "DLF=DT"));
		Else
			If LockParameters.End <> Date(1, 1, 1) Then
				Message = StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'from %1 to %2'"), 
					Format(LockParameters.Begin, "DLF=DT"), Format(LockParameters.End, "DLF=DT"));
			Else
	...
	For Each Section In Sections Do
		...
		Work.Presentation  = NStr("en = 'User operation locking'");
	...
	Else
		MessageText = MessageText +
		    NStr("en = '%1
		         ...
		         |ENTERPRISE %2 /CAllowUsersWork /UC%3'");
	EndIf;
...
Function EnabledSessionsMessage() Export
	...
	Message = NStr("en = 'Cannot disable sessions:'");
