...
Function ProcessStartParameters(Val LaunchParameterValue, Val LaunchParameters) Export
	...
	If LaunchParameterValue = Upper("AllowUsersWork") Then
		...
		If Not InfobaseConnectionsServerCall.AllowUsersWork() Then
			MessageText = NStr("en = 'Launch parameter AllowUsersWork is not processed. You are not authorized to administer the infobase.'");
	...
	// connecting to servers cluster in client-server option of system deployment. They should be passed if the current
	// user is not IB administrator.For the usage, see the EndUsersWork() procedure.
	ElsIf LaunchParameterValue = Upper("TerminateUserSessions") Then
		...
		If Not InfobaseConnectionsServerCall.SetConnectionLock() Then
			MessageText = NStr("en = 'The EndUsersWork launch parameter is not processed. You are not authorized to administer the infobase.'");
			ShowMessageBox(,MessageText);
			Return False;
		EndIf;
...
Procedure TerminateThisSession(ShowQuestion = True) Export
	...
	MessageText = NStr("en = 'User operation in the application is prohibited. Close this session?'");
	Title = NStr("en = 'End the current session'");
...
#Region HandlersOfTheBaseFunctionalitySubsystemEvents
