...
Procedure TerminateUserSessions() Export
	...
	If CurrentMoment < BeginTimeLock Then
		MessageText = NStr("en = 'Lock of the user operation is planned on %1.'");
		MessageText = StringFunctionsClientServer.SubstituteParametersInString(
			MessageText, BeginTimeLock);
		ShowUserNotification(NStr("en = 'User work completion'"), 
	...
	If Result Then
		...
		ShowUserNotification(NStr("en = 'User work completion'"), 
			"e1cib/app/DataProcessor.UserWorkBlocking", 
			NStr("en = 'Sessions are successfully closed'"), PictureLib.Information32);
	...
	Else
		...
		ShowUserNotification(NStr("en = 'User work completion'"), 
			"e1cib/app/DataProcessor.UserWorkBlocking", 
			NStr("en = 'Sessions are not completed. For more information, see event log.'"), PictureLib.Warning32);
