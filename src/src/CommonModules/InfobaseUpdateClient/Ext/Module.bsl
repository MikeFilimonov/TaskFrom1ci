...
Procedure StartInformationBaseUpdate(Parameters, ContinuationProcessor) Export
	...
	Else
		FormName = "DataProcessor.UpdateResults.Form.InfobaseUpdateProcessIndication";
...
// the user did not disable the display - Open the form ReleaseNotes.
//
Procedure ShowReleaseNotes()
	...
	If ClientWorkParameters.ShowReleaseNotes Then
		...
		OpenForm("CommonForm.ReleaseNotes", FormParameters);
	EndIf;
...
Procedure NotifyPendingHandlersNotImplemented() Export
	...
	ShowUserNotification(
		NStr("en = 'Application operations are temporarily restricted'"),
		ProcessorsURL(),
		NStr("en = 'Update to a new version is incomplete'"),
...
Function ProcessorsURL()
	Return "e1cib/app/DataProcessor.UpdateResults";
EndFunction
