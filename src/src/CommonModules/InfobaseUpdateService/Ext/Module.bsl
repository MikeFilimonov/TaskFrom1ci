...
Procedure OnAddOfficeEvent(ClientEvents, ServerEvents) Export
	...
	// EndDo;
	//
	//   PutReleaseNotes - Boolean (return value)- if you set True, then display the form with updates description.
...
Procedure SessionParametersSetting(Val ParameterName, SpecifiedParameters) Export
	...
	SessionParameters.InfobaseUpdateIsExecute = UpdateResults.InfobaseUpdateRequired();
...
// Returns a text of the lock reason if the IB update is required and the current user does not have sufficient rights,
// otherwise, returns an empty string.
//  
// Parameters:
//  ForPrivilegedMode - Boolean - if you select False, then during the checking of rights of the current user the
//                                presence of the privileged mode will be ignored.
//  
// Returns:
...
Function InfobaseLockedForUpdate(ForPrivilegedMode = True) Export
	...
	If Not UpdateResults.InfobaseUpdateRequired()
...
//                 The parameter is required to optimize the client launch mode not to update the application parameters
//                 twice. During an external call, for example, in the session
//                 of an external connection, work parameters should be updated to continue updating the IB.
//    *Restart             - Boolean    - (return value) restart is required, in some cases OnApplicationLaunch, for
//                                  example, when you return to the data base configuration of the RIB subnode, see
//                                  general module DataExchangeServer procedure. SynchronizeWhenNoInfobaseUpdate.
//    *SetIBLock - Structure - structure with properties see IBLock().
//    *InBackground                     - Boolean    - if the infobase update is executed in the background, then you
//                                                     need to select True, otherwise, False.
// 
// Returns:
...
Function RunInfobaseUpdate(UpdateParameters) Export
	...
	If Not AreRightsForInfobaseUpdate() Then
		Message = NStr("en = 'Insufficient rights to update the application version.'");
		WriteError(Message);
		Raise Message;
	EndIf;
	...
	Else 
		Message = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Initial data population up to version ""%1"" is in progress.'"),
			MetadataVersion);
	EndIf;
	...
	Message = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = 'Infobase is successfully updated to version ""%1"".'"), MetadataVersion);
	WriteInformation(Message);
	
	PutReleaseNotes = (DataUpdateMode <> "InitialFilling");
	...
	ExecuteHandlersAfterInfobaseUpdate(
		IterationsUpdate,
		Constants.DetailedEventLogOnInfobaseUpdate.Get(),
		PutReleaseNotes,
		OperationalUpdate);
	...
	DefineUpdatesDescriptionOutput(PutReleaseNotes);
...
//
Function ShowReleaseNotes() Export
	
	DataAboutUpdate = DataOnUpdatingInformationBase();
	If DataAboutUpdate.PutReleaseNotes = False Then
		Return False;
	EndIf;
	...
	OutputReleaseNotesForAdministrator = CommonUse.CommonSettingsStorageImport("UpdateInfobase", "OutputReleaseNotesForAdministrator",,, UserName());
	If OutputReleaseNotesForAdministrator = True Then
		Return True;
	EndIf;
...
#Region HandlersOfConditionalCallsIntoTheseSubsystems
...
Function IterationsUpdate() Export
	...
	For Each SubsystemName In SubsystemDescriptions.Order Do
		...
		IterationUpdate = IterationUpdate(SubsystemDescription.Name, SubsystemDescription.Version, 
			UpdateResults.NewUpdateHandlersTable(), SubsystemDescription.Name = MainConfigurationName);
...
Function RunUpdateIteration(Val IterationUpdate, Val Parameters) Export
	...
	If CurrentIBVersion <> "0.0.0.0"
		...
		If HandlersToExecute = Undefined Then
			If IterationUpdate.ThisMainConfiguration Then 
				MessagePattern = NStr("en = 'Update plan of configuration %1 from version %2 to version %3 is not found'");
			Else
				MessagePattern = NStr("en = 'Update plan of library %1 from version %2 to version %3 is not found'");
			EndIf;
	...
	RequiredSeparateHandlers = UpdateResults.NewUpdateHandlersTable();
	OriginalVersionOfDB = CurrentIBVersion;
	ToWriteInJournal = Constants.DetailedEventLogOnInfobaseUpdate.Get();
	
	For Each Version In HandlersToExecute.Rows Do
		
		If Version.Version = "*" Then
			Message = NStr("en = 'Required procedures of infobase update are in progress.'");
		Else
			NewInfobaseVersion = Version.Version;
			If CurrentIBVersion = "0.0.0.0" Then
				Message = NStr("en = 'Initial data population is in progress.'");
			ElsIf IterationUpdate.ThisMainConfiguration Then 
				Message = NStr("en = 'Updating the infobase from version %1 to version %2.'");
			Else
				Message = NStr("en = 'Updating data of library %3 from version %1 to version %2.'");
			EndIf;
			Message = StringFunctionsClientServer.SubstituteParametersInString(Message,
		...
		If Version.Version = "*" Then
			Message = NStr("en = 'Required procedures of infobase update are performed.'");
		Else
			If IterationUpdate.ThisMainConfiguration Then 
				Message = NStr("en = 'Infobase update from version %1 to version %2 is completed.'");
			Else
				Message = NStr("en = 'Data of library %3 is updated from version %1 to version %2.'");
			EndIf;
...
Procedure GoFromAnotherApplication()
	...
	If CommonUseReUse.DataSeparationEnabled() Then
		Raise NStr("en = 'When working in SaaS, transfer from another application is unexpected.'");
	EndIf;
...
#Region ProtocolTheUpdateProgress
...
Function EventLogMonitorEvent() Export
	...
	Return NStr("en = 'Infobase update'", CommonUseClientServer.MainLanguageCode());
...
//
Function DocumentReleaseNotes(Val Sections) Export
	
	DocumentReleaseNotes = New SpreadsheetDocument();
	If Sections.Count() = 0 Then
		Return DocumentReleaseNotes;
	EndIf;
	
	TemplateUpdatesDescriptionFull = Metadata.CommonTemplates.Find("ReleaseNotes");
	...
	For Each Version In Sections Do
		...
		DisplayLongDescChanges(Version, DocumentReleaseNotes, TemplateUpdatesDescriptionFull);
...
Procedure SetFlagDisplayDescriptionsForCurrentVersion(Val UserName = Undefined) Export
	...
	If UserName = Undefined AND Users.InfobaseUserWithFullAccess() Then
		...
		CommonUse.CommonSettingsStorageDelete("UpdateInfobase", "OutputReleaseNotesForAdministrator", UserName());
...
#Region MechanismOfADelayedUpdate
...
Procedure FormListDelayedUpdateHandlers(IterationsUpdate)
	...
	DataAboutUpdate.Insert("PutReleaseNotes", False);
...
Procedure WriteDataOnUpdatingInformationBase(Val DataAboutUpdate) Export
	...
	UpdateResults.WriteData(ManagerConstants);
...
// Adds parameters of the clients logic work during the system launch for a subsystem of the data exchange in the
// service model.
//
Procedure OnAddParametersJobsClientLogicStandardSubsystemsRunning(Parameters) Export
	...
	Parameters.Insert("ShowReleaseNotes", ShowReleaseNotes());
...
//    •  For the Inventory writing off  document a print form is added         - String - the complete path to the form
//    which you need to open at clicking the work hyperlink on the Current Work bar.
...
Procedure AtFillingToDoList(ToDoList) Export
	...
	If Not Users.InfobaseUserWithFullAccess(, True)
		Or ModuleToDoListService.WorkDisabled("PostponedUpdate") Then
		Return;
	EndIf;
	...
	Sections = ModuleToDoListServer.SectionsForObject(Metadata.DataProcessors.UpdateResults.FullName());
	...
	For Each Section In Sections Do
		...
		Work.Presentation = NStr("en = 'Application update is not completed'");
		Work.Form         = "DataProcessor.UpdateResults.Form.InfobaseDelayedUpdateProgressIndication";
		Work.Owner      = Section;
	EndDo;
...
Function DataUpdateModeInLocalWorkMode()
	...
	PackageExecutionResult = Query.Execute();
	If PackageExecutionResult.IsEmpty() Then
...
Procedure DisableUpdateHandlers(LibraryID, HandlersToExecute, MetadataVersion, HandlersExecutionProcess)
	...
	For Each DisabledHandler In SwitchableHandlers Do
		...
		ElsIf RunningHandler <> Undefined AND RunningHandler.Version <> "*"
			AND DisabledHandler.Version = MetadataVersion Then
			ErrorMessage = NStr("en = 'The handler of %1 update cannot be disabled as it is executed only when changing to version %2.'");
		...
		ElsIf RunningHandler = Undefined Then
			ErrorMessage = NStr("en = 'Disabled update handler %1 does not exist'");
			ErrorMessage = StringFunctionsClientServer.SubstituteParametersInString(ErrorMessage,
...
Procedure ExecuteHandlersAfterInfobaseUpdate(Val IterationsUpdate, Val ToWriteInJournal, PutReleaseNotes, Val OperationalUpdate)
...
Procedure ExecuteDeferredUpdate() Export
	// TODO: Get function from SL. Check how Catalogs.JobQueueTemplates element with name "DeferredInfobaseUpdate" should
	// be created.
EndProcedure
...
Procedure ValidateNestedTransaction(TransactionActiveOnExecutionStart, ProcessorsName)
	...
	EventName = EventLogMonitorEvent() + ". " + NStr("en = 'Executing handlers'", CommonUseClientServer.MainLanguageCode());
	...
	For Each Handler In IterationUpdate.Handlers Do
		...
		If IsBlankString(Handler.Version) Then
			...
			If Handler.InitialFilling <> True Then
				ErrorDescription = NStr("en = 'The Version or InitialFilling property is not filled out in the handler.'");
			EndIf;
			...
			Except
				...
				ErrorDescription = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'Handler has filled the Version property wrong: %1.
					     |Correct format, for example: 21.3.70.'"),
					Metadata.Version);
			EndTry;
		...
		If IterationUpdate.ThisMainConfiguration Then
			ErrorTitle = NStr("en = 'An error occurred in the property of the configuration update handler'");
		Else
			ErrorTitle = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'An error occurred in the property of library update handler %1 of version %2'"),
...
Function CountHandlersToCurrentVersion(IterationsUpdate)
	...
	Message = NStr("en = 'To update the application to a new version, the following handlers will be executed: %1'");
...
Procedure WriteInformationAboutUpdate(Handler, HandlersExecutionProcess, InBackground)
	...
	If Not CommonUseReUse.DataSeparationEnabled() Then
		Message = NStr("en = 'Update handler %1 is in progress (%2 of %3).'");
		Message = StringFunctionsClientServer.SubstituteParametersInString(
...
//
Procedure DisplayLongDescChanges(Val VersionNumber, DocumentReleaseNotes, TemplateUpdatesDescriptionFull)
	...
	DocumentReleaseNotes.Put(TemplateUpdatesDescriptionFull.GetArea("Header" + Number));
	DocumentReleaseNotes.StartRowGroup("Version" + Number);
	DocumentReleaseNotes.Put(TemplateUpdatesDescriptionFull.GetArea("Version" + Number));
	DocumentReleaseNotes.EndRowGroup();
	DocumentReleaseNotes.Put(TemplateUpdatesDescriptionFull.GetArea("Indent"));
...
// Gets a list of the versions from the ReleaseNotes
...
Procedure DefineUpdatesDescriptionOutput(PutReleaseNotes)
	...
	If PutReleaseNotes AND Not CommonUseReUse.DataSeparationEnabled() Then
		CommonUse.CommonSettingsStorageSave("UpdateInfobase", "OutputReleaseNotesForAdministrator", True, , UserName());
	EndIf;
	
	If CommonUseReUse.CanUseSeparatedData() Then
		InfobaseUpdateData = DataOnUpdatingInformationBase();
		InfobaseUpdateData.PutReleaseNotes = PutReleaseNotes;
...
#Region HelperProceduresAndFunctionsOfADeferredUpdate
	...
	For Each HandlerTreeLibrary In DataAboutUpdate.HandlerTree.Rows Do
		...
		For Each HandlerTreeVersion In HandlerTreeLibrary.Rows Do
			...
			For Each UpdateHandler In HandlerTreeVersion.Rows Do
				...
				Try
					MessageAboutRunningHandler = NStr("en = 'Updating ""%1"".'");
