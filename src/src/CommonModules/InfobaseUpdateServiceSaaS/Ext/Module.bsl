		...
		If LibraryID = Metadata.Name Then
			...
			While Selection.Next() Do
				...
				If Not Selection.Updated Then
					...
					CommentTemplate = NStr("en = 'Configuration version was updated before updating a version of library %1'");
					TextOfComment = StringFunctionsClientServer.SubstituteParametersInString(CommentTemplate, Selection.Name);
					WriteLogEvent(
						UpdateResults.EventLogMonitorEvent(),
						EventLogLevel.Error,
						,
						,
						TextOfComment);
				...
				If DescriptionUpdateYourPlan = Undefined Then
					...
					CommentTemplate = NStr("en = 'Update plan of library %1 is not found'");
					TextOfComment = StringFunctionsClientServer.SubstituteParametersInString(CommentTemplate, Selection.Name);
					WriteLogEvent(
						UpdateResults.EventLogMonitorEvent(),
						EventLogLevel.Error,
	...
	If CommonUseReUse.DataSeparationEnabled()
	   AND CommonUseReUse.CanUseSeparatedData() Then
		...
		If InfobaseUpdateService.NeedToDoUpdate(Metadata.Version, SharedDataVersion) Then
			...
			WriteLogEvent(UpdateResults.EventLogMonitorEvent(), EventLogLevel.Error,,, Message);
			Raise Message;
		EndIf;
	EndIf;
...
Procedure ScheduleDataAreaUpdate(Val LockAreas = True, Val LockMessage = "") Export
	...
	If IsBlankString(LockMessage) Then
		LockMessage = Constants.WarningMessageOnDatabaseUpdate.Get();
		If IsBlankString(LockMessage) Then
	...
	If RecordKey <> Undefined Then
		...
		Except
			WriteLogEvent(UpdateResults.EventLogMonitorEvent() + ". " 
				+ NStr("en = 'Update data area'", Metadata.DefaultLanguage.LanguageCode),
				EventLogLevel.Error,,,
				DetailErrorDescription(ErrorInfo()));
			Raise(NStr("en = 'An error occurred when updating the data area. Writing of data area versions is locked.'"));
		EndTry;
	EndIf;
	Return RecordKey;
