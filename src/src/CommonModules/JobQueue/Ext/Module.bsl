...
//   JobState - EnumRef.JobStatuses - Job state in the queue.
...
//   Pattern - CatalogRef.JobQueueTemplates - job template used
...
Function GetJobs(Val Filter) Export
	...
	If IsBlankString(QueryText) Then
		Raise NStr("en = 'Incorrect filter value. No catalog whose tasks meet the filter conditions was found.'");
	EndIf;
...
Function AddJob(JobParameters) Export
	...
	If Not JobParameters.Property("MethodName") Then
		Raise(NStr("en = 'Required parameter of the MethodName job is not specified'"));
	EndIf;
	...
	If JobParameters.Property("Key") AND ValueIsFilled(JobParameters.Key) Then
		...
		FilterDescription.Insert("Value", Enums.JobStatuses.Completed);
...
Procedure ChangeTask(ID, JobParameters) Export
	...
	Try
		...
		If Not CommonUse.RefExists(ID) Then
			MessagePattern = NStr("en = 'Job with ID %1 to change is not found. Data area: %2'");
...
//
// Returns:
//  CatalogRef.JobQueueTemplates - job template.
//
...
Procedure ValidateJobParameters(Parameters, Mode)
	...
	If TypeOf(Parameters) <> Type("Structure") Then
		MessagePattern = NStr("en = 'Invalid type of task parameters - %1 was passed'");
	...
	For Each KeyAndValue In Parameters Do
		...
		If ParameterDescription = Undefined 
			...
			MessagePattern = NStr("en = 'Invalid task parameter passed: %1.'");
			MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, 
				KeyAndValue.Key);
			Raise(MessageText);
		EndIf;
		...
		If Filter AND TypeOf(KeyAndValue.Value) = Type("Array") Then
			...
			For Each FilterDescription In KeyAndValue.Value Do
				If TypeOf(FilterDescription) <> Type("Structure") Then
					MessagePattern = NStr("en = 'Invalid type %1 was passed in filter description collection %2'");
					MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, 
				...
				If KindsCompare.Get(FilterDescription.ComparisonType) = Undefined Then
					MessagePattern = NStr("en = 'Invalid comparison kind in filter description in filter description collection %1'");
					MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, 
						KeyAndValue.Key);
					Raise(MessageText);
				EndIf;
	...
	If Parameters.Property("ScheduledStartTime")
		...
		MessagePattern = NStr("en = 'Invalid value %1 of task parameter %2 was passed'");
		MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, 
...
Procedure ValidateValueInAccordanceToDescriptionParameter(Val Value, Val ParameterDescription)
	...
	If Not ParameterDescription.Type.ContainsType(TypeOf(Value)) Then
		MessagePattern = NStr("en = 'Invalid type %1 of %2 task parameter was passed'");
		MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, 
...
Procedure ValidateRegistrationHandlerTasks(Val MethodName)
	...
	If JobQueueServiceReUse.AccordanceMethodNamesToAliases().Get(Upper(MethodName)) = Undefined Then
		MessagePattern = NStr("en = 'Method alias %1 is not registered to be used as a job queue handler.'");
...
Function TasksDetailsOnIdIdentificator(Val ID)
	...
	If Not ValueIsFilled(ID) OR Not CommonUse.RefExists(ID) Then
		MessagePattern = NStr("en = 'Invalid value %1 of job parameter ID was passed'");
		MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, ID);
		Raise(MessageText);
	EndIf;
	...
	If Jobs.Count() = 0 Then
		MessagePattern = NStr("en = 'Queue job with ID %1 is not found'");
		MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, ID);
		Raise(MessageText);
	EndIf;
...
Function GetJobsWithSameKeyDuplicationErrorMessage() Export
	...
	Return NStr("en = 'Job duplication with the same value of the ""Key"" field is not allowed.'");
...
Function GetAnotherAreaDataGettingExceptionText()
	...
	Return NStr("en = 'Cannot access data from another data area in this session.'");
