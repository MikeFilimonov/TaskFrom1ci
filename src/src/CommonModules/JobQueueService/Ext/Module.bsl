...
Procedure OnEnableSeparationByDataAreas() Export
	...
	If Constants.MaximumActiveBackgroundJobRunTime.Get() = 0 Then
		Constants.MaximumActiveBackgroundJobRunTime.Set(600);
	EndIf;
	
	If Constants.MaximumNumberOfActiveBackgroundJobs.Get() = 0 Then
		Constants.MaximumNumberOfActiveBackgroundJobs.Set(1);
	EndIf;
...
Procedure OnDetermineCatalogsForDataImport(ImportedCatalogs) Export
	...
	// Import to catalog JobQueueTemplates is prohibited.
	TableRow = ImportedCatalogs.Find(Metadata.Catalogs.JobQueueTemplates.FullName(), "FullName");
	If TableRow <> Undefined Then 
		ImportedCatalogs.Delete(TableRow);
	EndIf;
...
Procedure JobProcessingPlanning() Export
	...
	For Each CatalogJobs In CatalogsJobs Do
		...
		If CommonUseReUse.IsSeparatedConfiguration() AND CommonUseReUse.IsSeparatedMetadataObject(CatalogJobs.CreateItem().Metadata().FullName(), CommonUseReUse.SupportDataSplitter()) Then
			...
			|	CASE
			|		WHEN Queue.Pattern = VALUE(Catalog.JobQueueTemplates.EmptyRef)
			|			THEN Queue.Schedule
			...
			|WHERE
			|	Queue.JobState IN (VALUE(Enum.JobStatuses.Running), VALUE(Enum.JobStatuses.Completed), VALUE(Enum.JobStatuses.NotPlanned), VALUE(Enum.JobStatuses.ExecutionError))"
...
Procedure ProcessJobQueue(BackgroundJobKey) Export
	...
	MaximumActiveBackgroundJobRunTime = 
		Constants.MaximumActiveBackgroundJobRunTime.Get();
	MaximumNumberOfActiveBackgroundJobs =
		Constants.MaximumNumberOfActiveBackgroundJobs.Get();
	...
	For Each CatalogJobs In CatalogsJobs Do
		...
		If CommonUseReUse.IsSeparatedConfiguration() AND CommonUse.IsSeparatedMetadataObject(CatalogJobs.CreateItem().Metadata().FullName(), CommonUseReUse.SupportDataSplitter()) Then
			...
			|	AND Queue.JobState = VALUE(Enum.JobStatuses.Planned)
	...
	Query.Text = QueryText;
	SelectionSizeText = Format(MaximumNumberOfActiveBackgroundJobs * 3, "NZ=; NG=");
	...
	While CanExecute Do 
		...
		While Selection.Next() Do 
			Try
				...
				If ValueIsFilled(Selection.Pattern)
					...
					MessagePattern = NStr("en = 'Job template of queue with ID %1 is not found'");
		...
		If ExecutionTime > MaximumActiveBackgroundJobRunTime Then
			CanExecute = False;
		EndIf;
	EndDo;
...
Procedure HandleError(Val Task, Val ErrorInfoExecutionJobs = Undefined) Export
	...
	Except
		...
		WriteLogEvent(
			NStr("en = 'Scheduled job queue. An error of the error handler occurred'", 
				CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Error,
			,
			,
	...
	If CommonUse.RefExists(Task)
		AND Task.JobState = Enums.JobStatuses.ErrorProcessingOnAbort Then
	
		TaskObject = Task.GetObject();
		TaskObject.JobState = Enums.JobStatuses.ExecutionError;
...
Procedure RemoveErrorDataProcessorsJobs(Val CallParameters, Val ErrorInfo = Undefined, Val RecursionCounter = 1) Export
	...
	Try
		...
		Task.JobState = Enums.JobStatuses.ExecutionError;
		CommonUse.AuxilaryDataWrite(Task);
		...
		Else
			...
			WriteLogEvent(
				NStr("en = 'Scheduled job queue. End jobs of error processing'",
					CommonUseClientServer.MainLanguageCode()),
...
Procedure ExecuteConfigurationMethod(MethodName, Parameters = Undefined)
	...
	If TransactionActive() Then
		...
		ErrorMessageText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'There are active transactions before the handler %1 can start.'"),
				MethodName);
			
		WriteLogEvent(NStr("en = 'Scheduled job queue.Fulfillment'", 
			CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Error,
			,
			,
	...
	Try
		...
		If TransactionActive() Then
			...
			MessagePattern = NStr("en = 'Transaction was not closed after the handler %1 finished'");
		...
		If Not(DelimiterSet) AND CommonUse.UseSessionSeparator() Then
			...
			ErrorMessageText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Session separation was not disabled after the handler %1 finished.'"),
				MethodName);
		...
		ElsIf DelimiterSet AND SeparatorValue <> SaaSOperations.SessionSeparatorValue() Then
			...
			ErrorMessageText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Session separator value was changed after the handler %1 finished.'"),
				MethodName);
...
Function GetErrorHandlerParameters(Val Task,Val ErrorInfoExecutionJobs = Undefined)
	...
	If Result.ProcessorExist Then
		...
		If ErrorInfoExecutionJobs = Undefined Then
			...
			ErrorInfoExecutionJobs = ErrorInformationAssistant(NStr("en = 'Job was completed with an unknown error, it could have been caused by the process failure.'"));
...
Function ActiveBackgroundJobCountToStart()
	...
	ActiveBackgroundJobCountToStart = 
		Constants.MaximumNumberOfActiveBackgroundJobs.Get() - ActiveBackgroundJobCount;
...
Procedure ExecuteHandlerTasks(Pattern, Alias, Parameters)
	...
	If MethodName = Undefined Then
		MessagePattern = NStr("en = 'Method %1 cannot be called via the job queue.'");
	...
	Try
		...
		If CommonUseReUse.IsSeparatedConfiguration() AND CommonUseReUse.IsSeparatedMetadataObject(Task.Metadata().FullName(), CommonUseReUse.SupportDataSplitter()) Then
			...
			If ValueIsFilled(Task.Pattern)
				...
				MessagePattern = NStr("en = 'The %1 queue job template is not found'");
				MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, Task.Pattern);
				
				WriteLogEvent(NStr("en = 'Job queue.Planning'", 
					CommonUseClientServer.MainLanguageCode()), 
					EventLogLevel.Error,
					,
					,
		...
		If Task.JobState = Enums.JobStatuses.ExecutionError
		...
		ElsIf Task.JobState = Enums.JobStatuses.Running Then
			
			WriteExecutionControlEventLogMonitor(NStr("en = 'Scheduled job queue. Completed with errors'", 
				CommonUseClientServer.MainLanguageCode()), Selection.ID, 
				NStr("en = 'Active job was forcibly terminated'"));
			...
			If HandlerParameters.ProcessorExist Then
				...
				NewJob.Use = True;
				NewJob.JobState = Enums.JobStatuses.Planned;
				...
				Task.JobState = Enums.JobStatuses.ErrorProcessingOnAbort;
		...
		Else
			...
			If Schedule <> Undefined Then
				...
				If ValueIsFilled(Task.ScheduledStartTime) Then
					Task.JobState = Enums.JobStatuses.Planned;
				Else
					Task.JobState = Enums.JobStatuses.NotActive;
				EndIf;
				Task.ActiveBackgroundJob = Undefined;
				CommonUse.AuxilaryDataWrite(Task);
			...
			Else // No schedule
				...
				If CommonUseReUse.IsSeparatedConfiguration() AND CommonUseReUse.IsSeparatedMetadataObject(
					...
					If ValueIsFilled(Task.Pattern) Then // Job by template without schedule.
						...
						MessagePattern = NStr("en = 'The schedule was not found for the %1 queue job template'");
						MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, Task.Pattern);
						WriteLogEvent(NStr("en = 'Job queue.Planning'", 
							CommonUseClientServer.MainLanguageCode()), 
							EventLogLevel.Error,
...
Procedure ExecuteJobQueue(Val Ref, Val ActiveBackgroundJob, 
	...
	Try
		...
		If Task.JobState = Enums.JobStatuses.Planned
			AND Task.Use
			AND Task.ScheduledStartTime <= CurrentUniversalDate() Then 
			
			Task.JobState = Enums.JobStatuses.Running;
	...
	Try
		WriteExecutionControlEventLogMonitor(NStr("en = 'Scheduled job queue.Start'", 
		...
		WriteExecutionControlEventLogMonitor(NStr("en = 'Scheduled job queue. Successfully completed'", 
			CommonUseClientServer.MainLanguageCode()), Ref);
	...
	Try
		...
		If CommonUse.RefExists(Ref) Then // Else - the job could have been deleted inside the handler.
			...
			If CompletedSuccessfully Then
				Task.JobState = Enums.JobStatuses.Completed;
			Else
				Task.JobState = Enums.JobStatuses.ExecutionError;
			EndIf;
			CommonUse.AuxilaryDataWrite(Task);
