...
Function JobCompleted(Val JobID) Export
	...
	If Task = Undefined Then
		WriteLogEvent(NStr("en = 'Long actions. Background job is not found'", CommonUseClientServer.MainLanguageCode()),
	...
	Else
		...
		ElsIf Task.State = BackgroundJobState.Canceled Then
			WriteLogEvent(
				NStr("en = 'Long actions. Background job is canceled by administrator'", CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Error,
				,
				,
				NStr("en = 'Job was completed with an unknown error.'"));
		Else
			Return True;
		EndIf;
	EndIf;
	...
	If ShowFullErrorText Then
		ErrorText = Task.ErrorInfo.Description + " in " +
					Task.ErrorInfo.ModuleName  + " line N " +
					Task.ErrorInfo.LineNumber;
		Raise(ErrorText);
...
Procedure ExecuteProcedureDataProcessorsObjectModule(Parameters, StorageAddress) Export 
	...
	Except
		WriteLogEvent(NStr("en = 'Safe execution of the data processor method'", CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Error, , , DetailErrorDescription(ErrorInfo()));
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Method name ""%1"" does not meet variable naming conventions.'"),
			MethodName);
	EndTry;
