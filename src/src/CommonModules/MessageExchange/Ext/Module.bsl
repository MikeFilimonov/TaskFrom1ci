...
Procedure DeliverMessages() Export
	...
	If TransactionActive() Then
		Raise NStr("en = 'You cannot receive quick system messages in an active transaction.'");
	EndIf;
	...
	For Each CatalogMessages In CatalogsMessages Do
		...
		QueryText = StrReplace(QueryText, "[CatalogMessages]", CatalogMessages.EmptyRef().Metadata().FullName());
	...
	ElsIf TypeOf(Recipient) = Type("Array") Then
		...
		For Each Item In Recipient Do
			...
			If TypeOf(Item) <> Type("ExchangePlanRef.MessageExchange") Then
				...
				Raise NStr("en = 'Incorrect recipient is specified for the MessagesExchange.SendMessage() method.'");
...
Procedure SendMessageToRecipient(MessageChannel, MessageBody, Recipient, IsInstantMessage)
	...
	If Not ValueIsFilled(MessageChannel) Then
		...
		Raise NStr("en = 'Value of the MessagesChannel parameter is not specified for the MessagesExchange.SendMessage method.'");
		
	ElsIf StrLen(MessageChannel) > 150 Then
		
		Raise NStr("en = 'The length of message channel name cannot be longer than 150 characters.'");
		
	ElsIf Not ValueIsFilled(Recipient) Then
		
		Raise NStr("en = 'Value of the Recipient parameter is not specified for the MessagesExchange.SendMessage method.'");
		
	ElsIf CommonUse.ObjectAttributeValue(Recipient, "Locked") Then
		
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Attempting to send message to the locked endpoint ""%1"".'"),
			String(Recipient));
	EndIf;
...
//  in the body of the procedure-processor, there will be no standard processing of the event. Denial from the standard
//  processor does not stop action. Default value: True.
//
