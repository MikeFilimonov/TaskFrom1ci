...
#Region DataExchangeSubsystemEventHandlers
				...
				If ModuleSaaSOperations.DataAreaBlocked(DataArea) Then
					...
					Else
						Raise StringFunctionsClientServer.SubstituteParametersInString(
							NStr("en = 'Cannot perform an email exchange due to: data area %1 is locked.'"),
							DataArea);
					EndIf;
				EndIf;
		...
		Try
			...
			Try
				...
				If Selection.EndPointLocked Then
					...
					Raise NStr("en = 'Attempting to process the message received from the locked endpoint.'");
				EndIf;
				
				If FoundStrings.Count() = 0 Then
					MessageObject.Locked = True;
					Raise NStr("en = 'Message handler is not set.'");
				EndIf;
				...
				For Each TableRow In FoundStrings Do
					...
					If TransactionActive() Then
						...
						EndDo;
						MessageObject.Locked = True;
						Raise NStr("en = 'The transaction was not recorded in the message handler.'");
					EndIf;
			...
			Except
				...
				WriteLogEvent(ThisSubsystemEventLogMonitorMessageText(),
						...
						StringFunctionsClientServer.SubstituteParametersInString(
							NStr("en = 'An error occurred when processing message %1: %2'"),
			...
			If ProcessMessageInUndividedData AND CommonUseReUse.DataSeparationEnabled() AND CommonUseReUse.CanUseSeparatedData() Then
				...
				ErrorMessageText = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'After data processor of %1 channel message session separation was not disabled.'"),
...
Procedure SetLeadingEndPointAtRecipient(ThisEndPointCode, LeadingEndPointCode) Export
	...
	Try
		...
		If EndPointNode.IsEmpty() Then
			...
			Raise NStr("en = 'Endpoint is not found in the correspondent base.'");
...
Procedure SerializeDataToStream(DataSelection, Stream) Export
	...
	For Each Ref In DataSelection Do
		...
		MessageBody = ConvertInstantMessageData(MessageBody);
	...
	Try
		...
		While CanReadXML(XMLReader) Do
			...
			If TypeOf(Data) = Type("ObjectDeletion") Then
				...
				Raise NStr("en = 'Passing of object ObjectDeletion via instant message tool is not supported.'");
...
Function EndPointConnectionEventLogMonitorMessageText() Export
	...
	Return NStr("en = 'Message exchange.Endpoint connection'", CommonUseClientServer.MainLanguageCode());
...
Function LeadingEndPointSettingEventLogMonitorMessageText() Export
	...
	Return NStr("en = 'Message exchange.Setting leading endpoint'", CommonUseClientServer.MainLanguageCode());
...
Procedure ConnectEndPointAtSender(Cancel,
	...
	If Not EndPointNode.IsEmpty() Then
		Cancel = True;
		ErrorMessageString = NStr("en = 'Endpoint is already connected to the infobase; point name: %1'", CommonUseClientServer.MainLanguageCode());
...
#Region EventHandlersOfSendingAndReceivingMessages
