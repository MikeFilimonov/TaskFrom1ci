...
//  node, corresponding to message sender MessageProcessed - Boolean, a flag showing that the message is successfully
//  processed. The value of this parameter shall be set as equal to True if the message was successfully read in this handler
//
...
Procedure SetExchangeStep1(Message, Sender)
	...
	Try
		...
		If Not IsBlankString(ThisNodeCode)
			...
			MessageString = NStr("en = 'Predefined node code in the ""%1"" application does not match the expected one ""%2"". Exchange plan: %3'");
		...
		If CorrespondentEndPoint.IsEmpty() Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Correspondent endpoint with the ""%1"" code is not found.'"),
				Body.EndPoint);
		EndIf;
		...
		If Cancel Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Errors occurred while exporting catalogs for the correspondent %1.'"),
...
Procedure ImportExchangeMessage(Message, Sender)
	...
	Try
		...
		If Cancel Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Errors occurred while importing catalogs from the correspondent %1.'"),
				String(Correspondent));
		EndIf;
...
Function ExchangeCorrespondent(Val ExchangePlanName, Val Code)
	...
	If Not ValueIsFilled(Result) Then
		MessageString = NStr("en = 'Exchange plan node is not found; exchange plan name %1; node code %2'");
