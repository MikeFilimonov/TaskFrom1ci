...
Procedure PrepareDataArea(Val DataAreaCode, Val FromExporting, Val Variant, Val ExportID) Export
	...
	Try
		If Not ValueIsFilled(Constants.InfobaseUsageMode.Get()) Then
			MessageText = NStr("en = 'Infobase operation mode is not set'");
			Raise(MessageText);
		EndIf;
		...
		If RecordManager.Selected() Then
			If RecordManager.Status = Enums.DataAreaStatuses.Deleted Then
				MessagePattern = NStr("en = 'Data area %1 is removed'");
			...
			ElsIf RecordManager.Status = Enums.DataAreaStatuses.ToDelete Then
				MessagePattern = NStr("en = 'Data area %1 is being removed'");
				MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, DataAreaCode);
				Raise(MessageText);
			ElsIf RecordManager.Status = Enums.DataAreaStatuses.New Then
				MessagePattern = NStr("en = 'Data area %1 is being prepared for use'");
				MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, DataAreaCode);
				Raise(MessageText);
			ElsIf RecordManager.Status = Enums.DataAreaStatuses.Used Then
				MessagePattern = NStr("en = 'Data area %1 is being used.'");
				MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, DataAreaCode);
				Raise(MessageText);
			EndIf;
		EndIf;
...
Procedure SetInfobaseParameters(Parameters) Export
	...
	Except
		RollbackTransaction();
		WriteLogEvent(NStr("en = 'IB parameter setting'", 
			CommonUseClientServer.MainLanguageCode()), 
...
Function GetAreaUserByServiceUserID(Val ServiceUserID)
	...
	If Result.IsEmpty() Then
		MessagePattern = NStr("en = 'User with user identifier of service %1 is not found'");
...
Function GetInfobaseUserByDataAreaUser(Val DataAreaUser)
	...
	If IBUser = Undefined Then
		MessagePattern = NStr("en = 'The infobase user does not exist for the data area user with identifier %1'");
