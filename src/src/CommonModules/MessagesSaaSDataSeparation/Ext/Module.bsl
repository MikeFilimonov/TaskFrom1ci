...
//
// Parameters:
//  ArrayCatalog - Array - you need to add catalog managers to this parameter, these managers can be used to store queue
//                         jobs.
//    
//
	...
	If MessagesSaaS.BodyContainTypedMessage(MessageBody, Message) Then
		...
		If CommonUseReUse.CanUseSeparatedData()
			...
			If CommonUse.SessionSeparatorValue() <> Message.Body.Zone Then
				MessagePattern = NStr("en = 'Attempting to send message on behalf of area %1 from area %2'");
		...
		Else
			...
			If AreaKeyCheckAtMessageAvailable() Then
				...
				If AreaCurrentKey <> Message.Body.ZoneKey Then
					...
					Raise NStr("en = 'Incorrect key of the data area in the message.'");
