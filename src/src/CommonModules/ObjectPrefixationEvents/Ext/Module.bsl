...
// If the date is not included in the previous period or the Company attribute is changed, then the business process
// number is reset to zero. It is required to assign a new number to the business process number.
//
// Parameters:
//  Source - BusinessProcessObject - the source of the subscription event.
//  Cancel - Boolean - Failure flag.
...
// If the date is not included in the previous period or the Company attribute is changed, then the document number is
// reset to zero. It is required to assign a new number to the document.
//
// Parameters:
//  Source - DocumentObject - the source of the subscription event.
//  Cancel - Boolean - Failure flag.
...
Procedure SetPrefix(Source, Prefix, SetIBPrefix, SetCompanyPrefix)
	...
	If SetIBPrefix Then
		...
		ObjectsReprefixation.WhenDeterminingPrefixInformationBase(GlobalNumerationPrefix);
		
		SupplementStringWithZerosOnTheLeft(GlobalNumerationPrefix, 2);
	EndIf;
	...
	PrefixTemplate = StrReplace(PrefixTemplate, "[Infobase]", GlobalNumerationPrefix);
...
Function AttributeCompanyIsAvailable(Object)
	...
	If   (CommonUse.ThisIsCatalog(ObjectMetadata)
		...
		If AttributeCompany = Undefined Then
			...
			MessageString = NStr("en = 'Attribute with the ""%2"" name is not defined for metadata object %1.'");
