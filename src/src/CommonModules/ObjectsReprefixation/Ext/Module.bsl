...
Procedure ServiceSetIBPrefixAndReprefixAllObjectsCreatedInThisIB(
	...
	If TransactionActive() Then
		...
		Raise NStr("en = 'Cannot change infobase prefix during transaction.'");
		
	ElsIf Not Users.InfobaseUserWithFullAccess() Then
		
		Raise NStr("en = 'Insufficient rights to change infobase prefix.'");
...
Function DataSelection(
	...
	QueryText = StrReplace(QueryText, "[SelectDate]", ?(ThisIsDocument, "Table.Date AS Date,", "Undefined AS Date,"));
...
Procedure CheckPrefixSetPossibility()
	...
	If Not FunctionalOptionInUse Then
		...
		Raise NStr("en = 'Object reprefixation is not available.'");
...
Function EventLogMonitorMessageTextPereprefixionObjects()
	...
	Return NStr("en = 'Object prefixation.Infobase prefix change'", CommonUseClientServer.MainLanguageCode());
...
// Returns a flag showing that functional option GlobalNumerationPrefix exists in the configuration.
//
// Parameters:
//  FunctionalOptionInUse - Boolean - flag showing that functional
//                                    option GlobalNumerationPrefix exists in the configuration.
//
...
//
Procedure WhenDeterminingPrefixInformationBase(GlobalNumerationPrefix) Export
	
	If CommonUse.SubsystemExists("StandardSubsystems.DataExchange") Then
		ModuleDataExchangeServer = CommonUse.CommonModule("DataExchangeServer");
		GlobalNumerationPrefix = ModuleDataExchangeServer.GlobalNumerationPrefix();
	Else
