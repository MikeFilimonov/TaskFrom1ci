...
#Region ServiceProceduresAndFunctionsUsedOnTheServiceSide
...
Function GenerateOfflineWorkplacePrefix(Val LastPrefix = "") Export
	...
	ApplicationPrefix = Right(GetFunctionalOption("GlobalNumerationPrefix"), 1);
...
Function OfflineWorkplacesMonitor() Export
	...
	For Each SynchronizationSetting In SynchronizationSettings Do
		...
		Else
			SynchronizationSetting.SynchronizationDatePresentation = NStr("en = 'not executed'");
		EndIf;
...
Function EventLogMonitorEventCreatingOfflineWorkplace() Export
	...
	Return NStr("en = 'Offline work. Generation of offline work place'", CommonUseClientServer.MainLanguageCode());
...
Function EventLogMonitorEventDeletionOfflineWorkplace() Export
	...
	Return NStr("en = 'Offline work. Deletion of offline work place'", CommonUseClientServer.MainLanguageCode());
...
#Region ServiceProceduresAndFunctionsUsedOnTheSideOfOfflineWorkPlace
...
Procedure SynchronizeDataWithApplicationInInternet() Export
	...
	If Cancel Then
		Raise NStr("en = 'Errors occurred when synchronizing the data with an online application (see the event log).'");
	EndIf;
...
// to change the object Object can not be written in the Offline work place if it corresponds to the following
// conditions at the same time: 1. It is offline work place.
...
Procedure ImportParametersFromInitialImage()
	...
	Try
		...
		Constants.GlobalNumerationPrefix.Set(Parameters.Prefix);
		...
		Constants.ApplicationTitle.Set(Parameters.ApplicationTitle);
...
Function CalculateDataToStructure(XMLReader)
	...
	While XMLReader.NodeType <> XMLNodeType.EndElement Do
		...
		KeyProperty = XMLReader.Name;
		
		Result.Insert(KeyProperty, ReadXML(XMLReader));
...
Function EventLogMonitorEventDataSynchronization()
	...
	Return NStr("en = 'Offline.Data synchronization'", CommonUseClientServer.MainLanguageCode());
