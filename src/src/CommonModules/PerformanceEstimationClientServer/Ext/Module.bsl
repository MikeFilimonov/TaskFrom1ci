...
// But remember that if the measurement start time is received on client, then the measurement end time should be
// calculated on client. The same for server.
//
// Returns:
...
Procedure WriteToEventLogMonitor(EventName, Level, MessageText) Export
	...
	WriteLogEvent(EventName,
		Level,
		,
		NStr("en = 'Performance assessment'"),
		MessageText);
