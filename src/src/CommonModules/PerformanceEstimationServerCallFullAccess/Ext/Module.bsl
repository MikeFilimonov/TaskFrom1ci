...
Procedure ExportPerformanceEstimation(ExportDirectories) Export
	...
	If Not PerformanceEstimationServerCallReUse.MeasurePerformance() Then
	    Return;	
	EndIf;
...
#Region WritingMeasurementsOfKeyOperationsDuration
...
Procedure GenerateAPDEXCalculationRequest(Query)
	...
	|	END AS CurrentAPDEX,
	|	CASE
	|		WHEN TU_KeyOperations.AllowableLevel = VALUE(Enum.PerformanceMarks.Perfect)
	|			THEN 1
	|		WHEN TU_KeyOperations.AllowableLevel = VALUE(Enum.PerformanceMarks.Excellent)
	|			THEN 0.94
	|		WHEN TU_KeyOperations.AllowableLevel = VALUE(Enum.PerformanceMarks.Good)
	|			THEN 0.85
	|		WHEN TU_KeyOperations.AllowableLevel = VALUE(Enum.PerformanceMarks.Satisfactorily)
	|			THEN 0.7
	|		WHEN TU_KeyOperations.AllowableLevel = VALUE(Enum.PerformanceMarks.Bad)
	|			THEN 0.5
	|	END AS MinimalAPDEX
	|FROM
...
Procedure ExportResults(ExportDirectories, APDEXSelection, MeasurementArrays)
	...
	For Each KeyPerformDirectory In ExportDirectories Do
		...
		KeyProperty = KeyPerformDirectory.Key;
		If KeyProperty = PerformanceEstimationClientServer.LocalExportDirectoryTaskKey() Then
