...
//  until the end of the permissions application, ClosingAlert - AlertDetails, which will be called when permissions are
//  successfully got.
//  StandardProcessing - Boolean, a flag showing that standard permissions processor is applied to use external
//    resources (connection to a server agent through COM-connection or administration server with querying the cluster
//    connection parameters from the current user). Can be set to the value False inside the event handler, in this case
//    the standard processing of the session end will not be executed.
//
...
// It asynchronously (relative to code from which the Assistant was called) processes the alert description that was
// initially transferred from the form, for which the assistant was opened in pseudomodal mode, returning the OK return code.
//
// Parameters:
...
// It asynchronously (relative to code from which the Assistant was called) processes the alert description that was
// initially transferred from the form, for which the assistant was opened in pseudomodal mode, returning the Cancel
// return code.
//
// Parameters:
//  NotifyDescription - AlertDescription which was transferred from the calling code.
