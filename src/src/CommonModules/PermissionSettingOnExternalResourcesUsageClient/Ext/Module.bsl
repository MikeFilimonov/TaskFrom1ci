...
#Region StartClientApplications
//
...
#Region ApplicationOfPermissionsRequestsToUseTheExternalResources
//

// Applies permissions changes in the security profiles in the cluster servers by the script.
//
// Parameters:
//  OperationTypes - Structure that describes the enumeration values OperationsAdministrationSecurityProfiles:
//                   * Key - String - name of
...
//                      security
//  profile properties, see ClusterAdministrationClientServer.SecurityProfileProperties(),
//  ClusterAdministrationParameters - Structure - Cluster administration server parameters, see
//  ClusterAdministrationClientServer.ParametersAdministrationCluster(), InformationBaseAdministrationParameters -
//  Structure - information database administration parameters, see ClusterAdministrationClientServer.ClusterInformationDatabaseAdministrationOptions().
//
Procedure ApplyChangesPermissionsInSecurityProfilesOnServerCluster(Val OperationTypes, Val ScriptApplicationPermissions, Val ClusterAdministrationParameters, Val InformationBaseAdministrationParameters = Undefined) Export
	...
	For Each ScriptItem In ScriptApplicationPermissions Do
		...
		If ScriptItem.Operation = OperationTypes.Creating Then
			...
			If ClusterAdministrationClientServer.SecurityProfileExists(ClusterAdministrationParameters, ScriptItem.Profile) Then
				...
				CommonUseClientServer.MessageToUser(
					StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'Security profile %1 already exists in the server cluster. Settings in the security profile will be replaced...'"), ScriptItem.Profile));
		...
		ElsIf ScriptItem.Operation = OperationTypes.Purpose Then
		...
		ElsIf ScriptItem.Operation = OperationTypes.Update Then
			
			ClusterAdministrationClientServer.SetSecurityProfileProperties(ClusterAdministrationParameters, ScriptItem.permissions);
			
		ElsIf ScriptItem.Operation = OperationTypes.Delete Then
			...
			Else
				...
				CommonUseClientServer.MessageToUser(
					StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'Security profile %1 does not exist in the server cluster. Security profile might have been deleted earlier...'"), ScriptItem.Profile));
				
			EndIf;
			
		ElsIf ScriptItem.Operation = OperationTypes.DeleteDestination Then
...
// "as
// for that notifications of closing description Procedure PosleNastrojkiRazrešenijNaIspolzovanieVnešnihResursov or
// abort the preinstalled works assistant.
//
...
//
// Result of the peration is opening form
// "DataProcessor.PermissionSettingsForExternalResourcesUse.Form.EndPermissionsRequest" as for that notifications of
// closing description procedure AfterCompletingPermissionsQueryForExternalResourcesUsing or abort the preinstalled
// works assistant.
//
...
Procedure AfterCompletingPermissionsQueryForExternalResourcesUsing(Result, Status) Export
	...
	If Result = DialogReturnCode.OK Then
		...
		ShowUserNotification(NStr("en = 'Set permissions'"),,
			NStr("en = 'Changes made to security profile settings in server cluster.'"));
...
// It asynchronously (relative to the code from which the Assistant was called) processes the alert description that was
// initially transferred from the form for which the assistant was opened in pseudomodal mode.
//
// Parameters:
...
// property) the call of the assistant for checking the operation completion after closing the form from which the
// assistant was called.
//
// Parameters:
...
//    permissions request to use external resources that were applied within the operations and completion of which is
//    being checked.
//
...
//    body of the procedure, the parameter is required for assigning the procedure as a description of alert about
//    closing the forms.
...
// execution of all operations). When the assistant work is complete, the processor of an alert description will be
// called in the function of which the procedure will be set. AfterPermissionsCheckAfterOwnerFormClosure().
//
...
//  the end of the permissions application, ClosingAlert - NotifyDescription - which will be called if the permissions
