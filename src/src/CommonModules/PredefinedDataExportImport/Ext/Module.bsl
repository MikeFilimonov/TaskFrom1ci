...
//          process of data export. See more the review to the application processing interface DataExportImportContainerManager,
//        Serializer - XDTOSerializator, initialized with
//          support the executing of the annotation refs. If a random exporting handler requires additional data export
...
Procedure AfterObjectExport(Container, ObjectExportManager, Serializer, Object, Artifacts) Export
	...
	If CommonUseSTL.IsReferenceDataSupportPredefinedItems(Object.Metadata()) Then
		...
		Else
			...
			NStr("en = 'The %1 metadata object can not be processed by the
			     |PredefinedDataExportImport.BeforeObjectExport() handler as it is not required to ensure mapping of references to its predefined items!'", Metadata.DefaultLanguage.LanguageCode),
			...
			If Not QueryResult.IsEmpty() Then
				...
				Else
					...
					Raise StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en = 'Duplication of predefined items %1 is detected in table %2.'", Metadata.DefaultLanguage.LanguageCode),
