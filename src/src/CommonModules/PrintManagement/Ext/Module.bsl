...
//                                   "Document.Quote.PF_MXL_OrderInvoice".
...
//                                                     required to locate the Print submenu, by default, it is located
//                                                     in the form command pane.
...
Function PrintCommandsForms(Form, ObjectList = Undefined) Export
	...
	For LineNumber = -PrintCommands.Count() + 1 To 0 Do
		...
		FormsList		= StringFunctionsClientServer.DecomposeStringIntoSubstringsArray(PrintCommand.FormsList, ",", True, True);
...
//                                        several identifiers (as a line, separated by commas, or an array of lines, for
//                                        example: "OrderInvoice,WarrantyLetter".
//
...
//                                        print forms identifiers specified in the parameter. Example (2 invoices for
//                                        payment + 1 warranty letter): "OrderInvoice,OrderInvoice,WarrantyLetter".
//
...
//                                        manager module the Print procedure is located. The procedure generates tabular
//                                        documents for the command. Default value: name of the object manager module.
//                                         Example: "Document.Quote".
...
//                                        document logs where it is required to check a passed object type before
//                                        calling the print manager. If the list is not filled in, then while automatically
...
//                                        it is required to pass the management instead of the standard handler of the
//                                        print command. It is used, for example, when a print form is generated on client.
//                                         Example: "PrintHandlersClient.PrintBuyerInvoicesForPayment".
//
//  * Order       - Number             - (optional) Value from 1 to 100
//                                        that specifies command location order relatively to other commands. Commands
//                                        of menu Print are first sorted by field Order, then by presentation.
//                                        Default value: 99.
//
...
//                                        in which a command should be displayed. If the parameter is not specified,
//                                        then print command will appear in all forms of the object with the Print subsystem.
//                                         Example: "DocumentForm".
//
//  * Location - String          - (optional) Form command panel name to
//                                        which it is required to put the print command. The parameter shall be used
//                                        only when the form has more than one submenu "Print". IN the rest
...
//                                        it is required to check whether documents are posted before printing. If the
//                                        parameter is not specified, then posting check is not executed.
//
//  * StraightToPrinter - Boolean           - (optional) Shows that it is
//                                        required to print documents without a preview, pass it right to the printer.
//                                        If the parameter is not specified, then when you select a print command,
//                                        preview form "Print documents" is opened.
//
//  * SavingFormat - SpreadsheetDocumentFileType - (optional) It is applied for
//                                        a quick saving of a print form (without additional actions) to different
//                                        formats different from mxl. If the parameter is not specified, a normal mxl is
//                                        generated. Example: SpreadsheetDocumentFileType.PDF.
//
...
//                                        the mechanism of saving/restoring the number of copies to print selected by
//                                        user in form PrintDocuments. If the parameter is not specified, then the
//                                        saving/restoring mechanism will work on the form opening. PrintDocuments.
//
//  * ComplementKitWithExternalPrintForms - Boolean - (optional) Shows that
//                                        it is required to expand the documents set
//                                        with all external print forms connected to an object (the
//                                        AdditionalReportsAndDataProcessors subsystem). If the parameter is not
//                                        specified, the external print forms are not added to the kit.
//
//  * FixedKit - Boolean    - (optional) Shows that it is
//                                        required to lock changing the documents set content by a user. If the
//                                        parameter is not specified, then the user can exclude separate print forms
//                                        from the kit in form PrintDocuments and also modify their quantity.
//
...
//                                        required to disable the mechanism of object writing before running the print
//                                        command. Used in exceptional cases. If the parameter is not specified, the
//                                        object is recorded in case when the modification sign is set in the object form.
//
//  * RequiredFileOperationsExtension - Boolean - (optional) Shows that it
//                                        is required to enable work with files extension before executing the command.
//                                        If the parameter is not specified, the extension for work with files will not
//                                        be connected.
//
...
Function PrintedFormsTemplate(FullPathToTemplate) Export
	...
	Else
		Raise StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'Template ""%1"" is not found. Operation aborted.'"), FullPathToTemplate);
	EndIf;
...
Function QRCodeData(QRString, CorrectionLevel, Size) Export
	...
	Except
		WriteLogEvent(NStr("en = 'QR code generation'", CommonUseClientServer.MainLanguageCode()),
...
// 	* NotificationNumber                   - Number of notification, Earning, account.
// 	* NotificationDate                    - Date of notice/Earning/account/regulation (for STSI).
...
// 	* EarningIdentifier          - Unique Earning identifier.
...
Procedure ComplimentProductDescription(ProductDescription, ProductsSelection, SerialNumbersSelection = Undefined) Export
	...
	If Not ProductsSelection.ContentUsed Then
		...
		If AdditionalProductDescriptonComponents.Count() Then
			...
			AdditionalDescription = StringFunctionsClientServer.GetStringFromSubstringArray(AdditionalProductDescriptonComponents, ", ");
...
Procedure AtFillingToDoList(ToDoList) Export
	...
	If Not AccessRight("Edit", Metadata.InformationRegisters.UserPrintTemplates)
		Or ModuleToDoListService.WorkDisabled("PrintFormsTemplates") Then
		Return;
	EndIf;
	...
	OutputToDo = True;
	CheckedForVersion = CommonSettingsStorage.Load("ToDoList", "PrintForms");
...
Function permissions()
	...
	permissions.Add( 
		WorkInSafeMode.PermissionToUseExternalComponent("CommonTemplate.QRCodePrintingComponent", NStr("en = 'Print QR codes.'"))
	);
...
Function GeneratePrintForms(Val PrintManagerName, Val TemplateNames, Val ObjectsArray, Val PrintParameters, 
	...
	If TypeOf(TemplateNames) = Type("String") Then
		TemplateNames = StringFunctionsClientServer.DecomposeStringIntoSubstringsArray(TemplateNames,, True, True);
	Else // Type("Array")
	...
	For Each TemplateName In TemplateNames Do
		...
		For Each PrintFormDescription In TemporaryCollectionForOnePrintForm Do
			CommonUseClientServer.Validate(
				...
				StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'Number of copies is not specified for print form ""%1"".'"),
		...
		If Cancel Then
			ErrorMessageText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'An error occurred when generating print form ""%1"". Contact your administrator.'"), TemplateName);
...
//                   Ref                      - EnumRef.ReportSaveFormats - ref
//                                                                                               to metadata
...
Function RequiredAttributesString(DocumentData, MessageText)
	...
	For Each Item In MandatoryAttributes Do
		If Not ValueIsFilled(DocumentData[Item.Key]) Then
			MessageText = NStr("en = 'Required attribute is not entered: %1'");
...
Function PresentationsAndAttributesStructure()
	...
	ReturnStructure.Insert("EarningIdentifier",			"uin");
...
Procedure AddOtherAdditionalAttributes(DataStructure)
	...
	DataStructure.Insert("EarningIdentifier");
...
Procedure MessagePrintingFormIsUnavailable(Object)
	MessageText = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = '%1 was not printed. The selected print form is not available.'"),
		Object);
...
// Called from common form SavePrintForm.
//
// Parameters:
