...
#EndIf
	...
	Except
		...
		EventLogMonitorClient.AddMessageForEventLogMonitor(EventLogMonitorEvent(), "Error",
			DetailErrorDescription(ErrorInfo()),,True);
		Raise(NStr("en = 'An error occurred when opening a template file.'") + Chars.LF 
...
#Region FunctionToReceiveAreasFromTheLayout
...
//                              does not end right before the operator parenthesis, but it ends in one or several
//                              characters prior. The value
// must be negative.
//                              Default value:-1 - it is expected that a newline
//                                                         character is before the operator parenthesis of the area
//                                                         closure. The character is not to be included into the area
//                                                         being received.
//
...
#Region TheFunctionAddsAreasToThePrintForm
...
#If WebClient Then
	CorrectionText = NStr("en = 'Working via web client requires Windows Internet Explorer. See also documentation, chapter ""Web browser settings to work via web client""'");
#Else		
	CorrectionText = "";	
#EndIf
