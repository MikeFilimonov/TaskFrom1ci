...
Function InitializePrintFormOOWriter(Val Pattern = Undefined) Export
	...
	Except
		EventLogMonitorClient.AddMessageForEventLogMonitor(EventLogMonitorEvent(), "Error",
			NStr("en = 'An error occurred when contacting service manager (com.sun.star.ServiceManager).'") + 
	...
	Except
		EventLogMonitorClient.AddMessageForEventLogMonitor(EventLogMonitorEvent(), "Error",
			NStr("en = 'An error occurred when launching Desktop service (com.sun.star.frame.Desktop).'") + 
			+ Chars.LF + DetailErrorDescription(ErrorInfo()),,True);
		FailedToGeneratePrintForm(ErrorInfo());
	EndTry;
...
Function GetAreaBeginPosition(Val xDocument, Val AreaName)
	...
	If xFound = Undefined Then
		Raise NStr("en = 'Template area start is not found:'") + " " + AreaName;	
	EndIf;
	Return xFound.End;
...
Function GetAreaEndPosition(Val xDocument, Val AreaName)
	...
	xFound = xDocument.findFirst(xSearchDescr);
	If xFound = Undefined Then
		Raise NStr("en = 'End of template area is not found:'") + " " + AreaName;	
	EndIf;
	Return xFound.Start;
...
#If WebClient Then
	CorrectionText = NStr("en = 'Working via web client requires Windows Internet Explorer.'");
#Else		
	CorrectionText = "";	
#EndIf
