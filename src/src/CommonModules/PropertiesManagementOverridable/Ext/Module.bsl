...
//
//                    // Then, the form item properties of the FormGroup type, common group kind or page, which is
//                     formed if the number of sets is more than one disregarding the empty set that describes group
//                     properties of the remote attributes.
//                     
//                     
//                    
...
//                    ShowTitle         - Boolean.
//
//  StandardProcessing - Boolean - initial value is True. Specifies whether receive core set or not, when
//                                 PropertiesSets.Count() equals zero.
//                         
//
//  PurposeKey   - Undefined - (initial value) - specifies to calculate function key automatically and add to form
//                             property value.
//                      
//                      PurposeUseKey, to save form changes separately for different sets content.
//                      
//                      For example, for each type of products - their set structure.
//
//                    - String - (up to 32 letters) - use specified purpose key to add to the property value of the
//                                                    PurposeUseKey form.
//                      
...
Procedure FillObjectPropertiesSets(Object, ReferenceType, PropertiesSets, StandardProcessing, PurposeKey) Export
	...
	If ReferenceType = Type("CatalogRef.Products") AND NOT Object.IsFolder Then	
...
// 
// Now instead of specifying the attribute name containing the property owner kind, for example, ProductsKind
// of the CatalogRef.ProductsKinds which should have the attribute of the PropertySet type.
// 
// 
// CatalogRef.AdditionalDetailAndInformationSets you should feill proprty set for object CatalogRef.List in procedure
// FillOjectPropertiesSets as for few property sets. The difference is only that the set will be received internally
// from the owner kind object attribute which allows to use several different attributes with convenient names for
// different object kinds which have one object kind. For example, Catalog.Projects is a property owner kind of
// Catalog.Errors and Catalog.Tasks for which the ErrorsPropertiesSet, TasksPropertiesSet attributes will be in the
// Projects catalog.
// 
//
...
Function GetProductsAdditionalAttributes(Object)
	...
	CommonSet = Catalogs.AdditionalAttributesAndInformationSets.Catalog_Products_Common;
	SetList.Add(CommonSet, NStr("en = 'Common properties'"));
	
	GroupSet = CommonUse.ObjectAttributeValue(Object.ProductsCategory, "PropertySet");
	SetList.Add(GroupSet, NStr("en = 'Product group properties'"));	
...
Function FillAdditionalParameters(Object = Undefined, ItemNameForPlacement = Undefined, ArbitraryObject = Undefined, CommandBarItemName = Undefined, HideDeleted = Undefined) Export
	...
	If Object <> Undefined Then
		...
		AdditionalParameters.Вставить("Object", Object);
		
	EndIf;
	
	If ItemNameForPlacement <> Undefined Then
		
		AdditionalParameters.Вставить("ItemNameForPlacement", ItemNameForPlacement);
		
	EndIf;
	
	If ArbitraryObject <> Undefined Then
		
		AdditionalParameters.Вставить("ArbitraryObject", ArbitraryObject);
		
	EndIf;
	
	If CommandBarItemName <> Undefined Then
		
		AdditionalParameters.Вставить("CommandBarItemName", CommandBarItemName);
		
	EndIf;
	
	If HideDeleted <> Undefined Then
		
		AdditionalParameters.Вставить("HideDeleted", HideDeleted);
