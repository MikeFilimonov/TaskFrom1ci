////////////////////////////////////////////////////////////////////////////////
// Subsystem "Report options" (server).
//
////////////////////////////////////////////////////////////////////////////////

#Region ApplicationInterface

// Generates additional parameters of report form opening.
//
// Parameters:
//   OptionRef - CatalogRef.ReportOptions - opened report option reference.
//
Function OpenParameters(OptionRef) Export
	
	OpenParameters = New Structure("Ref, Report, ReportType, ReportName, VariantKey, MeasurementKey");
	
	If TypeOf(OptionRef) = ReportsVariantsClientServer.AdditionalReportRefType() Then
		// Support the possibility to directly select additional report references in report mailings.
		OpenParameters.Report     = OptionRef;
		OpenParameters.ReportType = "Additional";
	Else    
		...
		|	ReportOptions.VariantKey,
		|	"""" AS MeasurementKey
		|FROM
		|	Catalog.ReportsVariants AS ReportOptions
		|WHERE
		|	ReportOptions.Ref = &Ref";
		...
		If Not Selection.Next() Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Insufficient rights to open the ""%1"" variant.'"), 
				String(OptionRef));
		EndIf;
		
		FillPropertyValues(OpenParameters, Selection);
		OpenParameters.Ref			= OptionRef;
		OpenParameters.ReportType	= ReportsVariantsClientServer.ReportTypeAsString(Selection.ReportType, Selection.Report);
		
	EndIf;
	
	WhenConnectingReport(OpenParameters);
	
	Return OpenParameters;
	
EndFunction

// Connect additional reports.
Procedure WhenConnectingReport(OpenParameters) Export
	
	OpenParameters.Insert("Connected", False);
	
	If OpenParameters.ReportType = "Internal" Then
		
		ReportMetadata = CommonUse.MetadataObjectByID(OpenParameters.Report);
		
		If TypeOf(ReportMetadata) <> Type("MetadataObject") Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Cannot open the ""%1"" report.
				     |Maybe, the configuration extension with this report was disabled.'"),
				OpenParameters.Report);
		EndIf;
			
		OpenParameters.ReportName	= ReportMetadata.Name;
		OpenParameters.Connected	= True; // Configuration reports are always connected.
	...
	ElsIf OpenParameters.ReportType = "Additional" Then
		If CommonUse.SubsystemExists("StandardSubsystems.AdditionalReportsAndDataProcessors") Then
			...
			AdditionalReportsAndDataProcessorsModule.WhenConnectingReport(OpenParameters);
		EndIf;
	EndIf;
...
Function UsedSections() Export
	...
	For Each ItemOfList In SectionList Do
		...
		Else
			CaptionPattern = NStr("en = '""%1"" reports'");
		EndIf;
...
// Receives report option reference by key attributes set.
//
// Parameters:
//   Report        - Corresponds to the catalog attribute - Full name or report reference.
//   VariantKey - Corresponds to the catalog attribute - Report option name.
//
// Returns: 
//   * CatalogRef.ReportsVariants - When the option is found.
//   * Undefined                     - When the option is not found.
//
...
// Returns:
//   ValueTreeRow - Report.
//      See "Attributes for change" of the ConfigurationReportOptionSettingsTree() function.
//
...
// Finds settings of the report option. Used for placement setting.
//   For use in ReportsVariantsOverridable.SetReportsVariants().
//
// Parameters:
//   OptionsTree - ValueTree - Used to describe reports and
//       variants settings see ConfigurationReportOptionSettingsTree() function description.
...
//   VariantKey - String - Report option name as it is specified in the data layout schema.
//
// Returns: 
...
Function VariantDesc(OptionsTree, ReportTreeRowOrValueOrMetadata, VariantKey) Export
	...
	If RowOption = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Option ""%1"" was not found for report ""%2"".'"),
			VariantKey,
...
//       - True, "ByReports" - Default report options are hidden and the report (option with an empty key) is enabled
//       and seen. - False, "ByVariants" - Report options are visible by default and report is disabled. As in the
//       version 2.2.1 and earlier.
//
...
#Region ToCallConfigurationFromOnAddUpdateHandlersEvent

// Full update of report options search index.
...
#Region ToCallAppliedConfigurationFromUpdateHandlers
...
// // Or transfer user report options that are transferred to the "Report options" subsystem storage.
...
Procedure TransferUserOnesFromStandardStorage(NamesReports = "") Export
	ProcedureRepresentation = NStr("en = 'Direct conversion of report options'");
	...
	While True Do
		...
		Except
			...
			ErrorByVariant(
				Undefined,
				NStr("en = 'An error occurred when selecting report options from the standard storage:'")
				+ Chars.LF
...
// Imports report options subsystems to the
//   storage that were saved from the option system storage to the general settings storage.
//   It is used to load report options on full or partial implementation.
...
//       * ReportPresentaton   - String - Report presentation .
...
Procedure ImportCustom(VariantTable = Undefined) Export
	...
	ProcedureRepresentation = NStr("en = 'Complete conversion of report options'");
	ProcedureLaunch(ProcedureRepresentation);
	...
	For Each TableRow In DefinedVariants Do
		...
		// If option is already imported to the "Report options" catalog. - do not load it.
		...
		Else
			ErrorByVariant(
				VariantObject.Ref,
				NStr("en = 'Option ""%1"" of report ""%2"": author ""%3"" was not found'"),
				VariantObject.Description,
				TableRow.ReportPresentation,
				TableRow.Author);
		EndIf;
		
		// As user report options
...
Procedure OnAddOfficeEvent(ClientEvents, ServerEvents) Export
	...
	// Register changes in the report options names.
	...
	//
	// Parameters:
	//   Changes - ValueTable - Changes in report options names.
...
//      See description of method UpdateResults.UpdateHandlersNewTable().
//
Procedure OnAddUpdateHandlers(Handlers) Export
	...
	// 3.1. Fill in information to search for report options.
...
// Fills those metadata objects renaming that can not be automatically found by type, but the references to which are to
// be stored in the database (for example, subsystems, roles).
//
// See also:
...
Procedure OnGettingObligatoryExchangePlanObjects(Objects, Val DistributedInfobase) Export
	...
	If DistributedInfobase Then
		...
		Objects.Add(Metadata.Constants.ReportOptionParameters);
		...
		Objects.Add(Metadata.InformationRegisters.ReportOptionSettings);
...
// Receives report object by report option reference.
//
// Parameters:
//   ReferenceOfVariantOrReport -
//     - CatalogRef.ReportsVariants - Report option reference connected to the subsystem storage.
...
Function ConnectReportObject(ReferenceOfVariantOrReport) Export
	...
	If Result.Ref = Undefined Then
		Result.Errors = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Report option ""%1"" was not found in the application'"),
			String(ReferenceOfVariantOrReport));
		Return Result;
	ElsIf TypeOf(Result.Ref) = Type("String") Then
		Result.Errors = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'The ""%2"" report for the ""%1"" option is written as external and cannot be connected from the application'"),
			String(ReferenceOfVariantOrReport),
			Result.Ref);
		Return Result;
	EndIf;
	...
	If TypeOf(Result.Ref) = Type("CatalogRef.MetadataObjectIDs") Then
		...
		If Result.Metadata = Undefined Then
			Result.Errors = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Report ""%2"" for option ""%1"" was not found in the application'"),
				String(ReferenceOfVariantOrReport),
				Result.Name);
			Return Result;
		EndIf;
...
// Updates additional report options during its writing.
//
// Usage location:
...
Procedure OnWriteAdditionalReport(CurrentObject, Cancel, ExternalObject) Export
	...
	If Not ReportsVariantsReUse.AddRight() Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Insufficient access rights to write options of additional report ""%1"".'"),
	...
	// for the predefined report options (it does not clear for the user ones).
	QueryText =
	"SELECT ALLOWED
	|	Table.Ref,
	|	Table.VariantKey,
	...
	If DeletionMark = True Then
		// While setting mark for deletion of add. report mark for deletion is set for all report options - and
		// user and predefined.
	...
	While Selection.Next() Do
		...
		If Not OptionDeletionMark AND Not Selection.User AND ItemOfList = Undefined Then
			// Predefined option that is not found in the list of the predefined ones for the current report.
...
Procedure AddNotificationOnValueChangeConstants(Result, ConstantManager) Export
	// Report options are connected with constants via functional options.
	...
	For Each FunctionalOptionMetadata In Metadata.FunctionalOptions Do
		If FunctionalOptionMetadata.Location = ConstantMetadata Then
			...
			// Search for report options connection with this functional option.
			If Not VariablesInitialized Then
				VariablesInitialized = True;
				OptionsTable = Constants.ReportOptionParameters.Get().Get().TableFunctionalOptions;
...
Procedure Refresh(Settings = Undefined) Export
	...
	If Settings.SharedData = True Then
		...
		UpdateParameters.SeparatedHandlers = UpdateResults.NewUpdateHandlersTable();
...
Procedure GenerateReport(ReportGenerationParameters, StorageAddress) Export
	...
	If ReportObject = Undefined Then
		ErrorByVariant(Undefined, NStr("en = 'Cannot receive report object ""%1""'"), ReportRef);
		Return;
	EndIf;
...
// Generates a tree of customizing and placing predetermined report options.
//   Only for reports connected to the subsystem.
//
// Returns: 
//   OptionsTree - ValueTree - Settings of the predefined report options connected to subsystem.
//     Attributes open for change:
//       * Enabled              - Boolean - If False then the report option is not registered in the subsystem.
//       * VisibleByDefault - Boolean - If False then the report option is hidden by default in the reports panel.
//       * Description         - String - Report option description.
//       *Description             - String - Information about report option.
//       * Placement           - Map - Settings for report option location in sections.
//           ** Key     - MetadataObject - Subsystem that hosts the report or the report option.
...
//       * FunctionalOptions - Array from String - Names of the functional report option options.
//       * SettingsForSearch  - Structure - Additional settings for this report option search.
//           ** FieldsDescription - String - Report option fields names.
//           ** ParametersAndReportsDescriptions - String - Names of report option settings.
...
Function ReportVariantsTreesSettingsConfigurations() Export
	...
	For Each ReportMetadata In Metadata.Reports Do
		...
		If RowReport.SystemInfo.BuiltOnDCS Then
			...
			Except
				ErrorText = NStr("en = 'Cannot read report scheme ""%1"":'");
			...
			EndTry;
			// Read report options settings from schema.
			If DCSchema <> Undefined Then
				Try
					SettingVariants = DCSchema.SettingVariants;
				Except
					ErrorText = NStr("en = 'Cannot read a list of options of report ""%1"" from scheme:'");
					ErrorText = StrReplace(ErrorText, "%1", ReportMetadata.Name);
					ErrorText = ErrorText + Chars.LF + DetailErrorDescription(ErrorInfo());
					WarningByOption(Undefined, ErrorText);
				EndTry;
			EndIf;
			// Reading report options settings from the manager module (if failed from the scheme).
			If SettingVariants = Undefined Then
				Try
					SettingVariants = ReportManager.SettingVariants();
				Except
					ErrorText = NStr("en = 'Cannot read a list of options of report ""%1"" from the manager module:'");
					ErrorText = StrReplace(ErrorText, "%1", ReportMetadata.Name);
					ErrorText = ErrorText + Chars.LF + DetailErrorDescription(ErrorInfo());
			...
			If SettingVariants <> Undefined Then
				For Each DCSettingsVariant In SettingVariants Do
					...
					If AllowedIndexing AND TypeOf(DCSettingsVariant) = Type("DataCompositionSettingsVariant") Then
						...
						Except
							ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
								NStr("en = 'Cannot read settings of option ""%1"" of report ""%2"":'"),
								Variant.VariantKey,
								ReportMetadata.Name)
								+ Chars.LF
								+ DetailErrorDescription(ErrorInfo());
...
// Fills out setting description for the report option string if it is not filled out.
//
// Parameters:
//   RowOption - TreeRow - Description of report option settings.
...
// Sets the report options output mode in reports panels.
//
// Parameters:
...
Procedure SetReportOutputModeInReportsPanels(OptionsTree, Report, OutputReportsInsteadVariants)
	...
	Else
		...
		If RowReport = Undefined Then
			WarningByOption(Undefined, NStr("en = 'Report ""%1"" is not connected to the subsystem.'"), Report.Name);
			Return;
		EndIf;
	EndIf;
	RowReport.GroupByReport = OutputReportsInsteadVariants;
...
//       * ReportMetadata - MetadataObject: Report - Report metadata in the schema of which option name is changed.
//       * VariantOldName - String - Option old name before change.
//       * VariantActualName - String - Option current (last relevant) name.
		...
		If Found.Count() > 0 Then
			...
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'An error occurred while registering changes
				     |of report option name ""%1"": Relevant option
		...
		If Found.Count() > 2 Then
			...
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'An error occurred while registring report
				     |option name ""%1"": Old option name ""%2""
				     |(relevant name ""%3"") is listed as an
				     |old report option name ""%4"" (relevant name ""%5"").'"),
				String(Update.Report),
				Update.OldOptionName,
				Update.ActualOptionName,
...
Function GenerateInformationAboutReportByDescriptionFull(ReportFullName) Export
	...
	If Upper(Prefix) = "REPORT" Then
		...
		If Result.ReportMetadata = Undefined Then
			...
			WarningByOption(
				Undefined,
				NStr("en = 'Report ""%1"" is not found in the application, it will be marked as external.'"),
				ReportFullName);
		ElsIf Not AccessRight("view", Result.ReportMetadata) Then
			Result.ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Insufficient access rights to report ""%1"".'"),
				ReportFullName);
		EndIf;
	...
	Else
		Result.ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Type cannot be defined for the ""%1"" report (prefix is not set).'"),
			ReportFullName);
		Return Result;
	EndIf;
...
//   item of the "Report options" catalog connected to the "Report options" catalog item.
//
// Parameters:
...
// Checks whether report option name is vacant.
...
// Checks whether report option key is vacant.
...
Function GlobalSettings() Export
	...
	Result.Search.Insert("InputHint", NStr("en = 'Name, field or author of the report'"));
...
// Write error to the events log monitor by the report option.
...
// Write information in the events log monitor by the report option.
...
// Warning record in the events log monitor by the report option.
...
Procedure ProcedureLaunch(ProcedureRepresentation)
	InformationByOption(Undefined, NStr("en = 'Start the ""%1"" procedure.'"), ProcedureRepresentation);
	BeginTransaction();
EndProcedure
...
Procedure ProcedureCompletion(ProcedureRepresentation, Changed = Undefined)
	CommitTransaction();
	Text = NStr("en = 'Finish procedure ""%1"".'");
...
// Delete personal report options while deleting user.
...
Procedure UpdateCommonData(UpdateParameters) Export
	...
	////////////////////////////////////////////////////////////////////////////////
	// Executed only for the predefined report options.
	// Update plan:
	...
	////////////////////////////////////////////////////////////////////////////////
	// 1. Replace outdated report options keys to the relevant ones.
	UpdateKeysFixed(Cache);
	
	////////////////////////////////////////////////////////////////////////////////
	// 2. Actualize predefined report options
...
Procedure UpdateDividedData() Export
	...
	////////////////////////////////////////////////////////////////////////////////
	// 1. Actualize separated report options.
	UpdateContentAreas();
...
// Partial search index of the predefined report options. - only by changes in the variants settings.
Procedure DeferredGeneralDataIncrementalUpdate(Parameters = Undefined) Export
	DeferredGeneralDataUpdate(False);
EndProcedure

// Full update of search index of the predefined report options.
Procedure DeferredGeneralDataFullUpdate(Parameters = Undefined) Export
	DeferredGeneralDataUpdate(True);
EndProcedure

// Update search index of the predefined report options.
Procedure DeferredGeneralDataUpdate(Full)
	If Not UndividedDataIndexationAllowed() Then
		Return;
	EndIf;
	If Full Then
		ProcedureRepresentation = NStr("en = 'Deferred common data update (full)'");
	Else
		ProcedureRepresentation = NStr("en = 'Deferred common data update (by changes)'");
	EndIf;
	ProcedureLaunch(ProcedureRepresentation);
...
// Replace old report options key to the relevant ones.
Procedure UpdateKeysFixed(Cache)
	ProcedureRepresentation = NStr("en = 'Update report option keys'");
	ProcedureLaunch(ProcedureRepresentation);
	
	// Generate table of the variants keys to relevant ones replacements.
	Changes = KeysChanges();
	
	// Receive report options refs for keys
	//   replacement by deleting from the replacements list
	//   those report options, relevant keys
...
// Actualize predefined report options.
Procedure UpdateSettingsOfPredefined(Cache)
	ProcedureRepresentation = NStr("en = 'Update predefined'");
	ProcedureLaunch(ProcedureRepresentation);
	...
	OldValue = Constants.ReportOptionParameters.Get().Get();
	If CommonUse.ValueToXMLString(NewValue) <> CommonUse.ValueToXMLString(OldValue) Then
		ConstantObject = Constants.ReportOptionParameters.CreateValueManager();
		ConstantObject.Value = New ValueStorage(NewValue, New Deflation(9));
		UpdateResults.WriteData(ConstantObject, False, False);
	EndIf;
	
	ProcedureCompletion(ProcedureRepresentation);
EndProcedure

// Writes option settings to the catalog data.
...
// Determines whether key settings of the predetermined report option changed.
...
// Determines whether auxiliary settings of the predetermined report option changed.
...
// Defines whether search settings of the report predefined options are changed.
...
Procedure UpdateContentAreas()
	...
	ProcedureRepresentation = NStr("en = 'Update separated report options'");
	ProcedureLaunch(ProcedureRepresentation);
	...
	While Selection.Next() Do
		...
		VariantObject.Lock();
		UpdateResults.WriteData(VariantObject, VariantObject.IsNew(), False);
	EndDo;
...
// Set deletion mark of deleted report options.
Procedure MarkOnDeletionDeletedReportVariants(Cache, SharedData)
	...
	ProcedureRepresentation = NStr("en = 'Delete options of deleted reports'") + " (" + Suffix + ")";
	ProcedureLaunch(ProcedureRepresentation);
...
// Transfer option user settings from the corresponding storage.
...
// Generates subsystems tree by the option base data.
...
// Saves the placing settings changed by a user to a tabular section of report option.
//
// Parameters:
//   Form         - ManagedForm - Form where placement settings are stored.
//   VariantObject - CatalogObject.ReportsVariants, FormDataStructure - Report option object.
...
Procedure SubsystemsTreeWrite(Form, VariantObject, Cache = Undefined) Export
	...
	For Each Subsystem In SubsystemsChanges Do
		...
		If TabularSectionRow = Undefined Then
			// It is required to unconditionally register option placement setting (even the Usage disabled check box)
...
Function IndexSchemaContent(VariantObject) Export
	...
	// Receive report object, DLS settings and options.
	IsPredefined = False;
	...
	If TypeOf(VariantObject) = Type("CatalogObject.PredefinedReportsVariants") Then
		...
		If Found.Count() = 0 Then
			ErrorText = NStr("en = 'Option ""%1"" was not found for report ""%2""'");
		...
		// If option is disabled, then it does not take part in the search.
	...
	If DCSchema = Undefined Then
		If Not ValueIsFilled(VariantObject.FieldNames)
			...
			ErrorText = NStr("en = 'Search settings are not filled in for the ""%1"" option of the ""%2"" report: names of fields or parameters and filters.'");
...
Function DecreaseQuickSettingsQuantity(VariantObject, ReportObject) Export
	...
	If VariantObject = Undefined Then
		Return False; // The option does not exist in the base. Filling out is not required.
	EndIf;
	...
	DCSettings = VariantObject.Settings.Get();
	If TypeOf(DCSettings) <> Type("DataCompositionSettings") Then
		ErrorText = NStr("en = 'Empty settings of user option ""%1"" of report ""%2"" are detected.'");
		ErrorByVariant(VariantObject.Ref, ErrorText, VariantObject.VariantKey, VariantObject.Report);
		Return False; // Problem detected.
	EndIf;
...
//           Filled in with report options in which data all desired words are found.
...
//           All inserted report options should be output for such subsystems.
...
//               *** FoundWords - Array.
//       * VariantsConnectedToSubsystems - Map - Report options and their subsystems.
//           Filled in when some words are found in option data and the rest of them - in the names of its subsystems.
//           IN this case, the option should be displayed only in found subsystems (and in other - should not be shown).
//           Used in reports panel.
//           ** Key - CatalogRef.ReportsVariants - Option.
...
Function FindReferences(SearchParameters) Export
	...
	If FilterBySubsystemsExist Or HasSearchString Then
		...
		|		LEFT JOIN InformationRegister.ReportOptionSettings AS PersonalSettingsTip
		|		ON PlaceAll.Subsystem = PersonalSettingsTip.Subsystem
		|			AND PlaceAll.Ref = PersonalSettingsTip.Variant
		|			AND (PersonalSettingsTip.User = &CurrentUser)
		|		LEFT JOIN Variants AS Variants
		|		ON PlaceAll.Ref = Variants.Ref
		|WHERE
		|	ISNULL(PersonalSettingsTip.Visible, Variants.VisibleByDefault)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|SELECT
	...
	For IndexOf = 1 To Quantity Do
		...
		If IndexOf = Quantity Or TableRow.Ref <> Variant.Ref Then
			// Collected information about option analysis.
			AllWordsFound = True;
