...
Procedure WhenCompletingTablesOfParametersOfIB(Val ParameterTable) Export
	...
	If CommonUse.SubsystemExists("StandardSubsystems.SaaS") Then
		...
		ModuleSaaSOperations.AddConstantToInfobaseParameterTable(ParameterTable, "MaximumActiveBackgroundJobRunTime");
		ModuleSaaSOperations.AddConstantToInfobaseParameterTable(ParameterTable, "MaximumNumberOfActiveBackgroundJobs");
	EndIf;
...
#Region ProceduresAndFunctionsForWorkWithScheduledJobs
...
Function ExecuteScheduledJobManually(Val Task) Export
	...
	If LastBackgroundJobProperties <> Undefined
	   AND LastBackgroundJobProperties.State = BackgroundJobState.Active Then
		...
		EndIf;
	Else
		BackgroundJobDescription = StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'Start manually: %1'"), ScheduledJobPresentation(Task));
...
#Region ProceduresAndFunctionsForWorkWithBackgroundJobs
...
Procedure CancelBackgroundJob(ID) Export
	...
	Else
		Raise NStr("en = 'Background job is not found on the server.'");
	EndIf;
	
	If BackgroundJob.State <> BackgroundJobState.Active Then
		Raise NStr("en = 'Cannot complete the job, it cannot be canceled.'");
	EndIf;
