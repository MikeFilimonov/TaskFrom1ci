...
#Region ProceduresAndFunctionsOfTheOnlineWork
...
Procedure SetAdvancedApplicationCaption(OnLaunch = False) Export
	...
	Else
		CaptionPattern = "%1 / %2";
		ApplicationCaption = StringFunctionsClientServer.SubstituteParametersInString(CaptionPattern, 
			NStr("en = 'Separators are not set'"), ClientParameters.DetailedInformation);
	EndIf;
...
Procedure BeforeExit(Cancel = False, WarningText) Export
	...
	SetClientParameter("ShowWarningOnExit", Warnings);
...
Function ClientWorkParametersOnStart() Export
	Return StandardSubsystemsClientReUse.ClientWorkParametersOnStart();
EndFunction

Function ClientWorkParameters() Export	
	Return StandardSubsystemsClientReUse.ClientWorkParameters();	
EndFunction

#EndRegion

#Region SearchAndDeleteDuplicates
...
Function ApplicationStartComplete() Export
	
	ParameterName = "StandardSubsystems.ApplicationStartComplete";
...
Procedure AuthorizeOnUserSupportSite(FormOwner = Undefined, ClosingAlert = Undefined) Export
	...
	OpenForm("CommonForm.UserSupportAuthorization",, FormOwner,,,, ClosingAlert);
...
#Region ExecutionResultProcessor
...
Procedure ActionsBeforeSystemExit(Parameters) Export
		
	Parameters.Insert("ContinuationProcessor", Parameters.CompletionProcessor);
	
	ClientParameters = ClientWorkParameters();
	If ClientParameters.CanUseSeparatedData Then
		Try
...
Procedure ActionsBeforeExitCompletionProcessor(Notpecified, Parameters) Export
	
	If Not Parameters.Cancel AND Not Parameters.ContinuousExecution Then		
		ApplicationParameters.Insert("StandardSubsystems.SkipAlertBeforeExit", True);
		Exit();
	EndIf;
...
#Region OtherProceduresAndFunctionsToStartAndCloseTheApplication

// Checks a minimum available platform version for launch.
// If the platform version is earlier than RecommendedPlatformVersion, a warning is shown to the user. Exit the
// application if Exit = True.
//
// Return
...
Procedure PlatformVersionCheckOnlineProcessorOnStart(Parameters, NotSpecified) Export
	...
	Form = OpenForm("DataProcessor.PlatformUpdateTip.Form.PlatformUpdateTip", FormParameters,
...
Procedure SetSessionSeparation()
	...
	Except
		Raise
			NStr("en = 'The separator value in the LogOnDataArea parameter must be a number.'");
	EndTry;
...
Procedure OpenOnExitMessageForm(Parameters)
	...
	FormName = "CommonForm.WarningOnExit";
...
Procedure AskExitConfirmation(Parameters, ResponseProcessor) Export
	...
	ShowQuestionToUser(ResponseProcessor, NStr("en = 'Exit the application?'"), Buttons, QuestionParameters);
...
Procedure ProcessErrorOnStartOrEnd(Parameters, ErrorInfo, Event, StopWork = False)
	...
	WarningText = ErrorDescriptionBegin + Chars.LF
		+ NStr("en = 'Technical information on the error is recorded in the event log.'")
		+ Chars.LF + Chars.LF
