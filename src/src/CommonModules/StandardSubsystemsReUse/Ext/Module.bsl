	...
	For Each KeyAndValue In SubsystemDescriptions.ByNames Do
		...
		For Each RequiredSubsystem In KeyAndValue.Value.RequiredSubsystems Do
			...
			If Order < OrderRequiredSubsystem Then
				Interdependence = SubsystemDescriptions.ByNames[RequiredSubsystem].RequiredSubsystems.Find(Name) <> Undefined;
...
//                               example, Catalog.ProductsKinds, Only
...
#Region ReceiveIdentifierByTheMetadataObjectAndViceVersa
...
Function MetadataObjectID(FullMetadataObjectName) Export
	...
	If Exporting.Count() = 0 Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'An error occurred during the execution of CommonUse.MetadataObjectID() function.
			     |
			     |For the metadata object %1 an identifier is not found in the Metadata objects identifiers catalog.'")
			+ StandardSubsystemsServer.SpecificationOfErrorParametersWorkApplicationForDeveloper(),
			FullMetadataObjectName);
	ElsIf Exporting.Count() > 1 Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'An error occurred during the execution of CommonUse function.MetadataObjectID().
			     |
			     |For the metadata object %1 several identifiers are found in the Metadata objects identifiers catalog.'")
			+ StandardSubsystemsServer.SpecificationOfErrorParametersWorkApplicationForDeveloper(),
			FullMetadataObjectName);
	EndIf;
	...
	If CheckResult.NotCorresponds Then
		If CheckResult.MetadataObject = Undefined Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'An error occurred during the execution of CommonUse function.MetadataObjectID().
				     |
				     |For the metadata object %1 an identifier is found in the Metadata objects identifiers catalog that corresponds to the removed metadata object.'")
			+ StandardSubsystemsServer.SpecificationOfErrorParametersWorkApplicationForDeveloper(),
				FullMetadataObjectName);
		Else
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'An error occurred during the execution of CommonUse function.MetadataObjectID().
				     |
				     |For the metadata object %1 an identifier is found in the Metadata objects identifiers catalog that corresponds to another metadata object %2.'")
			+ StandardSubsystemsServer.SpecificationOfErrorParametersWorkApplicationForDeveloper(),
				FullMetadataObjectName,
...
#Region UseTheMetadataObjectsIDsCatalog
