...
#Region ParametersOfInfobaseAdministrationAndServersCluster
...
Procedure OnAddOfficeEvent(ClientEvents, ServerEvents) Export
	...
	If CommonUse.SubsystemExists("StandardSubsystems.ToDoList") Then
		...
		ModuleToDoListService.OnAddOfficeEvent(ClientEvents, ServerEvents);
	EndIf;
	
	If CommonUse.SubsystemExists("StandardSubsystems.DeleteMarkedObjects") Then
...
Procedure OnAddHandlersOfServiceEvents(ClientHandlers, ServerHandlers) Export
	...
	If CommonUse.SubsystemExists("StandardSubsystems.UserActivity") Then
	...
	If CommonUse.SubsystemExists("StandardSubsystems.Currencies") Then
		ModuleWorkWithExchangeRates = CommonUse.CommonModule("WorkWithExchangeRates");
		ModuleWorkWithExchangeRates.OnAddHandlersOfServiceEvents(ClientHandlers, ServerHandlers);
	EndIf;
	...
	If CommonUse.SubsystemExists("StandardSubsystems.PerformanceMeasurement") Then
	...
	If CommonUse.SubsystemExists("StandardSubsystems.SaaS.CurrenciesSaaS") Then
		...
		ModuleExchangeRatesServiceSaaS.OnAddHandlersOfServiceEvents(ClientHandlers, ServerHandlers);
	EndIf;
	...
	If CommonUse.SubsystemExists("StandardSubsystems.BackupInfobase") Then
...
#Region AdditionalBasicFunctionalityForAnalysisOfClientParametersOnServer
...
Function ClientParametersOnServer() Export
	...
	If ClientParameters.Count() = 0
	   AND CurrentRunMode() <> Undefined Then
		...
		Raise NStr("en = 'Client parameters on server are not entered.'");
	EndIf;
...
Function RunBackgroundJobWithClientContext(MethodName, Parameters = Undefined, Key = "", Description = "") Export
	...
	If CurrentRunMode() = Undefined Then
		Raise NStr("en = 'You can start background job with the customer context only when there is the customer.'");
	EndIf;
...
#Region AdditionalBasicFunctionalityOfTheInfobaseUpdate
...
#Region ProceduresForSettingupdatereceivingParametersOfApplicationWorkcaches
...
//                    received via the repeated use module to lock obtaining if not all  group parameters (constants)
//                    are updated. It is not required while receiving data in order to update it.
...
Procedure CheckForUpdatesApplicationWorkParameters(ConstantName, ParameterNames = "", Cancel = Undefined) Export
	...
	If ParameterNames <> "" Then
		...
		Else
			UpdateNeeded =
				UpdateResults.InfobaseUpdateRequired();
		EndIf;
		...
		If UpdateNeeded Then
			...
			If CurrentRunMode() = Undefined Then
				Raise
					NStr("en = 'Logon to the application is temporarily unavailable due to update to the newer version'");
			Else
				Raise
					NStr("en = 'Invalid access to non-updated application work parameters (for example, to some session parameters):
					     |- if they are accessed from the form on the initial page (desktop) , then you need to make sure there is the  procedure CommonUse.OnCreateAtServer  called in it.
					     |- otherwise, it is required to transfer the applied code call after the update of application work parameters.'");
			EndIf;
		EndIf;
	EndIf;
...
Procedure AddChangesToApplicationPerformenceParameters(ConstantName, ParameterName, Val ChangesParameter) Export
	...
	Try
		...
		If ValueIsFilled(ChangesParameter) Then
			...
			// next version. IN this case, updates executed not in the IB update will be considered during the transition to the
			// next change version.
			Version = Metadata.Version;
...
#Region AdditionalBasicFunctionalityForTheDataExchange
	...
	If EnableContent.Count() <> 0
		...
		If EnableContent.Count() <> 0 Then
			...
			DetailsExceptions1 = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'The following metadata objects should be included in exchange plan %1: %2'"),
				ExchangePlanName,
		...
		If ExcludeFromComposition.Count() <> 0 Then
			...
			DetailsExceptions2 = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'The following metadata objects should NOT be included in exchange plan %1: %2'"),
				ExchangePlanName,
...
Procedure RegisterObjectInAllNodes(Val Object, Val ExchangePlanName) Export
	...
	If CommonUseReUse.DataSeparationEnabled() Then
		...
		If CommonUseReUse.CanUseSeparatedData() Then
			Raise NStr("en = 'Changes registration of shared data in the split mode.'");
		EndIf;
		
		If Not CommonUseReUse.IsSeparatedMetadataObject("ExchangePlan." + ExchangePlanName,
				CommonUseReUse.MainDataSeparator())
			Then
			Raise NStr("en = 'Changes registration for undivided exchange plans is not supported.'");
		EndIf;
		
		If CommonUseReUse.IsSeparatedMetadataObject(Object.Metadata().FullName(),
				CommonUseReUse.MainDataSeparator())
			Then
			Raise NStr("en = 'Changes registration for divided objects is not supported.'");
		EndIf;
...
// Stores a reference to the main node in the MasterNode constant for the restoration possibility.
...
#Region HandlersOfSendingAndReceivingDataForExchangeInTheDistributedIB
...
#Region ServiceEventHandlersWithBasicFunctionality
...
//  DistributedInfobase - Boolean - (read only) Shows that objects for DIB exchange plan are received.
//                                              True - required to get the list of the exception objects of the DIB
//                                                     exchange plan;
//                                              False - it is required to receive a list for an exchange plan NOT RIB.
//
Procedure OnGettingObjectExceptionsOfExchangePlan(Objects, Val DistributedInfobase) Export
	...
	If DistributedInfobase Then
		...
		Objects.Add(Metadata.Constants.ApplicationTitle);
...
#Region AdditionalFunctionsForWorkWithTypes
...
#Region ProceduresAndFunctionsOfWorkWithForms
...
#Region ProceduresAndFunctionsOfWorkWithTheFileSystem
	...
	Except
		WriteLogEvent(
			...
			StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'An error occurred when cleaning up temporary file directory ""%1"":%2'"),
				PathToDirectory,
...
#Region OtherProcedureAndFunction
...
Procedure AddCommonClientParameters(Parameters) 
	...
	If Not Parameters.DataSeparationEnabled Or Parameters.CanUseSeparatedData Then
		...
		Parameters.Insert("ApplicationCaption", TrimAll(Constants.ApplicationTitle.Get()));
		SetPrivilegedMode(False);
...
Function EventsHandlers() Export
	...
	If AreRequiredEventsWithoutHandlers.Count() > 0 Then
		...
		Comment = NStr("en = 'Handlers are not defined for the following required events:'")
	...
	Try
		If CommonUse.IsSubordinateDIBNode() Then
			...
			If ValueIsFilled(CriticalChangesList) Then
				...
				If RunImport Then
					EventName = NStr("en = 'Metadata object IDs.Import critical changes'",
						CommonUseClientServer.MainLanguageCode());
				Else
					// Setting in the IB subordinate node during the first start.
					EventName = NStr("en = 'Metadata object IDs.It is required to execute critical changes'",
						CommonUseClientServer.MainLanguageCode());
				EndIf;
...
#Region ToUpdateTheMetadataObjectsIDsCatalog
...
Procedure MarkVersionCacheRecordsObsolete() Export
	...
	EndDo;
	UpdateResults.WriteData(RecordSet);
...
#Region ConfirmationOfApplicationEnding
...
Procedure DisablePredefinedItemsDeletionMarkupBeforeWriting(Source, Cancel) Export
	...
	If Source.IsNew() Then
		Raise
			NStr("en = 'Cannot create a predefined item marked for deletion.'");
	Else
...
#Region ProcessorOfSubscriptionsToDIBExchangePlansEvents
...
Procedure BeforeApplicationStart()
	...
	If Metadata.ScriptVariant <> Metadata.ObjectProperties.ScriptVariant.English Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Variant of the %1 ultimate fallback language of configuration is not supported.
			     |It is required to use %2 language.'"),
	...
	If CommonUseClientServer.CompareVersions(SystemInfo.AppVersion, MinimalPlatformVersion) < 0 Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'For launch 1C:Enterprise platform version %1 or higher is required.'"), MinimalPlatformVersion);
	EndIf;
	...
	If IsBlankString(Metadata.Version) Then
		Raise NStr("en = 'Property Version of the configuration is not entered.'");
	Else
		Try
...
#Region DeclarationOfEventsToWhichYouCanAddHandlers
...
Procedure WhenAddingOfficeEventBasicFunctionality(ClientEvents, ServerEvents) Export
	...
	//     box selection, then the information register record will be deleted at the same time the ref specified in the
	...
	//     attributes of which the ref of the deleted object is specified and select the method of ref clearing: clear
	//     attribute value, delete table row or delete the whole object.
	//
	// Parameters:
	...
	//                     ChartOfCalculationTypesObject - object of the predefined item after writing of which
	//                                                     nonuniqueness is found.
	//   WriteInJournal - Boolean - return value. If you specify False, then information about nonuniqueness will not be
	//                              added to the event log in general message.
...
#Region ForTheEventsHandlersFunction
...
#Region ForTheUpdateApplicationWorkParametersFuction
...
Procedure CheckUpdateApplicationWorkParameters(HasChanges, CheckOnly, WithoutChanges, InBackground)
	...
	If Not WithoutChanges.Property("UserSessionParameters") Then
		...
		If IsCurrentChanges Then
			...
			EndIf;
		Else
			WithoutChanges.Insert("UserSessionParameters");
		EndIf;
		If InBackground Then
...
#Region ForTheChangeApplicationFormParametersFunction
