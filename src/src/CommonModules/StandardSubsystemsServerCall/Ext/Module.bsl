...
Function WriteErrorToEventLogMonitorAtStartOrExit(StopWork, Val Event, Val ErrorText) Export
	...
	If Event = "Start" Then
		EventName = NStr("en = 'Start application'", CommonUseClientServer.MainLanguageCode());
		If StopWork Then
			ErrorDescriptionBegin = NStr("en = 'An exception is thrown when starting the application. Application launch is aborted.'");
		Else
			ErrorDescriptionBegin = NStr("en = 'An exception is thrown when starting the application.'");
		EndIf;
	Else
		EventName = NStr("en = 'Exit application'", CommonUseClientServer.MainLanguageCode());
		ErrorDescriptionBegin = NStr("en = 'An exception is thrown when closing the application.'");
	EndIf;
