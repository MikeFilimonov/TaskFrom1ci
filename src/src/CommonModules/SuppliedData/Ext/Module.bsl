...
//
// Can be used together with DataSuppliedFromManagerDescriptors for a manual data processor initialization. After the
// call, the system will behave as if it has just received a notification of the availability of new data with the
// specified handle - AvailableNewData will be called and later if needed, ProcessNewData for the corresponding handlers.
//
// Parameters:
...
//
// Data is saved either to the volume on disc, or to the SuppliedData table field depending on the
// StoreFilesInVolumesOnDisc constant and presence of the vacant volumes. Data can be extracted later using search by
// attributes or by specifying a unique identifier that is passed to the Descriptor.FileGUID field. If the base already
// contains data with the same data kind and key characteristics set - new data replaces the old one. Existing catalog
// item update is used not removal or creation of a new one.
//
// Parameters:
...
Function ProvidedDataFromCache(Val RefOrIdentifier) Export
	...
	Else
		...
		Except
			...
			WriteLogEvent(
				NStr("en = 'Supplied data.Receiving file from volume'", 
...
#Region UpdateInformationInTheDataAreas
...
Procedure OnAddOfficeEvent(ClientEvents, ServerEvents) Export
	...
	// 
	// IN case if AvailableNewData sets the argument to Import in value is true, the data is importing, the handle and the
	// path to the file with data pass to a procedure. ProcessNewData. File will be automatically deleted after procedure completed.
