...
// Announces service events of the ToDoList subsystem.
//
// Server events:
//   AtFillingToDoList.
//
...
Procedure OnAddOfficeEvent(ClientEvents, ServerEvents) Export
	...
	//
	ServerEvents.Add("StandardSubsystems.ToDoList\AtFillingToDoList");
...
Function UserToDoList(StorageAddress, ClientParametersOnServer = Undefined) Export
	...
	EventHandlers = CommonUse.ServiceEventProcessor(
		"StandardSubsystems.ToDoList\AtFillingToDoList");
	
	For Each Handler In EventHandlers Do
		Handler.Module.AtFillingToDoList(ToDoList);
	EndDo;
	
	// Adding works from applied configurations.
	WorkFillHandlers = New Array;
	ToDoListOverridable.AtDeterminingToDoListHandlers(WorkFillHandlers);
	
	For Each Handler In WorkFillHandlers Do
		Handler.AtFillingToDoList(ToDoList);
	EndDo;
	
	// Result postprocessing.
	ConvertToDoListTable(ToDoList);
	
	PutToTempStorage(ToDoList, StorageAddress);
...
Function SavedDisplaySettings() Export
	...
	DisplaySettings = CommonSettingsStorage.Load("ToDoList", "DisplaySettings");
...
//
Function ToDoListNumericIndicators(Query, CommonQueryParameters = Undefined) Export
...
//
Procedure ConvertToDoListTable(ToDoList)
	
	ToDoList.Columns.Add("IDOwner", New TypeDescription("String", New StringQualifiers(250)));
	ToDoList.Columns.Add("ThisIsSection", New TypeDescription("Boolean"));
	ToDoList.Columns.Add("PresentationOfSection", New TypeDescription("String", New StringQualifiers(250)));
	...
	For Each Work In ToDoList Do
		...
		Else
			ThisIsWorkIdentifier = (ToDoList.Find(Work.Owner, "ID") <> Undefined);
	...
	For Each ControllableWork In ControllableWorks Do
		ToDoList.Delete(ControllableWork);
	EndDo;
...
//                                        top level work, it is required to specify owner of a subsidiary work 
//                                        (description of the top level work).
//    * ToolTip String - The tooltip wording.
//
...
Function WorkDisabled(WorkIdentifier) Export
	...
	ToDoListOverridable.AtToDoListDisable(DisabledWork);
