...
Function ExecuteBroadcast(Val InitialObject, Val InitialVersionsDetails, Val DetailsOfResultingVersions) Export
	...
	If TransmissionChainOfInterface = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Handler of translation from version %1 to version %2 is not registered in the configuration.'"),
...
Function GetPackageDependencies(Val ObjectMessagePackage) Export
	...
	Result = New Array();
	PackageDependencies = XDTOFactory.packages.Get(ObjectMessagePackage).SubordinateDocumentStructure;
...
Function TransmitObject(Val Object, Val RulesBroadcastInterfaces)
	...
	If TransmissionChainOfInterface = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Cannot define a translation handler for package {%1}, cannot execute the standard translation to process this property'"), 
			SourceObjectPackage);
	EndIf;
	...
	Else
		...
		// the version is not changed, and it is necessary just to copy the values of the original object properties into the
		// resulting object.
...
Function StandardProcessing(Val Object, Val PackageResultObject, Val RulesBroadcastInterfaces)
	...
	Else
		...
		If ResultedObjectType = Undefined Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Cannot execute the standard translation processing of type %1 to package %2: type %1 does not exist in package %2.'"),
	...
	For Each Property In ResultedObjectType.Properties Do
		...
		If PropertyOfOriginal = Undefined Then
			...
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Cannot execute standard processing of conversion type %1 to type %2: property %3 is not defined for type %1.'"),
				"{" + SourceObjectType.NamespaceURI + "}" + SourceObjectType.Name,
	...
	For Each Property In SourceObjectType.Properties Do
		...
		If TranslatedProperty = Undefined Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Cannot execute standard processing of conversion type %1 to type %2: property %3 is not defined for type %2.'"),
				"{" + SourceObjectType.NamespaceURI + "}" + SourceObjectType.Name,
				"{" + ResultedObjectType.NamespaceURI + "}" + ResultedObjectType.Name,
				Property.LocalName);
		EndIf;
...
Function GetPackagesMessages(Val Message)
	...
	// SubordinateDocumentStructure of the XDTO-object package.
	SubordinateDocumentStructure = GetPackageDependencies(ObjectMessagePackage);
	CommonUseClientServer.SupplementArray(Result, SubordinateDocumentStructure, True);
