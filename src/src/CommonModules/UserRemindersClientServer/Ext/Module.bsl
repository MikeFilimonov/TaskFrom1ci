...
Function TimePresentation(Val Time, FullPresentation = True, OutputSeconds = True) Export
	...
	WeeksRepresentation		= NStr("en = 'week'")	+ "," + NStr("en = 'weeks'")		+ "," + NStr("en = 'weeks'");
		...
		For Position = 1 To StrLen(StringWithTime) Do
			...
			If (CurrentSymbolCode >= CharCode("0") AND CurrentSymbolCode <= CharCode("9"))
				...
				If Not IsBlankString(SubstringWithLetters) Then
					...
					StringForCalculation = StringForCalculation + "+" + ?(IsBlankString(SubstringWithNumbers), "1", SubstringWithNumbers)
...
Function ReplaceUnitDimensionsOnFactor(Val Unit)
	...
	If FirstThreeSymbols = NStr("en = 'week'") OR FirstThreeSymbols = NStr("en = 'w'") Then
		Result = 60*60*24*7;
	ElsIf FirstThreeSymbols = NStr("en = 'day'") 
		OR FirstThreeSymbols = NStr("en = 'days'")
		OR FirstThreeSymbols = NStr("en = 'd'")
		OR FirstThreeSymbols = NStr("en = 'da'") Then		
		
		Result = 60*60*24;
		
	ElsIf FirstThreeSymbols = NStr("en = 'hour'") OR FirstThreeSymbols = NStr("en = 'h'") Then
		Result = 60*60;
