...
Function GetStandardSchedulesToRemind() Export
	...
	Result.Insert(NStr("en = 'on Mondays at 9 a.m.'"), Schedule);
	...
	WeekDays.Add(5);
	Schedule.WeekDays = WeekDays;
	Result.Insert(NStr("en = 'on Fridays at 3 p.m.'"), Schedule);
...
Function GetReminderSettings() Export
	...
	Result.Insert("UseReminders", HasRightToUseNotifications() AND GetFunctionalOption("UseReminders"));
...
Procedure CheckDateChangesInSubject(Subject) Export
	...
	|WHERE
	|	Reminders.ReminderTimeSettingVariant = VALUE(Enum.ReminderTimeOptions.RelativelyToSubjectTime)
...
Procedure CheckDatesChangesInSubjectOnWrite(Source, Cancel) Export
	If Not CommonUse.ThisIsExchangePlan(Source.Metadata()) AND Source.DataExchange.Load Then 
		Return; 
	EndIf;
	If GetFunctionalOption("UseReminders") Then
...
Procedure DisableReminder(ReminderParameters, AttachOnSchedule = True) Export
	...
	If AttachOnSchedule AND Reminder <> Undefined Then
		If Reminder.ReminderTimeSettingVariant = PredefinedValue("Enum.ReminderTimeOptions.Periodically") Then
...
Function CreateReminder(ReminderParameters)
	...
	If Not ValueIsFilled(Reminder.ReminderTimeSettingVariant) Then
		If ValueIsFilled(Reminder.Source) AND Not IsBlankString(Reminder.SourceAttributeName) Then
			Reminder.ReminderTimeSettingVariant = Enums.ReminderTimeOptions.RelativelyToSubjectTime;
		Else
			Reminder.ReminderTimeSettingVariant = Enums.ReminderTimeOptions.InSpecifiedTime;
		EndIf;
	EndIf;
	
	If Reminder.ReminderTimeSettingVariant = Enums.ReminderTimeOptions.RelativelyToSubjectTime Then
	...
	ElsIf Reminder.ReminderTimeSettingVariant = Enums.ReminderTimeOptions.RelativelyToCurrentTime Then
		Reminder.ReminderTimeSettingVariant = Enums.ReminderTimeOptions.InSpecifiedTime;
