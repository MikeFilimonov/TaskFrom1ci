...
#Region ProceduresAndFunctionsUsedInManagedFormsItemsWork
...
//                  Parameter is ignored if FO UseUserGroups is disabled.
//
...
Procedure FillUserPictureNumbers(Val Table,
	...
	If TypeOf(Table) = Type("FormDataTree") Then
		...
		UsersTable.Columns.Add(FieldNameUser, Metadata.InformationRegisters.UserGroupMembers.Dimensions.UsersGroup.Type);
...
Function CreateAdministrator(IBUser = Undefined) Export
	...
	If IBUser = Undefined Then
		...
		If IBUser = Undefined Then
			...
			Unsafe = New UnsafeOperationProtectionDescription;
			Unsafe.UnsafeOperationWarnings = False;
			IBUser.UnsafeOperationProtection = Unsafe;
	...
	Else
		If Not IBUser.Roles.Contains(Metadata.Roles.FullRights)
		 OR Not IBUser.Roles.Contains(SystemAdministratorRole()) Then
...
#Region ProceduresAndFunctionsForWorkWithInfobaseUsers
		...
		If UpdatedProperties.Property("OldPassword")
		   AND TypeOf(UpdatedProperties.OldPassword) = Type("String") Then
			...
			If Not OldPasswordIsSame Then
				ErrorDescription = ErrorDescriptionOnIBUserWrite(
					NStr("en = 'When writing user of infobase %1, old password was specified incorrectly.'"),
					OldProperties.Name,
					OldProperties.UUID);
				Return False;
			EndIf;
...
Procedure FindAmbiguousInfobaseUsers(Val User,
	...
	ErrorDescription = NStr("en = 'Error in the data base:'") + Chars.LF;
		...
		If String.AmbiguousID <> CurrentAmbiguousID Then
			...
			Else
				...
				If CurrentInfobaseUser = Undefined Then
					NameForEntering = NStr("en = '<not specified>'");
				Else
...
//
Function ErrorDescriptionOnIBUserWrite(ErrorTemplate, NameForEntering, InfobaseUserID, ErrorInfo = Undefined, WriteInJournal = True)
	
	If WriteInJournal Then
		WriteLogEvent(
			NStr("en = 'Users. An error occurred when writing infobase user'",
			...
			StringFunctionsClientServer.SubstituteParametersInString(
				...
				+ ")",
				?(ErrorInfo = Undefined, "", DetailErrorDescription(ErrorInfo))));
	EndIf;
	
	Return StringFunctionsClientServer.SubstituteParametersInString(
		ErrorTemplate,
		"""" + NameForEntering + """",
		?(ErrorInfo = Undefined, "", BriefErrorDescription(ErrorInfo)));
