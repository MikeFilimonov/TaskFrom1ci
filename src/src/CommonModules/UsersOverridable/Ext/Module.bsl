...
//           CatalogRef.ExternalUserGroups - ref
...
Procedure OnReceiveOtherSettings(UserInfo, Settings) Export
	...
	If SettingValue <> Undefined Then
		...
		InformationAboutSetting.Insert("SettingName", NStr("en = 'Confirmation on closing the application'"));
...
Procedure LinkUserToEmployee(User) Export
	...
	LinkedUsersTable = DriveServer.GetUserEmployees(User);
	...
	If LinkedUsersTable.Count() = 0 Then
		...
		Else
			...
			NewEmployeeData.Insert("EmploymentContractType",				Enums.EmploymentContractTypes.FullTime);
			NewEmployeeData.Insert("SettlementsHumanResourcesGLAccount",	Catalogs.DefaultGLAccounts.GetDefaultGLAccount("PayrollPayable"));
			NewEmployeeData.Insert("AdvanceHoldersGLAccount",				Catalogs.DefaultGLAccounts.GetDefaultGLAccount("AdvanceHolders"));
			NewEmployeeData.Insert("OverrunGLAccount",						Catalogs.DefaultGLAccounts.GetDefaultGLAccount("AdvanceHoldersPayable"));
...
Procedure SwitchAllInTaxiInterface() Export
	...
	Except
		ErrorMessage = NStr("en = 'Switch users interface'", CommonUseClientServer.MainLanguageCode());
		WriteLogEvent(ErrorMessage, EventLogLevel.Error, , , ErrorDescription());
