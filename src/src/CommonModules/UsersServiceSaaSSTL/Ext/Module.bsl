#Region WorkWithUndividedUsersOfTheInfobase
...
#Region HandlersOfTheUsersSSLSubsystemEvents
...
#Region HandlersOfTheSaaSSSLSubsystemEvents
...
//      module of a handler, exported procedure
...
//          handler was called. Value that is passed to the BeforeObjectExport() procedure
...
//    BeforeImportType - Boolean, flag showing that a handler is to be called before importing all data objects that
//        belong to the metadata object. If the True value is set - in a general module of the handler, exported
...
//        BeforeImportType()- importing of all the data objects corresponding to the current metadata object will not be
//        executed.
...
//          is logically inextricably associated with the data object, but not being part of it. Created in exported
//          procedures BeforeObjectExport() of data export handlers (see a comment to procedure OnDataExportHandlersRegistration(). Each artifact must be the XDTOobject for which type, as the base type, abstract
//          XDTOtype is used {http://www.1c.ru/1cFresh/Data/Dump/1.0.2.1}Artefact. It is
...
Procedure GetUserFormProcessing(Source, FormKind, Parameters, SelectedForm, AdditionalInformation, StandardProcessing) Export
	...
	If FormKind = "ObjectForm"
		...
		If Not Query.Execute().IsEmpty() Then
			StandardProcessing = False;
			SelectedForm = Metadata.CommonForms.UserInfoUnavailableTip;
			Return;
		EndIf;
		...
		If CommonUseReUse.CanUseSeparatedData() Then
			...
			If Not UserRegisteredAsUnseparated(UserID) Then
				...
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'User with ID %1 is not registered as an undivided one.'"),
					String(UserID)
				);
...
Function ExceptionTextUnseparatedUsersWriteProhibited()
	...
	Return NStr("en = 'It is prohibited to write undivided users when the separator use is enabled.'");
	
EndFunction

#EndRegion

#Region IntegrationWithTheUsersSSLSubsystem
