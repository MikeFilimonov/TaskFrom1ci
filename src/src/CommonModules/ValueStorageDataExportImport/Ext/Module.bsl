...
//      object of the infobase. If the True value is set - in the general module of a handler, exported procedure must
//      be implemented BeforeObjectExport() that supports the following parameters:
//        Container - DataProcessorObject.DataExportImportContainerManager - container manager that is used in the
//          process of data export. See more the review to the application data processor interface DataExportImportContainerManager,
...
//          If a random exporting handler requires additional data export - Yo ushould use SerializerXDTO that is passed
//          to the procedure AfterExportType() as the parameter value of Serializer rather
//          than received by using the global context properties SerializerXDTO,
...
//      the data of which the registered handler must be called,
//     Handler - CommonModule, a common module in which a random handler of the data import is implemented. Set of
//      export procedures that must be implemented in the handler, depends on
//      the set values of the following table columns,
//     Version - String - Interface version number of exporting/importing
//      data handlers, supported by handler 
...
//                    IB, Remaining columns of equal fields of natural object key that were passed to the
//                    DataExportImportInfobase.RequiredMatchRefsOnImport() function during data export Function's return
//                    value MatchRefs() - ValueTable, columns:
...
//          It is allowed to use XDTOpackages in addition to the originally supplied content DataExportImport subsystem.
//        Cancel - Boolean. If you set this parameter value to True in the BeforeObjectImport() procedure - Import of
...
Procedure BeforeObjectExport(Container, ObjectExportManager, Serializer, Object, Artifacts, Cancel) Export
	...
	If AttributesWithValuesStorage = Undefined Then
		...
		Raise ServiceTechnologyIntegrationWithSSL.PlaceParametersIntoString(
			NStr("en = 'Metadata object %1 cannot be processed by handler ValueStorageDataExportImport.BeforeObjectExport().'"),
...
#Region DataExportOfTheValuesStorages
...
#Region DataImportOfTheValuesStorages
...
Procedure BeforeUpdateConstant(Container, Object, Artifact)
	...
	Else
		...
		Raise ServiceTechnologyIntegrationWithSSL.PlaceParametersIntoString(
			NStr("en = 'Type of owner {%1}%2 cannot be used for metadata object %3.'"),
...
Procedure ImportValueStorage(Container, ValueStorage, Artifact)
	...
	Else
		...
		Raise ServiceTechnologyIntegrationWithSSL.PlaceParametersIntoString(
			NStr("en = 'Unexpected placement type of value storage data in export container: (%1)%2.'"),
...
#Region FunctionsDefiningXDTOObjectsTypes
