...
#Region FunctionsconstructorsOfPermissions
//
...
//  in the specified directory of file system, Description - String - Description of a reason for which it is required
...
//  data to the directory of temporary files, Description - String - Description of a reason for which it is required to
//  grant a permission.
//
// Returns:
//  XDTODataObject - internal description of requested permission.
...
#Region FunctionsconstructorsOfQueriesForExternalResourcesUse
//
...
//  to an item of the MetadataObjectsIDs catalog, ReplacementMode - Boolean - defines the substitution mode of all
//  previously given permissions for this owner. If the parameter value equals to True, the clearance of all permissions
//  previously requested for the same owner will be added to the query in addition to the requested permissions.
//
// Returns:
...
//    etc. IN case if the permission is logically isolated (for example, canceled permissions are regulated by constant
//    value with Boolean type) - it is recommended to use reference
//  to an item of the catalog MetadataObjectsIdentifiers, CanceledPermissions - Array(XDTOObject) - array of XDTOObjects
//  corresponding to the internal descriptions of canceled permissions for access to the external resources. It is
//  assumed that all XDTOObjects transferred as parameters are generated by calling the functions WorkInSafeMode.Permission*().
//
// Returns:
//  UUID, reference to query for permissions recorded in IB. After
//  creation of all requests on permissions change, it is required to apply the requested changes by calling the procedure.
...
//    resources depending on utilized exchange transport) are logically associated with relevant nodes of exchange plans,
//    etc. IN case if the permission is logically isolated (for example, canceled permissions are regulated by constant
//    value with Boolean type) - it is recommended to use reference to an item of the catalog MetadataObjectsIdentifiers.
//
// Returns:
//  UUID, reference to query for permissions recorded in IB. After
//  creation of all requests on permissions change, it is required to apply the requested changes by calling the procedure.
...
Procedure ExecuteConfigurationMethod(Val MethodName, Val Parameters = Undefined) Export
	...
	Except
		ErrorInfo = ErrorInfo();
		Raise NStr("en = 'An error occurred when calling ExecuteConfigurationMethod procedure, WorkInSafeMode common module.'")
	...
	If Parameters <> Undefined AND Parameters.Count() > 0 Then
		For IndexOf = 0 To Parameters.UBound() Do 
			// The string has been edited to run the application in Arabic
			//ParametersString = ParametersString + "Parameters[" + IndexOf + "],";
			ParametersString = ParametersString + "Parameters[" + Format(IndexOf, "L=en_US; NZ=0") + "],";
		EndDo;
...
Procedure ExecuteObjectMethod(Val Object, Val MethodName, Val Parameters = Undefined) Export
	...
	Except
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Invalid value of MethodName parameter (%1)'"),
			MethodName);
	EndTry;
...
Procedure ValidateConfigurationMethodName(Val MethodName) Export
	...
	If NameParts.Count() <> 2 AND NameParts.Count() <> 3 Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Invalid format of MethodName parameter (%1)'"),
			MethodName);
	EndIf;
	...
	If NameParts.Count() = 3 Then
		...
		If Manager = Undefined Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Incorrect format of the parameter MethodName (%1):
				     |manager of the object ""%2"" is not found.'"),
				MethodName,
				FullObjectName);
		EndIf;
	EndIf;
	...
	Except
		WriteLogEvent(NStr("en = 'Method is safely performed'", 
			CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Error, , , DetailErrorDescription(ErrorInfo()));
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Incorrect format of parameter MethodName (%1): 
			     |Method name ""%2"" does not correspond to requirements of name generation for procedures and functions.'"),
			MethodName,
			ObjectMethodName);
	EndTry;
...
// for which it is impossible to execute handlers of session parameters setup and the list of actions that can be taken
	...
	If GetFunctionalOption("SecurityProfilesAreUsed") AND ValueIsFilled(InfobaseProfile) Then
		...
		If SafeMode() <> InfobaseProfile Then
			...
			Else
				...
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'Cannot execute handlers of session parameters setup due to:
					     |security profile %1 is absent in servers cluster of 1C:Enterprise,
					     |or it is prohibited to use it as security profile of safe mode.
					     |
					     |For restoration of the application it is required to disable the use of security profile
					     |through the cluster console and reconfigure security profiles using configuration interface
					     |(corresponding commands are located in the section of application settings'"),
					InfobaseProfile);
		...
		If Not PrivilegedModeAvailable Then
			...
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'It is impossible to execute handlers of session parameters setup due to:
				     |security profile %1 does not contain a permission to set privileged mode.
				     |It may have been edited through the cluster console.
				     |
				     |For restoration of the application it is required to disable the use of security profile
				     |through the cluster console and reconfigure security profiles using configuration interface
				     |(corresponding commands are located in the section of application settings'"),
		...
		Except
			...
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Cannot execute handlers of session parameters setup due to: %1.
				     |
				     |Maybe a security profile was set for infobase through the cluster console which does not allow to execute external modules without setup of safe mode.
				     |In this case, to restore application operation, it is required to disable the use of security profile through the cluster console and reconfigure security profiles using configuration interface (corresponding commands are located in the section of application settings).
				     |At the same time, the application will be automatically configured for shared use with enabled security profiles.'"),
...
// Returns the control amounts of the files belonging to the external component kit which is delivered in the
...
Function ObjectManagerByName(Name)
	...
	Raise StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = 'Cannot receive manager for object ""%1""'"),
		Name);
...
Function PossibleToExecuteSessionParametersSetupHandlersWithoutSafeModeInstallation() Export
	...
	Except
		...
		WriteLogEventTemplate = NStr("en = 'An error occurred while setting session parameters:
		                             |-------------------------------------------------------------------------------------------- 
		                             |%1
		                             |--------------------------------------------------------------------------------------------
		                             |Application start will be unavailable.'");
		...
		WriteLogEvent(
			NStr("en = 'Set session parameters'",
			CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Error,
			,
			,
