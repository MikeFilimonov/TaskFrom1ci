...
Function ExternalComponentsKitFilesControlSums(Val TemplateName) Export
	...
	Else
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Failed to generate a permission to use an external component:
	...
	If Template = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Failed to generate the permission to use the external component
			     |delivered with the template 1%:
			     |template %1 is not found in the configuration content!'"),
			TemplateName);
	EndIf;
	
	If Metadata.FindByFullName(TemplateName).TemplateType <> Metadata.ObjectProperties.TemplateType.BinaryData Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Failed to generate a permission for the use of the external component:
			     |template %1 does not contain binary data.'"),
			TemplateName);
	EndIf;
	...
	If IsBlankString(ManifestFile) Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Failed to generate the permission to use
			     |the external component delivered with the template %1:
			     |file MANIFEST.XML is not found in the archive!'"),
			TemplateName);
	EndIf;
...
#Region CreatingPermissionsQueries
//
...
//    for which permissions are requested. (Undefined when the permissions for a configuration are requested, not for
//    external modules).
//
...
//  be used for connection of an external module. If the connection mode is not registered for external module - returns
//  Undefined.
//
...
#Region ControlOfServiceDataRecording
//
...
Procedure OnWriteServiceData(Object) Export
	...
	If WorkInSafeMode.SafeModeIsSet() Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Writing of object %1 is unavailable: safe mode is set: %2.'"),
...
#Region DeclarationOfApplicationEventsHandlers
//
...
#Region DeclarationOfApplicationEvents
//
...
Procedure OnAddOfficeEvent(ClientEvents, ServerEvents) Export
	...
	//  until the end of the permissions application, ClosingAlert - AlertDetails, which will be called when permissions
	//  are successfully got.
	//  StandardProcessing - Boolean, a flag showing that standard permissions processor is applied to use external
	//      resources (connection to a server agent through COM-connection or administration server with querying the
	//      cluster connection parameters from the current user). Can be set to the
