...
Function ExternalModuleExecutionMode(Val ExternalModule) Export
	...
	If CommonUseReUse.DataSeparationEnabled() Then
		...
		KeyStructure = RegisterKeyByLink(ExternalModule);
		...
		If CommonUseReUse.CanUseSeparatedData() Then
			...
			Mode = InformationRegisters.ExternalModulesConnectionModesDataAreas.ExternalModuleExecutionMode(
				KeyStructure.Type, KeyStructure.ID);
...
Function AuthorizationKey(Val Resolution) Export
	...
	If StrLen(AuthorizationKey) > 32 Then
		Raise NStr("en = 'Key length excess'");
	EndIf;
...
Function PermissionsTableRow(Val PermissionTable, Val Filter, Val AddWithout = True) Export
	...
	Else
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Violation of row uniqueness in the permission table by filter %1'"),
...
//    corresponding to the external module, information about previously granted permissions on which it is required to
//    clear. If the parameter value is not set - information about granted permissions will be blocked by all external modules.
// LockExternalModulesConnectionModes - Boolean, a flag showing that it is required to additionally lock the connection
//                                      modes of external modules.
//
...
//  current cut will be collapsed by owners, WithoutDescriptions - Boolean - if True - cut will be returned with cleared
//  field Description of permissions.
//
...
Procedure AddPermission(Val SoftwareModuleType, Val SoftwareModuleID, Val OwnerType, Val IDOwner, Val AuthorizationKey, Val Resolution, Val AuthorizationAdding = Undefined) Export
	...
	If Manager.Selected() Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Duplication of permissions by key fields:
			     |- SoftwareModuleType: %1
			     |- SoftwareModuleIdentifier: %2
			     |- OwnerType: %3
			     |- OwnerIdentifier: %4
			     |- PermissionKey: %5.'"),
...
Procedure DeletePermission(Val SoftwareModuleType, Val SoftwareModuleID, Val OwnerType, Val IDOwner, Val AuthorizationKey, Val Resolution) Export
	...
	If Manager.Selected() Then
		...
		If Manager.AuthorizationBody <> CommonUse.ObjectXDTOInXMLString(Resolution) Then
			...
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Collection of permissions by the keys:
				     |- SoftwareModuleType: %1
				     |- SoftwareModuleIdentifier: 2%
				     |- OwnerType: %3
				     |- OwnerIdentifier: %4
	...
	Else
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Attempt to delete a nonexistent permission:
			     |- SoftwareModuleType: %1
			     |- SoftwareModuleIdentifier: 2%
			     |- OwnerType: %3
			     |- OwnerIdentifier: %4
...
Function PackageProcessingResult(Val PackageIdentifier) Export
	...
	Else
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Query pack %1 is not found'"),
			PackageIdentifier);
...
Procedure SetPackageProcessingResult(Val Result) Export
	...
	While Selection.Next() Do
		...
		If StandardProcessing Then
			...
			Else
				...
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'Unserialized key program module: - Type: 1% - ID: %2'"),
					Selection.SoftwareModuleType,
					Selection.SoftwareModuleID);
