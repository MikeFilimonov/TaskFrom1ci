...
// of days, included in the schedule, If the selected schedule is not filled and RaiseException = False, then it returns
// Undefined.
//
Function DatesOnSchedule(Val WorkSchedule, Val DateFrom, Val DaysArray, Val CalculateNextDateFromPrevious = False, CallingException = True) Export
	...
	While Selection.Next() Do
		If Selection.DateInCalendar = Undefined Then
			ErrorInfo = NStr("en = 'The ""%1"" work schedule is not filled in from date %2 for the specified number of working days.'");
...
Procedure CreateTSchedulesOfWorkOnPeriod(TempTablesManager, Graphics, StartDate, EndDate) Export
	...
	|	CASE
	|		WHEN BusinessCalendarData.DayKind = VALUE(Enum.BusinessCalendarDayType.Preholiday)
	|			THEN TRUE
	|		ELSE FALSE
	|	END AS HolidayDay
	...
	|			AND (BusinessCalendarData.DayKind = VALUE(Enum.BusinessCalendarDayType.Preholiday)
...
// Create a temporary table VTCalendarSchedules containing the data of schedule WorkSchedule for the years that are
// specified in the TTVariousYearsGraphics.
//
// Parameters:
