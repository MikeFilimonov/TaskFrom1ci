...
#Region EventHandlersOfTheUserListTabularSectionAttributes
...
Procedure EndSession(Command)
	...
	If SelectedRowsQuantity = 0 Then
		...
		ShowMessageBox(,NStr("en = 'Users for ending sessions are not selected.'"));
		Return;
...
Procedure AfterSessionEnd(Result, AdditionalParameters) Export
	...
	If Result = DialogReturnCode.OK Then
		...
		If AdditionalParameters.Property("SessionNumbers")
			...
			NotificationText = NStr("en = 'Sessions %1 are ended.'");
			...
			NotificationText = StringFunctionsClientServer.SubstituteParametersInString(NotificationText,
				SessionNumbers);
			ShowUserNotification(NStr("en = 'End sessions'"),, NotificationText);
			
		Else
			
			NotificationText = NStr("en = 'Session %1 is ended.'");
			NotificationText = StringFunctionsClientServer.SubstituteParametersInString(NotificationText,
			AdditionalParameters.SessionNumbers);
			ShowUserNotification(NStr("en = 'End session'"),, NotificationText);
