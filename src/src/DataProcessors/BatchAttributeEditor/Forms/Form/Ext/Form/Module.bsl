...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Else
		Title = NStr("en = 'Bulk attribute editing'");
	EndIf;
...
Procedure ChoiceProcessing(ValueSelected, ChoiceSource)
	...
	If Upper(ChoiceSource.FormName) = Upper("DataProcessor.BatchAttributeEditor.Form.AdditionalParameters") Then
...
Procedure Change(Command)
	...
	If ButtonFunction = "Change" Then
		If ObjectCountForDataProcessors = 0 Then
			ShowMessageBox(, NStr("en = 'Items for change are not specified'"));
			Return;
		EndIf;
		...
		Else
			...
			ShowQueryBox(NotifyDescription, QuestionText, QuestionDialogMode.OKCancel, , , NStr("en = 'Change items'"));
		EndIf;
...
Procedure PerformChangingSelectionCheckIsDone(QuestionResult = Undefined, AdditionalParameters = Undefined) Export
	...
	If Not ThereAreConfiguredChanges() Then
		QuestionText = NStr("en = 'Changes are not set. Rewrite the items without changes?'");
...
Procedure FulfillActionsAtContextOpening()
	...
	For Each TransferedObject In Parameters.ObjectsArray Do
		If TypeOf(TransferedObject) <> TransferedObjectType Then
			Raise NStr("en = 'Bulk edit is only for items of one type.'");
		EndIf;
	EndDo;
	
	ObjectCount    = Parameters.ObjectsArray.Count();
	CaptionPattern       = NStr("en = 'Change selected items ""%1"" (%2)'");
...
Procedure AddChangeToHistory(StructureChanges, ChangePresentation)
	...
	Settings = CommonSettingsStorageImport(
		"GroupObjectsChange", 
		"ChangeHistory/" + ChangingObjectKind);
	...
	CommonSettingsStorageSave("GroupObjectsChange", "ChangeHistory/" + ChangingObjectKind, Settings);
...
Procedure ImportHistoryOfOperations()
	...
	ChangeHistory = CommonSettingsStorageImport("GroupObjectsChange", "ChangeHistory/" + ChangingObjectKind);
...
Procedure QuestionGoToToAttributeUnblocking()
	...
	QuestionText = NStr("en = 'The attribute has been locked. Unlock attributes?'");
...
Procedure SetButtonsDuringModification(BeginChanges)
	...
	Else
		If ObjectsFailedToChange.Count() > 0 Then
			Items.FormChange.Title = NStr("en = 'Change again'");
		Else
...
Procedure ChangeObjects()
	...
	If Object.ChangeInTransaction Then
		...
		If TPAboutPortionSetting = 1 Then // processing by one call
			...
			ShowUserNotification(NStr("en = 'Change selected items'"), ,NStr("en = 'Please wait. Data processor may take some time...'"));
	...
	Else
		...
		Status(NStr("en = 'Processing items...'"), 0, NStr("en = 'Change selected items'"));
...
Procedure ProcessChangeResult(ResultOfChange = Undefined, ContinueProcessing = Undefined)
	...
	While ContinueProcessing Do
		...
		If Object.ChangeInTransaction Then
			WarningText = NStr("en = 'Errors occurred while editing the items. The changes were canceled.'");
		...
		ShowQueryBox(NotifyDescription, QuestionText, Buttons, , DialogReturnCode.Abort, NStr("en = 'Errors occurred when changing items'"));
		Return;
	EndDo;
	...
	If CurrentStateChanges.ShowPercentOfProcessed Then
		...
		Status(NStr("en = 'Processing items...'"), CurrentPercent, NStr("en = 'Change selected items'"));
	EndIf;
...
Procedure FinishChangeOfObjects()
	...
	If DataProcessorCompleted Then
		ShowUserNotification(NStr("en = 'Change item attributes'"), , 
			PlaceParametersIntoString(NStr("en = 'Items are changed (%1).'"), CurrentStateChanges.CountOfChanged));
		GoToPageAllDone();
		Return;
	EndIf;
	...
	If DataProcessorCompleted Then
		MessagePattern = NStr("en = 'Changes are made in all selected items (%2).'");
	Else
		If Object.ChangeInTransaction Or CurrentStateChanges.CountOfChanged = 0 Then
			MessagePattern = NStr("en = 'Not changed.'");
		Else
	...
	If Object.ChangeInTransaction AND Not DataProcessorCompleted Then
		...
		If SkippedQuantity > 0 AND Not CurrentStateChanges.InterruptChange Then
			...
			TableRow.Cause = NStr("en = 'Skipped as one or more items were not changed.'");
		EndIf;
	EndIf;
...
Function ChangeAtServer(Val StopChangingAtError)
	...
	Else
		JobDescription = NStr("en = 'Bulk edit of items'");
...
Function BlockedAttributes()
	...
	Else	
		...
		// IN configurations without SSL or for old SSL versions you shall try to determine whether there are lockable 
		// attributes in the object ("No-edit order for the object attributes").
		ObjectManager = ObjectManagerByFullName(ChangingObjectKind);
...
Function NonEditableAttributes()
	...
	If SSLVersionMeetsRequirements Then
		...
		EndIf;
	Else
		// For configurations without SSL or SSL in old versions try to determine whether there are noneditable attributes of
		// the object.
		ObjectManager = ObjectManagerByFullName(ChangingObjectKind);
		Try
			NotEditable = ObjectManager.NotEditableInGroupProcessingAttributes();
...
Procedure GenerateConfiguredChangesExplanation()
	...
	ElsIf ChangeableAttributes.Count() > 1 Then
		Explanation = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'attributes %1'"),
			Explanation);
	EndIf;
	
	If Not IsBlankString(Explanation) Then
		Explanation = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = '%1 in the selected items'"),
			Explanation);
	EndIf;
	...
	For Each TabularSection In ChangeableTableParts Do
		...
		If ChangeableAttributes.Count() > 3 Then
			...
			Explanation = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = '%1 attributes (%2)'"),
				Explanation,
				ChangeableAttributes.Count());
		...
		Explanation = StringFunctionsClientServer.SubstituteParametersInString( 
			NStr("en = '%1 in the ""%2"" tabular section'"),
			Explanation, 
			TabularSection.Key);
	EndDo;
	...
	If Not IsBlankString(Explanation) Then
		If ChangeableTableParts.Count() > 0 Then
			If FilterByStrings Then 
				Explanation = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = '%1 in the lines of the selected items that satisfy the filter conditions'"),
					Explanation);
			Else
				Explanation = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = '%1 in all lines of the selected items'"),
					Explanation);
			EndIf;
		EndIf;
	EndIf;
	...
	If ThereAreSelectedObjects Then
		...
		EndIf;
	Else
		Explanation = NStr("en = 'Items which attributes shall be changed are not selected.'");
	EndIf;
	...
	If ThereAreCustomizedFilters() Then
		...
		If IsBlankString(ErrorMessageText) Then
			LabelText = PlaceParametersIntoString(
				NStr("en = 'Selected items: %1'"), SelectedObjectsCount);
		Else
...
Procedure ConfigureChangeSetting(Val Setting)
	...
	If IsBlocked Then
		ShowMessageBox(, NStr("en = 'Some attributes are locked for editing. Changes were not defined.'"));
	EndIf;
		...
		Else
			...
			Else
				Raise PlaceParametersIntoString(
					NStr("en = 'Unknown metadata object type ""%1""'"), DescriptionFull);
			EndIf;
		EndIf;
