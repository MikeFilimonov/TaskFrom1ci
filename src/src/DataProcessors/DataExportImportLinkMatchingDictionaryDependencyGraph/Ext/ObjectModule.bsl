...
Function MetadataObject(Val DescriptionFull)
	...
	If MetadataObject = Undefined Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Metadata object %1 existing in the data file is missing in the current configuration.'"),
			DescriptionFull
		);
...
Function Vertex(Val MetadataObject, Val ExceptionIfNotExist = True)
	...
	ElsIf Rows.Count() = 0 Then
		...
		If ExceptionIfNotExist Then
			...
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'There is no vertex for metadata object %1 in the column.'"),
	...
	Else
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Unique column violation for the metadata object %1.'"),
			MetadataObject.FullName());
