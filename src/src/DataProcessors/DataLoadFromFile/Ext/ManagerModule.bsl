...
//                                                     of data match(optional parameter, default value is formed -
...
//     *ObjectName                               - String - Object Name.
//
Function LoadFromFileParametersExternalProcessing(CommandName, RefToDataProcessor, TemplateNameWithTemplate = Undefined) Export
	TemplateMandatoryColumns = New Array;
...
Procedure CreateCatalogsListForImport(CatalogsListForImport) Export
	...
	If CommonUse.SubsystemExists("StandardSubsystems.AdditionalReportsAndDataProcessors") Then
		...
		Query.SetParameter("Type", Enums.AdditionalReportsAndDataProcessorsTypes.AdditionalInformationProcessor);
	...
	For Each Attribute In CatalogMetadata.Attributes Do
		...
		ElsIf Attribute.Type.ContainsType(Type("Number")) Then 
			ColumnTypeDescription =  StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Digit, Length: %1, Accuracy: %2'"),
		...
		ElsIf Attribute.Type.ContainsType(Type("String")) Then
			If Attribute.Type.StringQualifiers.Length > 0 Then
				...
				ColumnTypeDescription =  StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'String, maximum characters: %1'"), StringLength);
			Else
				ColumnTypeDescription = NStr("en = 'String of unlimited length'");
			EndIf;
...
Procedure CreateColumnStandardAttributes(InformationByColumns, CatalogMetadata, ColumnName, Position)
	...
	If DataType = Type("String") Then 
		TypePresentation = StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'Row (no more than %1 characters)'"), DescriptionOfType.StringQualifiers.Length);
...
Procedure InitializeImportToTabularSection(TabularSectionFullName, TemplateNameWithTemplate, InformationByColumns, TemplateWithData, AdditionalParameters, Cancel) Export
	...
	If StructureObjectName.ObjectType = "Document" Then
		...
		Else
			Raise NStr("en = 'Template for data import from file is not found'");
			Cancel = True;
			Return;
		EndIf;
...
Procedure ImportCSVFileToTable(FileName, TemplateWithData, InformationByColumns) Export
	...
	If String = Undefined Then 
		MessageText = NStr("en = 'Cannot import data from this file. Make sure that data in the file is correct.'");
		Return;
	EndIf;
	
	ListSeparator = DriveServer.GetListSeparator(String);
	ColumnsHeaders = StringFunctionsClientServer.SplitStringIntoWordArray(String, ListSeparator);
	...
	While String <> Undefined Do
		NewRow = Source.Add();
		Position = Find(String, ListSeparator);
		IndexOf = 0;
		While Position > 0 Do
...
Procedure SaveTableToCSVFile(PathToFile, InformationByColumns) Export
	...
	TitleFormatForCSV = "";
	ListSeparator = DriveServer.GetListSeparator();
	
	For Each Column In InformationByColumns Do 
		TitleFormatForCSV = TitleFormatForCSV + Column.ColumnPresentation + ListSeparator;
	EndDo;
...
Procedure WriteMatchedData(ExportParameters, StorageAddress) Export
	...
	For Each TableRow In MatchedData Do 
		...
		Try
			...
			Else
				...
				If CatalogItem = Undefined Then
					...
					NStr("en = 'No products with SKU %1.'"), TableRow.SKU);
					Raise MessageText;
				EndIf;
			EndIf;
		...
		Except
			RollbackTransaction();
			TableRow.RowMatchResult = "Skipped";
			TableRow.ErrorDescription = NStr("en = 'Cannot write due to incorrect data'");
		EndTry;
...
Procedure GenerateReportTemplate(TableReport, TemplateWithData)
	...
	FillCellTemplateTitle(Cell, NStr("en = 'Result'"), 12, NStr("en = 'Data import result'"), True);
