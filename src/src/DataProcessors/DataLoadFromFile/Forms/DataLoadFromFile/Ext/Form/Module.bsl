...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	ElsIf Not Users.InfobaseUserWithFullAccess() Then
		Raise(NStr("en = 'Insufficient rights to open data import from file'"));
	Else
		Raise NStr("en = 'You can not open the data processor without context.'");
	EndIf;
	...
	If WebClient Then
		...
		Items.ExplanationForImportCatalogSelection.Title = NStr("en = 'Select a catalog to import data from spreadsheets located in external files (for example: Microsoft Office Excel, OpenOffice Calc, etc.).'");
		DataImportKind = 1;
	EndIf;
...
Procedure CancelMapping(Command)
	...
	ShowQueryBox(Notification, NStr("en = 'Cancel mapping?'"), QuestionDialogMode.YesNo);
EndProcedure
...
Procedure Back(Command)
	...
	If Items.AssistantPages.CurrentPage = Items.FillingTablesWithData Then
		...
		CommonUseClientServer.SetFormItemProperty(Items, "AnotherWayToImportData", "Visible", True);
		ThisObject.Title = NStr("en = 'Data import to catalog'");
		ClearTable();
	ElsIf Items.AssistantPages.CurrentPage = Items.ExportedDataComparison OR Items.AssistantPages.CurrentPage = Items.NotFound Then
		...
		If ImportType = "TabularSection" OR ImportType = "InsertionFromClipboard" Then
			...
			CommonUseClientServer.SetFormItemProperty(Items, "AnotherWayToImportData", "Visible", True);
		EndIf;
		Items.Back.Visible = (NOT VariantListImports.Count() = 0);
...
Procedure ExportTemplateToFile(Command)
	...
	If AttachFileSystemExtension() Then
		...
		If ValueIsFilled(SelectedFile.Name) Then
			...
			Else
				ShowMessageBox(, NStr("en = 'File template was not saved.'"));
			EndIf;
		EndIf;
	Else
...
&AtClient
Procedure BatchAttributeEditor(Command)
	...
	If CommonUseClient.SubsystemExists("StandardSubsystems.GroupObjectsChange") Then
		...
		If RefArray.Count() > 0 Then
			...
			OpenForm(ObjectName + "BatchAttributeEditor.Form", FormParameters);
		EndIf;
	EndIf;
	...
	ElsIf Items.AssistantPages.CurrentPage = Items.ExportedDataComparison Then
		...
		If ImportType = "TabularSection" Then
			...
			If Rows.Count() > 0 Then
				...
				ShowQueryBox(Notification, NStr("en = 'Strings with blank required columns will be skipped.'") + 
...
Procedure PerformMapping()
	...
	ShowUserNotification(NStr("en = 'Mapped'"),, NStr("en = 'Items mapped:'") + " " + String(MatchedQuantityByColumns));
	ShowStatisticsByMatchLoadFromFile();
EndProcedure
...
Procedure ShowStatisticsByMatchLoadFromFile()
	...
	TextAll = StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'All (%1)'"), Statistics.TotalAmount);
	
	Items.CreateIfNotMatched.Title = NStr("en = 'Unmapped ('") + Statistics.Unmatched + ")";
	Items.UpdateExisting.Title = NStr("en = 'Mapped items ('") + String(Statistics.Matched) + ")";
	...
	NStr("en = 'Unmapped (%1 out of %2)'"), Statistics.Unmatched, Statistics.TotalAmount));
	ChoiceList.Add("Mapped", StringFunctionsClientServer.SubstituteParametersInString(
	NStr("en = 'Mapped (%1 of %2)'"), Statistics.Matched, Statistics.TotalAmount));
	ChoiceList.Add("Ambiguous", StringFunctionsClientServer.SubstituteParametersInString(
	NStr("en = 'Ambiguous (%1 out of %2)'"), Statistics.Ambiguous, Statistics.TotalAmount));
...
Procedure ExecuteStepFillingTableWithDataOnClient()
	...
	CommonUseClientServer.SetFormItemProperty(Items, "AnotherWayToImportData", "Visible", False);
...
Procedure ExecuteImportedDataMatchStepOnServer(BackgroundJob = False)
	...
	Else
		...
		NStr("en = 'DataLoadFromFile: Execute server method of data processor FillMatchTableWithDataFromTemplate'"));
...
Procedure ExecuteImportedDataMatchStep()
	...
	If TableWithDataEmpty() Then
		ShowMessageBox(, (NStr("en = 'To go to the stage of data mapping and import, fill in the table.'")));	
		Return;
	EndIf;
	...
	If UnfilledColumnsList.Count() > 0 Then 
		If UnfilledColumnsList.Count() =1  Then 
			TextAboutColumns = NStr("en = 'Mandatory column ""'") + " " + UnfilledColumnsList[0] +
				NStr("en = '"" contains empty strings, these strings will be ignored during import'");
		Else
			TextAboutColumns = NStr("en = 'Mandatory columns ""'") + " " + StringFunctionsClientServer.RowFromArraySubrows(UnfilledColumnsList,", ") +
				NStr("en = '"" contain empty strings, these strings will be ignored during import'");
		EndIf;
...
Procedure ShowReport(Report)
	...
	Items.FilterReport.ChoiceList.Add("Skipped", NStr("en = 'Missed ('") + Report.Skipped+ ")");
	...
	If ImportType = "InsertionFromClipboard" Then
		...
		If Statistics.Matched > 0 Then
			...
			If Statistics.Ambiguous > 0 AND Statistics.NotFound > 0 Then 
				TextNotFound = NStr("en = '11 lines will be ignored:'") + Chars.LF + "  - " + NStr("en = 'No data in application: %1'") 
					+ Chars.LF + "  - " +NStr("en = 'Several variants to insert: %2'");
			...
			ElsIf Statistics.Ambiguous > 0 Then
				TextNotFound = NStr("en = 'Strings for which there are multiple variants in the application will be skipped: %1'");
			...
			ElsIf Statistics.NotFound > 0 Then
				TextNotFound = NStr("en = 'Strings for which there are no matching data in the application will be skipped: %1'");
				TextNotFound = StringFunctionsClientServer.SubstituteParametersInString(TextNotFound, Statistics.NotFound);
			EndIf;
			TextNotFound = TextNotFound + Chars.LF + NStr("en = 'To view skipped lines and data selection for insertion, click Next.'");
...
Procedure WriteImportedDataReport(BackgroundJob = False)
	...
	If ImportType = "UniversalImport" Then
		...
		BackgroundJobResult = LongActions.ExecuteInBackground(UUID, 
				...
				ServerCallParameters, 
				NStr("en = 'The ImportDataFromFile subsystem: Save imported data'"));
					...
					If InfoObject.ObjectType = "Catalog" Then
						...
						For Each InputRow In Catalog.InputByString Do 
							If InputRow.Name = "Code" AND Not Catalog.AutoNumber Then 
								InputConditionTextOnLine = "MatchCatalog." + Column.ColumnName+ ".Code " + ComparisonTypeString + " MatchingTable." + Column.ColumnName;	
							Else
								InputConditionTextOnLine = "MatchCatalog." + Column.ColumnName+ "." + InputRow.Name  + ComparisonTypeString + " MatchingTable." + Column.ColumnName;
							EndIf;	
...
Function ObjectManager(CorrelationObjectName)
		...
		Else
			Raise StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'Object ""%1"" is not found'"), CorrelationObjectName);
		EndIf;
...
Procedure ShowInformationRowAboutMandatoryColumns()
	...
	If Items.DataFillingPages.CurrentPage = Items.PageVariantLoadFromFile Then
		ToolTipText = NStr("en = 'To import data, save the form to a file and fill in the table using another application.
		                   |Then import the completed table in one of the following formats: Microsoft Excel Workbook (XLSX), Comma Separated Values (CSV), or spreadsheet document (MXL).'") + Chars.LF;
	Else
		ToolTipText = NStr("en = 'To fill in the table, copy data to the table from external file via clipboard.'") + Chars.LF;
	EndIf;
	...
	If MandatoryColumns.Count() > 0 Then 
		...
		If MandatoryColumns.Count() = 1 Then
			ToolTipText = ToolTipText + StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Required column: %1'"), 
				ListColumns);
		Else
			ToolTipText = ToolTipText + StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Required columns: %1'"),
				ListColumns);
		EndIf;
...
Procedure AddStandardColumnsInMatchTable(TemporaryVT, MappingObjectStructure, AddID,
	...
	If AddID Then 
		TemporaryVT.Columns.Add("ID", New TypeDescription("Number"), NStr("en = 'item'"));
	EndIf;
...
Procedure OnEndPlacingFile(Result, TemporaryStorageAddress, FileName, Parameter) Export
	...
	If Result = True Then
		...
		If Extension = "csv" OR Extension = "xlsx" OR Extension = "mxl" Then
			...
			EndIf;
		Else
			ShowMessageBox(,NStr("en = 'Cannot import data from this file. Make sure that data in the file is correct.'"));        
		EndIf;
	EndIf;
