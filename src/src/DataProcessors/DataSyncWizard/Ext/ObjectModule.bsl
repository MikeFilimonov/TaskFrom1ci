...
Procedure SetUpNewDataExchangeOverWebServiceInTwoBases(Cancel,
	...
	Try
		...
		If Cancel Then
			Raise NStr("en = 'When creating exchange setting in the second infobase, errors occurred.'");
		EndIf;
...
Procedure SetUpNewDataExchangeOverExternalConnection(Cancel,
	...
	Try
		...
		DataSyncWizard = ExternalConnection.DataProcessors.DataSyncWizard.Create();
		DataSyncWizard.ExchangePlanName = ExchangePlanName;
		
		// Import assistant parameters from string to assistant data processor.
		DataSyncWizard.ExternalConnectionImportAssistantParameters(Cancel, AssistantParameterStringXML);
		...
		If CorrespondentVersion_2_1_1_7 OR CorrespondentVersion_2_0_1_6 Then
			...
			DataSyncWizard.ExternalConnectionSetUpNewDataExchange_2_0_1_6(Cancel,
		...
		Else
			...
			DataSyncWizard.ExternalConnectionSetUpNewDataExchange(Cancel,
	...
	DataSyncWizard.ExternalConnectionRecordChangesForExchange();
...
Procedure ExternalConnectionSetUpNewDataExchange(Cancel, 
	...
	Try
		...
		If Not InfobasePrefixSet Then
			...
			ValueBeforeUpdate = GetFunctionalOption("GlobalNumerationPrefix");
			
			If ValueBeforeUpdate <> GlobalNumerationPrefix Then
				
				Constants.GlobalNumerationPrefix.Set(TrimAll(GlobalNumerationPrefix));
...
//                                      storage according to which the file data will be available on server or client
...
Procedure RefreshIBPrefixConstantValue()
	...
	If IsBlankString(ValueBeforeUpdate)
		...
		Constants.GlobalNumerationPrefix.Set(TrimAll(SourceInfobasePrefix));
...
Function GetThisBaseNodeCode(Val InfobasePrefixSpecifiedByUser)
	...
	Result = GetFunctionalOption("GlobalNumerationPrefix");
...
Procedure RunAssistantParametersImport(Cancel, XMLString) Export
	...
	If SettingsStructure.Property("ExchangePlanName")
		...
		ErrorMessageStringField = NStr("en = 'The file contains exchange settings for a different infobase.'");
	...
	If Not Cancel Then
		...
		If ExchangeDataSettingsFileFormatVersion = "1.0" Then
			...
			ThisInfobaseDescription = NStr("en = 'This is an infobase'");
			SecondInfobaseDescription = SettingsStructure.ExchangeProcessingSettingName;
			SecondInfobaseNewNodeCode = SettingsStructure.NewNodeCode;
			
		EndIf;
		
	EndIf;
	
	GlobalNumerationPrefix = GetFunctionalOption("GlobalNumerationPrefix");
	
	If Not IsBlankString(GlobalNumerationPrefix)
		AND GlobalNumerationPrefix <> SourceInfobasePrefix Then
