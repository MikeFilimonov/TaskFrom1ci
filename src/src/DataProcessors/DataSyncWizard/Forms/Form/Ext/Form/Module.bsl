...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Object.SourceInfobasePrefix           = GetFunctionalOption("GlobalNumerationPrefix");
	...
	If IsContinuedInDIBSubordinateNodeSetup Then
		...
		Items.AssistantRunModeChoiceSwitchGroup.Title = NStr("en = 'Continue to configure data synchronization with the main node'");
...
Procedure OnOpen(Cancel)
	...
	If CommonUseClient.OfferToCreateBackups() Then
		...
		Text = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'We recommend that you make a <a href = %1 >data backup</a> before configuring data synchronization.'"),
			"CreateBackup");
...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	If IsContinuedInDIBSubordinateNodeSetup Then
		...
		CommonUseClient.ShowArbitraryFormClosingConfirmation(ThisObject, Cancel, Exit, WarningText, "CloseFormWithoutWarnings", AlertDescriptionDenyContinueDIB);
		
		Return;
	EndIf;
	
	WarningText = NStr("en = 'Do you want to cancel synchronization customizing and quit the wizard?'");
	AlertDescriptionClose = New NotifyDescription("DeleteDataExchangeSetting", ThisObject);
	CommonUseClient.ShowArbitraryFormClosingConfirmation(ThisObject, Cancel, Exit, WarningText, "CloseFormWithoutWarnings", AlertDescriptionClose);
...
Procedure NotificationProcessing(EventName, Parameter, Source)
	...
	If EventName = "ClosingObjectMappingForm" Then
		...
		Status(NStr("en = 'Collecting mapping information...'"));
		
		RefreshDataOfMappingStatisticsAtServer(Cancel, Parameter);
		
		If Cancel Then
			ShowMessageBox(, NStr("en = 'Errors occurred when receiving statistics information.'"));
		Else
			
			ExpandTreeOfInformationStatistics(Parameter.UniqueKey);
			
			Status(NStr("en = 'Information collection is complete'"));
		EndIf;
...
Procedure DataExchangeSettingsForImportFileNameStartChoice(Item, ChoiceData, StandardProcessing)
	...
	DialogSettings.Insert("Filter", NStr("en = 'Data synchronization setting file (*.xml)'") + "|*.xml" );
...
#Region PageAssistantPageDataExchangeCreatedSuccessfullyEnd
...
#Region PageAssistantPageAssistantRunModeChoice
...
#Region PageAssistantPageSettingTransportParametersFILE
...
#Region PageAssistantPageSettingTransportParametersFTP
...
#Region PageAssistantPageSettingTransportParametersEMAIL
...
Procedure SaveDataExchangeSettingsFile(Command)
	...
	If Cancel Then
		...
		ShowMessageBox(, NStr("en = 'When saving file of data synchronization settings, errors occurred.'"));
	...
	Else
		...
		#Else
			...
			SuggestionText = NStr("en = 'To open the directory, install the file operation extension.'");
...
Procedure AfterWorksWithFilesExpansionCheck(Result, AdditionalParameters) Export
	...
	If Result Then
		...
		Dialog.Title      = NStr("en = 'Specify a name of the data synchronization setting file'");
...
Procedure CheckCOMConnectionEnd(Result, AdditionalParameters) Export
	...
	If Result = DialogReturnCode.OK Then
		...
		If IsBlankString(MessageText) Then
			MessageText = NStr("en = 'Connection check is completed successfully.'");
		EndIf;
...
Procedure CheckWSConnectionEnd(Result, AdditionalParameters) Export
	...
	If Result = DialogReturnCode.OK Then
		...
		If Not Cancel Then
			...
			ShowMessageBox(, NStr("en = 'Connection has been successfully established.'"));
...
#Region AppliedServiceProceduresAndFunctions
...
Procedure SetVisibleAtServer()
	...
	If Object.ThisIsSettingOfDistributedInformationBase Then
		...
		Items.AssistantRunModeChoiceSwitchGroup.Title = NStr("en = 'Create initial image of DIB subordinate node'");
...
Function GetDescriptionOfSettingsOfExchangeTransport()
	...
	If IsBlankString(Result) Then
		...
		Result = NStr("en = 'Connection settings are not set.'");
...
Procedure CheckConnection(TransportKind)
	...
	If Not Cancel Then
		...
		If TransportKind = PredefinedValue("Enum.ExchangeMessagesTransportKinds.FILE") Then
			WarningText = NStr("en = 'Writing data to the specified directory is permitted.'");
		Else
			WarningText = NStr("en = 'Connection has been successfully established.'");
		EndIf;
...
Procedure CheckWSConnectionAtServer(Cancel, ExtendedCheck, IsSuggestOpenEventLogMonitor)
	...
	If ExtendedCheck Then
		...
		If Not TargetParameters.ExchangePlanExists Then
			...
			Message = NStr("en = 'Another application is not for synchronization with the current one.'");
		...
		Object.TargetInfobasePrefix = TargetParameters.GlobalNumerationPrefix;
...
Procedure CheckAttributeFillingOnForm(Cancel, FormToCheckName, FormParameters, FormAttributeName)
	...
	If Not SettingsForm.CheckFilling() Then
		...
		CommonUseClientServer.MessageToUser(NStr("en = 'Configure required settings.'"),,, FormAttributeName, Cancel);
...
Procedure FinishFirstExchangeOverOrdinaryCommunicationChannelsSetupStage(Cancel)
	...
	If Not Object.ThisIsSettingOfDistributedInformationBase
		...
		NString = NStr("en = 'Save file with settings for another application'");
	...
	Else
		...
		If ExecuteDataExchangeNow Then
			...
			Status(NStr("en = 'Data is sent'"));
			
			If Cancel Then
				
				ShowMessageBox(, NStr("en = 'Errors occurred when sending the data (see the event log).'"));
...
Procedure FinishSecondExchangeOverOrdinaryCommunicationChannelsSetupStage(Cancel, Val OpenAfterClosingCurrent)
	...
	Status(NStr("en = 'Creating data synchronization setting'"));
	
	ConfigureNewDataExchangeAtServer(Cancel, FilterSsettingsAtNode, DefaultValuesAtNode);
	
	If Cancel Then
		
		ShowMessageBox(, NStr("en = 'When creating data synchronization setting, errors occurred.'"));
		Return;
	EndIf;
...
Procedure OpenMappingForm()
	...
	If Not CurrentData.UsePreview Then
		ShowMessageBox(, NStr("en = 'Cannot map this data.'"));
		Return;
	EndIf;
...
Function GetIndexesOfRowsOfInformationStatisticsTable(RowKeys)
	...
	For Each Element In RowKeys Do
		
		TableRows = Object.StatisticsInformation.FindRows(New Structure("Key", Element));
	...
	Except
		...
		WriteErrorInEventLogMonitor(
			StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'An error occurred while executing handler OnConnectingToCorrespondent:%1%2'"),
				Chars.LF,
...
#Region ValuesOfConstants

&AtClientAtServerNoContext
Function LabelNextFTP()
	
	Return NStr("en = 'Click Next to set connection via FTP resource.'");
...
Function LabelNextEMAIL()
	...
	Return NStr("en = 'Click Next to set email connection.'");
...
Function LabelNextSettings()
	...
	Return NStr("en = 'Click Next to set additional parameters of data synchronization.'");
...
Function Attachable_AssistantPageParameterSetup_OnGoingNext(Cancel)
	...
	If IsBlankString(Object.ThisInfobaseDescription) Then
		...
		NString = NStr("en = 'Specify this application name.'");
	...
	If IsBlankString(Object.SecondInfobaseDescription) Then
		...
		NString = NStr("en = 'Specify a different application name.'");
	...
	If IsBlankString(Object.TargetInfobasePrefix) Then
		...
		NString = NStr("en = 'Specify an existing or required prefix of the second infobase.'");
	...
	If TrimAll(Object.SourceInfobasePrefix) = TrimAll(Object.TargetInfobasePrefix) Then
		...
		NString = NStr("en = 'Infobase prefixes should be different.'");
		CommonUseClientServer.MessageToUser(NString,, "Object.TargetInfobasePrefix",, Cancel);
...
Function Attachable_AssistantPageFirstInfobaseExternalConnectionParameterSetup_OnGoingNext(Cancel)
	...
	If IsBlankString(Object.ThisInfobaseDescription) Then
		...
		NString = NStr("en = 'Specify an infobase name.'");
		CommonUseClientServer.MessageToUser(NString,, "Object.ThisInfobaseDescription",, Cancel);
		Return Undefined;
	EndIf;
...
Function Attachable_AssistantPageStart_OnGoingNext(Cancel)
	...
	If IsBlankString(Object.DataExchangeSettingsForImportFileName) Then
		ErrorText = NStr("en = 'Select a file with data synchronization settings'");
	...
	WarningText = NStr("en = 'To transfer the file of data synchronization settings, install extension for web client 1C:Enterprise.'");
...
Procedure EndSelectingDataExchangeSettingsForImportFile(Val FilesPlacingResult, Val AdditionalParameters) Export
	...
	If IsBlankString(ErrorText) AND IsBlankString(PlacedFileAddress) Then
		ErrorText = NStr("en = 'An error occurred when transferring the file of data synchronization settings to the server'");
	EndIf;
	...
	If IsBlankString(ErrorText) Then
		...
		If AssistantParametersImportError Then
			ErrorText = NStr("en = 'Invalid data synchronization setting file is specified. Specify a correct file.'");
		Else
...
Function Attachable_AssistantPageParameterSetup_OnOpen(Cancel, SkipPage, IsGoNext)
	...
	// If a user navigates to the page of additional parameters and does not select any kind of transport, then there will
	// be an error.
...
Function Attachable_AssistantPageWaitForDataAnalysisAutomaticMapping_LongOperationProcessing(Cancel, GoToNext)
	...
	If Cancel Then
		...
		ShowMessageBox(, NStr("en = 'Errors occurred when analyzing data.'"));
...
Procedure AssistantPageWaitForCatalogSynchronizationImport_LongOperationProcessing()
	...
	Result = LongActions.ExecuteInBackground(
		UUID,
		"DataProcessors.DataSyncWizard.RunCatalogImport",
		MethodParameters,
		NStr("en = 'Import catalogs from exchange message'"));
...
Procedure CheckAccountingSettingsAtServer(
	...
	If Not CorrespondentAccountingSettingsAreSet Then
		...
		If IsBlankString(CorrespondentErrorMessage) Then
			CorrespondentErrorMessage = NStr("en = 'Accounting parameters are not specified for the online application.'");
		EndIf;
...
Procedure CheckJobSettingsForFirstInfobase(Cancel, ConnectionType = "WebService")
	...
	If IsBlankString(Object.SecondInfobaseDescription) Then
		...
		NString = NStr("en = 'Specify the online application name.'");
		CommonUseClientServer.MessageToUser(NString,, "Object.SecondInfobaseDescription",, Cancel);
...
Procedure CheckJobSettingsForSecondInfobase(Cancel, ConnectionType)
	...
	If IsBlankString(Object.SecondInfobaseDescription) Then
		...
		NString = NStr("en = 'Specify application name.'");
		CommonUseClientServer.MessageToUser(NString,, "Object.SecondInfobaseDescription",, Cancel);
...
Procedure AssistantPageWaitForCheckExternalConnectionConnected_LongOperationProcessorOnServer(Cancel)
	...
	Else
		...
		If IsBlankString(Object.COMServerName1CEnterprise) Then
			...
			NString = NStr("en = 'Specify a server cluster name.'");
	...
	If Not ExchangePlanExists Then
		...
		Message = NStr("en = 'Data synchronization with the specified application is not applicable.'");
		CommonUseClientServer.MessageToUser(Message,,,, Cancel);
		Return;
	...
	If Lower(InfobaseConnectionString()) = Lower(ExternalConnection.InfobaseConnectionString()) Then
		...
		Message = NStr("en = 'Connection settings for this infobase are configured.'");
		CommonUseClientServer.MessageToUser(Message,,,, Cancel);
		Return;
		
	EndIf;
	
	Object.TargetInfobasePrefix           = ExternalConnection.GetFunctionalOption("GlobalNumerationPrefix");
	Object.TargetInfobasePrefixIsSet = ValueIsFilled(Object.TargetInfobasePrefix);
...
Procedure CheckWhetherDataExchangeWithSecondBaseExists(Cancel)
	...
	If Not IsBlankString(NodeCode)
		...
		Message = NStr("en = 'Data synchronization between applications has been configured earlier.'");
		CommonUseClientServer.MessageToUser(Message,,,, Cancel);
...
#Region InitializationOfTheAssistantTransitions
