...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not DataExchangeSaaSReUse.DataSynchronizationSupported() Then
		...
		Raise NStr("en = 'Data synchronization for this configuration is not supported.'");
	...
	Title = StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'Setting of data synchronization with ""%1""'"), CorrespondentDescription);
	Items.DataSynchronizationDescription.Title = StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'Description of data synchronization with ""%1""'"), CorrespondentDescription);
...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	WarningText = NStr("en = 'Do you want to cancel data synchronization setting?'");
...
// Parameters:
//
//  TransitionSequenceNumber (mandatory) - Number. Sequence number of transition that corresponds to the current
//                                         transition step
//  MainPageName  (mandatory) - String. Name of the MainPanel panel page that corresponds to the current number of the transition 
//  NavigationPageName (mandatory) - String. Page name of the NavigationPanel panel, which corresponds to the current
//                                   number of transition
//  DecorationPageName (optional) - String. Page name of the DecorationPanel panel, which corresponds to the current
//                                  number of transition
//  DeveloperNameOnOpening (optional) - String. Name of the function-processor of the assistant current page open event
//  HandlerNameOnGoingNext (optional) - String. Name of the function-processor of the transition to the next assistant
//                                      page event
//  HandlerNameOnGoingBack (optional) - String. Name of the function-processor of the transition to assistant previous
//                                      page event
//  LongAction (optional) - Boolean. Shows displayed long operation page.
...
#Region ServiceProceduresFunctions
...
Function Attachable_Begin_OnGoingNext(Cancel)
	...
	If NodeFiltersSettingsAvailable Then
		...
		If Not SettingsForm.CheckFilling() Then
			...
			CommonUseClientServer.MessageToUser(NStr("en = 'Configure required settings.'"),,, "DataExportSettingsDescription", Cancel);
...
// in this base Email message to correspondent:
//   Create exchange setting
...
Procedure DataAnalysisExpectation_LongOperationProcessing(Cancel)
	...
	Try
		...
		Result = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en = 'Setting of data synchronization between online applications (step 1)'"));
...
Procedure RunAutomaticDataMapping(Cancel)
	...
	Try
		...
		Result = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en = 'Setting of data synchronization between online applications (automatic data mapping)'"));
...
Procedure GetStatsComparison(Cancel)
	...
	Try
		...
		Result = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en = 'Setting of data synchronization between online applications (mapping statistics receipt)'"));
...
// catalogs) Email message to correspondent:
//   Import the exchange
...
Procedure DataSynchronizationExpectation_LongOperationProcessing(Cancel)
	...
	Try
		...
		Result = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en = 'Setting of data synchronization between online applications (catalog synchronization)'"));
...
Procedure ImportParametersAccountingCorrespondent(Cancel, ErrorInfo = "", CorrespondentErrorMessage = "")
	...
	Try
		...
		If IsBlankString(CorrespondentErrorMessage) Then
			CorrespondentErrorMessage = NStr("en = 'Accounting parameters are not specified in the ""%1"" application.'");
		...
		If IsBlankString(ErrorInfo) Then
			ErrorInfo = NStr("en = 'Accounting parameters are not specified in this application.'");
		EndIf;
...
Function Attachable_DataReceivingRules_OnGoingNext(Cancel)
	...
	If NodeDefaultValuesAvailable Then
		...
		If Not SettingsForm.CheckFilling() Then
			...
			CommonUseClientServer.MessageToUser(NStr("en = 'Configure required settings.'"),,, "ValuesDescriptionFullByDefault", Cancel);
	...
	If CorrespondentInfobaseNodeDefaultValuesAvailable Then
		...
		If Not SettingsForm.CheckFilling() Then
			...
			CommonUseClientServer.MessageToUser(NStr("en = 'Specify required settings for the second application.'")
...
Procedure SettingsSaveExpected_LongOperationProcessing(Cancel)
	...
	Try
		...
		Result = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en = 'Setting of data synchronization between online applications (step 2)'"));
...
Procedure FixCreatingExchangeSettingsInServiceManager(Cancel)
	...
	Try
		...
		// Email message to the service manager - include synchronization
		Message = MessagesSaaS.NewMessage(
		...
		// Email message to the service manager - request for synchronization between two applications
		Message = MessagesSaaS.NewMessage(
...
#Region InitializationOfAssistantTransitions
