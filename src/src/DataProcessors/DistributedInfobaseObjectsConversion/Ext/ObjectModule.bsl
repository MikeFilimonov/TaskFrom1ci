...
// It sets as
// local variable ExchangeMessageFileNameField the string with the full name of the exchange message file for import or
// export data. As a rule the exchange message file is located in the temporary directory of the user operating system.
//
// Parameters:
	...
	If MessageReader.Sender = ExchangePlans.MasterNode() Then
		...
		Try
			...
			If ValueIsFilled(PredefinedItemDoubles) Then
				WriteLogEvent(
					NStr("en = 'Predefined items.Non-unique entries found.'",
...
Procedure AddPredefinedItemDoubleDescription(WrittenObject, PredefinedItemDoubles, Cancel, DenialDescription)
	...
	While Selection.Next() Do
		...
		Else
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NonUniqueRecordErrorTemplate(),
				NStr("en = 'When importing predefined items, non-unique records were found.'"));
		EndIf;
	...
	If WriteInJournal Then
		If DoubleNumber = 1 Then
			Pattern = NStr("en = '(imported reference: %1, duplicate reference: %2)'");
		Else
			Pattern = NStr("en = '(imported reference: %1, duplicate references %2)'");
		EndIf;
...
Function NonUniqueRecordErrorTemplate()
	Return
		NStr("en = 'Import of important changes is not completed.
		     ...
		     |1. Open the configurator, go to the Administration menu, select item ""Testing and correction ..."".
		     |2. IN the opened form:
		     | - activate the ""Checking of the logical infobase integrity item only"";
...
#Region ExecutionContextErrorDetails

Function ErrorOfOpeningOfExchangeMessageFile()
	
	Return NStr("en = 'An error occurred when opening the exchange message file'", CommonUseClientServer.MainLanguageCode());
	
EndFunction

Function ErrorOfStartReadOfExchangeMessageFile()
	
	Return NStr("en = 'An error occurred when starting reading the exchange message file'", CommonUseClientServer.MainLanguageCode());
	
EndFunction

Function ErrorBeginningRecordsExchangeMessageFile()
	
	Return NStr("en = 'An error occurred when starting writing the exchange message file'", CommonUseClientServer.MainLanguageCode());
	
EndFunction

Function ErrorOfReadingOFExchangeMessageFile()
	
	Return NStr("en = 'An error occurred when reading an exchange message file'", CommonUseClientServer.MainLanguageCode());
	
EndFunction

Function ErrorOfExchangeMessageFileWrite()
	
	Return NStr("en = 'An error occurred when writing data to the exchange message file'");
	
EndFunction

Function ErrorOfDataExchangeKind()
	
	Return NStr("en = 'Exchange not according to conversion rules is not supported'", CommonUseClientServer.MainLanguageCode());
