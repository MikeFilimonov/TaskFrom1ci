...
Procedure RepresentScheduleDay()
	...
	|////////////////////////////////////////////////////////////////////////////////
	|SELECT
	|	ISNULL(ResourceWorkScheduleAdjustment.BeginTime, WorkSchedules.BeginTime) AS BeginTime,
	|	ISNULL(ResourceWorkScheduleAdjustment.EndTime, WorkSchedules.EndTime) AS EndTime,
	|	ISNULL(ResourceWorkScheduleAdjustment.NotABusinessDay, FALSE) AS NotABusinessDay,
	|	RecourcesWorkScheduleSliceLast.CompanyResource.ResourceValue AS Employee
	|FROM
	|	InformationRegister.WorkSchedulesOfResources.SliceLast(&DateOfSchedule, ) AS RecourcesWorkScheduleSliceLast
	...
	|		ON RecourcesWorkScheduleSliceLast.CompanyResource = ResourceWorkScheduleAdjustment.CompanyResource
	|			AND (YEAR(RecourcesWorkScheduleSliceLast.Period) = ResourceWorkScheduleAdjustment.Year)
	|			AND (ResourceWorkScheduleAdjustment.Day = BEGINOFPERIOD(&DateOfSchedule, DAY))
	|WHERE
...
Procedure AddTaskSchedule(Command)
	...
	OpenForm("Document.ObsoleteWorkOrder.ObjectForm", FillValue);
...
Procedure AddTaskMyCurrentDayTasks(Command)
	...
	OpenForm("Document.ObsoleteWorkOrder.ObjectForm", New Structure("Employee", Responsible), , , );
...
Procedure ScheduleDetailProcessing(Item, Details, StandardProcessing)
	...
	ElsIf RadioButton = "Day" Then
		...
		If Details.Property("DocumentRef")
		   AND TypeOf(Details.DocumentRef) = Type("DocumentRef.ObsoleteWorkOrder") Then
			OpenForm("Document.ObsoleteWorkOrder.ObjectForm", New Structure("Key", Details.DocumentRef), , , );
