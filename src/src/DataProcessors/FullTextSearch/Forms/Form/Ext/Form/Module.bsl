...
Procedure Search(Direction, Cancel = Undefined)
	...
	If IsBlankString(SearchString) Then
		CommonUseClientServer.MessageToUser(NStr("en = 'Enter a search object.'"), , "SearchString");
		Cancel = True;
		Return;
	EndIf;
	...
	Status(StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'Searching ""%1""...'"), SearchString));
	...
	If SearchResults.Count() > 0 Then
		...
		If Direction = 0 AND Result.CurrentPosition = 0 AND Result.TooManyResults Then
			ShowMessageBox(, NStr("en = 'Too many results. Refine your search criteria.'"));
		EndIf;
