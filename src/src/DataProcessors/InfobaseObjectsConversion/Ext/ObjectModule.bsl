...
Var AccumulationRecordTypeType;   // Type("EarningMovementKind")
...
Procedure ExecuteDataImport() Export
	...
	Try
		...
		Try
			...
			If ErrorFlag() Then
				Raise NStr("en = 'Errors occurred when importing data.'");
			EndIf;
...
Procedure ExecuteDataImportToInformationBase(TableToImport) Export
	...
	// Record in the events log monitor.
	MessageString = NStr("en = '%1, %2; %3 objects are processed'", CommonUseClientServer.MainLanguageCode());
...
Procedure RunExchangeMessageAnalysis(AnalysisParameters = Undefined) Export
	...
	Try
		...
		Try
			...
			If ErrorFlag() Then
				Raise NStr("en = 'Errors occurred when analyzing data.'");
			EndIf;
...
Procedure ExternalConnectionAfterDataImport() Export
	...
	If Not ErrorFlag() Then
		...
		If HasObjectChangeRecordData = True Then
			...
			InformationRegisters.MappingOfInfobaseObjects.DeleteOutdatedExportModeRecordsByRef(ExchangeNodeForDataImport);
...
Procedure WriteObjectToIB(Object, Type, WriteObject = False, Val SendBack = False) Export
	...
	If CommonUseReUse.DataSeparationEnabled()
		...
		ErrorMessageString = NStr("en = 'Attempting to change shared data (%1) in split session.'");
	...
	Try
		...
		MappingOfInfobaseObjects = Undefined;
		If Object.AdditionalProperties.Property("MappingOfInfobaseObjects", MappingOfInfobaseObjects)
			AND MappingOfInfobaseObjects <> Undefined Then
			
			MappingOfInfobaseObjects.UniqueSourceHandle = Object.Ref;
			
			InformationRegisters.MappingOfInfobaseObjects.AddRecord(MappingOfInfobaseObjects);
		EndIf;
		CommitTransaction();
	Except
		RollbackTransaction();
...
Function DumpByRule(Source					= Undefined,
	...
	If CommentObjectProcessingFlag Then
		...
		NameActions = ?(GetRefNodeOnly, NStr("en = 'Conversion of reference to object'"), NStr("en = 'Object conversion'"));
...
Procedure ExchangeProtocolInitialization()
	...
	Except
		DataLogFile = Undefined;
		MessageString = NStr("en = 'An error occurred when attempting to write to data protocol file: %1. Error description: %2'",
...
Procedure StartMessageReader(MessageReader, DataAnalysis = False)
	...
	If ExchangeFile.NodeType <> XMLNodeType.StartElement Then
		Raise NStr("en = 'Error in exchange message format.'");
	EndIf;
	...
	If ConversionRulesTable.Count() = 0 Then
		...
		If ErrorFlag() Then
			Raise NStr("en = 'Errors occurred when importing data exchange rules.'");
		EndIf;
	Else
	...
	If Not IsBlankString(Conversion.BeforeDataImport) Then
		...
		If Cancel Then
			Raise NStr("en = 'Cancel importing exchange message in the BeforeDataImport handler (conversion).'");
		EndIf;
	...
	If ExchangeFile.LocalName = "DataTypeInfo" Then
		...
		Else
			...
			If ErrorFlag() Then
				Raise NStr("en = 'Errors occurred when importing information on data types.'");
			EndIf;
		EndIf;
		ExchangeFile.Read();
	EndIf;
	...
	If TransactionActive() Then
		Raise NStr("en = 'Lock of data receipt cannot be set in an active transaction.'");
	EndIf;
	...
	Try
		...
		If Cancel = True Then
			...
			If Not IsBlankString(CancelReason) Then
				...
				MessageString = NStr("en = 'Decline exchange message import in handler AfterParametersImport (conversion) due to: %1'");
				MessageString = StringFunctionsClientServer.SubstituteParametersInString(MessageString, CancelReason);
				Raise MessageString;
			Else
				Raise NStr("en = 'Cancel importing exchange message in the AfterImportOfParameters handler (conversion).'");
			EndIf;
...
Procedure DeleteDocumentRegisterRecords(DocumentObject)
	...
	For Each RegisterRecordRow In RegisterRecordTable Do
		...
		If Not AccessRight("Update", Set.Metadata()) Then
			...
			ErrorMessage = NStr("en = 'Access right violation: %1'");
	...
	For Each RegisterRecordRow In RecordTableRowToProcessArray Do		
		...
		Except
			...
			ErrorMessage = NStr("en = 'Operation is not executed: %1 %2'");
...
Procedure ReadSearchPropertiesFromFile(SearchProperties, SearchPropertiesDontReplace, TypeInformation,
	SearchByEqualDate, ObjectParameters, Val MainObjectSearchMode, ObjectMapFound, MappingOfInfobaseObjects)
	...
	While ExchangeFile.Read() Do
		...
		If    NodeName = "Property"
			...
			If UUIDProperty Then
				...
				ExecuteReplacementOfUUIDIfNeeded(PropertyValue, SourceTypeAsString, ReceiverTypeAsString, MainObjectSearchMode, ObjectMapFound, MappingOfInfobaseObjects);
...
Procedure ExecuteReplacementOfUUIDIfNeeded(
	...
	If QueryResult.IsEmpty() Then
		...
		MappingOfInfobaseObjects = New Structure;
		MappingOfInfobaseObjects.Insert("InfobaseNode", ExchangeNodeForDataImport);
		MappingOfInfobaseObjects.Insert("ReceiverType", ReceiverTypeAsString);
		MappingOfInfobaseObjects.Insert("SourceType", SourceTypeAsString);
		MappingOfInfobaseObjects.Insert("UniqueReceiverHandle", UUID);
		...
		MappingOfInfobaseObjects.Insert("UniqueSourceHandle", Undefined);
...
// 
// Parameters:
//  RefTypeAsString - String - object type in a string presentation, for example, "CatalogRef.Products."
// 
// Returns:
...
Function FindItemBySearchProperties(ObjectType, ObjectTypeName, SearchProperties, 
	...
	// by a unique ref to object, it is required to search only by those properties that are available in the property
	// names string. If it is empty there, then by all available search properties.
...
Procedure ReadInformationAboutSearchProperties(ObjectType, SearchProperties, SearchPropertiesDontReplace,
	SearchByEqualDate = False, ObjectParameters = Undefined, Val MainObjectSearchMode, ObjectMapFound, MappingOfInfobaseObjects)
	...
	ReadSearchPropertiesFromFile(SearchProperties, SearchPropertiesDontReplace, TypeInformation, SearchByEqualDate, ObjectParameters, MainObjectSearchMode, ObjectMapFound, MappingOfInfobaseObjects);
...
Function FindObjectByRef(ObjectType, 
	...
	// 1. Object search by IB objects match register.
	ReadInformationAboutSearchProperties(ObjectType, SearchProperties, SearchPropertiesDontReplace, SearchByEqualDate, ObjectParameters, MainObjectSearchMode, ObjectMapFound, MappingOfInfobaseObjects);
...
Procedure CommentsToObjectImport(NPP, Rulename, Source, ObjectType, GNPP = 0)
	...
	If CommentObjectProcessingFlag Then
		...
		MessageString = NStr("en = 'Import object #%1'");
...
Function ReadInformationAboutObjectRegistration()
	...
	WriteObjectToIB(RecordSet, "InformationRegisterRecordSet.MappingOfInfobaseObjects");
...
Procedure ReadInformationComparingCorrection()
	...
	InformationRegisters.MappingOfInfobaseObjects.AddRecord(RecordStructure);
...
Function ReadObject(UUIDString = "")
	...
	While ExchangeFile.Read() Do
		...
		ElsIf NodeName = "Ref" Then
			...
			Object.AdditionalProperties.Insert("MappingOfInfobaseObjects", MappingOfInfobaseObjects);
...
//  PropertyCollectionNode - properties collection xml-node.
//  CollectionObject      - if it is specified, then collection object properties are exported, otherwise, Source
			...
			If ExportRefOnly Then
				...
				If ExportObjectByRef(Value, NodeForExchange) Then
					...
					If Not ObjectPassingFilterOfAllowedObjects(Value) Then
						...
						InformationRegisters.MappingOfInfobaseObjects.AddRecord(RecordStructure, True);
...
Function GetRegistrationAttributesStructure(PCRTable)
	...
	For Each PCR In PCRRowArray Do
		...
		Except
			WriteLogEvent(NStr("en = 'Data exchange.Conversion rule import'", CommonUseClientServer.MainLanguageCode()),
...
Procedure RunObjectRegistrationAttributesCheck(ChangeRecordAttributeTable)
	...
	For Each TableRow In ChangeRecordAttributeTable Do
		...
		Except
			...
			MessageString = NStr("en = 'Object type is undefined: %1'");
			...
			For Each Attribute In TableRow.ChangeRecordAttributeStructure Do
				...
				If CommonUse.ThisIsTask(MDObject) Then
					...
					If Not (MDObject.Attributes.Find(Attribute.Key) <> Undefined
						...
						MessageString = NStr("en = 'Attributes of ""%1"" object header are specified incorrectly. Attribute ""%2"" does not exist.'");
		...
		Else
			...
			If MetaTable = Undefined Then
				WriteInExecutionProtocol(StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'Tabular section (standard tabular section, movements) %1 of object %2 does not exist.'"),
			...
			For Each Attribute In TableRow.ChangeRecordAttributeStructure Do
				...
				If Not AttributeFoundInObjectsRegistrationAttributesTabularSection(MetaTable, Attribute.Key) Then
					WriteInExecutionProtocol(StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en = 'Attribute ""%3"" does not exist in the table section (standard table section, motion) ""%1"" of object ""%2"".'"),
...
Function GetInformationAboutRules(AreCorrespondentRules = False) Export
	...
	If AreCorrespondentRules Then
		InfoString = NStr("en = 'Conversion rules of correspondent (%1) from %2'");
	Else
		InfoString = NStr("en = 'Conversion rules of this infobase (%1) from %2'");
	EndIf;
...
Procedure ImportExchangeRules(Source="",
	...
	If ExchangeMode <> "Import" AND Conversion.Property("AfterExchangeRuleImport", AfterExchangeRuleImportEventText)
		...
		Except
			ErrorMessageString = WriteInformationAboutErrorConversionHandlers(75, ErrorDescription(), NStr("en = 'AfterImportOfExchangeRules (conversion)'"));
			Cancel = True;
					...
					If TableRow = Undefined Then
						...
						For Each Column In ExchangeMessageDataTable.Columns Do
							...
							If Column.ValueType.ContainsType(StringType) Then
								...
								TableRow[Column.Name] = NStr("en = 'Deleting object'");
		...
		If NodeName = "Object" Then
			...
			If ObjectTypeAsString <> "ConstantsSet" Then
				...
				ElsIf Not (OCR.SynchronizeByID AND OCR.SearchBySearchFieldsIfNotFoundByID) AND OCR.SynchronizeByID Then
					...
					// Objects for which the automatic match is executed during exchange (classifiers) are not displayed to a user in
					// statistics information table. Therefore, statistics information collection is not required for them.
				...
				Try
					...
					If Cancel = True Then
						...
						If Not IsBlankString(CancelReason) Then
							...
							MessageString = NStr("en = 'Data import was canceled as: %1'");
							MessageString = StringFunctionsClientServer.SubstituteParametersInString(MessageString, CancelReason);
							Raise MessageString;
						Else
...
Procedure ReadExchangeData(MessageReader, DataAnalysis)
	...
	If DataImportToInformationBaseMode() Then
		...
		If ReceivedNo >= MessageReader.MessageNo Then // Message number is either smaller or equals to the previously accepted one.
			...
			Raise NStr("en = 'Exchange message was previously received'");
		EndIf;
		...
		If DeleteChangeRecords Then // Delete registration of changes.
			...
			If TransactionActive() Then
				Raise NStr("en = 'Deletion of the data change registration cannot be completed in an active transaction.'");
			EndIf;
	...
	For Each MappingObject In ObjectsForPostponedRecording() Do
		...
		Try
			...
			Else
				...
				ErrorDescription = NStr("en = 'An error occurred when checking the attribute population'");
...
Function ObjectPassingFilterOfAllowedObjects(Value)
	...
	Return InformationRegisters.MappingOfInfobaseObjects.ObjectIsInRegister(Value, NodeForExchange);
...
Procedure RunDumpOfObjectMappingRegister(WriteMessage, ErrorMessageString)
	...
	ChangeSelection = DataExchangeServer.SelectChanges(WriteMessage.Recipient, WriteMessage.MessageNo, Metadata.InformationRegisters.MappingOfInfobaseObjects);
...
Procedure RunDumpOfRegisteredData(WriteMessage, ErrorMessageString, UsedUnloadRulesTable)
	...
	If MetadataToExportArray.Find(Metadata.InformationRegisters.MappingOfInfobaseObjects) <> Undefined Then
		
		CommonUseClientServer.DeleteValueFromArray(MetadataToExportArray, Metadata.InformationRegisters.MappingOfInfobaseObjects);
...
Function ExportObjectMappingInformation(InfobaseNode)
	...
	|SELECT TOP 1 1
	|FROM
	|	InformationRegister.MappingOfInfobaseObjects.Changes AS MappingOfInfobaseObjectsChanges
	|WHERE
	|	MappingOfInfobaseObjectsChanges.Node = &InfobaseNode
	|";
...
Procedure MessagesInitialization()
	...
	ErrorMessages.Insert(2,  NStr("en = 'An error occurred when unpacking an exchange file. The file is locked.'"));
	ErrorMessages.Insert(3,  NStr("en = 'The specified exchange rule file does not exist.'"));
	ErrorMessages.Insert(4,  NStr("en = 'An error occurred when creating COM object Msxml2.DOMDocument'"));
	ErrorMessages.Insert(5,  NStr("en = 'An error occurred when opening the exchange file'"));
	ErrorMessages.Insert(6,  NStr("en = 'An error occurred when importing exchange rules'"));
	ErrorMessages.Insert(7,  NStr("en = 'Error in exchange rule format'"));
	ErrorMessages.Insert(8,  NStr("en = 'Incorrect file name for data export'")); // not used
	ErrorMessages.Insert(9,  NStr("en = 'Error in exchange file format'"));
	ErrorMessages.Insert(10, NStr("en = 'File name for data export is not specified (Data file name)'"));
	ErrorMessages.Insert(11, NStr("en = 'Link to a non-existing metadata object in the exchange rules'"));
	ErrorMessages.Insert(12, NStr("en = 'File name with exchange rules is not specified (Rule file name)'"));
			
	ErrorMessages.Insert(13, NStr("en = 'An error occurred when receiving a value of the object property (by the name of the source property)'"));
	ErrorMessages.Insert(14, NStr("en = 'An error occurred when receiving the object property value (by the target property name)'"));
	
	ErrorMessages.Insert(15, NStr("en = 'File name for data import is not specified (File name for import)'"));
			
	ErrorMessages.Insert(16, NStr("en = 'An error occurred while receiving value of subobject property (by source property name)'"));
	ErrorMessages.Insert(17, NStr("en = 'An error occurred while receiving value of subobject property (by target property name)'"));
	ErrorMessages.Insert(18, NStr("en = 'An error occurred when creating a data processor with the handler code'"));
	ErrorMessages.Insert(19, NStr("en = 'An error occurred in event handler BeforeObjectImport'"));
	ErrorMessages.Insert(20, NStr("en = 'An error occurred in event handler OnObjectImport'"));
	ErrorMessages.Insert(21, NStr("en = 'An error occurred in event handler AfterObjectImport'"));
	ErrorMessages.Insert(22, NStr("en = 'An error occurred in event handler BeforeDataImport (conversion)'"));
	ErrorMessages.Insert(23, NStr("en = 'An error occurred in event handler AfterDataImport (conversion)'"));
	ErrorMessages.Insert(24, NStr("en = 'An error occurred when removing an object'"));
	ErrorMessages.Insert(25, NStr("en = 'An error occurred when writing the document'"));
	ErrorMessages.Insert(26, NStr("en = 'An error occurred when writing the object'"));
	ErrorMessages.Insert(27, NStr("en = 'An error occurred in event handler BeforeProcessClearingRule'"));
	ErrorMessages.Insert(28, NStr("en = 'An error occurred in event handler AfterClearingRuleProcessing'"));
	ErrorMessages.Insert(29, NStr("en = 'An error occurred in event handler BeforeDeleteObject'"));
	
	ErrorMessages.Insert(31, NStr("en = 'An error occurred in event handler BeforeProcessExportRule'"));
	ErrorMessages.Insert(32, NStr("en = 'An error occurred in event handler AfterDumpRuleProcessing'"));
	ErrorMessages.Insert(33, NStr("en = 'An error occurred in event handler BeforeObjectExport'"));
	ErrorMessages.Insert(34, NStr("en = 'An error occurred in event handler AfterObjectExport'"));
			
	ErrorMessages.Insert(41, NStr("en = 'An error occurred in event handler BeforeObjectExport'"));
	ErrorMessages.Insert(42, NStr("en = 'An error occurred in event handler OnObjectExport'"));
	ErrorMessages.Insert(43, NStr("en = 'An error occurred in event handler AfterObjectExport'"));
			
	ErrorMessages.Insert(45, NStr("en = 'Object conversion rule is not found'"));
		
	ErrorMessages.Insert(48, NStr("en = 'An error occurred in event handler BeforeExportProcessor of the property group'"));
	ErrorMessages.Insert(49, NStr("en = 'An error occurred in event handler AfterExportProcessor of the property group'"));
	ErrorMessages.Insert(50, NStr("en = 'Error in event handler BeforeExport (of collection object)'"));
	ErrorMessages.Insert(51, NStr("en = 'Error in event handler OnExport (of collection object)'"));
	ErrorMessages.Insert(52, NStr("en = 'Error in event handler AfterExport (of collection object)'"));
	ErrorMessages.Insert(53, NStr("en = 'An error occurred in global event handler BeforeObjectImporting (conversion)'"));
	ErrorMessages.Insert(54, NStr("en = 'An error occurred in global event handler AfterObjectImport (conversion)'"));
	ErrorMessages.Insert(55, NStr("en = 'An error occurred in event handler BeforeExport (properties)'"));
	ErrorMessages.Insert(56, NStr("en = 'An error occurred in event handler OnExport (properties)'"));
	ErrorMessages.Insert(57, NStr("en = 'An error occurred in event handler AfterExport (properties)'"));
	
	ErrorMessages.Insert(62, NStr("en = 'An error occurred in event handler BeforeDataExport (conversion)'"));
	ErrorMessages.Insert(63, NStr("en = 'An error occurred in event handler AfterDataExport (conversion)'"));
	ErrorMessages.Insert(64, NStr("en = 'An error occurred in global event handler BeforeObjectConversion (conversion)'"));
	ErrorMessages.Insert(65, NStr("en = 'An error occurred in global event handler BeforeObjectExport (conversion)'"));
	ErrorMessages.Insert(66, NStr("en = 'An error occurred when receiving a subordinate object collection from the incoming data'"));
	ErrorMessages.Insert(67, NStr("en = 'An error occurred when receiving the subordinate object properties from the incoming data'"));
	ErrorMessages.Insert(68, NStr("en = 'An error occurred when receiving the object properties from the incoming data'"));
	
	ErrorMessages.Insert(69, NStr("en = 'An error occurred in global event handler AfterObjectExport (conversion)'"));
	
	ErrorMessages.Insert(71, NStr("en = 'Match for the Source value is not found'"));
	
	ErrorMessages.Insert(72, NStr("en = 'An error occurred when exporting data for the exchange plan node'"));
	
	ErrorMessages.Insert(73, NStr("en = 'An error occurred in event handler SearchFieldsSequence'"));
	ErrorMessages.Insert(74, NStr("en = 'Import exchange rules for data export again.'"));
	
	ErrorMessages.Insert(75, NStr("en = 'An error occurred in event handler AfterImportOfExchangeRules (conversion)'"));
	ErrorMessages.Insert(76, NStr("en = 'An error occurred in event handler BeforeSendingUninstallInformation (conversion)'"));
	ErrorMessages.Insert(77, NStr("en = 'An error occurred in event handler OnObtainingInformationAboutDeletion (conversion)'"));
	
	ErrorMessages.Insert(78, NStr("en = 'An error occurred when executing the algorithm after import of the parameter values'"));
	
	ErrorMessages.Insert(79, NStr("en = 'An error occurred in event handler AfterObjectExportToFile'"));
	...
	//
	ErrorMessages.Insert(83, NStr("en = 'An error occurred while accessing the object tabular section. The object tabular section cannot be changed.'"));
	ErrorMessages.Insert(84, NStr("en = 'Collision of change closing dates.'"));
	
	ErrorMessages.Insert(173, NStr("en = 'An error of the exchange node lock occurred. Maybe, data synchronization is already in progress'"));
	ErrorMessages.Insert(174, NStr("en = 'Exchange message was previously received'"));
	ErrorMessages.Insert(175, NStr("en = 'An error occurred in event handler BeforeModifiedObjectsReceiving (conversion)'"));
	ErrorMessages.Insert(176, NStr("en = 'An error occurred in event handler AfterGettingInformationAboutExchangeNodes (conversion)'"));
		
	ErrorMessages.Insert(1000, NStr("en = 'An error occurred when creating a temporary file of data export'"));
...
Procedure AttributesAndModuleVariablesInitialization()
	...
	MapRegisterType    = Type("InformationRegisterRecordSet.MappingOfInfobaseObjects");
	...
	ObjectMappingRegisterManager = InformationRegisters.MappingOfInfobaseObjects;
	...
	|SELECT TOP 1
	|	MappingOfInfobaseObjects.UniqueSourceHandleAsString AS UniqueSourceHandleAsString
	|FROM
	...
	|	AND MappingOfInfobaseObjects.UniqueReceiverHandle = &UniqueReceiverHandle
	|	AND MappingOfInfobaseObjects.ReceiverType                     = &ReceiverType
	|	AND MappingOfInfobaseObjects.SourceType                     = &SourceType
	|";
	//
