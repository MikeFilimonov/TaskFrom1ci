...
// Records are stored in the MappingOfInfobaseObjects information register.
//
// Parameters:
//      Cancel - Boolean - processing denial flag which is set to True in case of an error.
// 
...
Procedure ExecuteDataImportToInformationBase(Cancel, TableToImport) Export
	...
	If DataExchangeDataProcessor.ErrorFlag() Then
		NString = NStr("en = 'Errors occurred when importing exchange message: %1'");
...
Procedure GetTableOfObjectMapping(SourceTable, UserFields, TempTablesManager)
	...
	If DataExchangeReUse.ThisIsExchangePlanXDTO(InfobaseNode) Then
		QueryText = QueryText + "
			...
			|FROM
			|	InformationRegister.SynchronizedObjectPublicIDs AS MappingOfInfobaseObjects
			|WHERE
			|	  MappingOfInfobaseObjects.InfobaseNode = &InfobaseNode
			|	AND MappingOfInfobaseObjects.Ref REFS #TargetTable#
			|
			|INDEX BY
	...
	Else
		QueryText = QueryText + "
			...
			|WHERE
			|	  MappingOfInfobaseObjects.InfobaseNode = &InfobaseNode
			|	AND MappingOfInfobaseObjects.ReceiverType = &SourceType
			|	AND MappingOfInfobaseObjects.SourceType = &ReceiverType
			|INDEX BY
			|	UniqueReceiverHandle,
			|	ReceiverType,
			|	SourceType
	...
	If DataExchangeReUse.ThisIsExchangePlanXDTO(InfobaseNode) Then
		QueryText = QueryText + "
			...
			|		
			|		ISNULL(MappingOfInfobaseObjects.UniqueSourceHandle, TargetTable.Ref) AS Ref,
			|		
			|		#SourceTableIsFolder#                      AS IsSourceFolder,
			|		#MappingOfInfobaseObjectsIsFolder# AS IsTargetFolder,
			|			
			|		// {MAPPING REGISTER DATA}
			|		ISNULL(MappingOfInfobaseObjects.UniqueSourceHandle, TargetTable.Ref)                  AS UniqueReceiverHandle,
			|		ISNULL(MappingOfInfobaseObjects.UniqueReceiverHandle, SourceTable.UUID) AS UniqueSourceHandle,
			|		ISNULL(MappingOfInfobaseObjects.ReceiverType, ""#SourceType#"")                                           AS SourceType,
			|		ISNULL(MappingOfInfobaseObjects.SourceType, ""#ReceiverType#"")                                           AS ReceiverType
			|	FROM
			|		SourceTable AS SourceTable
			|	LEFT JOIN
			|		InfobaseObjectMappingRegisterTable AS MappingOfInfobaseObjects
			|	ON
			|		  MappingOfInfobaseObjects.UniqueReceiverHandle = SourceTable.UUID
			|		AND MappingOfInfobaseObjects.UniqueSourceHandle REFS #TargetTable#
			|	LEFT JOIN
			|		#TargetTable# AS TargetTable
			|	ON
			|		  SourceTable.Ref = TargetTable.Ref
			|	WHERE
			|		NOT MappingOfInfobaseObjects.UniqueSourceHandle IS NULL
			...
			|			
			|		// {MAPPING REGISTER DATA}
			|		MappingOfInfobaseObjects.UniqueSourceHandle AS UniqueReceiverHandle,
			|		MappingOfInfobaseObjects.UniqueReceiverHandle AS UniqueSourceHandle,
			|		MappingOfInfobaseObjects.ReceiverType                     AS SourceType,
			|		MappingOfInfobaseObjects.SourceType                     AS ReceiverType
			|	FROM
			|		#TargetTable# AS TargetTable
			|	LEFT JOIN
			|		InfobaseObjectMappingRegisterTable AS MappingOfInfobaseObjects
			|	ON
			|		  MappingOfInfobaseObjects.UniqueSourceHandle = TargetTable.Ref
			|	LEFT JOIN
	...
	Else
		QueryText = QueryText + "
			...
			|		#custom_Fields_MappedSourceObjectTableByRegister_NestedSelect#
			|		
			|		MappingOfInfobaseObjects.UniqueSourceHandle AS Ref,
			|		
			|		#SourceTableIsFolder#                      AS IsSourceFolder,
			|		#MappingOfInfobaseObjectsIsFolder# AS IsTargetFolder,
			|			
			...
			|	ON
			|		  MappingOfInfobaseObjects.UniqueReceiverHandle = SourceTable.UUID
			|		AND MappingOfInfobaseObjects.ReceiverType                     = SourceTable.ObjectType
			|	WHERE
			|		NOT MappingOfInfobaseObjects.UniqueSourceHandle IS NULL
			|	) AS NestedSelect
			|;
	...
	If DataExchangeReUse.ThisIsExchangePlanXDTO(InfobaseNode) Then
		QueryText = StrReplace(QueryText, "#USER_FIELDS_MappedSourceObjectsTableByRegister_NestedSelect#", GetUserFields(UserFields, "CAST(UNF(MappingOfInfobaseObjects.UniqueSourceHandle, TargetTable.Ref) AS [ReceiverTableName]).# AS ReceiverFieldNN, SourceTable.# AS SourceFieldNN,"));
	Else
		QueryText = StrReplace(QueryText, "#USER_FIELDS_MappedSourceObjectsTableByRegister_NestedSelect#", GetUserFields(UserFields, "CAST(MappingOfInfobaseObjects.UniqueSourceHandle AS [ReceiverTableName]).# AS ReceiverFieldNN, SourceTable.# AS SourceFieldNN,"));
	EndIf;
	...
	If UserFields.Find("IsFolder") <> Undefined Then
		...
		QueryText = StrReplace(QueryText, "#InfobaseObjectsMatchingIsFolder#", "CAST(MappingOfInfobaseObjects.UniqueSourceHandle AS [ReceiverTableName]).IsFolder");
	...
	For Each Item In TableFieldList Do
		...
		If Attribute = Undefined Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Attribute with the ""%2"" name is not defined for the ""%1"" metadata object'"),
