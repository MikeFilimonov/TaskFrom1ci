...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	VariantsOfFilterStatusMap.Insert(ChoiceList.Add(
		"UnmappedObjectsSink", NStr("en = 'Unmapped data of this base'"),
	).Value, New FixedStructure("MappingState",  1));
	
	VariantsOfFilterStatusMap.Insert(ChoiceList.Add(
		"UnmappedSourceObjects", NStr("en = 'Unmapped second base data'"),
	).Value, New FixedStructure("MappingState", -1));
	...
	SecondApplicationName = ?(IsBlankString(SecondApplicationName), NStr("en = 'The second application'"), SecondApplicationName);
...
Procedure ExecuteDataImport(Command)
	NString = NStr("en = 'Receive data to the infobase?'");
...
Procedure SetConditionalAppearance()
	...
	Item.Appearance.SetParameterValue("Text", NStr("en = 'No mapping for object. The object will be copied'"));
...
Procedure ImportDataAfterQuestionOnDataReceiptConfirmation(Val QuestionResult, Val AdditionalParameters) Export
	...
	If Object.DataSuccessfullyImported Then
		NString = NStr("en = 'Data is already received. Receive data again?'");
...
Procedure CancelApprovedMappingAtServer(Filter)
	...
	InformationRegisters.MappingOfInfobaseObjects.DeleteRecord(Filter);
...
Procedure RunUserFieldsTaskCheck(Cancel, UserFields)
	...
	ElsIf UserFields.Count() > MaximumQuantityOfCustomFields() Then
		...
		MessageString = NStr("en = 'Reduce the number of fields (select no more than %1 fields)'");
...
Procedure SetConnectionInteractively()
	...
	QuestionText = NStr("en = 'Map the selected objects?'");
...
Procedure PerformMappingOfObjects(Cancel)
	...
	Except
		Cancel = True;
		WriteLogEvent(NStr("en = 'Object mapping wizard.Data analysis'", CommonUseClientServer.MainLanguageCode()),
