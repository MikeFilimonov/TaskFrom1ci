...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	WriteLogEvent(ConfigurationUpdate.EventLogMonitorEvent(), EventLogLevel.Information,,,
		NStr("en = 'Opening configuration update wizard...'"));
...
Procedure FieldUpdateFileStartChoice(Item, ChoiceData, StandardProcessing)
	...
	Dialog.Filter		= NStr("en = 'All delivery files (*.cf*;*.cfu)|*.cf*;*.cfu|Configuration delivery files (*.cf)|*.cf|Configuration update delivery files(*.cfu)|*.cfu'");
	Dialog.Title	= NStr("en = 'Select a configuration update delivery'");
...
Procedure DecorationUpdatePlatformNavigationRefDataProcessor(Item, URL, StandardProcessing)
	...
	OpenForm("DataProcessor.PlatformUpdateTip.Form.PlatformUpdateOrder", FormParameters);
...
Procedure ConfigureProxyServerParameters(Command)
	OpenForm("CommonForm.ProxyServerSettings", New Structure("ProxySettingAtClient", True), ThisObject);
EndProcedure
...
Procedure NewInVersion(Command)
	...
	Else
		ShowMessageBox(, NStr("en = 'No update information is available.'"));
	EndIf;
...
Function InsertScriptParameters(Val Text, Val ParametersStructure)
	...
	Result = StrReplace(Result, "[BlockInfobaseConnections]"		, ?(BlockInfobaseConnections, "true", "false"));
	...
	While CurrentVersionFrom <> VersionBefore Do
		...
		If CurrentVersionFrom = NewCurrentVersionFrom AND NewCurrentVersionFrom <> VersionBefore Then
			...
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Update to version %1 from the current version %2 is not available'"), VersionBefore, VersionFrom);
		EndIf;
...
Procedure RunImportOfListOfUpdates(Val ImportFileAddress, TableUpdates = Undefined)
	...
	ErrorInfo = NStr("en = 'An error occurred when reading the update list file'") + " " + FullPathFileExport;
	...
	If TableUpdates = Undefined Then // errors at file reading
		Raise NStr("en = 'An error occurred when reading the file'") + " " + FullPathFileExport;
   	EndIf;
...
Procedure BeforePageOpen(NewCurrentPage = Undefined)
	...
	ElsIf NewCurrentPage = Pages.ConnectionToSite Then
		...
		If Not IsBlankString(Object.TechnicalErrorInfo) Then
			Object.TechnicalErrorInfo = NStr("en = 'Technical information on the error:'") + Chars.LF + Object.TechnicalErrorInfo;
		EndIf;
	...
	ElsIf NewCurrentPage = Pages.UpdatesNotDetected Then
		...
		Items.InscriptionVersionForUpdate.Title			= ?(TypeOf(AvailableUpdateStructure) = Type("Structure"), NStr("en = 'Version available for update -'") + " " + AvailableUpdateStructure.Version, "");
		...
		If CommonUseClientServer.CompareVersions(StandardSubsystemsClientReUse.ClientWorkParameters().ConfigurationVersion,
			...
			Items.GroupUpdateIsNotDetected.Title = NStr("en = 'No update required'");
		...
		Else 
			...
			Items.GroupUpdateIsNotDetected.Title = NStr("en = 'Configuration update is not detected'");
		EndIf;
	...
	ButtonNext.Title = ?(NextButtonFunction, NStr("en = 'Next >'"), NStr("en = 'Finish'"));
...
Function RestoreResultsOfPreviousStart(Cancel = False)
	...
	If PageName = Pages.AvailableUpdate.Name Then
		...
		If CheckUpdateFilesReceived() Then
			...
			EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents, "Information",
				NStr("en = 'Configuration update files have already been received and saved locally.'"));
...
Function ProcessPageConnectionToSite(OutputMessages = True)
	...
	If Not ValueIsFilled(Object.UpdateServerUserCode) Then
		If OutputMessages Then
			ShowMessageBox(, NStr("en = 'Specify a user code for update.'"));
		EndIf;
	...
	If Not ValueIsFilled(Object.UpdatesServerPassword) Then
		If OutputMessages Then
			ShowMessageBox(, NStr("en = 'Specify a user password for update.'"));
		EndIf;
...
Function ProcessPageChoiceUpdateMode(OutputMessages = True, FlagCompleteJobs = False)
	...
	If FileInfobase AND Not SimulationModeOfClientServerIB() AND Object.CreateBackup = 2 Then
		...
		If Not File.Exist() Or Not File.IsDirectory() Then
			ShowMessageBox(, NStr("en = 'Specify an existing directory to save the IB backup.'"));
		EndIf;
	...
	If Object.UpdateMode = 0 Then   // Update now
		If FileInfobase AND Not SimulationModeOfClientServerIB() Then
			...
			If AvailabilityOfConnections Then
				ShowMessageBox(, NStr("en = 'The configuration update cannot proceed as connections with infobase were not terminated.'"));
				Return CurrentPage.Name;
			EndIf; 
		EndIf;
		ExecuteUpdate		= True;
	...
	Message = NStr("en = 'Update mode is selected:'") + " ";
	If Object.UpdateMode = 0 Then   // Update now
		Message = Message + NStr("en = 'now'");
	ElsIf Object.UpdateMode = 1 Then  // On closing application
		Message = Message + NStr("en = 'On exiting the application'");
	ElsIf Object.UpdateMode = 2 Then  // Plan update
...
Function ProcessPageSuccessfulUpdate(OutputMessages = True)
	...
	If ShowNewInVersion Then
		OpenForm("CommonForm.ReleaseNotes", New Structure("ShowOnlyChanges", True));
	EndIf;
...
Function ProcessPageUpdateFile(OutputMessages = True)
	...
	If Object.NeedUpdateFile = 1 Then
		...
		If Not ValueIsFilled(Object.UpdateFileName) Then
			If OutputMessages Then
				CommonUseClientServer.MessageToUser(NStr("en = 'Specify a configuration update delivery file.'"),,"Object.UpdateFileName");
			EndIf;
		...
		If Not File.Exist() OR Not File.IsFile() Then
			If OutputMessages Then
				CommonUseClientServer.MessageToUser(NStr("en = 'Delivery file of configuration update is not found.'"),,"Object.UpdateFileName");
			EndIf;
			CurrentItem = Items.FieldUpdateFile;
			Return Pages.UpdateFile.Name;
		EndIf;
...
Function GoToChoiceOfUpdateMode(IsGoNext = False)
	...
	If AdministrationParameters = Undefined Then
		...
		FormTitle = NStr("en = 'Install update'");
...
Function CheckAccessToIB()
	...
	Except
		Result = False;
		MessageText = NStr("en = 'Cannot connect to the infobase:'") + Chars.LF;
		EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents, 
	...
	If Result AND Not ClientWorkParameters.FileInfobase Then
		...
		Except
			Result = False;
			MessageText = NStr("en = 'Cannot connect to the server cluster:'") + Chars.LF;
			EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents,
				"Error", MessageText + DetailErrorDescription(ErrorInfo()));
			DetectedConnectionError = MessageText + BriefErrorDescription(ErrorInfo());
		EndTry;
...
Function CheckEMailSettings(CurrentPageName, OutputMessages = True)
	If Not CommonUseClientServer.EmailAddressMeetsRequirements(Object.EmailAddress) Then
		If OutputMessages Then
			ShowMessageBox(, NStr("en = 'Specify an allowable email address.'"));
		EndIf;
...
Function CreateSchedulerTask(Val ApplicationFileName, Val DateTime)
	...
	Try
		...
		If ErrorCode <> 0 Then	// Error codes: http://msdn2.microsoft.com/en-us/library/aa389389(VS.85).aspx.
			EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents, 
				"Error", NStr("en = 'An error occurred while creating a scheduler task:'")
					+ " " + ErrorCode);
			Return 0;
		EndIf;
		MessageText = NStr("en = 'Scheduler task is successfully planned (command: %1; date: %2; task code: %3).'");
	...
	Except
		...
		EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents, "Error",
			NStr("en = 'An error occurred while creating a scheduler task:'")
				+ " " + ErrorDescription());
		Return 0;
	EndTry;
...
Function ValidateAcceptableDateUpdateAtServer(DateTime)
	...
	If DateTime < Now Then
		Return NStr("en = 'Configuration update can be scheduled only for a future date and time.'");
	EndIf;
	If DateTime > AddMonth(Now, 1) Then
		Return NStr("en = 'Configuration update can be scheduled not later than in a month from the current date.'");
	EndIf;
...
Function DeleteSchedulerTask(CodeTasks)
	...
	Try
		...
		If Not Result Then	// Error codes: http://msdn2.microsoft.com/en-us/library/aa389957(VS.85).aspx.
			EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents, "Error",
				NStr("en = 'An error occurred while deleting the scheduler task:'")
					+ " " + ErrorCode);
			Return Result;
		EndIf;
		MessageText = NStr("en = 'Scheduler task is successfully removed (task code: %1).'");
		EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents, "Information",
...
Function UnpackUpdateInstallationPackage()
	...
	NameLogEvents = ConfigurationUpdateClient.EventLogMonitorEvent();
	EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents, 
		"Information", NStr("en = 'Unpacking update distribution...'"));
	...
	For Each Update In Object.AvailableUpdates Do
		...
		Try 
			...
			If Not FileExistsAtClient(UpdateFileName) Then
				EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents, "Error",
					NStr("en = 'Update distribution does not contain 1cv8.cfu:'")
			...
			EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents, "Information",
				NStr("en = 'Update distribution files are successfully uncompressed:'")
					+ " " + UpdateFileName);
		Except
			EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents, "Error",
		...
		Try
			...
			If Not FileExistsAtClient(UpdateFileName) Then
				EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), 
					"Error", NStr("en = 'Update distribution does not contain 1cv8.cfu:'")
						+ " " + Update.PathToLocalFile);
				Return False;
			EndIf;
			Update.PathToLocalUpdateFile = UpdateFileName;
			EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), 
				"Information", NStr("en = 'Update distribution files are successfully uncompressed:'")
					+ " " + UpdateFileName);
			ZipReader.Close();
		Except
			EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), 
				"Error",  NStr("en = 'Error while unzipping the updates distribution'")
					+ " " + ErrorDescription());
			Return False;
		EndTry;
	EndDo;
...
Function GetFileOfTemplatesList(Val OutputMessages = True, AvailableUpdateForNextEdition = False)
	...
	If Result.Status <> True Then
		...
		Except
			EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(),
				"Error", 
				NStr("en = 'An error occurred while deleting a temporary file'") + " " + PathToTemplatesListFile + Chars.LF +
				DetailErrorDescription(ErrorInfo()));
		EndTry;
		ErrorText = NStr("en = 'An error occurred when receiving a template list file:'") + " " + Result.ErrorInfo;
		If OutputMessages Then
	...
	Except
		ErrorText	= NStr("en = 'An error occurred when unpacking a file with available update list:'") + " ";
...
Function GetUpdate(OutputMessages = True)
	...
	Message = "";
	If Object.UpdateSource = 0 Then
		Message = Message + NStr("en = 'Receiving files from the Internet...'");
	Else
		Message = Message + NStr("en = 'Receiving update file from the specified source...'");
	EndIf;
	EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), 
		"Information", Message);
		...
		If File.Value <> Undefined AND File.Value.Received <> True Then	// It can also be Undefined.
			If Object.UpdateSource = 0 Then
				...
				If Result.Status <> True Then
					...
					Except
						EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), 
							"Error", NStr("en = 'An error occurred while deleting a temporary file'") + " " +
	...
	If CheckUpdateFilesReceived() Then
		EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), 
			"Information", NStr("en = 'Update files are successfully received.'"));
		GoToChoiceOfUpdateMode(True);
		Return Undefined;
	Else
		Message = NStr("en = 'An error occurred when receiving update files.'");
		EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), 
			"Error", Message);
...
Function GetUpdateFilesViaInternet(OutputMessages, AvailableUpdateForNextEdition = False)
	...
	If AvailableUpdate.PageName = Pages.AvailableUpdate.Name Then
		...
		If Object.AvailableUpdates.Count() = 0 Then
			EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), 
				"Information", NStr("en = 'The update cannot proceed: no updates available.'"));
			Return Pages.UpdatesNotDetected.Name;
		EndIf;
...
Function CheckUpdateInternet(OutputMessages = True) 
	Pages		= Items.AssistantPages.ChildItems;
	EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), 
		"Information", NStr("en = 'Checking for updates online...'"));
	...
	If Object.NeedUpdateFile = 1 Then
		...
		If Not File.Exist() OR Not File.IsFile() Then
			...
			EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), "Information",
				NStr("en = 'The update cannot proceed: the delivery file for configuration update was not found.'"));
...
Function LabelTextInfobaseBackup()
	...
	Result = NStr("en = 'Do not back up the infobase'");
	
	If Object.CreateBackup = 1 Then
		Result = NStr("en = 'Create temporary backup of infobase'");
	ElsIf Object.CreateBackup = 2 Then
		Result = NStr("en = 'Create infobase backup'");
	EndIf; 
	
	If Object.RestoreInfobase Then
		Result = Result + " " + NStr("en = 'and roll back in case of error'");
	Else
		Result = Result + " " + NStr("en = 'and do not roll back in case of error'");
	EndIf;
...
Procedure RunConfigurationUpdate()
	...
	EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), "Information",
		NStr("en = 'Updating configuration:'")
			+ " " + ScriptMainFileName);
...
Function ItIsPossibleToStartUpdate()
	...
	#If WebClient Then
		...
		MessageText = NStr("en = 'Updating the application in web client is unavailable.'");
	#EndIf
	
	If CommonUseClientServer.IsLinuxClient() Then
		ItIsPossibleToStartUpdate = False;
		MessageText = NStr("en = 'Updating the application is unavailable under Linux OS.'");
	EndIf;
	
	If CommonUseClient.ClientConnectedViaWebServer() Then
		ItIsPossibleToStartUpdate = False;
		MessageText = NStr("en = 'Updating the application using web server connection is unavailable.'");
	EndIf;
...
Procedure DisplayUpdateOrder()
	...
	If Not IsBlankString(FileNameOrderUpdate) Then
		ConfigurationUpdateClient.OpenWebPage(FileNameOrderUpdate);
	Else
		ShowMessageBox(, NStr("en = 'Update order description is missing.'"));
	EndIf;
