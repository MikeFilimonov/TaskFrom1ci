...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	ElsIf Parameters.Property("CodeOfInfobaseNode", CodeOfInfobaseNode) Then
		...
		If Object.InfobaseNode.IsEmpty() Then
			...
			DataExchangeServer.ShowMessageAboutError(NStr("en = 'Data exchange setting is not found.'"), Cancel);
			Return;
	...
	Else
		...
		DataExchangeServer.ShowMessageAboutError(NStr("en = 'Wizard cannot be opened.'"), Cancel);
		Return;
	...
	If Not DataExchangeReUse.IsUniversalDataExchangeNode(Object.InfobaseNode) Then
		...
		DataExchangeServer.ShowMessageAboutError(
			NStr("en = 'Data exchange execution with the setting is not required for the selected node.'"), Cancel);
		Return;
	...
	If NodesArray.Find(Object.InfobaseNode) = Undefined Then
		...
		CommonUseClientServer.MessageToUser(NStr("en = 'Data mapping is not required for the selected node.'"),,,, Cancel);
		Return;
...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	If CurrentPage = Items.BeginningPage Then
		ConfirmationText = NStr("en = 'Do you want to cancel data synchronization?'");		
	Else
		ConfirmationText = NStr("en = 'Do you want to stop data synchronization?'");		
	EndIf;
...
#Region PagePageQuestionAboutExportContent
...
&AtClient
Procedure External1CInfobaseConnectionSettings(Command)
...
Procedure ExecuteDataImportForRow(Command)
	...
	If SelectedRows.Count() = 0 Then
		NString = NStr("en = 'Select a table name in the statistical information field.'");
...
Procedure OpenMappingForm(Command)
	...
	If Not CurrentData.UsePreview Then
		ShowMessageBox(, NStr("en = 'Cannot map objects for the data type.'"));
		Return;
	EndIf;
...
&AtClient
Procedure DataImportLog(Command)
...
&AtClient
Procedure DataExportLog(Command)
...
Procedure ExportAdditionFilterHistory(Command)
	...
	Text = NStr("en = 'Saving the current configuration...'");
...
Procedure ExportAdditionFiltersHistoryEnd(Response, SettingRepresentation) Export
	...
	If Response = DialogReturnCode.Yes Then
		...
		If ScriptJobsAssistantInteractiveExchange = "OnlineSynchronizationDocuments" Then
			UpperLevelItems = FilterByDocumentTypesTree.GetItems();
...
Procedure ImportDataForStringQuestionContinuation(Val SelectedRows) 
	...
	Status(NStr("en = 'Data import is complete.'"));
EndProcedure
...
Procedure AfterWorksWithFilesExpansionCheck(Result, AdditionalParameters) Export
	...
	If Result Then
		File = New File(AdditionalParameters.DirectoryName);
		CheckFileExistence(File, AdditionalParameters);
	Else
		WarningText = NStr("en = 'File operation extension is not installed. Directory cannot be opened.'");
...
Procedure AfterFileExistenceCheck(Exist, AdditionalParameters) Export
	...
	Else
		WarningText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Directory ""%1"" does not exist or cannot be accessed.'"),
...
Procedure DetermineIsDirectory(IsDirectory, AdditionalParameters) Export
	...
	Else
		WarningText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = '""%1"" is a file, not a catalog.'"),
			AdditionalParameters.DirectoryName
		);
		ShowMessageBox(, WarningText);
	EndIf;
...
Procedure DeleteTemporaryDirectoryOfExchangeMessages(TempDirectoryName)
	...
	If Not IsBlankString(TempDirectoryName) Then
		...
		Try
			...
			TempDirectoryName = "";
		Except
			WriteLogEvent(NStr("en = 'Data exchange.Temporary file deletion'", CommonUseClientServer.MainLanguageCode()),
...
Procedure RefreshTransportSettingsPages()
	...
	Items.External1CInfobaseConnectionSettings.Visible = IsInRoleAddChangeOfDataExchanges;
...
Procedure RefreshRepresentationOfDataExchangeItemsState()
	...
	If Items.PagesOfDataImportStatus.CurrentPage=Items.ImportStateUndefined Then
		Items.DataImportLog.Title = NStr("en = 'Data has not been imported'");
	Else
		Items.DataImportLog.Title = DataExchangeClient.HyperlinkHeadersOfDataImport()[DataImportResult];
	EndIf;
	...
	If Items.PagesOfDataDumpStatus.CurrentPage=Items.ExportStateUndefined Then
		Items.DataExportLog.Title = NStr("en = 'Data is not exported'");
	Else
		Items.DataExportLog.Title = DataExchangeClient.HyperlinkHeadersOfDataDump()[DataExportResult];
	EndIf;
...
Function Attachable_BeginningPage_OnGoingNext(Cancel)
	...
	If Object.InfobaseNode.IsEmpty() Then
		...
		NString = NStr("en = 'Specify an infobase node'");
	...
	ElsIf Object.ExchangeMessageTransportKind.IsEmpty() Then
		...
		NString = NStr("en = 'Specify connection option'");
...
Procedure DataAnalysis()
	...
	Try
		...
		Result = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en = 'Analysis of exchange message data'")
		);
...
Procedure DataImport()
	...
	Try
		...
		Result = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en = 'Import data from the exchange message'"));
...
Function ExportAdditionSetSettingsServer(SettingRepresentation)
	...
	If ScriptJobsAssistantInteractiveExchange = "OnlineSynchronizationDocuments" Then
		...
		GenerateTreeSpeciesDocuments(DocumentTypes);
...
Procedure CompanyFilterClean(Command)
	...
	HeaderText = NStr("en = 'Confirmation'");
	QuestionText   = NStr("en = 'Clear filter by companies?'");
...
&AtClient
Procedure IncludeAllDocumentTypes(Command)
	
	NoteDocumentTypes(True);
	
EndProcedure

&AtClient
Procedure DisableAllDocumentTypes(Command)
	
	NoteDocumentTypes(False);
...
Procedure DocumentTypesFilterCheckOnChange(Item)
	...
	If CurrentData <> Undefined Then
		...
		If CurrentData.GetParent() = Undefined Then
			NoteDocumentTypes(MarkValue, CurrentData.GetID());
		EndIf;
...
&AtClient
Procedure FilterByDocumentTypesChoice(Item, SelectedRow, Field, StandardProcessing)
...
Procedure InitializeScriptTransfer(ExchangeOverConnectionToCorrespondent)
	...
	If ExchangeOverConnectionToCorrespondent Then
		...
		Else
			...
			Raise NStr("en = 'This data synchronization scenario is not supported.'");
...
Procedure FillAdditionalRegistration(AddAdditionalFilters = True)
	...
	FilterTree = FormAttributeToValue("FilterByDocumentTypesTree", Type("ValueTree"));
...
Function EditingRowFilterAdditionalListServer(ChoiceStructure)
	...
	CurrentData = FilterByDocumentTypesTree.FindByID(-ChoiceStructure.ActionSelect);
	If CurrentData=Undefined Then
		Return Undefined
	EndIf;
...
&AtClient
Procedure NoteDocumentTypes(MarkValue, ItemIdentificator = Undefined)
	
	If ItemIdentificator <> Undefined Then
		TreeItem = FilterByDocumentTypesTree.FindByID(ItemIdentificator);
...
Procedure GenerateTreeSpeciesDocuments(ArraySelectedValues = Undefined)
	...
	AddLineTreeOfDocumentsKind(MetaDocuments.SalesInvoice,			UpperLevelRow);
	AddLineTreeOfDocumentsKind(MetaDocuments.Quote,		UpperLevelRow);
	AddLineTreeOfDocumentsKind(MetaDocuments.AccountSalesFromConsignee,			UpperLevelRow);
	AddLineTreeOfDocumentsKind(MetaDocuments.ShiftClosure,			UpperLevelRow);
	...
	AddLineTreeOfDocumentsKind(MetaDocuments.AdditionalExpenses,			UpperLevelRow);
	AddLineTreeOfDocumentsKind(MetaDocuments.AccountSalesToConsignor,			UpperLevelRow);
	...
	AddLineTreeOfDocumentsKind(MetaDocuments.Stocktaking,	UpperLevelRow);
	AddLineTreeOfDocumentsKind(MetaDocuments.InventoryIncrease,			UpperLevelRow);
	...
	StringDetails.MetadataObjectName	= MetaDocuments.SalesOrder.Name;
	StringDetails.FullMetadataName		= MetaDocuments.SalesOrder.FullName();
	StringDetails.Presentation			= "Job-order";
	
	UpperLevelRow = FilterTree.Rows.Add();
	UpperLevelRow.Presentation = "Production";
	
	AddLineTreeOfDocumentsKind(MetaDocuments.Production,	UpperLevelRow);
	AddLineTreeOfDocumentsKind(MetaDocuments.InventoryTransfer,	UpperLevelRow);
	AddLineTreeOfDocumentsKind(MetaDocuments.SubcontractorReportIssued,	UpperLevelRow);
	...
	AddLineTreeOfDocumentsKind(MetaDocuments.CashVoucher,		UpperLevelRow);
	...
	AddLineTreeOfDocumentsKind(MetaDocuments.ArApAdjustments, UpperLevelRow);
	...
	ValueToFormAttribute(FilterTree, "FilterByDocumentTypesTree");
