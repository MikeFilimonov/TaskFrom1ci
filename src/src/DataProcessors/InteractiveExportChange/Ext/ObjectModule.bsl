...
Function InitializeThisObject(Val Source = "") Export
	...
	ComposerAllDocumentsFilter.Initialize(
		New DataCompositionAvailableSettingsSource(Data.CompositionSchema));
	ComposerAllDocumentsFilter.LoadSettings(Data.Settings);
	
	If IsBlankString(Source.AddressLinkerAllDocuments) Then
		AddressLinkerAllDocuments = PutToTempStorage(Data, Source.AddressOfFormStore);
	Else 
		AddressLinkerAllDocuments = Source.AddressLinkerAllDocuments;
	EndIf;
...
Function InitializeComposer(ListOfMetadataNames = Undefined, LimitUseOfSelection = False, AddressConservationScheme = Undefined)
	...
	AdditionalParameter.Value = NStr("en = 'Additional Information'");
	
	ReferenceParameter = Parameters.Find("ReasonForRegistrationLink");
	ReferenceParameter.Value = NStr("en = 'By reference'");
...
//                          restorable settings by variants 0 - without filter, 1 - filter all documents, 2 - detailed,
//                          3 - node script.
//
//  Returns:
//      ValueList - settings.
