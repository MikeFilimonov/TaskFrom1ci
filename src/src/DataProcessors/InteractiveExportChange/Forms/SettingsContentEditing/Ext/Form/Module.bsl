...
Procedure SettingsOptionsBeforeDeleting(Item, Cancel)
	...
	QuestionText   = NStr("en = 'Remove setting ""%1""?'");
...
Procedure SaveSetting(Command)
	...
	If IsBlankString(ViewCurrentSettings) Then
		CommonUseClientServer.MessageToUser(
			NStr("en = 'Name for the current setting is not entered.'"), , "ViewCurrentSettings");
		Return;
	EndIf;
		
	If SettingVariants.FindByValue(ViewCurrentSettings)<>Undefined Then
		HeaderText = NStr("en = 'Confirmation'");
		QuestionText   = NStr("en = 'Rewrite existing setting ""%1""?'");
