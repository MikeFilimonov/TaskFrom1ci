...
//        * MessageNumber                - Number, NULL  - If an object is specified, then message number is for it,
//                                                         otherwise, NULL.
//        * NotExported                 - Boolean, NULL - If an object is specified, then check box of the export,
//                                                        otherwise, NULL.
//        Appearance picture setup mark                       - Boolean       - If an object is specified, then 0 = no
//        registration, 1 - is present, otherwise, always 0.
	...
	For Each Meta In Metadata.ExchangePlans Do
		...
		If Not Result.IsEmpty() Then
			...
			For Each NodeString In TemporaryTable Do;
				...
				NewRow.InitialMark = ?(NodeString.ChangesOnNodeQuantity > 0, 1, 0);
...
//         * StructureName              - Structure - Key - metadata group (constants, catalogs
//                                                    etc), value - full names array.
//         * PresentationsStructure     - Structure - Key - metadata group (constants, catalogs
//                                                    etc), value - full names array.
//         * StructureAutoRecord   - Structure - Key - metadata group (constants, catalogs
...
// Parameters:
	CurParameters = New Structure("StructureName, PresentationsStructure, StructureAutoRecord, Rows", 
	...
	Result = New Structure("Tree, StructureName, PresentationsStructure, StructureAutoRecord", 
	...
	MetadataLevelForm(CurStringNumber, CurParameters, 9,  10, True, "ChartsOfCharacteristicTypes",	NStr("en = 'Charts of characteristic types'"));
...
Function RegisterSetDimensions(TableName, AllDimensions = False) Export
	...
	If ThisIsInformationRegister AND Meta.MainFilterOnPeriod Then
		...
		String.Title   = NStr("en = 'Accounting period'");
	EndIf;
...
Function CheckSettingsCorrectness(SettingKey = "") Export
		...
		Try
			...
			Else
				If ThisIsFileBase() Then
					Text = NStr("en = 'The ""%1"" file is not available'");
				Else
					Text = NStr("en = 'File ""%1"" is not available on the server'");
				EndIf;
	...
	Else
		...
		If Metadata.DataProcessors.Find(SettingAddressExternalDataQueryProcessors) = Undefined Then
			Text = NStr("en = 'Data processor ""%1"" is not found in the configuration'");
			Result.SettingAddressExternalDataQueryProcessors = StrReplace(Text, "%1", SettingAddressExternalDataQueryProcessors);
...
Function ExternalDataProcessorInfo() Export
	...
	Info.Insert("Information",    NStr("en = 'Data processor to manage object registration on exchange nodes before export generating. When using configuration with SL version 2.1.2.0 and higher, data processor controls data migration limitations for the exchange nodes.'"));
	...
	Command.Presentation = NStr("en = 'Editing of object changes registration'");
...
Procedure RecordChanges(Val Node, Val RegistrationObject)
	...
	If TypeOf(RegistrationObject) = Type("MetadataObject") Then
		...
		If Characteristics.IsReference Then
			...
			While Selection.Next() Do
				ModuleDataExchangeEvents.RecordChangesData(Node, Selection.Ref, SettingObjectsExportControl);
			EndDo;
	...
	// Regular object
	ModuleDataExchangeEvents.RecordChangesData(Node, RegistrationObject, SettingObjectsExportControl);
EndProcedure
...
Procedure MetadataLevelForm(CurrentLineNumber, Parameters, PictureIndex, NodsPictureIndex, AddSubordinated, MetaName, TemplatePresentation)
	...
	If WereSubordinate Then
		...
		If Not AddSubordinated Then
			Parameters.StructureAutoRecord.Insert(MetaName, AutoRecords);
		EndIf;
	Else
