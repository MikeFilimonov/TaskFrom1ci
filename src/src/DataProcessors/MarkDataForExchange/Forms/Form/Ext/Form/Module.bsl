...
// ProhibitedToChooseExchangeNode - Boolean           - Check box showing that user is not allowed to change the
//                                                  specified node. The ExchangeNode parameter should be specified.
...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Else
		...
		Else
			Raise StrReplace(
				NStr("en = 'Incorrect parameters for command ""%1"" - destination objects array is empty!'"),
	...
	If Not MonitorSettings() AND OpenNodeParameter Then
		...
		MessageText = StrReplace(
			NStr("en = 'Editing object registration is not available for ""%1"".'"),
...
Procedure ChoiceProcessing(ValueSelected, ChoiceSource)
	...
	Then
		Error = NStr("en = 'Unexpected result when querying from console'");
	Else
...
Procedure ShowExportResult(Command)
	...
	If Serialization.Count() > 0 Then
		...
		TitleText = NStr("en = 'Export result (in DIB mode)'");
		Text.Show(TitleText);
	EndIf;
...
//                                     is expected that the same parameter has been transferred and processed while
//                                     creating a form.
...
Procedure SetTitleNumbersMessages()
	...
	Text = NStr("en = '# of sent %1, # of received %2'");
...
Procedure MessageAboutResultsOfRegistration(Command, Results)
	...
	If TypeOf(Command) = Type("Boolean") Then
		If Command Then
			TitleWarnings = NStr("en = 'Change registration:'");
			WarningText = NStr("en = '%1 changes are registered
			                   |from %2 on the node ""%0""'");
		Else
			TitleWarnings = NStr("en = 'Cancel registration:'");
			WarningText = NStr("en = 'Registration of %1 changes is canceled on the ""%0"" node.'");
		EndIf;
	Else
		TitleWarnings = NStr("en = 'Change message number:'");
...
Procedure AddSelectedObjectsRegistration(WithoutAccountingAutoRecord = True)
	...
	Text = NStr("en = 'Register %1 for export on the ""%2"" node? Changing registration of a large number of objects can take a long time.'");
...
Procedure DeleteSelectedObjectsRegistration(WithoutAccountingAutoRecord = True)
	...
	Text = NStr("en = 'Cancel %1 registration for export on the ""%2"" node? Changing registration of a large number of objects can take a long time.'");
...
Procedure OperationWithResultsOfQuery(CommandOperations)
	...
	If CurFormName <> Undefined Then
		...
		If CommandOperations Then
			Text = NStr("en = 'Registration of query result changes'");
		Else
			Text = NStr("en = 'Cancel registration of request result changes'");
		EndIf;
...
Procedure FillCountRegistrationsInTree()
	...
	For Each Root In MetadataTree.GetItems() Do
		...
		For Each GroupOne In Root.GetItems() Do
			AmountGroup = New Structure("CountChanges, CountExported, CountNotExported", 0,0,0);
			
			ListOfNodes = GroupOne.GetItems();
			If ListOfNodes.Count() = 0 AND StructureNameMetadata.Property(GroupOne.MetaFullName) Then
				// Collection of nodes without nodes, sum manually, take auto registration from the structure.
				For Each MetaName In StructureNameMetadata[GroupOne.MetaFullName] Do
			...
			EndIf;
			FillPropertyValues(GroupOne, AmountGroup);
			
			AmountRoot.CountChanges     = AmountRoot.CountChanges     + GroupOne.CountChanges;
			AmountRoot.CountExported    = AmountRoot.CountExported    + GroupOne.CountExported;
			AmountRoot.CountNotExported = AmountRoot.CountNotExported + GroupOne.CountNotExported;
			
			FormatCountChanges(GroupOne);
		EndDo;
...
Function QuerySelectControlLinks(Address)
	...
	If TypeOf(Result) = Type("Array") Then 
		...
		If Result.Columns.Find("Ref") = Undefined Then
			Return NStr("en = 'Column ""Ref"" is missing in the last query result.'");
		EndIf;
	Else		
		Return NStr("en = 'An error occurred when receiving the query result data'");
	EndIf;
...
// Output changes for a reference type (catalog, document, chart of characteristic types, chart of accounts, calculation
// type, business processes, jobs).
// 
&AtServer
...
Procedure CustomizeBlankPage(Description, TableName = Undefined)
	...
	Else
		...
		If String <> Undefined Then
			CountsText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Objects registered:
				     |%1 Objects exported: %2 Objects not exported: %3'"),
				Format(String.CountChanges, "NFD=0; NZ="),
				Format(String.CountExported, "NFD=0; NZ="),
				Format(String.CountNotExported, "NFD=0; NZ="));
		EndIf;
	EndIf;
	

	Items.EmptyDecorationPage.Title = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = '%1.
		     |
		     |%2
...
Procedure CustomizeRecordSet(TableName, Dimensions, Description)
	...
	For Each String In Dimensions Do
		Name = String.Name;
		TextSelect = TextSelect + ",ChangesTable." + Name + " AS " + Prefix + Name + Chars.LF;
		// Not to interfere with the dimension "MessageNo" or "NotExported".
...
Function GetDescriptionSelectedMetadata(WithoutAccountingAutoRecord, MetaNameGroup = Undefined, MetaNameNode = Undefined)
	...
	If MetaNameGroup = Undefined AND MetaNameNode = Undefined Then
		...
		Text = NStr("en = 'all objects %1 by the selected kind hierarchy'");
	...
	If WithoutAccountingAutoRecord Then
		FlagText = "";
	Else
		FlagText = NStr("en = 'with automatic registration flag'");
	EndIf;
...
Procedure HandleProhibitionChangesNode()
	...
	Else
		...
		Title = StrReplace(NStr("en = 'Register changes for exchange with ""%1""'"), "%1", String(ExchangeNodeRef));
	EndIf;
