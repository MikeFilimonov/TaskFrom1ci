...
Function GetInformationAboutRules() Export
	...
	InfoString = NStr("en = 'Object registration rules of this infobase (%1) from %2'");
...
#Region ImportObjectRegistrationRules
...
#Region ObjectsRegistrationRulesCompilationByExchangePlanProperties
...
Function GetConditionTextForProperty(Rule, PropertiesOfObject)
	...
	// registered Object are located differently in CD 2 configuration.0 while setting ORR and in the query to Exchange
	// plan in this module.
...
#Region CompilationOfObjectsRegistrationRulesByTheObjectProperties
...
Procedure RunExchangePlanPresenceCheckup()
	...
	If Registration.ExchangePlanName <> ExchangePlanImportName Then
		...
		ErrorDescription = NStr("en = 'Exchange plan %1 is specified in the registration rules, and import in progress is for exchange plan %2'");
...
#Region WorkWithTheXMLReadingObject
...
#Region InitializeAttributesAndModuleVariables
...
Function MessagesInitialization()
	...
	Messages.Insert(1, NStr("en = 'An error occurred when opening the rule file'", MainLanguageCode));
	Messages.Insert(2, NStr("en = 'An error occurred when importing rules'", MainLanguageCode));
	Messages.Insert(3, NStr("en = 'Rule format error'", MainLanguageCode));
	Messages.Insert(4, NStr("en = 'An error occurred when receiving a reading rule file'", MainLanguageCode));
	Messages.Insert(5, NStr("en = 'Imported registration rules are not for the current exchange plan.'", MainLanguageCode));
