...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If OverallSystemPerformance.IsEmpty() Then
		Object.OverallSystemPerformance = NStr("en = 'Overall application performance'");
	Else
	...
	Except
		MessageText = NStr("en = 'Cannot import settings.'");
...
Procedure ChoiceProcessing(ValueSelected, ChoiceSource)
	...
	If Upper(ChoiceSource.FormName) = Upper("DataProcessor.PerformanceMeasurement.Form.FilterForm") Then
...
Procedure PerformanceRange(Item, SelectedRow, Field, StandardProcessing)
	...
	OpenForm("DataProcessor.PerformanceMeasurement.Form.ExecutionHistory", OpenParameters, ThisObject);
...
Procedure Setting(Command)
	...
	OpenForm("DataProcessor.PerformanceMeasurement.Form.AutomaticExportPerformanceMeasurements", , ThisObject);
...
Procedure SpecifyAPDEX(Command)
	...
	ToolTip = NStr("en = 'Enter required APDEX value'"); 
	APDEX = 0;
...
Procedure SetFilter(Command)
	...
	OpenForm("DataProcessor.PerformanceMeasurement.Form.FilterForm", , ThisObject);
...
Procedure RefreshIndicators(FilterValues = Undefined)
	...
	If TableOfKeyOperations = Undefined Then
		CommonUseClientServer.MessageToUser(NStr("en = 'Period was set up incorrectly.'"));
		Return;
	EndIf;
...
Procedure RefreshChart(TableOfKeyOperations)
	...
	HeaderText = StrReplace(HeaderText, "%1", Format(Object.StartDate, "DLF=D"));
	HeaderText = StrReplace(HeaderText, "%2", Format(Object.EndDate, "DLF=D"));
...
Function CalculateDateTimeSegment(StartDate, EndDate, IndexPeriod)
	...
	If NumberOfSteps <= IndexPeriod Then
		Raise NStr("en = 'Number of steps cannot be less than index.'");
	EndIf;
	...
	For Each TSRow In Object.Performance Do
		...
		Then
			...
			CommonUseClientServer.MessageToUser(
				NStr("en = 'Target time must be filled in.'"),
				,
