...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	...
	If Not ValueIsFilled(DataProcessorFileName) Or Not ValueIsFilled(DataProcessorFileAddress) Then
		...
		CommonUseClientServer.MessageToUser(NStr("en = 'Specify an external report or a data processor file'"), , "DataProcessorFileAddress");
		Cancel = True;
	...
	If Not ValueIsFilled(SafeMode) Then
		...
		CommonUseClientServer.MessageToUser(NStr("en = 'Specify the safe mode for the external module connection'"), , "SafeMode");
		Cancel = True;
...
Function EnableOnServer()
	...
	Else
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'File %1 is not a file of an external report or data processor'"),
			DataProcessorFileName);
