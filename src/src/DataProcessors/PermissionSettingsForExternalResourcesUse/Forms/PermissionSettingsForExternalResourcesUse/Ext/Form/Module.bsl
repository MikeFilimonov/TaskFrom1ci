...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	...
	If RequiredInfobaseAdministrationParameters Then
		...
		If Not Users.InfobaseUserWithFullAccess(IBUser, True) Then
			CommonUseClientServer.MessageToUser(NStr("en = 'User does not have administrative rights.'"),,
				FieldName,,Cancel);
			Return;
		EndIf;
...
Procedure ApplyPermissions()
	...
	OperationTypes = UsageParameters.OperationTypes;
	...
	PermissionSettingOnExternalResourcesUsageClient.ApplyChangesPermissionsInSecurityProfilesOnServerCluster(
		OperationTypes, Script, ClusterAdministrationParameters, InfobaseAdministrationParameters);
...
Function StartRequestsUsage(Val StorageAddress)
	...
	OperationTypes = New Structure();
	For Each EnumValue In Metadata.Enums.OperationsAdministrationSecurityProfiles.EnumValues Do
		OperationTypes.Insert(EnumValue.Name, Enums.OperationsAdministrationSecurityProfiles[EnumValue.Name]);
	EndDo;
	
	Return New Structure("OperationTypes, RequestsUsageScenario, RequiredInfobaseAdministrationParameters",
		OperationTypes, RequestsUsageScenario, RequiredInfobaseAdministrationParameters);
