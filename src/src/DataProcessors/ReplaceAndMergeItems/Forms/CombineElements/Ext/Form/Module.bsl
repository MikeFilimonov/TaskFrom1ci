...
Procedure UsagePlacesBeforeDeletion(Item, Cancel)
	...
	QuestionText = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = 'Delete item ""%1"" from the list for merging?'"),
...
Function CheckMergedRefs(Val RefsSet)
	...
	If QuantityRefs < 2 Then
		Result.Error = NStr("en = 'Specify several items for merging.'");
		Return Result;
	EndIf;
	...
	ElsIf Control.QuantityRefs <> QuantityRefs Then
		Result.Error = NStr("en = 'All mergable items must be of the same type.'");
	Else 
		// Everything is ok
...
Function EndingMessage()
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = 'Items (%1) are merged into ""%2""'"),
		UsagePlaces.Count(),
		MainItem);
...
Procedure FillUsagePlaces(Val UsageTable)
	...
	HeaderText = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = 'Merge items of %1 into a single one'"),
		Presentation
	);
EndProcedure
...
Procedure InitializeAssistantScript()
	...
	ButtonsAssistant.Cancel.ToolTip = NStr("en = 'Refuse to merge items'");
	...
	ButtonsAssistant.Next.ToolTip = NStr("en = 'Start merging items'");
	...
	ButtonsAssistant.Next.Visible = False;
	ButtonsAssistant.Cancel.Title = NStr("en = 'Abort'");
	ButtonsAssistant.Cancel.ToolTip = NStr("en = 'Stop joining items'");
	...
	ButtonsAssistant.Cancel.ToolTip = NStr("en = 'Close grouping results'");
	...
	ButtonsAssistant.Back.ToolTip = NStr("en = 'Go back to the main item selection'");
	ButtonsAssistant.Next.DefaultButton = True;
	ButtonsAssistant.Next.Title = NStr("en = 'Repeat'");
	ButtonsAssistant.Next.ToolTip = NStr("en = 'Repeat grouping'");
	ButtonsAssistant.Cancel.Title = NStr("en = 'Cancel'");
	ButtonsAssistant.Cancel.ToolTip = NStr("en = 'Close grouping results'");
	...
	If Not InformationAboutJob.Completed Then
		If ShowDialogBeforeClosing Then
			...
			If StepByStepAssistantSettings.ProcedureName = "DefineUsagePlacess" Then
				QuestionText = NStr("en = 'Stop searching for usage locations and close the form?'");
			ElsIf StepByStepAssistantSettings.ProcedureName = "ReplaceRefs" Then
				QuestionText = NStr("en = 'Stop merging items and close the form?'");
			EndIf;
...
Function BackGroundJobStart(Val ProcedureName)
	...
	If ProcedureName = "DefineUsagePlacess" Then
		...
		MethodName = NStr("en = 'Search and deletion of duplicates: Determine usage locations'");
		MethodParameters = RefsArrayFromSet(UsagePlaces);
		
	ElsIf ProcedureName = "ReplaceRefs" Then
		
		MethodName = NStr("en = 'Search and deletion of duplicates: Merge items'");
		SubstitutionsPairs = New Map;
		For Each String In UsagePlaces Do
			SubstitutionsPairs.Insert(String.Ref, MainItem);
		EndDo;
