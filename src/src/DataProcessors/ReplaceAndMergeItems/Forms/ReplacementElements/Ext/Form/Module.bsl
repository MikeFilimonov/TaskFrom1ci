...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	EventReplacementNotifications        = DataProcessors.ReplaceAndMergeItems.EventReplacementNotifications();
	...
	If ReplaceReferences.Count() > 1 Then
		Items.LabelSelectedType.Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Select one of the ""%1"" items the selected values (%2) should be replaced with:'"),
			MainMetadata.Presentation(), ReplaceReferences.Count());
	Else
		Title = NStr("en = 'Replace item'");
		Items.LabelSelectedType.Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Select one of the ""%1"" items ""%2"" should be replaced with:'"),
			MainMetadata.Presentation(), ReplaceReferences[0].Ref);
	EndIf;
	Items.ToolTipSelectTargetItem.Title = NStr("en = 'Replacement item is not selected.'");
...
Function EndingMessage()
	...
	If Count = 1 Then
		ResultText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Item ""%1"" is replaced with ""%2""'"),
			ReplaceReferences[0].Ref,
			TargetItemResult);
	Else
		ResultText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Items (%1) are replaced with ""%2""'"),
			Count,
			TargetItemResult);
	EndIf;
...
Function FillFailedReplacements(Val ReplacementResults)
	...
	For Each ResultRow In ReplacementResults Do
		...
		If ErrorType = "UnknownData" Then
			ErrorString.Cause = NStr("en = 'Data which processing was not planned is detected.'");
		...
		ElsIf ErrorType = "DataChanged" Then
			ErrorString.Cause = NStr("en = 'Data is changed by another user.'");
			
		ElsIf ErrorType = "RecordingError" Then
			ErrorString.Cause = ResultRow.ErrorText;
			
		ElsIf ErrorType = "ErrorDelete" Then
			ErrorString.Cause = NStr("en = 'You cannot delete data.'");
...
Procedure InitializeReplacedReferences(Val RefArray)
	...
	If QuantityRefs = 0 Then
		ParametersErrorText = NStr("en = 'No item for replacement is specified.'");
		Return;
	EndIf;
	...
	ElsIf Conditions.QuantityRefs <> QuantityRefs Then
		ParametersErrorText = NStr("en = 'All replaceable items must be of the same type.'");
		Return;
	EndIf;
	...
	If Result[2].Unload().Count() = 0 Then
		If QuantityRefs > 1 Then
			ParametersErrorText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'There is nothing to replace the selected items (%1) with.'"), QuantityRefs);
		Else
			ParametersErrorText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'There is nothing to replace selected item ""%1"" with.'"), CommonUse.SubjectString(TargetItem));
		EndIf;
		Return;
	EndIf;
...
Procedure InitializeAssistantScript()
	...
	ButtonsAssistant.Next.ToolTip = NStr("en = 'Start replacing items'");
	ButtonsAssistant.Cancel.Title = NStr("en = 'Cancel'");
	ButtonsAssistant.Cancel.ToolTip = NStr("en = 'Refuse to replace items'");
	...
	ButtonsAssistant.Cancel.Title = NStr("en = 'Abort'");
	ButtonsAssistant.Cancel.ToolTip = NStr("en = 'Stop item replacement'");
	...
	ButtonsAssistant.Cancel.ToolTip = NStr("en = 'Close results of item replacement'");
	...
	ButtonsAssistant.Back.ToolTip = NStr("en = 'Return to target item selection'");
	ButtonsAssistant.Next.Title = NStr("en = 'Repeat replacement >'");
	ButtonsAssistant.Next.ToolTip = NStr("en = 'Repeat item replacement'");
	ButtonsAssistant.Cancel.Title = NStr("en = 'Close'");
	ButtonsAssistant.Cancel.ToolTip = NStr("en = 'Close results of item replacement'");
...
Procedure StepSelectTargetItemBeforeNextAction(Val StepParameters, Val AdditionalParameters) Export
	...
	ElsIf ReplaceReferences.Count() = 1 AND CurrentData.Ref = ReplaceReferences.Get(0).Ref Then
		ShowMessageBox( , NStr("en = 'An item cannot be replaced with itself.'"));
		Return;
		
	ElsIf AttributeValue(CurrentData, "IsFolder", False) Then
		ShowMessageBox( , NStr("en = 'Cannot replace item with group.'"));
		Return;
	EndIf;
...
Procedure CheckAppliedAreaReplacementAdmissibility(Val StepParameters)
	...
	WarningParameters.Insert("Title", NStr("en = 'You cannot replace items'"));
	WarningParameters.Insert("MessageText", ErrorText);
	OpenForm("DataProcessor.ReplaceAndMergeItems.Form.MultilineWarning", WarningParameters, ThisObject);
...
Function InitializeMaster(Val PagesGroup, Val ButtonNext, Val ButtonBack, Val ButtonCancel)
	...
	Result.Insert("HomePage", Undefined);
...
//                                            be executed after clicking the Next button before going to the next page.
//                                            The procedure will be called with two parameters:
...
//
// Parameters:
//     HomePage - Number, String, FormGroup - Number of a step, group-page, or its identifier.
//
&AtClient
Procedure SetAssistantInitialPage(Val Page)
	
	StepByStepAssistantSettings.HomePage = AssistantStepNumberByIdentifier(Page);
...
Procedure RunAssistant()
	If StepByStepAssistantSettings.HomePage = Undefined Then
		Raise NStr("en = 'Before you run this wizard, set the home page.'");
		
	ElsIf StepByStepAssistantSettings.HomePage = -1 Then
		...
		For Each StepDescription In StepByStepAssistantSettings.Steps Do
			For Each KeyValue In PossibleActions Do
				...
				If Not IsBlankString(HandlerName) Then
					...
					Except
						Text = StringFunctionsClientServer.SubstituteParametersInString(
							NStr("en = 'An error occurred when creating event handler %1 for page %2, procedure %3 is not defined'"),
							NameActions, 
	...
	Else
		...
		// Deferred launch
		StepByStepAssistantSettings.CurrentStepNumber = StepByStepAssistantSettings.HomePage;
		StepByStepAssistantSettings.HomePage    = -1;
...
Procedure GoToAssistantStep(Val IdentifierStep, Val TriggerEvents = False)
	...
	If NextStep = Undefined Then
		Error = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Step of wizard %1 is not found'"),
			IdentifierStep
		);
		Raise Error;
	EndIf;
...
Procedure AssistantStepEnd(Val StepParameters)
	...
	ElsIf StepParameters = 1 AND NextStep > LastStep Then
		...
		Raise NStr("en = 'Attempting to go over the last wizard step'");
		
	ElsIf StepParameters = -1 AND NextStep < 0 Then
		// You are trying to take a step outside back.
		Raise NStr("en = 'Attempting to go back from the first wizard step'");
...
Function AssistantStepNumberByIdentifier(Val IdentifierStep)
	...
	Raise StrReplace(NStr("en = 'Step ""%1"" is not found.'"), "%1", SearchName);
EndFunction
...
Function BackgroundJobImportOnClient(InterruptIfNotCompleted, ShowDialogBeforeClosing)
	...
	If Not InformationAboutJob.Completed Then
		If ShowDialogBeforeClosing Then
			...
			QuestionText = NStr("en = 'Stop items replacement and close the form?'");
...
Function BackGroundJobStart()
	...
	Try
		Task = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en = 'Search and delete duplicates: Reference replacement'"));
	Except
		ErrorInfo = ErrorInfo();
	EndTry;
