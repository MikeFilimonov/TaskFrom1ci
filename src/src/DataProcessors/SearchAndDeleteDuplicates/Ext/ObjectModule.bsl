...
Function DuplicatesGroups(Val SearchParameters, Val ReferenceObject = Undefined) Export
	...
	EndDo;
	IdentityCondition = Mid(IdentityCondition, 4);
	...
	If Not IdentityFieldsStructure.Property("Description") AND Not SimilarityFieldsStructure.Property("Description") Then
		QueryCommonPart = QueryCommonPart + "," + ?(HasName, "Description", "UNDEFINED") + " AS Description";
	EndIf;
	...
	Else
		...
		EndIf;
		If Not IdentityFieldsStructure.Property("Description") AND Not SimilarityFieldsStructure.Property("Description") Then
			TextPreselection = TextPreselection + "," + ?(HasName, "Description", "UNDEFINED") + " AS Description";
		EndIf;
	...
	While Selection.Next() Do
		...
		If ReturnPortionSize > 0 AND (DuplicatesTable.Count() > ReturnPortionSize) Then
			...
			If DuplicatesTable.Count() = 0 Then
				Result.ErrorDescription = NStr("en = 'Too many items are found, not all duplicate groups are defined.'");
			Else
				Result.ErrorDescription = NStr("en = 'Too many items are found. Specify the duplicate search criteria.'");
			EndIf;
			Break;
		EndIf;
...
Procedure RunCommand(ExecuteParameters, ResultAddress) Export
	...
	Else
		...
		Raise NStr("en = 'Search and delete duplicates: Command ""%1"" is not supported.'");
