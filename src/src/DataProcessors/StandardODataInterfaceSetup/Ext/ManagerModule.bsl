...
//		* Used - Boolean - flag of authorization for access to standard OData interface;
//		* Login - String - user login for authorization in standard OData interface;
//		* Password - String - user password for authorization in standard OData interface.
// Value is sent in structure content only in case when it is required to change the password.
//
	...
	If AuthorizationSettings.Used Then
		...
		Try
			...
			Comment = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'User is written for standard interface OData. IB user description:------------------------------------------- %1 ------------------------------------------- Result: ------------------------------------------- %2 -------------------------------------------'",
					LangCode),
		...
		Except
			...
			Comment = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'An error occurred while writing a user for standard interface OData. IB user description: ------------------------------------------- %1 ------------------------------------------- Error text: ------------------------------------------- %2 -------------------------------------------'",
					LangCode),
				CommonUse.ValueToXMLString(IBUserDescription),
...
//                                              can get an access
//                         to object record, * SubordinateDocumentStructure -      Array(MetadataObject) - array of metadata
...
Function DataModelProvidedForStandardODataInterface() Export
	...
	Result.Columns.Add("SubordinateDocumentStructure", New TypeDescription("Array"));
...
Procedure CheckODataRoleContent() Export
	...
	If ExcessiveRights.Count() > 0 Then
		ErrorText = Chars.NBSp + NStr("en = 'The following rights are redundant in the role:'") + Chars.LF + Chars.CR + 
	...
	If Errors.Count() > 0 Then
		...
		ErrorMessage = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Errors in the role right content were found %1:'"),
			RoleForStandardODataInterface().Name);
...
Procedure FillDataModelProvidedForStandardODataInterface(Val Result, Val DescriptionFull, Val Excluded)
	...
	SubordinateDocumentStructure = CommonUseSTL.MetadataObjectDependence(DescriptionFull);
	
	For Each KeyAndValue In SubordinateDocumentStructure Do
		...
		String.SubordinateDocumentStructure.Add(DependencyFullName);
...
Procedure CheckODataRoleContentWhenUpdating() Export
	...
	Else
		Raise NStr("en = 'Handler shall not be used when data area division is enabled'");
	EndIf;
...
Procedure CheckPossibilityToCreateUserForStandardODataInterfaceCalls()
	...
	If UserCount = 0 Then
		...
		Raise NStr("en = 'Cannot create separate login and password for automatic REST service because there are no other users in the application.'");
