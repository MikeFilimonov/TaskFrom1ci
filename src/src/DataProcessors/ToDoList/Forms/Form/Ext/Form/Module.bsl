...
Procedure NotificationProcessing(EventName, Parameter, Source)
	...
	If EventName = "ToDoList_AutoUpdateEnabled" Then
		...
		AttachIdleHandler("AutomaticallyUpdateToDoList", UpdatePeriod);
	ElsIf EventName = "ToDoList_AutoUpdateDisabled" Then
		DetachIdleHandler("AutomaticallyUpdateToDoList");
	EndIf;
...
Procedure Configure(Command)
	...
	FormParameters.Insert("ToDoList", ToDoListInStorage);
	OpenForm("DataProcessor.ToDoList.Form.PanelSettings", FormParameters,,,,,ResultHandler);
...
#Region FormingAListOfUserWorks

&AtClient
Procedure AutomaticallyUpdateToDoList()
	LaunchToDoListUpdate(True);
EndProcedure

&AtServer
Procedure UpdateToDoList(ToDoList)
	
	ToDoList.Sort("ThisIsSection Desc, PresentationOfSection Asc, Important Desc");
	
	SectionsWithImportantWork = New Structure;
	SavedDisplaySettings = ToDoListService.SavedDisplaySettings();
...
Procedure LaunchBackgroundJobOnServer(ExecutionBroken = False)
	...
	If ToDoListInStorage = "" Then
		ToDoListInStorage = PutToTempStorage(Undefined, UUID);
	EndIf;
	
	If CommonUseClientServer.DebugMode() Then
		ToDoListService.UserToDoList(ToDoListInStorage);
	Else
		ProcedureParameters = New Array;
		ProcedureParameters.Add(ToDoListInStorage);
		ProcedureParameters.Add(SessionParameters.ClientParametersOnServer);
		
		JobParameters = New Array;
		JobParameters.Add("ToDoListService.UserToDoList");
		JobParameters.Add(ProcedureParameters);
		
		JobDescription = NStr("en = 'Update current to-dos'");
...
&AtServer
Procedure ImportToDoList()
	
	ToDoList = GetFromTempStorage(ToDoListInStorage);
	...
	If OutputToDosAndSections.Count() = 0 Then
		Items.NoToDoListPage.Visible = True;
	Else
		Items.NoToDoListPage.Visible = False;
...
Procedure SaveCollapsedSections(CollapsedSections)
	...
	DisplaySettings = CommonUse.CommonSettingsStorageImport("ToDoList", "DisplaySettings");
	...
	CommonUse.CommonSettingsStorageSave("ToDoList", "DisplaySettings", DisplaySettings);
...
&AtServer
Procedure ActionOnClosingToDoListPanel()
