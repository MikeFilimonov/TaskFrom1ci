...
//                     according to which you should search for an object: "Code", "Name", <AttributeName>, "Name"
//                     (predefined value).
//
// Returns:
...
// Generates debugging module with handlers and algorithms and all required global variables, general functions wrappers
// and comments.
//
// Parameters:
...
Procedure ExportEventHandlers(Cancel) Export
	...
	If ThisIsInteractiveMode Then
		...
		If ErrorFlag Then
			...
			MessageToUser(NStr("en = 'Errors occurred when exporting handlers.'"));
			
		Else
			
			MessageToUser(NStr("en = 'Handlers are successfully exported.'"));
...
// exported while exporting handlers from file with data as handler code is not in the exchange rules node but in the
// separate node. This algorithm is exported like the other ones while exporting handlers from file.
...
Procedure AddInStreamFullHandler(Result, HandlerInterface, Handler)
	...
	For IndexOf = 1 To StrLineCount(Handler) Do
		...
		// IN the debugging mode of "Code integration" algorithms insert algorithms code directly to the handler code. Insert
		// algorithm code instead of its call. IN the algorithms code algorithms nesting is already considered.
		If AlgorithmsDebugMode = mAlgorithmDebugModes.CodeIntegration Then
...
// Expands the "HandlerAlgorithms" array with algorithm names that are called from the passed "HandlerString" handler
// procedure string.
//
// Parameters:
...
// algorithm call operator "Execute(Algorithms.Algorithm_1);" full code of the called algorithm is inserted with
// "PrefixString" shift. Function recursively calls itself until all nested algorithms are considered.
//  
// Parameters:
		...
		If HandlerAlgorithms.Count() <> 0 Then // IN this string there are algorithms calls.
			...
			For Each Algorithm In HandlerAlgorithms Do
				...
				If RequestedItemArray.Find(Upper(Algorithm)) <> Undefined Then // Recursive algorithm call.
					...
					OperatorString = NStr("en = 'CallException ""ALGORITHM RECURSIVE CALL: %1"";'");
...
Function CreateNewObject(Type, SearchProperties, Object = Undefined, 
	...
	// should add field to the possible search fields and specify fields without Owner in the SEARCHFIELDS event if the
	// search by it is unnecessary.
...
Function ImportObjectTypes(Source)
	...
	AllowedLengthValue =  deAttribute(Source, deStringType, "AllowedLength");
	...
	If TypeArray.Count() > 0 Then
		...
		If Length > 0 Then
			If AllowedLengthValue = "Fixed" Then
				AllowedLengthValue = AllowedLength.Fixed;
			Else
				AllowedLengthValue = AllowedLength.Variable;
			EndIf;
			StringQualifiers = New StringQualifiers(Length, AllowedLengthValue);
		Else
...
Procedure CommentsToObjectImport(NPP, Rulename, Source, ObjectType, GNPP = 0)
	...
	If CommentObjectProcessingFlag Then
		...
		If NPP <> 0 Then
			MessageString = PlaceParametersIntoString(NStr("en = 'Import object #%1'"), NPP);
		Else
			MessageString = PlaceParametersIntoString(NStr("en = 'Import object #%1'"), GNPP);
		EndIf;
...
Procedure DumpGroupOfProperties(Source, Receiver, IncomingData, OutgoingData, OCR, PGCR, PropertyCollectionNode, 
	...
	If ExportGroupToFile Then
		...
		Except
			...
			WriteInformationAboutErrorConversionHandlers(1000, ErrorDescription(), NStr("en = 'An error occurred when creating a temporary file for data export'"));
...
Function DumpByRule(Source					= Undefined,
	...
	If CommentObjectProcessingFlag Then
		...
		StringForUser = ?(GetRefNodeOnly, NStr("en = 'Conversion of reference to object: %1'"), NStr("en = 'Object conversion: %1'"));
...
Procedure ImportDataEchangeParameterValues()
	...
	If EventsAfterParameterImport.Property(Name, AfterParameterImportAlgorithm)
		...
		If HandlersDebugModeFlag Then
			...
			Raise NStr("en = 'Debugging of handler ""After parameter import"" is not supported.'");
	...
	If Conversion.Property("AfterExchangeRuleImport", AfterExchangeRuleImportEventText)
		...
		Try
			...
			If HandlersDebugModeFlag Then
				...
				Raise NStr("en = 'Debugging of handler ""After exchange rules import"" is not supported.'");
...
Procedure ReadData(ErrorInfoResultString = "") Export
				...
				If Not IsBlankString(AlgorithmText) Then
					...
					Try
						...
						If HandlersDebugModeFlag Then
							...
							Raise NStr("en = 'Debugging of handler ""After parameters import"" is not supported.'");
						...
						If Cancel = True Then
							...
							If Not IsBlankString(CancelReason) Then
								ExceptionString = PlaceParametersIntoString(NStr("en = 'Data import was canceled as: %1'"), CancelReason);
	...
	Except
		...
		ErrorString = PlaceParametersIntoString(NStr("en = 'An error occurred when importing data: %1'"), ErrorDescription());
...
Procedure ExecuteActionsAfterDataReadIsCompleted() Export
	...
	WriteInExecutionProtocol(PlaceParametersIntoString(
		NStr("en = 'Imported objects: %1'"), mImportedObjectCounter), , False, , , True);
	
	FinishExchangeProtocolLogging();
	
	If ThisIsInteractiveMode Then
		MessageToUser(NStr("en = 'Data import is complete.'"));
	EndIf;
...
Procedure PassInformationAboutRecordsToReceiver(InformationToWriteToFile, ErrorStringInTargetInfobase = "") Export
	...
	If Not IsBlankString(ErrorStringInTargetInfobase) Then
		...
		MessageString = PlaceParametersIntoString(NStr("en = 'Import in the target: %1'"), ErrorStringInTargetInfobase);
...
Procedure RunExport() Export
	...
	Except
		...
		WriteInExecutionProtocol(PlaceParametersIntoString(
			NStr("en = 'An error occurred when exporting data: %1'"), ErrorString), Undefined, True, , , True);
	...
	WriteInExecutionProtocol(PlaceParametersIntoString(
		NStr("en = 'Exported objects: %1'"), mExportedObjectCounter), , False, , , True);
	...
	If ThisIsInteractiveMode Then
		MessageToUser(NStr("en = 'Data is exported.'"));
	EndIf;
...
Procedure MessagesInitialization()
	...
	deMessages.Insert(2,  NStr("en = 'An error occurred when unpacking an exchange file. The file is locked'"));
	deMessages.Insert(3,  NStr("en = 'The specified exchange rules file does not exist'"));
	deMessages.Insert(4,  NStr("en = 'An error occurred when creating COM object Msxml2.DOMDocument'"));
	deMessages.Insert(5,  NStr("en = 'An error occurred when opening the exchange file'"));
	deMessages.Insert(6,  NStr("en = 'An error occurred when importing exchange rules'"));
	deMessages.Insert(7,  NStr("en = 'Error in exchange rule format'"));
	deMessages.Insert(8,  NStr("en = 'Incorrect file name for data export'"));
	deMessages.Insert(9,  NStr("en = 'Error in exchange file format'"));
	deMessages.Insert(10, NStr("en = 'File name for data export is not specified (Data file name)'"));
	deMessages.Insert(11, NStr("en = 'Link to a non-existing metadata object in the exchange rules'"));
	deMessages.Insert(12, NStr("en = 'File name with exchange rules is not specified (Rule file name)'"));
	
	deMessages.Insert(13, NStr("en = 'An error occurred when receiving a value of the object property (by the name of the source property)'"));
	deMessages.Insert(14, NStr("en = 'An error occurred when receiving the object property value (by the target property name)'"));
	
	deMessages.Insert(15, NStr("en = 'File name for data import is not specified (File name for import)'"));
	
	deMessages.Insert(16, NStr("en = 'An error occurred while receiving value of subobject property (by source property name)'"));
	deMessages.Insert(17, NStr("en = 'An error occurred while receiving value of subobject property (by target property name)'"));
	
	deMessages.Insert(19, NStr("en = 'An error occurred in event handler BeforeObjectImport'"));
	deMessages.Insert(20, NStr("en = 'An error occurred in event handler OnObjectImport'"));
	deMessages.Insert(21, NStr("en = 'An error occurred in event handler AfterObjectImport'"));
	deMessages.Insert(22, NStr("en = 'An error occurred in event handler BeforeDataImport (conversion)'"));
	deMessages.Insert(23, NStr("en = 'An error occurred in event handler AfterDataImport (conversion)'"));
	deMessages.Insert(24, NStr("en = 'An error occurred when removing an object'"));
	deMessages.Insert(25, NStr("en = 'An error occurred when writing the document'"));
	deMessages.Insert(26, NStr("en = 'An error occurred when writing the object'"));
	deMessages.Insert(27, NStr("en = 'An error occurred in event handler BeforeProcessClearingRule'"));
	deMessages.Insert(28, NStr("en = 'An error occurred in event handler AfterClearingRuleProcessing'"));
	deMessages.Insert(29, NStr("en = 'An error occurred in event handler BeforeDeleteObject'"));
	
	deMessages.Insert(31, NStr("en = 'An error occurred in event handler BeforeProcessExportRule'"));
	deMessages.Insert(32, NStr("en = 'An error occurred in event handler AfterDumpRuleProcessing'"));
	deMessages.Insert(33, NStr("en = 'An error occurred in event handler BeforeObjectExport'"));
	deMessages.Insert(34, NStr("en = 'An error occurred in event handler AfterObjectExport'"));
	
	deMessages.Insert(39, NStr("en = 'An error occurred when executing the algorithm from the exchange file'"));
	
	deMessages.Insert(41, NStr("en = 'An error occurred in event handler BeforeObjectExport'"));
	deMessages.Insert(42, NStr("en = 'An error occurred in event handler OnObjectExport'"));
	deMessages.Insert(43, NStr("en = 'An error occurred in event handler AfterObjectExport'"));
	
	deMessages.Insert(45, NStr("en = 'Object conversion rule is not found'"));
	
	deMessages.Insert(48, NStr("en = 'An error occurred in event handler BeforeExportProcessor of the property group'"));
	deMessages.Insert(49, NStr("en = 'An error occurred in event handler AfterExportProcessor of the property group'"));
	deMessages.Insert(50, NStr("en = 'Error in event handler BeforeExport (of collection object)'"));
	deMessages.Insert(51, NStr("en = 'Error in event handler OnExport (of collection object)'"));
	deMessages.Insert(52, NStr("en = 'Error in event handler AfterExport (of collection object)'"));
	deMessages.Insert(53, NStr("en = 'An error occurred in global event handler BeforeObjectImporting (conversion)'"));
	deMessages.Insert(54, NStr("en = 'An error occurred in global event handler AfterObjectImport (conversion)'"));
	deMessages.Insert(55, NStr("en = 'An error occurred in event handler BeforeExport (properties)'"));
	deMessages.Insert(56, NStr("en = 'An error occurred in event handler OnExport (properties)'"));
	deMessages.Insert(57, NStr("en = 'An error occurred in event handler AfterExport (properties)'"));
	
	deMessages.Insert(62, NStr("en = 'An error occurred in event handler BeforeDataExport (conversion)'"));
	deMessages.Insert(63, NStr("en = 'An error occurred in event handler AfterDataExport (conversion)'"));
	deMessages.Insert(64, NStr("en = 'An error occurred in global event handler BeforeObjectConversion (conversion)'"));
	deMessages.Insert(65, NStr("en = 'An error occurred in global event handler BeforeObjectExport (conversion)'"));
	deMessages.Insert(66, NStr("en = 'An error occurred when receiving a subordinate object collection from the incoming data'"));
	deMessages.Insert(67, NStr("en = 'An error occurred when receiving the subordinate object properties from the incoming data'"));
	deMessages.Insert(68, NStr("en = 'An error occurred when receiving the object properties from the incoming data'"));
	
	deMessages.Insert(69, NStr("en = 'An error occurred in global event handler AfterObjectExport (conversion)'"));
	
	deMessages.Insert(71, NStr("en = 'Match for the Source value is not found'"));
	
	deMessages.Insert(72, NStr("en = 'An error occurred when exporting data for the exchange plan node'"));
	
	deMessages.Insert(73, NStr("en = 'An error occurred in event handler SearchFieldsSequence'"));
	
	deMessages.Insert(74, NStr("en = 'Import exchange rules for data export again'"));
	
	deMessages.Insert(75, NStr("en = 'An error occurred when executing the algorithm after import of the parameter values'"));
	
	deMessages.Insert(76, NStr("en = 'An error occurred in event handler AfterObjectExportToFile'"));
	
	deMessages.Insert(77, NStr("en = 'File of external data processor with connected procedures of event handlers is not specified'"));
	
	deMessages.Insert(78, NStr("en = 'An error occurred when creating an external data processor from a file with event handler procedures'"));
	...
	deMessages.Insert(80, NStr("en = 'Data exchange can be executed only if you have full rights'"));
	
	deMessages.Insert(1000, NStr("en = 'An error occurred when creating a temporary file of data export'"));
...
Function DefineSufficiencyOfParametersForConnectionToInformationBase(ConnectionStructure, ConnectionString = "", ErrorMessageString = "")
	...
	If ConnectionStructure.FileModeVersion  Then
		...
		If IsBlankString(ConnectionStructure.InfobaseDirectory) Then
			...
			ErrorMessageString = NStr("en = 'Directory of the target infobase is not specified'");
	...
	Else
		...
		If IsBlankString(ConnectionStructure.ServerName) Then
			...
			ErrorMessageString = NStr("en = 'Name of 1C: Enterprise server of the target infobase is not specified'");
		...
		If IsBlankString(ConnectionStructure.InfobaseNameAtServer) Then
			...
			ErrorMessageString = NStr("en = 'Name of the target infobase is not specified on 1C: Enterprise server'");
...
Function RunConnectionToReceiverIB() Export
	...
	Except
		...
		Text = NStr("en = 'When trying to create handler UniversalXMLDataExchange, an error occurred: %1'");
		Text = PlaceParametersIntoString(Text, BriefErrorDescription(ErrorInfo()));
		MessageToUser(Text);
		ConnectionResult = Undefined;
	EndTry;
