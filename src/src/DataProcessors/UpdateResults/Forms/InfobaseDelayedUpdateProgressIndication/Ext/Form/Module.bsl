...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Else
		TitleRefreshCompleted = NStr("en = 'The application version was successfully updated to version %1'");
		Items.InformationRefreshEnabledCompleted.Title = StringFunctionsClientServer.SubstituteParametersInString(
	...
	Else
		...
		DataAboutUpdate = InfobaseUpdateService.DataOnUpdatingInformationBase();
		CaptionPattern = NStr("en = 'Additional data processor procedures are completed %1 at %2'");
...
Procedure InformationStateUpdateClick(Item)
	OpenForm("DataProcessor.UpdateResults.Form.DelayedHandlers");
EndProcedure
...
Function MessageAboutUpdateResult(DataAboutUpdate)
	...
	If TotalHandlers = SuccessfullyCompletedHandlers Then
		...
		Else
			MessageText = NStr("en = 'All update procedures are completed successfully (%1)'");
		EndIf;
		Items.PictureInformation1.Picture = PictureLib.Successfully32;
	Else
		MessageText = NStr("en = 'Not all procedures were executed (%1 is executed out of %2)'");
...
Procedure ChangeScheduleAfterScheduleSetup(Schedule, AdditionalParameters) Export
	...
	If Schedule <> Undefined Then
		If Schedule.RepeatPeriodInDay = 0 Then
			...
			QuestionButtons.Add("ConfigureSchedule", NStr("en = 'Configure schedule'"));
