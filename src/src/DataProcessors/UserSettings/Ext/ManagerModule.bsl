...
#Region GettingOfTheFormSettings
...
Function AllFormsSettings(UserName) Export
	...
	FormsList.Add("ExternalDataProcessor.StandardEventLogMonitor.Form.EventsJournalFilter", 
		NStr("en = 'Standard.Event log, Customize event selection'") , False, PictureLib.Form);
	FormsList.Add("ExternalDataProcessor.StandardFindByRef.Form.MainForm", 
		NStr("en = 'Standard.Search for links to object'") , False, PictureLib.Form);
	...
	FormsList.Add("ExternalDataProcessor.StandardMarkedObjectDeletion.Form.Form", 
		NStr("en = 'Standard.Delete selected objects'") , False, PictureLib.Form);
	FormsList.Add("ExternalDataProcessor.StandardExternalDataSourceManagement.Form.Form", 
		NStr("en = 'Standard.Manage external data sources'") , False, PictureLib.Form);
	FormsList.Add("ExternalDataProcessor.StandardTotalsManagement.Form.MainForm", 
		NStr("en = 'Standard.Total management'") , False, PictureLib.Form);
...
Function MetadataObjectsForms()
	...
	StandardFormNames = New ValueList;
	StandardFormNames.Add("Form");
	GetFormListOfMetadataObject(Metadata.DocumentJournals, "DocumentJournal", NStr("en = 'Document journal'"),
	...
	StandardFormNames = New ValueList;
	StandardFormNames.Add("Form");
	GetFormListOfMetadataObject(Metadata.DataProcessors, "DataProcessor", NStr("en = 'Processing'"),
	...
	StandardFormNames.Add("ListForm");
	StandardFormNames.Add("ChoiceForm");
	GetFormListOfMetadataObject(Metadata.ChartsOfAccounts, "ChartOfAccounts", NStr("en = 'Primary chart of accounts'"),
...
#Region CopyAndDeletionOfAllUserSettings
...
Procedure DeleteUserSettings(ClearedSettings, Sources, UserVariantsReportsTable = Undefined) Export
	...
	MapStorageSetting.Insert("PersonalSettingsTip",		CommonSettingsStorage);
	MapStorageSetting.Insert("Favorites",				SystemSettingsStorage);
	MapStorageSetting.Insert("PrintSettings",			SystemSettingsStorage);
	MapStorageSetting.Insert("OtherUserSettings",		SystemSettingsStorage);
...
#Region CopyAndDeletionOfSelectedSettings
...
#Region ListOfUsersAndUserGroups
...
// UserExternal - Boolean - If True then users are selected from the ExternalUserGroups catalog.
Procedure FillGroupsTree(GroupsTree, UserExternal) Export
	
	GroupsArray = New Array;
	ParentGroupsArray = New Array;
	GroupsListAndFullStaff = UserGroups(UserExternal);
	UserGroupsList = GroupsListAndFullStaff.UserGroupsList;
	...
	GenerateFilter(UserGroupsList, EmptyGroup, GroupsArray);
	...
	While GroupsArray.Count() > 0 Do
		...
		For Each Group In GroupsArray Do
			...
			If Group.Parent = EmptyGroup Then
				...
				GroupContent = UserGroupsStaff(Group.Ref, UserExternal);
		...
		For Each Item In ParentGroupsArray Do
			GenerateFilter(UserGroupsList, Item, GroupsArray);
		EndDo;
...
Function UserGroups(UserExternal)
	...
	UserGroupsList = Query.Execute().Unload();
	
	Query = New Query;
	Query.Text =
	"SELECT
	|	UserGroupMembers.UsersGroup AS UsersGroup,
	...
	UserGroupMembers = Query.Execute().Unload();
	
	TableGroupsAndStaff = UserGroupsFullStaff(UserGroupMembers);
	
	Return New Structure("UserGroupsList, TableGroupsAndStaff",
							UserGroupsList, TableGroupsAndStaff);
EndFunction

Function UserGroupsFullStaff(UserGroupMembers)
	...
	For Each ContentRow In UserGroupMembers Do
		
		If TypeOf(ContentRow.UsersGroup) = Type("CatalogRef.UserGroups")
			Or TypeOf(ContentRow.UsersGroup) = Type("CatalogRef.ExternalUserGroups") Then
...
Function UserGroupsStaff(GroupReference, UserExternal)
...
Function FormPresentation(Object, Form, MetadataObjectType)
	...
	Else // Getting the presentation of form for Document, Primary chart of accounts, Chart of settlement kinds, Business processes and Tasks.
...
Procedure GenerateFilter(UserGroupsList, GroupReference, GroupsArray)
	
	FilterParameters = New Structure("Parent", GroupReference);
	FilteredRows = UserGroupsList.FindRows(FilterParameters);
...
Function CopyingReportGenerating(NOTCopiedReportsSettings,
	...
	If UserVariantsReportsTable <> Undefined
		...
		HeaderArea.Parameters.Definition = NStr("en = '
		                                        |It is impossible to copy the personal options of reports.
		                                        |If you want to make personal report option to
		                                        |be available to other users, then you need to resave it with the ""Only for author"" mark removed.
		                                        |List of the skipped report options:'
		|");
	...
	If NotCopiedReportsSettings.Count() <> 0 Then
		HeaderArea = TabTemplate.GetArea("Title");
		HeaderArea.Parameters.Definition = NStr("en = 'Following users have insufficient rights for the reports:'");
		Spreadsheet.Put(HeaderArea);
...
#Region UsersSettingsAndSettingsChoiceForms
...
Procedure FillReportsSettingsList(Form)
	...
	For Each Setting In Settings Do
		...
		// If the report option (report) was deleted but setting remained - do not display it to user.
		If ReportVariantPresentation = "" Then
			Continue;
		EndIf;
		
		// Check if it is the custom option of report.
		...
		// Fill string of the report option.
		...
		// Fill the object key and the setting key for the report option.
...
Procedure FillOtherSettingsList(Form)
	...
	If Keys.Count() > 0 Then
		...
		If OutputSetting Then
			...
			SettingType = "PersonalSettingsTip";
