...
Procedure Select(Command)
	...
	Result.Insert("PersonalSettingsTip", OtherSettingsStructure.PersonalSettingsArray);
...
Procedure UpdatePageTitle(SettingsCount)
	...
	If SelectedSettingsPage = "ReportsSettingsPage" Then
		...
		HeaderText = ?(ReportsSettingsAmount = 0, NStr("en = 'Report settings'"), NStr("en = 'Report settings (%1)'"));
...
Function SelectedSettings(SettingsTree)
	...
	For Each Setting In SettingsTree.GetItems() Do
		...
		If Setting.Check = 1 Then
			...
			If Setting.Type = "PersonalSettingsTip" Then
...
Procedure ImportMarksValues(ValueTree, MarkedSettings, KindSettings)
	...
	If MarkedCount > 0 Then
		...
		If KindSettings = "ReportsSettings" Then
			...
			Items.ReportsSettingsPage.Title = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Report settings (%1)'"), MarkedCount);
