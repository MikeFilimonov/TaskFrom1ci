...
Procedure CopyAllSettings(Command)
	...
	CopiedSettings.Add("PersonalSettingsTip", NStr("en = 'Personal settings'"));
	CopiedSettings.Add("Favorites", NStr("en = 'Favorites'"));
	CopiedSettings.Add("PrintSettings", NStr("en = 'Print settings'"));
	CopiedSettings.Add(
		"OtherUserSettings", NStr("en = 'Settings of additional reports and data processors'"));
...
Procedure Clear(Command)
	...
	If SelectedRows.Count() = 0 Then
		...
		ShowMessageBox(,NStr("en = 'Select settings to delete.'"));
		Return;
...
Procedure ClearSelectedUserSettings(Command)
	...
	QuestionText = NStr("en = 'Clear the selected settings? It will open the user selection window whose settings should be cleared.'");
...
Procedure ClearAllSettings(Command)
	...
	QuestionText = NStr("en = 'Clear all settings of user ""%1""?'");
...
Procedure ClearReportsAndAppearanceSettings(Command)
	...
	QuestionText = NStr("en = 'Clear all settings of reports and appearance of user ""%1""?'");
	QuestionText = StringFunctionsClientServer.SubstituteParametersInString(QuestionText, UserRef);
	QuestionButtons = New ValueList;
	QuestionButtons.Add("Clear", NStr("en = 'Clear'"));
	QuestionButtons.Add("Cancel", NStr("en = 'Cancel'"));
...
#Region CalculationOfSettingCounts
...
#Region CopyDeleteAndClearSettings
...
Procedure ClearSettingsOfAllUsersAtServer()
	...
	ClearedSettings.Add("PersonalSettingsTip");
		...
		If Parameter.CopyAll Then
			...
			If SettingsArray.Count() = 7 Then
				ExplanationText = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'All settings copied %1'"), ExplanationToWhomSettingsAreCopied);
			Else
				ExplanationText = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = '%1 are copied %2'"), SettingsNames, ExplanationToWhomSettingsAreCopied);
			EndIf;
				
			Status(NStr("en = 'Copying settings'"));
			
			Report = Undefined;
			CopyAllSettingsAtServer(InfobaseUser, UsersTarget, SettingsArray, Report);
			
			If Report <> Undefined Then
				QuestionText = NStr("en = 'Not all report options and settings were copied.'");
				QuestionButtons = New ValueList;
			...
			ShowUserNotification(
				NStr("en = 'Copy settings'"), , ExplanationText, PictureLib.Information32);
...
Procedure ClearAllUserSettingsEnd(Response, AdditionalParameters) Export
	...
	ShowUserNotification(NStr("en = 'Clear settings'"), ,
		NStr("en = 'All settings of all users are cleaned up'"), PictureLib.Information32);
...
Procedure ClearAllSettingsEnd(Response, AdditionalParameters) Export
	...
	ClearedSettings = New Array;
	ClearedSettings.Add("ReportSettings");
	ClearedSettings.Add("ExternalViewSettings");
	ClearedSettings.Add("FormsData");
	ClearedSettings.Add("PersonalSettingsTip");
	ClearedSettings.Add("Favorites");
	...
	ExplanationText = NStr("en = 'All settings of user ""%1"" are cleaned up'");
...
Procedure ClearReportsAndAppearanceSettingsEnd(Response, AdditionalParameters) Export
	...
	ExplanationText = NStr("en = 'All settings of reports and appearance of user ""%1"" are cleaned up'");
	ExplanationText = StringFunctionsClientServer.SubstituteParametersInString(ExplanationText, UserRef);
	ShowUserNotification(
		NStr("en = 'Clear settings'"), , ExplanationText, PictureLib.Information32);
...
Procedure NotifyAboutDeleting(SettingsCount, SettingName = Undefined, UserCount = Undefined)
	...
	If SettingsCount = 1
		...
		ExplanationText = NStr("en = '""%1"" is cleared for user ""%2""'");
		ExplanationText = StringFunctionsClientServer.SubstituteParametersInString(
...
Procedure CopySettings()
	...
	If SettingsTree.SelectedRows.Count() = 0 Then
		ShowMessageBox(,NStr("en = 'Select settings to copy.'"));
		Return;
	ElsIf SettingsTree.SelectedRows.Count() = 1 Then
		
		If SettingsTree.CurrentData.Type = "PersonalVariant" Then
			ShowMessageBox(,NStr("en = 'Impossible to copy the personal report option.
			                     |If you want to make personal report option to be
		...
		ElsIf SettingsTree.CurrentData.Type = "SettingPersonal" Then
			ShowMessageBox(,NStr("en = 'Impossible to copy setting of the personal report option.
			                     |Copying of the individual report option settings is not provided.'"));
			Return;
		EndIf;
...
Function SelectedSettings()
	...
	For Each SelectedItem In SelectedItems Do
		...
		If SelectedSetting.Type = "PersonalSettingsTip" Then
		...
		If SelectedSetting.Type = "PersonalVariant" Then
			...
			// Filling array of personal report options.
