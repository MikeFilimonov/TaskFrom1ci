...
Procedure SwitchClearedSettingsOnChange(Item)
	...
	If SwitchWhomSettingsCleared = "SelectedUsers"
		...
		ShowMessageBox(,NStr("en = 'Cleanup of separate settings is available only when one user is selected.'"));
...
Function ClearingSettings()
	...
	If SwitchWhomSettingsCleared = "SelectedUsers" Then
		...
		If UserCount = 1 Then
			ExplanationWhoHasClearedSettings = NStr("en = 'user ""%1""'");
		...
		Else
			ExplanationWhoHasClearedSettings = NStr("en = '%1 users'");
			ExplanationWhoHasClearedSettings = StringFunctionsClientServer.SubstituteParametersInString(
	...
	If SwitchClearedSettings = "SomeSettings"
		...
		CommonUseClientServer.MessageToUser(
			NStr("en = 'Select settings to be cleared.'"), , "SwitchClearedSettings");
		Return False;
	EndIf;
	...
	If SwitchClearedSettings = "SomeSettings" Then
		...
		If SettingsCount = 1 Then
			...
			ExplanationText = NStr("en = '""%1"" is cleared for %2'");
		...
		Else
			...
			ExplanationText = NStr("en = '%1 is cleaned up for %2'");
			ExplanationText = StringFunctionsClientServer.SubstituteParametersInString(
	...
	ElsIf SwitchClearedSettings = "ClearAll" Then
		...
		ExplanationText = NStr("en = 'All settings %1 are cleaned up'");
		ExplanationText = StringFunctionsClientServer.SubstituteParametersInString(
...
Procedure ClearSelectedSettings()
	...
	If SelectedSettings.PersonalSettingsTip.Count() > 0 Then
		DataProcessors.UserSettings.DeleteSelectedSettings(
			User, SelectedSettings.PersonalSettingsTip, "CommonSettingsStorage");
	EndIf;
...
Procedure ClearAllSettings()
	...
	SettingsArray.Add("PersonalSettingsTip");
