...
Procedure NotificationProcessing(EventName, Parameter, Source)
	...
	ElsIf Upper(EventName) = Upper("SettingsChoice") Then
		...
		SelectedSettings.Insert("PersonalSettingsTip", Parameter.PersonalSettingsTip);
...
Procedure Copy(Command)
	...
	If UserRef = Undefined Then
		CommonUseClientServer.MessageToUser(
			NStr("en = 'Select a user whose settings should be copied.'"), , "UserRef");
		Return;
	EndIf;
	
	If UserCount = 0 AND SwitchToWhomCopySettings <> "AllUsers" Then
		CommonUseClientServer.MessageToUser(
			NStr("en = 'Select one or multiple users whose settings should be copied.'"), , "Receiver");
		Return;
	EndIf;
	
	If SwitchCopiedSettings = "CopySelected" AND SettingsCount = 0 Then
		CommonUseClientServer.MessageToUser(
			NStr("en = 'Select settings to be copied.'"), , "SwitchCopiedSettings");
		Return;
	EndIf;
...
Procedure CopySettings(CommandName)
	...
	If SwitchCopiedSettings = "CopySelected" Then
		...
		If Report = Undefined Then
			...
			ShowUserNotification(NStr("en = 'Copy settings'"), , ExplanationText, PictureLib.Information32);
	...
	Else
		...
		If Not SettingsCopied Then
			...
			WarningText = NStr("en = 'Settings were not copied as user ""%1"" has not saved any settings.'");
		...
		ExplanationText = NStr("en = 'All settings copied %1'");
...
Procedure CopySelectedSettings(Report)
	...
	If SelectedSettings.PersonalSettingsTip.Count() > 0 Then
		DataProcessors.UserSettings.CopyReportsSettingsAndPersonalSettings(CommonSettingsStorage,
			User, Receivers, SelectedSettings.PersonalSettingsTip);
	EndIf;
...
Function CopyingAllSettings()
	...
	CopiedSettings.Add("PersonalSettingsTip");
