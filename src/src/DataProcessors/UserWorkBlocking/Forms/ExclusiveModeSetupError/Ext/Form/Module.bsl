...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	DeleteMarkedObjects = Parameters.DeleteMarkedObjects;
	If DeleteMarkedObjects Then
		Title = NStr("en = 'Cannot delete the marked objects'");
		Items.ErrorMessageText.Title = NStr("en = 'Cannot delete selected objects as other users are using the application:'");
	EndIf;
...
Procedure TimeOutCompleteWorkUsers()
	...
	If DurationOfUsersWorkCompletion >= 3 Then
		...
		Items.ErrorMessageText.Title = NStr("en = 'Cannot update application version as some user sessions are still active:'");
...
Procedure InstallLockOfFileBase()
	...
	If DeleteMarkedObjects Then
		...
		Object.MessageForUsers = NStr("en = 'The application is locked for deleting selected objects.'");
	Else
		Object.LockBegin = CurrentSessionDate() + 2*60;
		Object.LockEnding = Object.LockBegin + 5*60;
		Object.MessageForUsers = NStr("en = 'The application is locked for updating.'");
	EndIf;
