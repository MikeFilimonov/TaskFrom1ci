...
Procedure OnCopy(CopiedObject)
	...
	If EventType = Enums.EventTypes.Email
		...
		Raise NStr("en = 'You can not copy an incoming message.'");
	EndIf;
...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	If EventEnding < EventBegin Then
		CommonUseClientServer.MessageToUser(
			NStr("en = 'The end date cannot be earlier than the start date.'"),
			ThisObject,
			"EventEnding",
			,
			Cancel
...
Procedure FillingHandler(FillingData) Export
	...
	If FillingData.EventType = Enums.EventTypes.PhoneCall
		...
		Content = StrTemplate(NStr("en = 'Call from number:%1.'"), FillingData.PhoneNumber);
	EndIf;
	...
	ElsIf TypeOf(FillingData.FillingBasis) = Type("DocumentRef.ObsoleteWorkOrder") Then
		
		FillByObsoleteWorkOrder(FillingData.FillingBasis);
...
Procedure FillByStructure(FillingBasis)
	...
	If FillingBasis.Property("Basis")
		AND TypeOf(FillingBasis.Basis)= Type("DocumentRef.ObsoleteWorkOrder") Then
		
		FillByCurrentRowObsoleteWorkOrder(FillingBasis);
		Return;
...
Procedure FillByObsoleteWorkOrder(ObsoleteWorkOrder)
	
	Participants.Clear();
	
	// Filling out a document header.
	Query = New Query;
	Query.SetParameter("Ref", ObsoleteWorkOrder);
	...
	|	Works.Day AS Day
	|FROM
	|	Document.ObsoleteWorkOrder.Works AS Works
	|WHERE
	|	Works.Ref = &Ref";
	...
	If EventType = Enums.EventTypes.Email Then
		RowDocumentsBases = BasisDocuments.Add();
		RowDocumentsBases.BasisDocument = ObsoleteWorkOrder;
	Else
		BasisDocument = ObsoleteWorkOrder;
	EndIf;
...
Procedure FillByCurrentRowObsoleteWorkOrder(FillingStructure)
