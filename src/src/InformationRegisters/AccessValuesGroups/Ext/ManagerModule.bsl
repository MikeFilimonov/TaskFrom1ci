...
//    - UserGroups: one users group or array.
//
...
//
Procedure UpdateUserGroups(Parameters = Undefined, HasChanges = Undefined) Export
	...
	Else
		Raise
			NStr("en = 'An error occurred
			     |in the UpdateUserGroups procedure of the information register manager module Access value groups.
			     |
	...
	Try
		...
		If UpdateType = "UsersAndUserGroups" Then
			...
			If Parameters.Property("UserGroups") Then
				RefreshUserGroups(Parameters.UserGroups, HasChanges);
			EndIf;
...
Function DeleteExtraRecords(HasChanges = Undefined)
	...
	|									AND VALUETYPE(AccessValuesGroups.AccessValuesGroup) <> Type(Catalog.ExternalUserGroups)
	|						ELSE TRUE
	|					END
	...
	|							THEN VALUETYPE(AccessValuesGroups.AccessValuesGroup) <> Type(Catalog.Users)
	|									AND VALUETYPE(AccessValuesGroups.AccessValuesGroup) <> Type(Catalog.UserGroups)
	|						WHEN VALUETYPE(AccessValuesGroups.AccessValue) = Type(Catalog.ExternalUserGroups)
	|							THEN VALUETYPE(AccessValuesGroups.AccessValuesGroup) <> Type(Catalog.ExternalUsers)
	|									AND VALUETYPE(AccessValuesGroups.AccessValuesGroup) <> Type(Catalog.ExternalUserGroups)
	|						ELSE TRUE
	|					END
	...
	|								OR VALUETYPE(AccessValuesGroups.AccessValue) = Type(Catalog.ExternalUserGroups)
	|							THEN TRUE
...
Procedure RefreshUsers(Users1 = Undefined,
	...
	QueryText =
	"SELECT
	|	UserGroupMembers.User AS AccessValue,
	|	UserGroupMembers.UsersGroup AS AccessValuesGroup,
	|	1 AS DataGroup,
	|	&InsertFieldsRowChangeKind
	|FROM
	|	InformationRegister.UserGroupMembers AS UserGroupMembers
	|WHERE
	|	VALUETYPE(UserGroupMembers.User) = Type(Catalog.Users)
	...
	|	InformationRegister.UserGroupMembers AS UserGroupMembers
	|WHERE
	|	VALUETYPE(UserGroupMembers.User) = Type(Catalog.ExternalUsers)
	|	AND &UsersFilterCondition1";
	...
	AccessManagementService.SetCriteriaForQuery(Query, Users1, "Users",
		"&UsersFilterCondition1:UserGroupMembers.User 
...
//
//
Procedure RefreshUserGroups(UserGroups = Undefined,
                                      HasChanges       = Undefined)
	...
	QueryText =
	"SELECT DISTINCT
	|	UserGroupMembers.UsersGroup AS AccessValue,
	|	UserGroupMembers.UsersGroup AS AccessValuesGroup,
	|	2 AS DataGroup,
	|	&InsertFieldsRowChangeKind
	|FROM
	|	InformationRegister.UserGroupMembers AS UserGroupMembers
	|WHERE
	|	VALUETYPE(UserGroupMembers.UsersGroup) = Type(Catalog.UserGroups)
	|	AND &UserGroupsFilterCondition1
	|
	|UNION ALL
	|
	|SELECT
	...
	|WHERE
	|	VALUETYPE(UserGroupMembers.UsersGroup) = Type(Catalog.UserGroups)
	|	AND VALUETYPE(UserGroupMembers.User) = Type(Catalog.Users)
	|	AND &UserGroupsFilterCondition1
	|
	|UNION ALL
	|
	...
	|WHERE
	|	VALUETYPE(UserGroupMembers.UsersGroup) = Type(Catalog.ExternalUserGroups)
	|	AND VALUETYPE(UserGroupMembers.User) = Type(Catalog.ExternalUsers)
	|	AND &UserGroupsFilterCondition1";
	
	// Prepare the selected fields with an optional filter.
	Fields = New Array; 
	Fields.Add(New Structure("AccessValue",         "&UserGroupsFilterCondition2"));
	Fields.Add(New Structure("AccessValuesGroup"));
	Fields.Add(New Structure("DataGroup",            "&UpdatedDataGroupsFilterCondition"));
	...
	AccessManagementService.SetCriteriaForQuery(Query, UserGroups, "UserGroups",
		"&UserGroupsFilterCondition1:UserGroupMembers.UsersGroup
   |&UserGroupsFilterCondition2:OldData.AccessValue");
...
Procedure UpgradeGroupsOfPerformers(PerformersGroups = Undefined,
	...
	|WHERE
	|	VALUETYPE(TableOfExecutantGroups.GroupPerformers) <> Type(Catalog.UserGroups)
	|	AND VALUETYPE(TableOfExecutantGroups.GroupPerformers) <> Type(Catalog.Users)
	|	AND VALUETYPE(TableOfExecutantGroups.GroupPerformers) <> Type(Catalog.ExternalUserGroups)
	...
	|		ON UsersOfExecutantGroups.User = UserGroupMembers.User
	|			AND (VALUETYPE(UserGroupMembers.UsersGroup) = Type(Catalog.UserGroups))
	|
	|UNION ALL
	|
	|SELECT
	...
	|	ExternalUsersOfExecutantGroups AS ExternalUsersOfExecutantGroups
	|		INNER JOIN InformationRegister.UserGroupMembers AS UserGroupMembers
	|		ON ExternalUsersOfExecutantGroups.ExternalUser = UserGroupMembers.User
	|			AND (VALUETYPE(UserGroupMembers.UsersGroup) = Type(Catalog.ExternalUserGroups))";
...
Procedure RefreshAuthorizationObjects(AuthorizationObjects = Undefined, HasChanges = Undefined)
	...
	QueryText =
	"SELECT
	|	CAST(UserGroupMembers.User AS Catalog.ExternalUsers).AuthorizationObject AS AccessValue,
	|	UserGroupMembers.UsersGroup AS AccessValuesGroup,
	|	4 AS DataGroup,
	|	&InsertFieldsRowChangeKind
	|FROM
	...
	|		ON (VALUETYPE(UserGroupMembers.User) = Type(Catalog.ExternalUsers))
