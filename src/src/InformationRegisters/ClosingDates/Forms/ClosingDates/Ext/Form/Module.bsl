...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	// Form fields setting
	If Parameters.DataClosingDatesOfDataImport Then
		
		If Not SectionsProperties.UseProhibitionDatesOfDataImport Then
			Raise(NStr("en = 'Closing dates of data import are not used.'"));
		EndIf;
		
		Title = NStr("en = 'Closing dates of data import'");
		Items.ProhibitionDateSetting.ChoiceList.FindByValue("NoProhibition").Presentation =
			NStr("en = 'Data import allowed'");
		...
		Commands.ProhibitionDatesByUsers.Title
			= NStr("en = 'Closing dates by infobases'");
		...
		Commands.ProhibitionDatesBySectionsObjectsForUsers.Title =
			NStr("en = 'Closing dates by sections and objects for infobases'");
		...
		TypesUser =
			Metadata.InformationRegisters.ClosingDates.Dimensions.User.Type.Types();
	...
	Else
		ValueForAllUsers = Enums.ClosingDateAreas.ForAllUsers;
...
Procedure ChoiceProcessing(ValueSelected, ChoiceSource)
	...
	If Upper(ChoiceSource.FormName)
	   = Upper("InformationRegister.ClosingDates.Form.ProhibitionDateEditing") Then
...
Procedure ProhibitionDateSettingChoiceProcessing(Item, ValueSelected, StandardProcessing)
	...
	ProhibitionDateCurrentSetting = ProhibitionDateCurrentSetting(Parameters.DataClosingDatesOfDataImport);
	...
	Else
		...
		ClosingDatesetting(ValueSelected, False);
	EndIf;
...
#Region SameEventHandlersOfFormsClosingDatesAndProhibitionDateEditing
	   ...
	   AND Not CurrentData.WithoutProhibitionDate Then
		...
		Else
			...
			ElsIf TypeOf(CurrentData.User) = Type("CatalogRef.UserGroups") Then
				QuestionText = QuestionTextDeleteProhibitionDatesForUserGroups();
			...
			ElsIf TypeOf(CurrentData.User) = Type("CatalogRef.ExternalUserGroups") Then
				QuestionText = QuestionTextDeleteProhibitionDatesForExternalUserGroups();
			Else
...
#Region HandlerOfTheFullPresentationItemEventsOfTheUsersFormTable
...
Procedure UsersFullPresentationChoiceProcessing(Item, ValueSelected, StandardProcessing)
	...
	If Rows.Count() = 0 Then
		...
		NotifyChanged(Type("InformationRegisterRecordKey.ClosingDates"));
...
#Region HandlerOfTheCommentItemEventsOfTheUsersFormTable
...
#Region HandlersOfTheFullPresentationItemEventsOfTheProhibitionDateFormTable
...
#Region HandlersOfTheProhibitionDateItemEventsOfTheProhibitionDateFormTable
...
Procedure ProhibitionDatesByUsers(Command)
	...
	If Parameters.DataClosingDatesOfDataImport Then
		ReportFormName = "Report.ClosingDatesOfDataImport.Form.ReportForm";
	Else
		ReportFormName = "Report.ClosingDates.Form.ReportForm";
	EndIf;
...
Procedure SetConditionalAppearance()
	...
	ValueList.Add(Enums.ClosingDateAreas.ForAllUsers);
	ValueList.Add(Enums.ClosingDateAreas.ForAllDatabases);
...
Procedure ProhibitionDateSettingSelectionProcessingContinue(Response, ValueSelected) Export
	...
	ProhibitionDateSetting = ValueSelected;
	ClosingDatesetting(ValueSelected, True);
...
Procedure ProhibitionDateSpecificationMethodSelectionProcessingContinue(Response, AdditionalParameters) Export
	...
	DeleteUnneededOnClosingDatespecifiedMode(
...
Function RecalculateDatesAtServer()
	...
	ResultDescription = "";
	ClosingDatesService.RecalculateCurrentValuesOfRelativeProhibitionDates(
...
Procedure UpdateAtServer()
	...
	ProhibitionDateSetting = ProhibitionDateCurrentSetting(Parameters.DataClosingDatesOfDataImport);
...
Procedure ReadUsers()
	...
	Try
		...
		Query.SetParameter("DataClosingDatesOfDataImport", Parameters.DataClosingDatesOfDataImport);
		Query.Text =
		"SELECT DISTINCT
		|	PRESENTATION(ClosingDates.User) AS FullPresentation,
		|	ClosingDates.User,
		|	CASE
		...
		|		ELSE 1
		|	END AS CommonUse,
		|	PRESENTATION(ClosingDates.User) AS Presentation,
		|	MAX(ClosingDates.ProhibitionDate) AS ProhibitionDate,
		|	MAX(ClosingDates.ProhibitionDateDescription) AS ProhibitionDateDescription,
		|	MAX(ClosingDates.Comment) AS Comment,
		|	FALSE AS WithoutProhibitionDate,
		|	-1 AS PictureNumber
		|FROM
		|	InformationRegister.ClosingDates AS ClosingDates
		|WHERE
		|	ClosingDates.Object <> UNDEFINED
		|	AND (NOT(ClosingDates.Section <> ClosingDates.Object
		|				AND VALUETYPE(ClosingDates.Object) = Type(ChartOfCharacteristicTypes.ClosingDateSections)))
		|	AND (NOT(VALUETYPE(ClosingDates.Object) <> Type(ChartOfCharacteristicTypes.ClosingDateSections)
		...
		|
		|HAVING
		|	ClosingDates.User <> UNDEFINED AND
		|	CASE
		|		WHEN VALUETYPE(ClosingDates.User) = Type(Catalog.Users)
		|				OR VALUETYPE(ClosingDates.User) = Type(Catalog.UserGroups)
		|				OR VALUETYPE(ClosingDates.User) = Type(Catalog.ExternalUsers)
		|				OR VALUETYPE(ClosingDates.User) = Type(Catalog.ExternalUserGroups)
		|				OR ClosingDates.User = VALUE(Enum.ClosingDateAreas.ForAllUsers)
		|			THEN &DataClosingDatesOfDataImport = FALSE
		|		ELSE &DataClosingDatesOfDataImport = TRUE
		|	END";
		
		// Incorrect records are excluded from the selection using conditions:
		// - object with the value of CCT type.ClosingDateSections can be only equal to the section.
...
Function ReadUserDataWithSections(Val User,
	...
	Try
		...
		|FROM
		|	(SELECT
		|		VALUE(ChartOfCharacteristicTypes.ClosingDateSections.EmptyRef) AS Ref,
		...
		|		ClosingDateSections.Description,
		|		NULL
		|	FROM
		...
		|		ClosingDates.Section.Description,
		|		NULL
		|	FROM
		|		InformationRegister.ClosingDates AS ClosingDates
		|	WHERE
		|		ClosingDates.Section <> VALUE(ChartOfCharacteristicTypes.ClosingDateSections.EmptyRef)) AS Sections
		|
		|INDEX BY
		|	Sections.Ref
		|;
		...
		|	ClosingDates.Object,
		|	PRESENTATION(ClosingDates.Object) AS FullPresentation,
		|	PRESENTATION(ClosingDates.Object) AS Presentation,
		|	CASE
		|		WHEN ClosingDates.Object IS NULL 
		|			THEN TRUE
		|		ELSE FALSE
		...
		|		LEFT JOIN InformationRegister.ClosingDates AS ClosingDates
		|		ON Sections.Ref = ClosingDates.Section
		|			AND (ClosingDates.User = &User)
		|			AND (ClosingDates.Object <> UNDEFINED)
		|			AND (NOT(ClosingDates.Section <> ClosingDates.Object
		|					AND VALUETYPE(ClosingDates.Object) = Type(ChartOfCharacteristicTypes.ClosingDateSections)))
		|			AND (NOT(VALUETYPE(ClosingDates.Object) <> Type(ChartOfCharacteristicTypes.ClosingDateSections)
		|					AND &AllSectionsWithoutObjects))
		...
		|				AND ClosingDates.Section IS NULL )
		|
		|ORDER BY
		|	Predefined DESC,
		|	SectionPresentation
...
Function ReadUserDataWithoutSections(Val User, Val FirstSection, Val ValueForAllUsers)
	...
	Try
		...
		Query.Text =
		"SELECT
		|	VALUE(ChartOfCharacteristicTypes.ClosingDateSections.EmptyRef) AS Section,
		|	VALUE(ChartOfCharacteristicTypes.ClosingDateSections.EmptyRef) AS Object,
		...
		|		LEFT JOIN (SELECT
		|			ClosingDates.ProhibitionDate AS ProhibitionDate,
		|			ClosingDates.ProhibitionDateDescription AS ProhibitionDateDescription
		|		FROM
		|			InformationRegister.ClosingDates AS ClosingDates
		|		WHERE
		|			ClosingDates.User = &User
		|			AND ClosingDates.Section = VALUE(ChartOfCharacteristicTypes.ClosingDateSections.EmptyRef)
		|			AND ClosingDates.Object = VALUE(ChartOfCharacteristicTypes.ClosingDateSections.EmptyRef)) AS CommonDate
		|		ON (TRUE)
		|WHERE
		|	CASE
		...
		|	&FirstSection,
		|	ClosingDates.Object,
		|	PRESENTATION(ClosingDates.Object),
		|	PRESENTATION(ClosingDates.Object),
		|	ClosingDates.ProhibitionDate,
		|	ClosingDates.ProhibitionDateDescription,
		|	FALSE,
		...
		|WHERE
		|	ClosingDates.User = &User
		|	AND ClosingDates.Section = &FirstSection
		|	AND ClosingDates.Object <> UNDEFINED
		|	AND VALUETYPE(ClosingDates.Object) <> Type(ChartOfCharacteristicTypes.ClosingDateSections)";
...
Procedure LockSetOfUserRecords(User, CurrentRow = Undefined)
	...
	Try
		For Each RecordDescription In Exporting Do
			...
			If FoundStrings.Count() = 0 Then
				// Add a new lock.
				RecordKey = InformationRegisters.ClosingDates.CreateRecordKey(
					KeyValuesRecords);
...
Procedure UnlockAllRecordsAtServer(LockedRecordsValuesOfKeys, FormID)
	...
	For Each KeyValuesRecords In LockedRecordsValuesOfKeys Do
		RecordKey = InformationRegisters.ClosingDates.CreateRecordKey(KeyValuesRecords);
...
Procedure FillProhibitionDatesUserPictureNumbers(CurrentRow = Undefined)
	...
	If Parameters.DataClosingDatesOfDataImport Then
		...
		For Each String In ProhibitionDatesUsers Do
			...
			If String.User
			   = Enums.ClosingDateAreas.ForAllDatabases Then
...
Function TextNotificationsOfUnusedSettingModes()
	...
	AdditionalParameters = New Structure;
	AdditionalParameters.Insert("DataClosingDatesOfDataImport",    Parameters.DataClosingDatesOfDataImport);
...
&AtServerNoContext
Function ProhibitionDateCurrentSetting(DataClosingDatesOfDataImport)
	
	BeginTransaction();
	Try
		Query = New Query;
		Query.SetParameter("DataClosingDatesOfDataImport", DataClosingDatesOfDataImport);
		Query.Text =
		"SELECT TOP 1
		...
		|				ELSE &DataClosingDatesOfDataImport = TRUE
		|			END)
		|	AND VALUETYPE(ClosingDates.User) <> Type(Enum.ClosingDateAreas)";
...
&AtServer
Function ClosingDatesetting(Val ValueSelected, Val DeleteUnneeded)
	...
	If DeleteUnneeded Then
		...
		Try
			Query = New Query;
			Query.SetParameter("DataClosingDatesOfDataImport",
				Parameters.DataClosingDatesOfDataImport);
			...
			Else
				Query.SetParameter("DataClosingDatesOfDataImport", Undefined);
			EndIf;
...
Function ProhibitionDateCurrentSpecifiedMode(Val User, Val SingleSection, Val FirstSection, Val ValueForAllUsers, Data = Undefined)
	...
	Try
		...
		|	InformationRegister.ClosingDates AS ClosingDates
		|WHERE
		|	Not(&User <> ClosingDates.User
		|				AND &User <> ""*"")
		|	AND Not(ClosingDates.Section = VALUE(ChartOfCharacteristicTypes.ClosingDateSections.EmptyRef)
		|				AND ClosingDates.Object = VALUE(ChartOfCharacteristicTypes.ClosingDateSections.EmptyRef))
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|SELECT TOP 1
		...
		|	AND ClosingDates.Section <> VALUE(ChartOfCharacteristicTypes.ClosingDateSections.EmptyRef)
		|	AND ClosingDates.Object <> VALUE(ChartOfCharacteristicTypes.ClosingDateSections.EmptyRef)
		|	AND ClosingDates.Object <> ClosingDates.Section
		|	AND ClosingDates.Object <> UNDEFINED
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		...
		|				AND &User <> ""*"")
		|	AND ClosingDates.Section <> VALUE(ChartOfCharacteristicTypes.ClosingDateSections.EmptyRef)
		|	AND ClosingDates.Section = ClosingDates.Object";
...
&AtServer
Function DeleteUnneededOnClosingDatespecifiedMode(Val ValueSelected, Val CurrentUser, Val ProhibitionDateSetting)
...
Procedure EditProhibitionDateInFormEnd(SelectedRows) Export
	...
	OpenForm("InformationRegister.ClosingDates.Form.ProhibitionDateEditing",
...
Function PurposeForAll(User)
	...
	Return TypeOf(User) = Type("EnumRef.ClosingDateAreas");
...
#Region SameProcedureAndFunctionOfFormsClosingDatesAndProhibitionDateEditing
	...
	Else
		...
		If Context.AllowDataChangingToProhibitionDate Then
			...
			If Context.CurrentDateAtServer > PermissionTerm Then
				LabelText = Chars.LF +
					NStr("en = 'Period of changing data from %3 to %4 expired on %2'");
			Else
				If CalculateProhibitionDate Then
					Context.ProhibitionDate = CalculatedProhibitionDates.Previous;
				EndIf;
				LabelText = Chars.LF +
					NStr("en = 'For %2 data might change from %3 to %4'") + Chars.LF +
					NStr("en = 'After %2 it will be prohibited to change %4 data'") + Chars.LF;
			EndIf;
		Else
		...
		Context.Items.AutomaticalDateExplanation.Title =
			StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Changing of data on %1 is prohibited'") + LabelText,
...
#Region HelperFunctionsOfTheUserInterfaceStrings

&AtClientAtServerNoContext
Function DescriptionsProhibitionDateList()
	
	List = New ValueList;
	List.Add("",                "(" + NStr("en = 'Default'") + ")");
	List.Add("Custom",      NStr("en = 'Custom date'"));
	List.Add("LastYearEnd",     NStr("en = 'Last year end'"));
	List.Add("LastQuarterEnd", NStr("en = 'Last quarter end'"));
	List.Add("LastMonthEnd",   NStr("en = 'Last month end'"));
	List.Add("LastWeekEnd",    NStr("en = 'Last week end'"));
...
Function UserPresentationText(Context, User)
	...
	If Context.Parameters.DataClosingDatesOfDataImport Then
...
Function QuestionTextRecalculateProhibitionDates()
	...
	Return NStr("en = 'Clear the unfilled lines and recalculate relative closing dates?'");
...
Function QuestionTextDeleteAllProhibitionDatesExceptDatesForAllUsers()
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = 'Delete all closing dates except for the set ones %1?'"),
		ValueForAllUsers);
...
Function QuestionTextDeleteAllProhibitionDates()
	...
	Return NStr("en = 'Delete all closing dates?'");
...
Function MessageTextValueForAllUsersNotChange()
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = 'Value %1 is not being changed.'"),
		PresentationTextForAllUsers(ThisObject));
...
Function MessageTextCommentForAllUsersNotChange()
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = 'Comment %1 cannot be changed.'"),
		PresentationTextForAllUsers(ThisObject));
...
Function MessageTextFirstSelectUser()
	...
	Return NStr("en = 'Please select a user.'");
...
Function QuestionTextClearBlankRows()
	...
	Return NStr("en = 'Clear unfilled lines?'");
...
Function MessageTextForDeletingSelectOneRow()
	...
	Return NStr("en = 'Mark one line for deletion.'");
...
Function QuestionTextDeleteProhibitionDatesForAllUsers()
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = 'Delete the %1 closing dates?'"), Lower(ValueForAllUsers));
...
Function QuestionTextDeleteProhibitionDatesForUser()
	...
	Return NStr("en = 'Delete closing dates for a user?'");
	
EndFunction

&AtClientAtServerNoContext
Function QuestionTextDeleteProhibitionDatesForUserGroups()
	
	Return NStr("en = 'Delete closing dates for a user group?'");
...
Function QuestionTextDeleteProhibitionDatesForExternalUser()
	...
	Return NStr("en = 'Delete closing dates for an external user?'");
	
EndFunction

&AtClientAtServerNoContext
Function QuestionTextDeleteProhibitionDatesForExternalUserGroups()
	
	Return NStr("en = 'Delete closing dates for external users group?'");
...
Function QuestionTextDeleteProhibitionDates()
	...
	Return NStr("en = 'Remove closing dates?'");
...
Function MessageTextForAllUsersProhibitionDatesIsNotSet()
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = '%1 closing dates are not set.'"), ValueForAllUsers);
...
Function MessageTextValueAlreadyAddedToList()
	...
	Return NStr("en = 'The value has already been added to the list.'");
...
Function MessageTextUpdateFormF5Data()
	...
	Return NStr("en = 'Update form data (F5).'");
...
Function QuestionTextDeleteProhibitionDatesForSectionsAndObjects()
	...
	Return NStr("en = 'Delete closing dates for sections and objects?'");
...
Function QuestionTextDeleteProhibitionDatesForObjects()
	...
	Return NStr("en = 'Delete closing dates for objects?'");
...
Function QuestionTextDeleteProhibitionDatesForSections()
	...
	Return NStr("en = 'Delete closing dates for sections?'");
...
Function MessageTextInSelectedSectionProhibitionDatesForObjectNotSet()
	...
	Return NStr("en = 'Closing dates for objects are not set in the selected section.'");
...
Function MessageTextCommonDateCanBeSet()
	...
	Return NStr("en = '<Common date> can be set.'");
...
Function QuestionTextDeleteProhibitionDatesForSection()
	...
	Return NStr("en = 'Delete a closing date for a section?'");
...
Function QuestionTextDeleteCommonProhibitionDate()
	...
	Return NStr("en = 'Delete common closing date?'");
...
Function QuestionTextDeleteObjectProhibitionDate()
	...
	Return NStr("en = 'Delete a closing date for an object?'");
...
Function MessageTextForAllUsersSectionsAlwaysShow()
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = '%1 sections are always shown.'"), ValueForAllUsers);
...
Function MessageTextWhenDeletingProhibitionDateCleared()
	...
	Return NStr("en = 'On deletion, closing date is cleared.'");
...
Function MessageTextForAllUsersCommonDateAlwaysShow()
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en = '%1 <Common date> is always shown.'"), ValueForAllUsers);
...
Function MessageTextNotEnoughRightsForChoiceOfExternalUsers()
	...
	Return NStr("en = 'Insufficient rights to select external users.'");
...
Function MessageTextSettingsWithUnselectedUsersNotSaved(Context)
	...
	If Context.Parameters.DataClosingDatesOfDataImport Then
		Return NStr("en = 'Settings with unselected infobases are not saved.'");
	EndIf;
	
	Return NStr("en = 'Settings with unselected users are not saved.'");
...
Function MessageTextSettingsWithUnfilledProhibitionDatesForObjectsNotSaved()
	...
	Return NStr("en = 'Settings with unfilled closing dates for objects are not saved.'");
...
Function MessageTextSettingsWithUnfilledProhibitionDatesForSectionsNotSaved()
	...
	Return NStr("en = 'Settings with unfilled closing dates for sections are not saved.'");
...
Function MessageTextCommonDateAlreadyShown()
	...
	Return NStr("en = '<Common date> is already shown.'");
...
Function MessageTextAllSectionsAlreadyShown()
	...
	Return NStr("en = 'All sections are already shown.'");
...
Function TitleChoiceOfRequiredSections()
	...
	Return NStr("en = 'Select required sections'");
...
Function MessageTextUnfilledRowsAreUnmarked()
	...
	Return NStr("en = 'Unfilled rows are unchecked.'");
...
Function MessageTextSelectedStringsNotFilledIn()
	...
	Return NStr("en = 'The selected lines are not filled in.'");
