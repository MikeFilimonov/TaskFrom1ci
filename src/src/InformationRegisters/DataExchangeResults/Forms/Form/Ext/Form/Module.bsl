...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	UsedClosingDates = False;
	DataExchangeServer.OnDefenitionOfUsageOfProhibitionDatesChange(UsedClosingDates);
	...
	ElsIf Not UsedClosingDates Then
	...
	If CommonUseReUse.DataSeparationEnabled() AND UseVersioning Then
		...
		Items.CollisionsOtherVersionAuthor.Title = NStr("en = 'Version is received from the application'");
...
Procedure AcceptVersionUnaccepted(Command)
	...
	ShowQueryBox(NotifyDescription, NStr("en = 'Accept version despite import prohibition?'"), QuestionDialogMode.YesNo, , DialogReturnCode.No);
...
Procedure ConflictResultChange(Command)
	...
	If Items.Collisions.CurrentData <> Undefined Then
		...
		If Items.Collisions.CurrentData.OtherVersionAccepted Then
			...
			QuestionText = NStr("en = 'Replace the version received from another application with the version of this application?'");
			
		Else
			
			QuestionText = NStr("en = 'Replace a version of this application with the version received from another application?'");
...
Procedure FillNodeList()
	...
	If ContextOpen AND ExchangeByRulesAbsent Then
		Title = NStr("en = 'Conflicts during data synchronization'");
...
Procedure ShowDifference(Item)
	...
	If ComparedVersions.Count() <> 2 Then
		...
		CommonUseClientServer.MessageToUser(NStr("en = 'There is no version to compare.'"));
		Return;
...
Procedure PageHeaderRefresh()
	...
	If UseVersioning Then
		...
		SetPageHeader(Items.PageUnacceptedByProhibitionDate, NStr("en = 'Not accepted by closing date'"), NumberOfUnaccepted());
	EndIf;
