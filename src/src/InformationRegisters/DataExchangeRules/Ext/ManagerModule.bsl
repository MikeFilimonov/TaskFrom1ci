	...
	If IsArchive Then
		...
		If DataExchangeServer.UnpackZIPFile(TemporaryArchiveName, TempFolderName) Then
			...
			If UnpackedFileList.Count() = 0 Then
				NString = NStr("en = 'When unzipping archive, file with rules was not found.'");
			...
			If AreConversionRules Then
				...
				ElsIf UnpackedFileList.Count() = 1 Then
					NString = NStr("en = 'Archive contains one file with conversion rules. Expected number of files in the archive is two. Expected files: ExchangeRules.xml - rules for the current application conversion; CorrespondentExchangeRules.xml - rules for the correspondent application conversion.'");
					DataExchangeServer.ShowMessageAboutError(NString, Cancel);
				// There are several files in the archive but there should be one - deny importing.
				ElsIf UnpackedFileList.Count() > 1 Then
					NString = NStr("en = 'When unzipping archive, multiple files were found. There should be only one file with rules.'");
					DataExchangeServer.ShowMessageAboutError(NString, Cancel);
				EndIf;
		...
		Else // If you failed to unpack file - deny importing.
			NString = NStr("en = 'Cannot unpack an archive with rules.'");
			DataExchangeServer.ShowMessageAboutError(NString, Cancel);
		EndIf;
	...
	If ConversionRulesRecord.RulesSource = Enums.RuleSourcesForDataExchange.File Then
		...
		If DataExchangeServer.UnpackZIPFile(TemporaryArchiveName, TempFolderName) Then
			...
			For Each ReceivedFile In UnpackedFileList Do
				...
				Else
					NString = NStr("en = 'File names in archive does not correspond to the expected ones. Files are expected:
					               |ExchangeRules.xml - conversion rules for the current application;
					               |CorrespondentExchangeRules.xml - conversion rules for application-correspondent;
					               |RegistrationRules.xml - rules of registration for the current application.'");
...
Procedure ImportProvidedRules(ExchangePlanName, RulesFilename) Export
	...
	If DataExchangeServer.UnpackZIPFile(RulesFilename, TempFolderName) Then
		...
		// There was no file in the archive - deny importing.
		If UnpackedFileList.Count() = 0 Then
			Raise NStr("en = 'When unzipping archive, file with rules was not found.'");
		EndIf;
		...
		For Each ReceivedFile In UnpackedFileList Do
			...
			ElsIf ReceivedFile.Name ="RegistrationRules.xml" Then
				RegistrationBinaryData = New BinaryData(ReceivedFile.DescriptionFull);
			Else
				Raise NStr("en = 'File names in archive does not correspond to the expected ones. Files are expected:
				           |ExchangeRules.xml - conversion rules for the current application.
				           |CorrespondentExchangeRules.xml - conversion rules for application-correspondent.
				           |RegistrationRules.xml - rules of registration for the current application.'");
			EndIf;
	...
	Else
		// If you failed to unpack file - deny importing.
		Raise NStr("en = 'Cannot unpack an archive with rules.'");
	EndIf;
...
Procedure DeleteProvidedRules(ExchangePlanName) Export
	...
	For Each RuleKind In Enums.DataExchangeRuleKinds Do
		...
		If HasErrors Then
			Raise NStr("en = 'An error occurred when importing rules from configuration.'");
		Else
			RecordManager.Write();
		EndIf;
...
Function GetBinaryDataFromConfigurationTemplate(Cancel, ExchangePlanName, TemplateName)
	...
	Except
		...
		MessageString = NStr("en = 'An error occurred when receiving a template of configuration %1 for exchange plan %2'");
...
Procedure DefineRuleKindForDataExchange(RuleKind, FileName, Cancel)
	...
	If Rules.NodeType = XMLNodeType.StartElement Then
		...
		Else
			...
			NString = StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en = 'Unable to determine rules kind because an error occurred while parsing XML-file %1. 
						     |The wrong file may be selected or XML-file has an incorrect structure. Choose the correct file.'"),
						FileName);
			DataExchangeServer.ShowMessageAboutError(NString, Cancel);
...
Function RulesFromFileInformation(RulesFilename)
	...
	If Not ((ExchangeRules.LocalName = "ExchangeRules") AND (ExchangeRules.NodeType = XMLNodeType.StartElement)) Then
		Raise NStr("en = 'Error in exchange rule format'");
	EndIf;
...
Function ConversionRulesAreCompatibleWithCurrentVersion(ExchangePlanName, ErrorDescription, RulesData)
	...
	If ComparisonResult <> 0 Then
		...
		If ComparisonResult < 0 Then
			...
			ErrorText = NStr("en = 'Data may be synced incorrectly because you use rules designed for the previous version of application ""%1"" (%2). Use rules designed for the configuration or import the set of rules designed for the current application version (%3).'");
			ErrorKind = "OutdatedConfigurationVersion";
			
		Else
			
			ErrorText = NStr("en = 'Data may be synced incorrectly because you use rules designed for the newer version of application ""%1"" (%2). Update version of the application or use the set of rules designed for the current application version (%3).'");
