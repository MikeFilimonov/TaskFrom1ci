...
Procedure OnOpen(Cancel)
	...
	TemplateUserWebsite = NStr("en = 'user support website of 1C:Enterprise 8'");
...
// Procedure continued (see above).
&AtClient
Procedure InitializeFile(File, AdditionalParameters)
...
Procedure DetermineFileExistence(Exists, AdditionalParameters) Export
	...
	If Exists Then
		...
		#If Not WebClient Then
			Text	= New TextReader(AdditionalParameters.TemplateUserSettings, "UTF-16");
			Str		= "";
	...
	File = New File(AdditionalParameters.FileLocation);
	...
	If DebugMode Then
		...
		If ExportDebuggingMode Then
			...
			If Not ValueIsFilled(FileName) Then
				...
				MessageString = NStr("en = 'Name of external data processor file is not specified.'");
...
Procedure ImportRules(Command)
	...
	DialogueParameters.Insert("Title", NStr("en = 'Specify an archive with exchange rules'"));
...
Procedure ImportRulesEnd(Val FilesPlacingResult, Val AdditionalParameters) Export
	...
	If Lower(NameParts.Extension) <> ".zip" Then
		CommonUseClientServer.MessageToUser(NStr("en = 'Incorrect format of the rule set file. Awaiting zip archive containing three files:
		                                         |ExchangeRules.xml - conversion rules for the current application.
		                                         |CorrespondentExchangeRules.xml - conversion rules for the application-correspondent.
...
Procedure ImportRulesExecute(Val PlacedFileAddress, Val FileName, ErrorDescription = Undefined)
	...
	If TypeOf(ErrorDescription) <> Type("Boolean") AND ErrorDescription <> Undefined Then
		...
		FormParameters.OfferDontAskAgain = False;
		If ErrorDescription.ErrorKind = "IncorrectConfiguration" Then
			FormParameters.Title = NStr("en = 'Rules cannot be imported'");
		Else
...
Procedure UpdateRuleInfo()
	...
	If RulesSource = "ExportedFromTheFile" Then
		UsageInformation = NStr("en = 'Rules imported from file are used.'");
	Else
		UsageInformation = NStr("en = 'Standard rules from configuration are used.'");
	EndIf;
