...
Function AuthorizationKey(Val Resolution) Export
	...
	PermissionKey = XDTOFactory.Create(XDTOFactory.Type("http://www.w3.org/2001/XMLSchema", "hexBinary"), Hashing.HashSum).LexicalValue;
	
	If StrLen(PermissionKey) > 32 Then
		Raise NStr("en = 'Key length exceeds the allowed one'");
	EndIf;
...
Procedure DeletePermission(Val SoftwareModuleType, Val SoftwareModuleID, Val OwnerType, Val IDOwner, Val AuthorizationKey, Val Resolution) Export
	...
	If Manager.Selected() Then
		...
		If Manager.AuthorizationBody <> CommonUse.ObjectXDTOInXMLString(Resolution) Then
			...
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Permissions position by keys:
				     |- ProgramModuleType: %1
				     |- ProgramModuleID: %2
				     |- OwnerType: %3
				     |- OwnerID: %4 
