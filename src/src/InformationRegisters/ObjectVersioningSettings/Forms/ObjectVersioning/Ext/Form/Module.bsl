...
Procedure SetVersioningVariantOnPosting(Command)
	...
	If SelectNoPostingDocumentsType() Then
		ShowMessageBox(, NStr("en = 'Documents that cannot be posted are set to versioning mode ""Version on writing"".'"));
	EndIf;
...
Procedure RunScheduledJob()
	...
	Else
		BackgroundJobDescription = StringFunctionsClientServer.SubstituteParametersInString(NStr("en = 'Start manually: %1'"), ScheduledJobMetadata.Synonym);
...
Function StatusTextCalculation()
	Return NStr("en = 'Search for outdated versions...'");
EndFunction

&AtClientAtServerNoContext
Function StatusTextClearing()
	Return NStr("en = 'Clearing obsolete versions...'");
EndFunction
...
Procedure DisplayInformationAboutOutdatedVersions()
	...
	If InformationAboutLegacyVersions.DataSize > 0 Then
		Items.InformationAboutLegacyVersions.Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Total obsolete versions: %1 (%2)'"),
			InformationAboutLegacyVersions.CountVersions,
			InformationAboutLegacyVersions.DataSizeString);
	Else
		Items.InformationAboutLegacyVersions.Title = NStr("en = 'All obsolete versions: no'");
	EndIf;
