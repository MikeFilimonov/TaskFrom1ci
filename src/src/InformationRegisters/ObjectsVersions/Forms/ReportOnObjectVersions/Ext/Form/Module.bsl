	...
	If TypeOf(Parameters.ComparedVersions) = Type("Array") Then
		...
		If ComparedVersions.Count() > 1 Then
			...
			Title = StringFunctionsClientServer.SubstituteParametersInString(
			                 NStr("en = 'Compare revision ""%1"" (#%2)'"),
			                 CommonUse.SubjectString(ObjectReference),
			                 StringOfVersionsNumber);
		Else
			Title = StringFunctionsClientServer.SubstituteParametersInString(
			                 NStr("en = 'Revision #%2 of the object ""%1""'"),
			                 ObjectReference,
...
#Region TheFunctionForReceivingReportOnChanges
...
Procedure GenerateChangesReport(ReportTS, Val VersionArray)
	...
	// change characteristics version identification is a string that identifies the object version among the rest ones and
	...
	// of the object version identification is a string that identifies the object version among the rest ones and provides
	...
	For VersionIndex = 2 To VersionsNumbersArray.Count() Do
		...
		For Each MapItem In ObjectVersioning.TabularSections Do
			...
			TablesVersionRef.Columns.Add("Versioning_RowId");
			...
			If TableWithChanges <> Undefined Then
				...
				For Each TSItem In ModifiedRows Do
					...
					TablesVersionRef[TSItem.IndexOfTS1 - 1].Versioning_RowId		= RowTCH.Versioning_RowId;
				...
				For Each TSItem In AddedStrings Do
					TablesVersionRef[TSItem.IndexOfTS1 - 1].Versioning_RowId		= IncrementCounter(CounterUniqueId, TableName);
				...
				For IndexOf = 1 To TablesVersionRef.Count() Do
					If TablesVersionRef[IndexOf - 1].Versioning_RowId = Undefined Then
						...
						For IndByVT_Current = 1 To RowArrayCurrentPM.Count() Do
							If IndByVT_Current <= ArrayOfStringsPreviousTS.Count() Then
								RowArrayCurrentPM[IndByVT_Current - 1].Versioning_RowId = ArrayOfStringsPreviousTS[IndByVT_Current - 1].Versioning_RowId;
							EndIf;
				...
				For Each TSItem In DeletedStrings Do
					...
					VirtualRow.Versioning_RowId			= ChangesTableTableParts[TableName][PreviousVersionNumber][TSItem.IndexTTN0 - 1].Versioning_RowId;
	...
	For Each ChangedTPItem In ChangesTableTableParts Do
		...
		For TechCounterUniqueId = 1 To CounterUniqueId[TabularSectionName] Do
			...
			While VersionsIndex >= 1 Do
				...
				If CurrentVersionOfTS.Columns.Find("Versioning_RowId") <> Undefined Then
					FoundString = CurrentVersionOfTS.Find(TechCounterUniqueId, "Versioning_RowId");
				EndIf;
...
Function OutputHeader(ReportTS, VersionsNumbersArray, NumberOfVersions)
	...
	AreaHeader.Parameters.ReportDescription = NStr("en = 'Object version change report'");
...
//       part corresponding table |- Versioning_RowId     - unique, within the table, ID
...
Function ReadInitialAttributeValuesAndTabularSections(AttributesTable,
	...
	ObjectVersioningStructure  = ObjectVersioning.VersionParsing(ObjectReference, MinObjectVersioning);
	AddLineNumbersInTabularSections(ObjectVersioningStructure.TabularSections);
	
	Attributes      = ObjectVersioningStructure.Attributes;
	TabularSections = ObjectVersioningStructure.TabularSections;
	...
	For Each TSItem In TabularSections Do
		...
		CurrentTV.Columns.Add("Versioning_RowId");
		...
		For IndexOf = 1 To CurrentTV.Count() Do
			CurrentTV[IndexOf-1].Versioning_RowId			= IndexOf;
			CurrentTV[IndexOf-1].Versioning_Modification	= False;
		EndDo;
	
	EndDo;
	
	Return ObjectVersioningStructure;
...
Function GetDescriptionByVersion(VersionNumber)
	...
	Definition = StringFunctionsClientServer.SubstituteParametersInString(NStr("en = '#%1 / (%2) / %3'"), 
...
Procedure AddLineNumbersInTabularSections(TabularSections)
	...
	For Each Map In TabularSections Do
		...
		If Table.Columns.Find("LineNumber") <> Undefined Then
			Continue;
		EndIf;
		Table.Columns.Insert(0, "LineNumber",,NStr("en = 'line #'"));
...
// While comparing values in the strings, values collections are compared by the references (they are not compared by
