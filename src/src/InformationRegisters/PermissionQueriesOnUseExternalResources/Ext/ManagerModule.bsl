...
//  previously requested for the same owner will be added to the request in addition to the requested permissions.
...
Function PossiblePermissionQueryToUseExternalResources()
	...
	If CommonUse.FileInfobase(InfobaseConnectionString()) OR Not GetFunctionalOption("SecurityProfilesAreUsed") Then
		...
		// the record of permissions requests is possible in the privileged mode or during using user personal account with
	...
	Try
		...
		While Selection.Next() Do
			...
			Try
				...
				KeyProperty = CreateRecordKey(New Structure("IDRequest", Selection.IDRequest));
				LockDataForEdit(KeyProperty);
