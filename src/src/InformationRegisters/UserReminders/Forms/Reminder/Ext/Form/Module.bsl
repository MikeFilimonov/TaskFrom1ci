...
Procedure BeforeWriteAtServer(Cancel, CurrentObject, WriteParameters)
	...
	If CurrentObject.ReminderTimeSettingVariant = Enums.ReminderTimeOptions.RelativelyToCurrentTime Then
		CurrentObject.EventTime = CurrentSessionDate() + Object.ReminderInterval;
		CurrentObject.ReminderPeriod = CurrentObject.EventTime;
		CurrentObject.ReminderTimeSettingVariant = Enums.ReminderTimeOptions.InSpecifiedTime;
	ElsIf CurrentObject.ReminderTimeSettingVariant = Enums.ReminderTimeOptions.RelativelyToSubjectTime Then
		...
		EndIf;
	ElsIf CurrentObject.ReminderTimeSettingVariant = Enums.ReminderTimeOptions.InSpecifiedTime Then
		CurrentObject.ReminderPeriod = Object.EventTime;
	ElsIf CurrentObject.ReminderTimeSettingVariant = Enums.ReminderTimeOptions.Periodically Then
	...
	If CurrentObject.ReminderTimeSettingVariant <> Enums.ReminderTimeOptions.Periodically Then
		Schedule = Undefined;
	EndIf;
...
Procedure OnOpen(Cancel)
	...
	If Not ValueIsFilled(Object.SourceRecordKey) Then
		If Items.SourceAttributeName.ChoiceList.Count() > 0 Then
			...
			Object.ReminderTimeSettingVariant = PredefinedValue("Enum.ReminderTimeOptions.RelativelyToSubjectTime");
		EndIf;
	...
	FillOptionsAlerts();
	If Items.SourceAttributeName.ChoiceList.Count() = 0 Then
		Items.ReminderTimeSettingVariant.ChoiceList.Delete(Items.ReminderTimeSettingVariant.ChoiceList.FindByValue(GetKeyByValueInAccordance(GetPredefinedWaysNotifications(),PredefinedValue("Enum.ReminderTimeOptions.RelativelyToSubjectTime"))));
	EndIf;		
	...
	If Object.ReminderTimeSettingVariant = PredefinedValue("Enum.ReminderTimeOptions.RelativelyToCurrentTime") Then
		ReminderTimeSettingVariant = NStr("en = 'every'") + " " + UserRemindersClientServer.TimePresentation(Object.ReminderInterval);
	Else
...
Procedure ReminderTimeSettingVariantOnChange(Item)
	...
	If TimeInterval > 0 Then
		...
		ReminderTimeSettingVariant = NStr("en = 'every'") + " " + TimeIntervalAsString;
	Else
		If Items.ReminderTimeSettingVariant.ChoiceList.FindByValue(ReminderTimeSettingVariant) = Undefined Then
			CommonUseClientServer.MessageToUser(NStr("en = 'Time interval not specified.'"), , "ReminderTimeSettingVariant");
		EndIf;
	EndIf;
	...
	If SelectedMethod = Undefined Then
		Object.ReminderTimeSettingVariant = PredefinedValue("Enum.ReminderTimeOptions.RelativelyToCurrentTime");
	Else
...
Procedure TimeIntervalOnChange(Item)
	...
	If Object.ReminderInterval > 0 Then
		TimeIntervalAsString = UserRemindersClientServer.TimePresentation(Object.ReminderInterval);
	Else
		CommonUseClientServer.MessageToUser(NStr("en = 'Time interval not specified'"), , "TimeIntervalAsString");
	EndIf;
EndProcedure
...
Function GetPredefinedWaysNotifications()
	...
	Result = New Map;
	Result.Insert(NStr("en = 'relative to subject'"), PredefinedValue("Enum.ReminderTimeOptions.RelativelyToSubjectTime"));
	Result.Insert(NStr("en = 'at a specified time'"), PredefinedValue("Enum.ReminderTimeOptions.InSpecifiedTime"));
	Result.Insert(NStr("en = 'periodic'"), PredefinedValue("Enum.ReminderTimeOptions.Periodically"));
...
Procedure FillOptionsAlerts()
	...
	For Each Interval In IntervalsTime Do
		OptionsAlerts.Add(NStr("en = 'every'") + " " + Interval);
...
Procedure FillInOptionsForFrequency()
	...
	EndDo;
	Items.FrequencyVariant.ChoiceList.Add("", NStr("en = 'on the specified schedule...'"));	
EndProcedure
...
Procedure SetVisible()
	...
	If SelectedMethod <> Undefined Then
		If SelectedMethod = PredefinedValue("Enum.ReminderTimeOptions.InSpecifiedTime") Then
			Items.DetailedSettingsPanel.CurrentPage = Items.DateTime;
		ElsIf SelectedMethod = PredefinedValue("Enum.ReminderTimeOptions.RelativelyToSubjectTime") Then
			Items.DetailedSettingsPanel.CurrentPage = Items.SourceSetting;
		ElsIf SelectedMethod = PredefinedValue("Enum.ReminderTimeOptions.Periodically") Then
...
Procedure OpenDialogScheduleSettingsEnd(SelectedSchedule, AdditionalParameters) Export
	...
	If Not ScheduleMeetsRequirements(Schedule) Then 
		ShowMessageBox(, NStr("en = 'Daily frequency is not supported. Corresponding settings cleared.'"));
	EndIf;
