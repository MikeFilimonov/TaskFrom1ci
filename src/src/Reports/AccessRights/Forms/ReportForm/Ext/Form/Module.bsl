...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Parameters.User <> Users.AuthorizedUser()
	   AND Not Users.InfobaseUserWithFullAccess() Then
		...
		Raise NStr("en = 'Insufficient rights to view the report.'");
	EndIf;
...
Procedure OutputReport(Ref)
	...
	ElsIf TypeOf(Ref) = Type("CatalogRef.ExternalUsers") Then
		...
		Properties.Insert("RolesByProfilesGrouping",	NStr("en = 'Roles of external user by profiles'"));
	...
	ElsIf TypeOf(Ref) = Type("CatalogRef.UserGroups") Then
		...
		Properties.Insert("RolesByProfilesGrouping",	NStr("en = 'User group roles by profiles'"));
		Properties.Insert("ObjectPresentation",			NStr("en = 'User group: %1'"));
	Else
		Properties.Insert("ReportHeader",				NStr("en = 'Report on group rights of external users'"));
		Properties.Insert("RolesByProfilesGrouping",	NStr("en = 'Roles of external user group by profiles'"));
	...
	|						WHERE
	|							UserGroupMembers.UsersGroup = AccessGroupsUsers.User
	|							AND UserGroupMembers.User = &User)
	|			END)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|						InformationRegister.UserGroupMembers AS UserGroupMembers
	|					WHERE
	|						UserGroupMembers.UsersGroup = ObjectRightsSettings.User
	|						AND UserGroupMembers.User = &User)
	|		END
	|
	|UNION ALL
	...
	If DetailedInformationAboutAccessRights Then
		...
		If OnePersonalGroup Then
			If TypeOf(Ref) = Type("CatalogRef.Users") Then
				AccessPresentation = NStr("en = 'User access limitations'");
				
			ElsIf TypeOf(Ref) = Type("CatalogRef.ExternalUsers") Then
				AccessPresentation = NStr("en = 'Access restrictions of external user'");
				
			ElsIf TypeOf(Ref) = Type("CatalogRef.UserGroups") Then
				AccessPresentation = NStr("en = 'Access restrictions of user group'");
			Else
				AccessPresentation = NStr("en = 'Access restrictions of external user group'");
			EndIf;
		Else
			If TypeOf(Ref) = Type("CatalogRef.Users") Then
			...
			ElsIf TypeOf(Ref) = Type("CatalogRef.ExternalUsers") Then
				AccessPresentation = NStr("en = 'Groups of external user access'");
				
			ElsIf TypeOf(Ref) = Type("CatalogRef.UserGroups") Then
				AccessPresentation = NStr("en = 'Access groups user groups'");
			Else
		...
		For Each DescriptionOfAccessGroup In AccessGroupsDescriptionFulls Do
			...
			Else
				...
				UserGroupsDescriptionFull = DescriptionOfAccessGroup.Rows.FindRows(Filter);
				If UserGroupsDescriptionFull.Count() > 0 Then
					
					Area = Template.GetArea(
						"AccessGroupDetailsUserIsInGroupAsParticipantOfGroupsUsers");
					
					Document.Put(Area, 3);
					For Each UserGroupLongDesription In UserGroupsDescriptionFull Do
		...
		For Each ObjectDescription In ObjectsKindDescriptionFull.Rows Do
			...
			If ObjectDescription.ReadingNotLimited Then
				If ObjectDescription.view Then
					ObjectPresentationClarification = NStr("en = '(view, not limited)'");
				Else
					ObjectPresentationClarification = NStr("en = '(view*, not limited)'");
				EndIf;
			Else
				If ObjectDescription.view Then
					ObjectPresentationClarification = NStr("en = '(view, limited)'");
				Else
					ObjectPresentationClarification = NStr("en = '(view*, limited)'");
				EndIf;
			EndIf;
					...
					ElsIf RoleKindsDescriptionFull.Rows[0].Rows.Count() > 0 Then
						...
						For Each DescriptionOfAccessGroup In RoleKindsDescriptionFull.Rows[0].Rows Do
							...
							If AllRolesWithRestriction Then
								If GetFunctionalOption("UseRowLevelSecurity") Then
	...
	Area = Template.GetArea("ObjectRightsGrouping");
	Area.Parameters.ObjectRightsGroupingPresentation = NStr("en = 'Object editing'");
	Document.Put(Area, 1);
		...
		For Each ObjectDescription In ObjectsKindDescriptionFull.Rows Do
			...
			Else
				If ObjectDescription.Update1 Then
					If ObjectDescription.ChangingNotLimited Then
						If ObjectDescription.Edit Then
							ObjectPresentationClarification = NStr("en = '(change, not limited)'");
						Else // NO ObjectDescription.Editing
							ObjectPresentationClarification = NStr("en = '(change *, not limited)'");
						EndIf;
					Else
						If ObjectDescription.Edit Then
							ObjectPresentationClarification = NStr("en = '(change, limited)'");
						Else // NO ObjectDescription.Editing
							ObjectPresentationClarification = NStr("en = '(change *, limited)'");
						EndIf;
					EndIf;
	...
	For Each ObjectTypeDescription In RightSettings.Rows Do
		...
		PromtForSubfolders = NStr("en = 'Rights are not only to the current folder but also for its subfolders'");
		...
		TitleSettingIsReceivedFromGroup = NStr("en = 'Rights setting received from group'");
		
		Area = Template.GetArea("RightsSettingsLegendRowInheritance");
		Area.Parameters.ToolTip = NStr("en = 'Rights inheritance from upstream folders'");
		Document.Put(Area, 2);
		...
		DisplayUserGroups = ObjectTypeDescription.GroupInvolvement AND Not OutputGroupRights;
		ColumnsCount = RightsDescriptionFull.Count() + ?(DisplayUserGroups, 2, 1);
...
Function AccessKindPresentationTemplate(AccessTypeDescription, OwnersOfRightsSettings)
	...
	If AccessTypeDescription.Rows.Count() = 0 Then
		...
		ElsIf AccessTypeDescription.AllAllowed Then
			If AccessTypeDescription.AccessKind = Catalogs.Users.EmptyRef() Then
				AccessKindPresentationTemplate =
					NStr("en = '%1 (no prohibited, the current user is always permitted)'");
				
			ElsIf AccessTypeDescription.AccessKind = Catalogs.ExternalUsers.EmptyRef() Then
				AccessKindPresentationTemplate =
					NStr("en = '%1 (no prohibited, the current external user is always permitted)'");
			Else
		...
		Else
			...
			ElsIf AccessTypeDescription.AccessKind = Catalogs.ExternalUsers.EmptyRef() Then
				AccessKindPresentationTemplate =
					NStr("en = '%1 (no permitted, the current external user is always permitted)'");
			Else
	...
	Else
		If AccessTypeDescription.AllAllowed Then
			If AccessTypeDescription.AccessKind = Catalogs.Users.EmptyRef() Then
				AccessKindPresentationTemplate =
					NStr("en = '%1 (prohibited, the current user is always permitted):'");
				
			ElsIf AccessTypeDescription.AccessKind = Catalogs.ExternalUsers.EmptyRef() Then
				AccessKindPresentationTemplate =
					NStr("en = '%1 (prohibited, the current external user is always permitted):'");
			Else
		...
		Else
			If AccessTypeDescription.AccessKind = Catalogs.Users.EmptyRef() Then
				AccessKindPresentationTemplate =
					NStr("en = '%1 (permitted, the current user is always permitted):'");
				
			ElsIf AccessTypeDescription.AccessKind = Catalogs.ExternalUsers.EmptyRef() Then
				AccessKindPresentationTemplate =
					NStr("en = '%1 (permitted, the current external user is always permitted):'");
			Else
				AccessKindPresentationTemplate = NStr("en = '%1 (permitted):'");
			EndIf;
		EndIf;
	EndIf;
...
Function StartModePresentation(RunMode)
	...
	ElsIf RunMode = "OrdinaryApplication" Then
		StartModePresentation = NStr("en = 'Standard application'");
...
// Function returns value table containing the
		...
		If AccessTypeProperties = Undefined Then
			...
			Else
				String.Presentation = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'Set user rights to %1'"), RightSettingsOwnerMetadata.Presentation());
			EndIf;
