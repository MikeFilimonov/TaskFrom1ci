...
Procedure GenerateFieldList(MetadataResource, TableOfFields, FieldList)
	...
	For Each Resource In MetadataResource Do
		...
		If (Resource.Type.ContainsType(Type("CatalogRef.ProductsCharacteristics")) AND Not Constants.UseCharacteristics.Get())
			...
			OR (Resource.Type.ContainsType(Type("CatalogRef.Projects")) AND Not Constants.UseProjects.Get()) Then
...
Procedure AddPeriodToFieldList(TableOfFields, FieldList)
	...
	TableOfFields.Columns.Add("Period", , NStr("en = 'Period'"));
...
Procedure ProcessDataOutputByArray(FieldList, ResourcesTable, TableOfDimensions, AttributesTable, TableKindOfMovements = Undefined, Val RegisterName, SynonymRegister)
	...
	If Report.SubordinateDocumentsStructureGrouping = Enums.SubordinateDocumentsStructureGrouping.Horizontal Then
		...
		If TableKindOfMovements <> Undefined Then
			AreaTitleCell.Parameters.ColumnsTitle = NStr("en = 'Record type'");
...
Procedure DoOutputPostingJournal()
	...
	HeaderArea.Parameters.SynonymRegister = "Accounting register ""Journal entries""";
	SpreadsheetDocument.Put(HeaderArea);
	SpreadsheetDocument.StartRowGroup();
...
Procedure GenerateReport()
    ...
    If Not ValueIsFilled(Report.Document) Then
		CommonUseClientServer.MessageToUser(NStr("en = 'Document not selected.'"));
		Return;
	EndIf;
	...
	For Each PropertiesOfObject In DocumentRegisterRecords Do
		...
		If RegisterType = "Information" Then
			SynonymRegister = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Information register ""%2""'"),
				RegisterType,
				PropertiesOfObject.Synonym);
		ElsIf RegisterType = "Accumulation" Then
			SynonymRegister = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Accumulation register ""%2""'"),
				RegisterType,
				PropertiesOfObject.Synonym);
		ElsIf RegisterType = "Accounting" Then
			SynonymRegister = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Accounting register ""%2""'"),
				RegisterType,
				PropertiesOfObject.Synonym);
		ElsIf RegisterType = "Calculation" Then
			SynonymRegister = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en = 'Calculation register ""%2""'"),
				RegisterType,
				PropertiesOfObject.Synonym);
		EndIf;
		...
		If RegisterType = "Information" OR RegisterType = "Accumulation" Then
			...
			If (RegisterType = "Accumulation") AND (PropertiesOfObject.RegisterType = Metadata.ObjectProperties.AccumulationRegisterType.Balance) Then
			    ...
			    TableKindOfMovements.Columns.Add("RecordType", , NStr("en = 'Record type'"));
