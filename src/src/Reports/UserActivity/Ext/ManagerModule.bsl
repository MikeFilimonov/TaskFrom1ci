...
Procedure ConfigureReportsVariants(Settings, ReportSettings) Export
	...
	VariantSettings.Definition = NStr("en = 'List of critical entries in the event log.'");
	...
	VariantSettings.Definition = NStr("en = 'Displays scheduled job schedule in the application.'");
...
Procedure Generate(ReportParameters, StorageAddress) Export
	...
	ReportObject = Reports.UserActivity.Create();
...
#Region ScheduledJobsWorkDuration
...
Function GenerateReportForDurationJobsOfScheduledJobs(FillingParameters) Export
	...
	Template = Reports.UserActivity.GetTemplate("ScheduledJobsWorkDuration");
...
Function DecryptionScheduledJobs(Details) Export
	...
	Template = Reports.UserActivity.GetTemplate("ScheduledJobDetails");
	...
	Area = Template.GetArea("Table");
	Area.Parameters.TypeTasks = NStr("en = 'Scheduled'");
...
#Region EventsLogMonitorControl
...
Function GenerateReportEventLogMonitorControl(StartDate, EndDate) Export	
	...
	Result = New Structure; 	
	Report = New SpreadsheetDocument; 	
	Template = Reports.UserActivity.GetTemplate("ErrorsReportTemplateInEventLogMonitor");   	
	DataEventLog = BugEventLogInformation(StartDate, EndDate);
	...
	Rollup_Errors.Sort("TotalForComment Desc");
	Rollup_Warnings = RollOnComments.Copy(RowArray_LevelWarning);
	Rollup_Warnings.Sort("TotalForComment Desc");
