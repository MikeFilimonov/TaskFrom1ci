...
Procedure OnComposeResult(ResultDocument, DetailsDataObject, StandardProcessing, StorageAddress) 	
	...
	If ReportVariant = "EventLogMonitorControl" Then
		FormingResultReport = Reports.UserActivity.
	...
	Else
		...
		CompositionProcessor.Initialize(CompositionTemplate,
			Reports.UserActivity.DataFromEventLog(ReportParameters), DetailsDataObject, True);
...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	If ReportVariant = "GanttChart" Then
		...
		If Not ValueIsFilled(DayPeriod.Date) Then
			CommonUseClientServer.MessageToUser(
				NStr("en = 'Value of field Day is not entered.'"), , );
			Cancel = True;
			Return;
		EndIf;
		...
		AND EndSelection.Use Then
			CommonUseClientServer.MessageToUser(
				NStr("en = 'The period start value cannot be later than the period end value.'"), , );
			Cancel = True;
			Return;
		EndIf;
	...
	ElsIf ReportVariant = "ActiveUser" Then
		...
		If Not ValueIsFilled(User) Then
			CommonUseClientServer.MessageToUser(
				NStr("en = 'Value of field User is not entered.'"), , );
			Cancel = True;
			Return;
		EndIf;
		
		If Reports.UserActivity.IBUserName(User) = Undefined Then
			CommonUseClientServer.MessageToUser(
				NStr("en = 'The report can be generated only for the user for whom a name for logon to the application is specified.'"), , );
			Cancel = True;
			Return;
		EndIf;
	...
	ElsIf ReportVariant = "UsersActivityAnalysis" Then
		...
		If TypeOf(UsersAndGroups) = Type("CatalogRef.Users") Then
			...
			If Reports.UserActivity.IBUserName(UsersAndGroups) = Undefined Then
				CommonUseClientServer.MessageToUser(
					NStr("en = 'The report can be generated only for the user for whom a name for logon to the application is specified.'"), , );
				Cancel = True;
				Return;
		...
		If Not ValueIsFilled(UsersAndGroups) Then
			CommonUseClientServer.MessageToUser(
				NStr("en = 'Value of field Users is not entered.'"), , );
			Cancel = True;
			Return;
		EndIf;
