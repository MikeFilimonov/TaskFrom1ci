...
Procedure ResultDetailProcessing(Item, Details, StandardProcessing)
	...
	If Not CurrentVariantDescription = NStr("en = 'Duration of scheduled jobs'") Then
	...
	If TypeDetails = "DecryptionScheduledJobs" Then
		...
		VariantDetails.Add("InfoAboutScheduledJob", NStr("en = 'Scheduled job information'"));
		VariantDetails.Add("OpenEventLogMonitor", NStr("en = 'Go to event log'"));
...
Procedure InfoAboutScheduledJob(Details)
	...
	OpenForm("Report.UserActivity.Form.InfoAboutScheduledJob", FormParameters);
EndProcedure

&AtServer
Function ReportOnScheduledTask(Details)
	FormingResultReport = Reports.UserActivity.DecryptionScheduledJobs(Details);
...
Function GenerateReportServer(ReportParameters)
	...
	ExecutionResult = LongActions.ExecuteInBackground(
		UUID,
		"Reports.UserActivity.Generate",
		ReportParameters,
		NStr("en = 'Running report: Event log analysis'"));
