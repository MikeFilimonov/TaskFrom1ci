...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not Parameters.Property("OptionName", OptionName) Then
		Raise NStr("en = 'Service parameter VariantName is not passed.'");
	EndIf;
	...
	If CurrentCDHostIdentifier <> Undefined Then
		...
		If Not Parameters.Property("Title", Title) Then
			Raise NStr("en = 'Service parameter ""Title"" is not transferred.'");
		EndIf;
		
		If Not Parameters.Property("CurrentCDHostType", CurrentCDHostType) Then
			Raise NStr("en = 'Service parameter CurrentDCNode is not passed.'");
		EndIf;
...
#Region AttachedValuesListWithTheSelectionButton
...
&AtClient
Procedure Attachable_ListWithSelection_Value_AutoComplete(Item, Text, ChoiceData, Parameters, Wait, StandardProcessing)
...
&AtClient
Procedure Attachable_ListWithSelection_BeforeRowChange(Item, Cancel)
	...
	Else
		For Each ListItemDoubleInForm In ValuesListInForm Do
			If ListItemDoubleInForm.Value = Value AND ListItemDoubleInForm <> ListItemInForm Then
				Status(NStr("en = 'Duplicate records are detected. Editing canceled.'"));
...
Procedure Attachable_ListWithSelection_Pick_OpenChoiceForm(SelectedItem, ItemParameters) Export
	...
	//   even for read, that is why to store the preset selection form names, the MetadataObjectsNamesQuickSearch
	//   collection is used.
...
&AtClient
Procedure Attachable_FixedList_BeforeAddRow(Item, Cancel, Copy, Parent, Group, Parameter)
	Cancel = True;
EndProcedure

&AtClient
Procedure Attachable_FixedList_BeforeDeleteRow(Item, Cancel)
	Cancel = True;
EndProcedure
...
#Region TABLEEVENTSSelectedFields
...
#Region TABLEEVENTSVariantStructure
...
#Region TABLEEVENTSGroupsContent
...
#Region ClientStepsOfAScriptExecution
...
#Region ClientTablesOfFieldsscriptExecutionSupported
	...
	For Each RowIdentifier In SelectedRows Do
		...
		If Script.TableName = "Filters" Then
			...
			ElsIf TreeRow.IsParameter Then
				If Script.Action = "Move" Then
					Script.ErrorText = StrReplace(NStr("en = 'Parameters cannot be transferred.'"), "%1", TreeRow.Presentation);
				ElsIf Script.Action = "Group" Then
					Script.ErrorText = NStr("en = 'Parameters cannot be group participants.'");
				ElsIf Script.Action = "Delete" Then
					Script.ErrorText = NStr("en = 'Parameters cannot be deleted.'");
				EndIf;
				Return False; // Pause script.
			EndIf;
		EndIf;
		If Script.Action = "Move" Or Script.Action = "Group" Then
			...
			ElsIf Script.CurrentParent <> Parent Then
				If Script.Action = "Move" Then
					Script.ErrorText = NStr("en = 'The selected items cannot be transferred as they have different parents.'");
				ElsIf Script.Action = "Group" Then
					Script.ErrorText = NStr("en = 'The selected items cannot be grouped as they have different parents.'");
				EndIf;
				Return False; // Pause script.
			EndIf;
		EndIf;
...
#Region ClientTableOfParametersAndFilters
...
Procedure FiltersSelectAccessLevel(TableName, RowIdentifier, TableRow)
	...
	List = New ValueList;
	List.Add(2, NStr("en = 'In report header'"), , PictureLib.QuickAccess);
	If Not TableRow.IsParameter Then
		List.Add(1, NStr("en = 'Only check box in report title'"), , PictureLib.RapidAccessWithCheckBox);
	EndIf;
	List.Add(4, NStr("en = 'In report settings'"), , PictureLib.Attribute);
	If Not TableRow.IsParameter Then
		List.Add(3, NStr("en = 'Only check box in report settings'"), , PictureLib.UsualAccessWithCheckBox);
	EndIf;
...
Procedure FiltersShowListWithCheckBoxes(TableRow)
	...
	OpenForm("CommonForm.FilterFormForSimpleModeOfReportSettings", FormParameters, ThisObject, , , , Handler, Block);
...
#Region ClientFieldsTablesfunctionalPart
...
Procedure FieldsTablesChangeNodeVariantStructure(TableName, RowIdentifier, TableRow)
	...
	If TableRow.Type = "Chart" Then
		FormParameters.Insert("Title", NStr("en = 'Set up chart of the %2 report'"));
	Else
		FormParameters.Insert("Title", NStr("en = 'Set up %1 grouping of the %2 report'"));
	EndIf;
...
Function FieldsTablesFindNode(ThisObject, TableName, TableRow = Undefined, DCNodeIdentifier = Undefined)
	If ThisObject.ExtendedMode = 1 Then
		...
		ElsIf TableName = "VariantStructure" Then
			Return RootNode;
		Else
			Raise StrReplace(NStr("en = 'Changing of nodes of the ""%1"" table is not supported.'"), "%1", TableName);
		EndIf;
	Else
...
Procedure AddConditionalDesign()
	...
	Instruction = ReportsVariants.ConditionalDesignInstruction();
	Instruction.Filters.Insert("Filters.DisplayCheckbox", False);
...
Procedure QuickSettingsCreateControlItemsAndLoadValues(FillingParameters, Information)
	...
	For Each SettingProperty In AddedValueLists Do
		...
		FormTable.SetAction("BeforeRowChange", "Attachable_ListWithSelection_BeforeRowChange");
		FormTable.SetAction("BeforeEditEnd", "Attachable_ListWithSelection_BeforeEditEnd");
		FormTable.SetAction("OnChange", "Attachable_ListWithSelection_OnChange");
		If SettingProperty.LimitChoiceWithSpecifiedValues Then
			FormTable.SetAction("BeforeAddRow", "Attachable_FixedList_BeforeAddRow");
			FormTable.SetAction("BeforeDeleteRow", "Attachable_FixedList_BeforeDeleteRow");
		Else
			FormTable.SetAction("ChoiceProcessing", "Attachable_ListWithSelection__ChoiceProcessing");
			Column_Value.SetAction("AutoComplete", "Attachable_ListWithSelection_Value_AutoComplete");
		EndIf;
		ColumnUse.SetAction("OnChange", "Attachable_ListWithSelection_Use_OnChange");
	EndDo;
