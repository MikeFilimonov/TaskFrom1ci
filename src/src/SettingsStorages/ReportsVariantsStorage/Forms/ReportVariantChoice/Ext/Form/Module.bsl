...
Procedure ChooseAndClose()
	...
	If Variant.PictureIndex = 4 Then
		QuestionText = NStr("en = 'Selected report option is marked for deletion.
...
Procedure FillVariantsList()
	...
	|	MAX(ISNULL(PersonalSettingsTip.Visible, Variants.VisibleByDefault)) AS VisibleInReportsPanel
	|INTO ttVisible
	|FROM
	|	ttVariantPlacement AS Placement
	|		LEFT JOIN InformationRegister.ReportOptionSettings AS PersonalSettingsTip
	|		ON Placement.Subsystem = PersonalSettingsTip.Subsystem
	|			AND Placement.Ref = PersonalSettingsTip.Variant
	...
	// Add predefined options of external report to the option table (for sorting when adding to a tree).
	...
	For Each TableRow In VariantTable Do
		...
		If TreeRowSet = Undefined Then
			...
			If TableRow.GroupNumber = 2 Then
				TreeGroup.Description = NStr("en = 'Hidden in report panels'");
