	...
	Try
		...
		If CommonUseReUse.DataSeparationEnabled() Then
			...
			If CommonUseReUse.CanUseSeparatedData() Then
				...
				Raise NStr("en = 'Operation cannot be called in session which is started with specified separators. Verify that the web service is published correctly.'");
			...
			Else
				...
				If DataAreaKey <> Constants.DataAreaKey.Get() Then
					Raise NStr("en = 'Incorrect data area key.'");
				EndIf;
		...
		Else
			Raise NStr("en = 'The operation cannot be run for the infobase in which separation by data areas is disabled.'");
		EndIf;
